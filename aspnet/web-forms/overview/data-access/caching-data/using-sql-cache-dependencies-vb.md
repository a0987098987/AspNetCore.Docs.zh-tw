...
[架構中的快取資料](caching-data-in-the-architecture-vb.md)教學課程會探討使用個別的快取層，在架構中，而不是緊密結合使用 ObjectDataSource 快取的優點。 在本教學課程，我們建立`ProductsCL`類別示範以程式設計方式使用的資料快取。 若要利用快取層中的 SQL 快取相依性，使用`SqlCacheDependency`類別。
