...
* 判斷提示的 cookie `SameSite=None` 也必須標記為 `Secure` 。
* 使用的應用程式 [`<iframe>`](https://developer.mozilla.org/docs/Web/HTML/Element/iframe) 可能會遇到 `sameSite=Lax` 或 cookie 的問題， `sameSite=Strict` 因為會被 `<iframe>` 視為跨網站案例。
* `SameSite=None` [2016 標準](https://tools.ietf.org/html/draft-west-first-party-cookies-07)不允許此值，而會導致某些實作為將這類 cookie 視為 `SameSite=Strict` 。 請參閱本檔中的[支援舊版瀏覽器](#sob)。
...
.NET Core 2.2 支援自12月2019的更新發行以來，適用于 SameSite 的 2019 draft standard。 開發人員可以使用屬性，以程式設計方式控制 sameSite 屬性的值 `HttpCookie.SameSite` 。 將屬性設定為 [Strict]、[寬鬆] 或 [無] 會 `SameSite` 導致這些值在網路上使用 cookie 寫入。 將其設定為等於（SameSiteMode）（-1）表示不應在具有 cookie 的網路上包含任何 sameSite 屬性

[!code-csharp[](samesite/snippets/Privacy.cshtml.cs?name=snippet)]

.NET Core 3.0 支援已更新的 SameSite 值，並將額外的列舉值新增 `SameSiteMode.Unspecified` 至 `SameSiteMode` 列舉。
...
.NET Framework 和 .NET Core 2.1 的特定行為變更是 `SameSite` 屬性解讀值的方式 `None` 。 在修補程式之前，「 `None` 不要發出屬性」的值，在修補程式之後，表示「發出屬性，其值為」 `None` 。 在修補程式之後，的 `SameSite` 值 `(SameSiteMode)(-1)` 會導致不發出屬性。
...
* 重新定義發出的行為 `SameSiteMode.None``SameSite=None`
* 加入新的值 `SameSiteMode.Unspecified` ，以省略 SameSite 屬性。
* 所有 cookie Api 都會預設為 `Unspecified` 。 某些使用 cookie 的元件會針對其案例設定更具體的值。 如需範例，請參閱上表。

::: moniker-end

::: moniker range=">= aspnetcore-3.0"

在 ASP.NET Core 3.0 和更新版本中，SameSite 預設值已變更，以避免與不一致的用戶端預設值發生衝突。 下列 Api 已將預設值從變更 `SameSiteMode.Lax ` 為 `-1` ，以避免發出這些 Cookie 的 SameSite 屬性：
...
SameSite 支援首次使用[2016 draft 標準](https://tools.ietf.org/html/draft-west-first-party-cookies-07#section-4.1)在2.0 的 ASP.NET Core 中執行。 2016標準是加入宣告。 預設會將數個 cookie 設定為，以 ASP.NET Core 加入宣告 `Lax` 。 在驗證發生數個[問題](https://github.com/aspnet/Announcements/issues/318)之後，大部分 SameSite 的使用量都會[停用](https://github.com/aspnet/Announcements/issues/348)。
...
* 指定明確判斷提示 `SameSite=None` 以啟用跨網站傳遞的 cookie 應該標記為 `Secure` 。 `None`是要退出的新專案。
...
2016 SameSite 標準規定必須將未知的值視為 `SameSite=Strict` 值。 從支援 2016 SameSite standard 的舊版瀏覽器存取的應用程式，可能會在取得值為的 SameSite 屬性時中斷 `None` 。 如果 Web 應用程式想要支援較舊的瀏覽器，則必須執行瀏覽器偵測。 ASP.NET Core 不會實作為瀏覽器偵測，因為使用者代理程式的值會高度變動並經常變更。 中的擴充點 <xref:Microsoft.AspNetCore.CookiePolicy> 允許插入使用者代理程式特定的邏輯。

在中 `Startup.Configure` ，新增在呼叫之前呼叫的程式碼， <xref:Microsoft.AspNetCore.Builder.CookiePolicyAppBuilderExtensions.UseCookiePolicy*> <xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication*> 或*任何*寫入 cookie 的方法：

[!code-csharp[](samesite/sample/Startup.cs?name=snippet5&highlight=18-19)]

在中 `Startup.ConfigureServices` ，新增類似下列的程式碼：
...
從抑制的版本開始 `80.0.3975.0` ，您可以使用新的旗標來停用最寬鬆的 + POST 暫時緩和， `--enable-features=SameSiteDefaultChecksMethodRigorously` 以允許在已移除緩和功能的最終狀態中測試網站和服務。 如需詳細資訊，請參閱 Chromium Projects [SameSite Updates](https://www.chromium.org/updates/same-site)

### <a name="test-with-safari"></a>使用 Safari 進行測試

Safari 12 已嚴格實作為先前的草稿，當新的 `None` 值在 cookie 中時就會失敗。 `None`透過支援本檔中[較舊流覽](#sob)器的瀏覽器偵測程式碼來避免。 使用 MSAL、ADAL 或您使用的任何程式庫，測試 Safari 12、Safari 13 和以 WebKit 為基礎的 OS 樣式登入。 問題取決於基礎作業系統版本。 OSX Mojave （10.14）和 iOS 12 已知具有新 SameSite 行為的相容性問題。 將 OS 升級至 OSX Catalina （10.15）或 iOS 13 會修正問題。 Safari 目前沒有加入宣告旗標來測試新的規格行為。

### <a name="test-with-firefox"></a>使用 Firefox 進行測試

您可以使用功能旗標在頁面上進行選擇，以在第68版上測試新標準的 Firefox 支援 `about:config` `network.cookie.sameSite.laxByDefault` 。 尚未報告舊版 Firefox 的相容性問題。
...
SameSite 旗標是在頁面上設定的 `edge://flags/#same-site-by-default-cookies` 。 Edge Chromium 未發現相容性問題。

### <a name="test-with-electron"></a>測試方式Electron

的版本 Electron 包含較舊版本的 Chromium。 例如，小組所使用的版本 Electron 是 Chromium 66，這會展現較舊的行為。 您必須使用您的產品版本來執行自己的相容性測試 Electron 。 請參閱下一節中的[支援舊版瀏覽器](#sob)。
