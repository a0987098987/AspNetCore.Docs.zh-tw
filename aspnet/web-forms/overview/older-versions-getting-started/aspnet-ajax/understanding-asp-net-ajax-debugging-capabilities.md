...
> *> [!NOTE] 如果看不到列出的指令碼總管功能表，請移至工具**自訂**在 Visual Studio.NET 功能表上的命令。在 [類別] 區段中找出偵錯項目，然後按一下以顯示所有可用的功能表項目。在 [命令] 清單中，捲動到指令碼總管，並將它拖曳拖曳至偵錯* *Windows 功能表中的先前所述。如此一來，即可使用 [指令碼總管] 功能表項目每次執行 Visual Studio.NET。*
