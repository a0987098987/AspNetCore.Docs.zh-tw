...
ASP.NET Core MVC 有內建支援格式化回應資料的功能，使用固定的格式或偵測來自用戶端的規格。
...
某些動作結果型別有特定的格式，例如`JsonResult`和`ContentResult`，該動作永遠已特定的格式回傳結果。例如，回傳`JsonResult`會回傳 JSON 格式的資料，而忽略用戶端喜好設定。同樣地，回傳`ContentResult`（例如只回傳一個字串），就會回傳純文字格式的字串資料。
...
繼承自`Controller`基底類別的控制器，若要以特定格式傳回資料，請使用內建的輔助方法`Json`回傳 JSON，或使用`Content`回傳純文字。你的動作方法應該傳回特定結果型別 (例如`JsonResult`) 或`IActionResult`。
...
請注意，上述範例的回應內容類型為`application/json`，可參考網路要求清單已及回應標頭區段。也請注意在瀏覽器的選項清單中（在此情況下，Microsoft Edge），要求標頭區段的 Accept 標頭，目前的技術會忽略此標頭，以下將討論它。
...
>[!TIP]
> 對於非簡單式且具有多個動作的傳回類型或選項（例如，根據所執行作業的結果不同而回傳不同的 HTTP 狀態碼），建議使用`IActionResult`當做回傳型別。
...
除非要求另一種格式，而且伺服器可以回傳所要求的格式，預設會回傳 JSON 格式的回應。您可以使用[Fiddler](http://www.telerik.com/fiddler)這類的工具，建立包含 Accept 標頭的要求，並指定另一種格式，在此情況下，如果伺服器有*格式器*可以產生回應要求的格式，回傳結果將會使用用戶端喜好的格式。

![Fiddler 主控台顯示手動建立取得與應用程式/xml 的 Accept 標頭值的要求](formatting/_static/fiddler-composer.png)

在上面的螢幕擷取畫面中，使用 Fiddler 編輯器來產生指定`Accept: application/xml`標頭的要求。 ASP.NET Core MVC 預設僅支援 JSON，所以即使指定另一種格式，回傳的結果仍是 JSON 格式。下一節中您會看到如何加入其他格式器。
...
在範例中，對於有效的作者別名要求，會回應 `200 OK` 以及作者的資料。無效的別名要求，將會收到`204 No Content`回應。
...
不同於一般的 API 用戶端，Web 瀏覽器通常會提供包含廣泛格式的`Accept`標頭，包括使用萬用字元的標頭。根據預設，當框架偵測到來自瀏覽器的要求時，它將會忽略`Accept`標頭，並改為傳回應用程式中所設定的內容預設格式 (預設為 JSON)。這會讓你使用不同的瀏覽器來呼叫 API 時，有更一致的體驗。

如果您想要讓您的應用程式使用瀏覽器的 `Accept` 標頭，您可以設定 MVC 組態，在*Startup.cs*的`ConfigureServices`方法中，設定 MVC 的組態選項 `RespectBrowserAcceptHeader`為`true`。
...
若您的應用程式需要支援 JSON 預設值以外的其他格式，您可以新增 NuGet 封裝，並設定 MVC 支援它們。NuGet 上有多種輸入和輸出的格式器。輸入的格式器用於[模型繫結](model-binding.md)；輸出格式器會用來格式化回應。您也可以設定[自訂格式器](../advanced/custom-formatters.md)。

### <a name="adding-xml-format-support"></a>加入 XML 格式的支援

若要支援 XML 格式，請安裝`Microsoft.AspNetCore.Mvc.Formatters.Xml` NuGet 封裝。
...
這兩種方法將使用`System.Xml.Serialization.XmlSerializer`來序列化結果。如果您想要的話，您可以加上其相關聯的格式器`System.Runtime.Serialization.DataContractSerializer`：
...
一旦您加入支援 XML 格式，控制器方法會根據要求的`Accept`標頭回傳適當的格式，如同這個 Fiddler 範例所示範的:
...
您可以在 Inspectors 的 RAW 索引標籤中，看到未經處理的 GET 要求，其中包含`Accept: application/xml`標頭。回應窗格顯示`Content-Type: application/xml`標頭，而`Author`物件序列化為 XML 格式。

使用編輯器索引標籤，將要求修改成指定`application/json`中`Accept`標頭並執行要求，會將回應的物件格式化為 JSON 格式：
...
如果您想要限制特定動作的回應格式，您可以套用`[Produces]`篩選器。`[Produces]`篩選條件會將所指定的動作（或控制器站）使用所設定的回應格式。像大部分[篩選](../controllers/filters.md)的功能，這可套用在動作、控制器或全域範圍。
...
舉例來說，移除`TextOutputFormatter`後，對於傳回型別為`string`時，將傳回 `406 Not Acceptable`。請注意，如果移除`TextOutputFormatter`後，尚存在 XML 格式器，傳回型別將會格式化成`string`。
...
此路由會讓要求的格式使用指定為選擇性的副檔名。`[FormatFilter]`屬性會檢查格式值是否存在於`RouteData`，並根據回應格式挑選適當的格式器。
