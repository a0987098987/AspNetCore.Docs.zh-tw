...
* 移除的套件參考的 「 版本 」 屬性`Microsoft.AspNetCore.App`。 專案使用`<Project Sdk="Microsoft.NET.Sdk.Web">`不需要設定版本。 版本是隱含目標架構，並選取最能符合 ASP.NET Core 2.1 的運作的方式。 如需詳細資訊，請參閱 <<c0> [ 規則，以共用的 framework 為目標的專案](#rules-for-projects-targeting-the-shared-framework)一節。
...
* 更新 *\_LoginPartial.cshtml*檔案：
  * 移動*頁 /\_LoginPartial.cshtml*要*共用頁面 / /\_LoginPartial.cshtml*。
...
* 在 *領域/身分識別/頁面/\_ViewStart.cshtml*，變更`Layout = "/Pages/_Layout.cshtml"`到`Layout = "/Pages/Shared/_Layout.cshtml"`。
* *\_Layout.cshtml*檔案有下列變更：

  * `<partial name="_CookieConsentPartial" />` 會加入。 如需詳細資訊，請參閱 [ASP.NET Core 的 GDPR 支援](xref:security/gdpr)。
  * jQuery 的 2.2.0 3.3.1 變更。
...
<xref:Microsoft.AspNetCore.Mvc.FileResult> 不會再處理[Accept-ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges)預設標頭。 若要啟用`Accept-Ranges`標頭，設定<xref:Microsoft.AspNetCore.Mvc.FileResult.EnableRangeProcessing>至`true`。

### <a name="controllerbasefile-and-physicalfile-range-header"></a>ControllerBase.File 和 PhysicalFile 範圍標頭

下列<xref:Microsoft.AspNetCore.Mvc.ControllerBase>方法不會再處理[Accept-ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges)預設標頭：

* 多載[ControllerBase.File](/dotnet/api/microsoft.aspnetcore.mvc.controllerbase.file#Microsoft_AspNetCore_Mvc_ControllerBase_File_System_String_System_String_System_String_System_Boolean_)
* [ControllerBase.PhysicalFile](xref:Microsoft.AspNetCore.Mvc.ControllerBase.PhysicalFile*)

若要啟用`Accept-Ranges`標頭，設定`EnableRangeProcessing`參數來`true`。
