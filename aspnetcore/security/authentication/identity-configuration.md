...
| 屬性 | 描述 |
| -------- | ----------- |
| [AccessDeniedPath](/dotnet/api/microsoft.aspnetcore.authentication.cookies.cookieauthenticationoptions.accessdeniedpath) | 通知處理常式，它應該變更傳出*403 禁止*狀態程式碼到*302 重新導向*到指定的路徑。<br><br>預設值是 `/Account/AccessDenied`。 |
...
| [Cookie.Expiration](/dotnet/api/microsoft.aspnetcore.http.cookiebuilder.expiration) | 取得或設定 cookie 的壽命。 |
...
| [CookieManager](/dotnet/api/microsoft.aspnetcore.authentication.cookies.cookieauthenticationoptions.cookiemanager) | 用來從要求取得 cookie 或設定回應上的元件。 | [ChunkingCookieManager](/dotnet/api/microsoft.aspnetcore.authentication.cookies.chunkingcookiemanager) |
...
| [ReturnUrlParameter](/dotnet/api/microsoft.aspnetcore.authentication.cookies.cookieauthenticationoptions.returnurlparameter) | 決定是由中介軟體附加的查詢字串參數名稱時*401 未授權*狀態碼變更為*302 重新導向*到登入路徑。<br><br>預設值是 `ReturnUrl`。 |
