...
<span data-ttu-id="413f1-141">如果您不熟悉 Visual Studio 程式碼片段，而且想来了解如何使用它們，您可以從這份文件參考附錄&quot;[附錄 c： 使用程式碼片段](#AppendixC)&quot;。</span><span class="sxs-lookup"><span data-stu-id="413f1-141">If you are not familiar with the Visual Studio Code Snippets, and want to learn how to use them, you can refer to the appendix from this document &quot;[Appendix C: Using Code Snippets](#AppendixC)&quot;.</span></span>
...
<span data-ttu-id="413f1-154">在這項工作，您會發現新強型別繫結用於 ASP.NET 4.5。</span><span class="sxs-lookup"><span data-stu-id="413f1-154">In this task, you will discover the new strongly-typed bindings available in ASP.NET 4.5.</span></span>
...
5. <span data-ttu-id="413f1-216">建立**GetCategories**方法來擷取使用 LINQ 的類別目錄的清單。</span><span class="sxs-lookup"><span data-stu-id="413f1-216">Create a **GetCategories** method to retrieve the list of categories using LINQ.</span></span> <span data-ttu-id="413f1-217">查詢將包括**產品**屬性，好 GridView 可以顯示每個類別目錄的產品數量。</span><span class="sxs-lookup"><span data-stu-id="413f1-217">The query will include the **Products** property so the GridView can show the amount of products for each category.</span></span> <span data-ttu-id="413f1-218">請注意，方法會傳回未經處理的 IQueryable 物件，代表要查詢之後執行網頁生命週期。</span><span class="sxs-lookup"><span data-stu-id="413f1-218">Notice that the method returns a raw IQueryable object that represent the query to be executed later on the page lifecycle.</span></span>
    ...
    <span data-ttu-id="413f1-219">(程式碼片段- *Web Form 實驗室-Ex01-GetCategories*)</span><span class="sxs-lookup"><span data-stu-id="413f1-219">(Code Snippet - *Web Forms Lab - Ex01 - GetCategories*)</span></span>
...
<span data-ttu-id="413f1-288">在先前工作中，您已使用主要用於選取資料的模型繫結，在這項工作中，您將學習如何在更新作業中使用模型繫結。</span><span class="sxs-lookup"><span data-stu-id="413f1-288">In the previous task, you have used model binding mainly for selecting data, in this task you will learn how to use model binding in update operations.</span></span>
...
2. <span data-ttu-id="413f1-396">停止偵錯。</span><span class="sxs-lookup"><span data-stu-id="413f1-396">Stop debugging.</span></span> <span data-ttu-id="413f1-397">在**Products.aspx.cs**程式碼後置檔案，更新**UpdateCategory**方法以處理 db 所擲回的例外狀況。Savechanges （） 方法呼叫，並加入錯誤**ModelState**物件。</span><span class="sxs-lookup"><span data-stu-id="413f1-397">In the **Products.aspx.cs** code-behind file, update the **UpdateCategory** method to handle the exceptions thrown by the db.SaveChanges() method call and add an error to the **ModelState** object.</span></span>
    ...
    <span data-ttu-id="413f1-399">(程式碼片段- *Web Form 實驗室-Ex02-UpdateCategory 處理錯誤*)</span><span class="sxs-lookup"><span data-stu-id="413f1-399">(Code Snippet - *Web Forms Lab - Ex02 - UpdateCategory Handle Errors*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample27.cs)]

    > [!NOTE]
    > <span data-ttu-id="413f1-400">在理想情況下，您必須找出 DbUpdateException 的原因，並檢查的根本原因是否為唯一的索引鍵條件約束違規。</span><span class="sxs-lookup"><span data-stu-id="413f1-400">Ideally, you would have to identify the cause of the DbUpdateException and check if the root cause is the violation of a unique key constraint.</span></span>
3. <span data-ttu-id="413f1-401">開啟**Products.aspx**並加入**Aggregate**類別 GridView 以顯示模型錯誤清單 下的控制項。</span><span class="sxs-lookup"><span data-stu-id="413f1-401">Open **Products.aspx** and add a **ValidationSummary** control below the categories GridView to show the list of model errors.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample28.aspx)]
4. <span data-ttu-id="413f1-402">執行網站，並移至 [產品] 頁面。</span><span class="sxs-lookup"><span data-stu-id="413f1-402">Run the site and go to the Products page.</span></span> <span data-ttu-id="413f1-403">嘗試更新類別目錄使用重複的值的名稱。</span><span class="sxs-lookup"><span data-stu-id="413f1-403">Try to update the name of a category using an duplicated value.</span></span>

    <span data-ttu-id="413f1-404">請注意，已處理的例外狀況和錯誤訊息會出現在**Aggregate**控制項。</span><span class="sxs-lookup"><span data-stu-id="413f1-404">Notice that the exception was handled and the error message appears in the **ValidationSummary** control.</span></span>

    <span data-ttu-id="413f1-405">![重複的類別目錄錯誤](whats-new-in-web-forms-in-aspnet-45/_static/image17.png "重複類別目錄錯誤")</span><span class="sxs-lookup"><span data-stu-id="413f1-405">![Duplicated category error](whats-new-in-web-forms-in-aspnet-45/_static/image17.png "Duplicated category error")</span></span>

    <span data-ttu-id="413f1-406">*重複的類別目錄錯誤*</span><span class="sxs-lookup"><span data-stu-id="413f1-406">*Duplicated category error*</span></span>

<a id="Task_4_-_Request_Validation_in_ASPNET_Web_Forms_45"></a>
#### <a name="task-4---request-validation-in-aspnet-web-forms-45"></a><span data-ttu-id="413f1-407">工作 4-要求的 ASP.NET Web Form 4.5 中的驗證</span><span class="sxs-lookup"><span data-stu-id="413f1-407">Task 4 - Request Validation in ASP.NET Web Forms 4.5</span></span>

<span data-ttu-id="413f1-408">在 ASP.NET 要求驗證的功能提供特定層級的預設保護，防止跨網站指令碼 (XSS) 攻擊。</span><span class="sxs-lookup"><span data-stu-id="413f1-408">The request validation feature in ASP.NET provides a certain level of default protection against cross-site scripting (XSS) attacks.</span></span> <span data-ttu-id="413f1-409">在舊版 ASP.NET 中，依預設會啟用要求驗證，並才可停用整個頁面。</span><span class="sxs-lookup"><span data-stu-id="413f1-409">In previous versions of ASP.NET, request validation was enabled by default and could only be disabled for an entire page.</span></span> <span data-ttu-id="413f1-410">使用新版本的 ASP.NET Web Form 您現在可以停用單一控制項的要求驗證、 執行延遲要求的驗證或存取 （務必小心，如果您這麼做 ！） 的未驗證的要求資料。</span><span class="sxs-lookup"><span data-stu-id="413f1-410">With the new version of ASP.NET Web Forms you can now disable the request validation for a single control, perform lazy request validation or access un-validated request data (be careful if you do so!).</span></span>

1. <span data-ttu-id="413f1-411">按**Ctrl + F5**啟動網站，但不偵錯，並移至 [產品] 頁面。</span><span class="sxs-lookup"><span data-stu-id="413f1-411">Press **Ctrl+F5** to start the site without debugging and go to the Products page.</span></span> <span data-ttu-id="413f1-412">選取類別目錄，然後按一下**編輯**連結上的任何產品。</span><span class="sxs-lookup"><span data-stu-id="413f1-412">Select a category and then click the **Edit** link on any of the products.</span></span>
2. <span data-ttu-id="413f1-413">輸入的描述包含有潛在危險的內容，例如包括 HTML 標記。</span><span class="sxs-lookup"><span data-stu-id="413f1-413">Type a description containing potentially dangerous content, for instance including HTML tags.</span></span> <span data-ttu-id="413f1-414">需要請注意，由於要求驗證擲回的例外狀況。</span><span class="sxs-lookup"><span data-stu-id="413f1-414">Take notice of the exception thrown due to the request validation.</span></span>

    <span data-ttu-id="413f1-415">![編輯具有潛在危險的內容之產品的](whats-new-in-web-forms-in-aspnet-45/_static/image18.png "編輯產品有潛在危險的內容")</span><span class="sxs-lookup"><span data-stu-id="413f1-415">![Editing a product with potentially dangerous content](whats-new-in-web-forms-in-aspnet-45/_static/image18.png "Editing a product with potentially dangerous content")</span></span>

    <span data-ttu-id="413f1-416">*編輯具有潛在危險的內容的產品*</span><span class="sxs-lookup"><span data-stu-id="413f1-416">*Editing a product with potentially dangerous content*</span></span>

    <span data-ttu-id="413f1-417">![因為要求的驗證所以擲回例外狀況](whats-new-in-web-forms-in-aspnet-45/_static/image19.png "因為要求的驗證所以擲回例外狀況")</span><span class="sxs-lookup"><span data-stu-id="413f1-417">![Exception thrown due to request validation](whats-new-in-web-forms-in-aspnet-45/_static/image19.png "Exception thrown due to request validation")</span></span>

    <span data-ttu-id="413f1-418">*因為要求的驗證所以擲回例外狀況*</span><span class="sxs-lookup"><span data-stu-id="413f1-418">*Exception thrown due to request validation*</span></span>
3. <span data-ttu-id="413f1-419">關閉網頁，並在 Visual Studio 中，按**SHIFT + F5**停止偵錯。</span><span class="sxs-lookup"><span data-stu-id="413f1-419">Close the page and, in Visual Studio, press **SHIFT+F5** to stop debugging.</span></span>
4. <span data-ttu-id="413f1-420">開啟**ProductDetails.aspx**頁面上，並找出**描述**文字方塊。</span><span class="sxs-lookup"><span data-stu-id="413f1-420">Open the **ProductDetails.aspx** page and locate the **Description** TextBox.</span></span>
5. <span data-ttu-id="413f1-421">加入新**ValidateRequestMode**文字方塊中的屬性並將其值設定為**已停用**。</span><span class="sxs-lookup"><span data-stu-id="413f1-421">Add the new **ValidateRequestMode** property to the TextBox and set its value to **Disabled**.</span></span>

    <span data-ttu-id="413f1-422">新**ValidateRequestMode**屬性可讓您停用每個控制項精細的要求驗證。</span><span class="sxs-lookup"><span data-stu-id="413f1-422">The new **ValidateRequestMode** attribute allows you to disable the request validation granularly on each control.</span></span> <span data-ttu-id="413f1-423">當您想要使用的輸入，可能會收到 HTML 程式碼，但想要保留頁面的其餘部分所使用的驗證，這非常有用。</span><span class="sxs-lookup"><span data-stu-id="413f1-423">This is useful when you want to use an input that may receive HTML code, but want to keep the validation working for the rest of the page.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample29.aspx)]
6. <span data-ttu-id="413f1-424">按**F5**執行 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="413f1-424">Press **F5** to run the web application.</span></span> <span data-ttu-id="413f1-425">再次開啟編輯產品頁面，並完成產品描述，包括 HTML 標記。</span><span class="sxs-lookup"><span data-stu-id="413f1-425">Open the edit product page again and complete a product description including HTML tags.</span></span> <span data-ttu-id="413f1-426">請注意，您現在可以將 HTML 內容加入描述。</span><span class="sxs-lookup"><span data-stu-id="413f1-426">Notice that you can now add HTML content to the description.</span></span>

    <span data-ttu-id="413f1-427">![要求驗證停用產品描述](whats-new-in-web-forms-in-aspnet-45/_static/image20.png "要求驗證停用產品描述")</span><span class="sxs-lookup"><span data-stu-id="413f1-427">![Request validation disabled for the product description](whats-new-in-web-forms-in-aspnet-45/_static/image20.png "Request validation disabled for the product description")</span></span>

    <span data-ttu-id="413f1-428">*產品說明停用要求驗證*</span><span class="sxs-lookup"><span data-stu-id="413f1-428">*Request validation disabled for the product description*</span></span>

    > [!NOTE]
    > <span data-ttu-id="413f1-429">在實際執行應用程式中，您應該處理以確定只有安全的 HTML 標記的使用者所輸入的 HTML 程式碼 (例如，有任何&lt;指令碼&gt;標記)。</span><span class="sxs-lookup"><span data-stu-id="413f1-429">In a production application, you should sanitize the HTML code entered by the user to make sure only safe HTML tags are entered (for example, there are no &lt;script&gt; tags).</span></span> <span data-ttu-id="413f1-430">若要這樣做，您可以使用[Microsoft Web 保護文件庫](https://www.nuget.org/packages/AntiXSS)。</span><span class="sxs-lookup"><span data-stu-id="413f1-430">To do this, you can use [Microsoft Web Protection Library](https://www.nuget.org/packages/AntiXSS).</span></span>
7. <span data-ttu-id="413f1-431">再次編輯產品。</span><span class="sxs-lookup"><span data-stu-id="413f1-431">Edit the product again.</span></span> <span data-ttu-id="413f1-432">在 [名稱] 欄位中輸入的 HTML 程式碼，然後按一下**儲存**。</span><span class="sxs-lookup"><span data-stu-id="413f1-432">Type HTML code in the Name field and click **Save**.</span></span> <span data-ttu-id="413f1-433">請注意，要求驗證只會停用 [描述] 欄位與其餘 re 欄位仍驗證有潛在危險的內容。</span><span class="sxs-lookup"><span data-stu-id="413f1-433">Notice that Request Validation is only disabled for the Description field and the rest of the fields re still validated against the potentially dangerous content.</span></span>

    <span data-ttu-id="413f1-434">![要求其他的欄位中啟用驗證](whats-new-in-web-forms-in-aspnet-45/_static/image21.png "要求其他的欄位中啟用驗證")</span><span class="sxs-lookup"><span data-stu-id="413f1-434">![Request validation enabled in the rest of the fields](whats-new-in-web-forms-in-aspnet-45/_static/image21.png "Request validation enabled in the rest of the fields")</span></span>

    <span data-ttu-id="413f1-435">*其餘欄位中啟用要求驗證*</span><span class="sxs-lookup"><span data-stu-id="413f1-435">*Request validation enabled in the rest of the fields*</span></span>

    <span data-ttu-id="413f1-436">ASP.NET Web Form 4.5 包含新的要求驗證模式來執行要求的驗證延遲的方式。</span><span class="sxs-lookup"><span data-stu-id="413f1-436">ASP.NET Web Forms 4.5 includes a new request validation mode to perform request validation lazily.</span></span> <span data-ttu-id="413f1-437">要求的驗證模式設定為與**4.5**，如果一段程式碼存取*Request.Form [&quot;金鑰&quot;]*，ASP.NET 4.5 要求驗證就會只觸發要求驗證表單集合中特定項目。</span><span class="sxs-lookup"><span data-stu-id="413f1-437">With the request validation mode set to **4.5**, if a piece of code accesses *Request.Form[&quot;key&quot;]*, ASP.NET 4.5's request validation will only trigger request validation for that specific element in the form collection.</span></span>

    <span data-ttu-id="413f1-438">此外，ASP.NET 4.5 現在會包含從 Microsoft 反 XSS 文件庫 v4.0 核心編碼常式。</span><span class="sxs-lookup"><span data-stu-id="413f1-438">Additionally, ASP.NET 4.5 now includes core encoding routines from the Microsoft Anti-XSS Library v4.0.</span></span> <span data-ttu-id="413f1-439">編碼常式由新反 XSS *AntiXssEncoder*找到在新的型別**System.Web.Security.AntiXss**命名空間。</span><span class="sxs-lookup"><span data-stu-id="413f1-439">The Anti-XSS encoding routines are implemented by the new *AntiXssEncoder* type found in the new **System.Web.Security.AntiXss** namespace.</span></span> <span data-ttu-id="413f1-440">與**encoderType**參數設定為使用*AntiXssEncoder*，所有輸出的編碼方式在 ASP.NET 中會自動使用新的編碼常式。</span><span class="sxs-lookup"><span data-stu-id="413f1-440">With the **encoderType** parameter configured to use *AntiXssEncoder*, all output encoding within ASP.NET automatically uses the new encoding routines.</span></span>
8. <span data-ttu-id="413f1-441">ASP.NET 4.5 要求驗證也支援未驗證的存取要求資料。</span><span class="sxs-lookup"><span data-stu-id="413f1-441">ASP.NET 4.5 request validation also supports un-validated access to request data.</span></span> <span data-ttu-id="413f1-442">ASP.NET 4.5，將新的集合屬性，以**HttpRequest**呼叫物件**Unvalidated**。</span><span class="sxs-lookup"><span data-stu-id="413f1-442">ASP.NET 4.5 adds a new collection property to the **HttpRequest** object called **Unvalidated**.</span></span> <span data-ttu-id="413f1-443">當您瀏覽到**HttpRequest.Unvalidated**您具有存取權的所有要求資料，包括表單、 QueryStrings、 Cookie、 Url、 等等的通用部分。</span><span class="sxs-lookup"><span data-stu-id="413f1-443">When you navigate into **HttpRequest.Unvalidated** you have access to all of the common pieces of request data, including Forms, QueryStrings, Cookies, URLs, and so on.</span></span>

    <span data-ttu-id="413f1-444">![Request.Unvalidated 物件](whats-new-in-web-forms-in-aspnet-45/_static/image22.png "Request.Unvalidated 物件")</span><span class="sxs-lookup"><span data-stu-id="413f1-444">![Request.Unvalidated object](whats-new-in-web-forms-in-aspnet-45/_static/image22.png "Request.Unvalidated object")</span></span>

    <span data-ttu-id="413f1-445">*Request.Unvalidated 物件*</span><span class="sxs-lookup"><span data-stu-id="413f1-445">*Request.Unvalidated object*</span></span>

    > [!NOTE]
    > <span data-ttu-id="413f1-446">**請謹慎使用 HttpRequest.Unvalidated 屬性 ！**</span><span class="sxs-lookup"><span data-stu-id="413f1-446">**Please use the HttpRequest.Unvalidated property with caution!**</span></span> <span data-ttu-id="413f1-447">請確定您仔細未經處理的要求資料，以確保危險的文字不往返並呈現察覺的客戶執行自訂驗證 ！</span><span class="sxs-lookup"><span data-stu-id="413f1-447">Make sure you carefully perform custom validation on the raw request data to ensure that dangerous text is not round-tripped and rendered back to unsuspecting customers!</span></span>

<a id="Exercise3"></a>

<a id="Exercise_3_Asynchronous_Page_Processing_in_ASPNET_Web_Forms"></a>
### <a name="exercise-3-asynchronous-page-processing-in-aspnet-web-forms"></a><span data-ttu-id="413f1-448">練習 3： 非同步網頁處理中 ASP.NET Web Form</span><span class="sxs-lookup"><span data-stu-id="413f1-448">Exercise 3: Asynchronous Page Processing in ASP.NET Web Forms</span></span>

<span data-ttu-id="413f1-449">在此練習中，您將介紹新的非同步頁面處理 ASP.NET Web Form 中的功能。</span><span class="sxs-lookup"><span data-stu-id="413f1-449">In this exercise, you will be introduced to the new asynchronous page processing features in ASP.NET Web Forms.</span></span>

<a id="Task_1_-_Updating_the_Product_Details_Page_to_Upload_and_Show_Images"></a>
#### <a name="task-1---updating-the-product-details-page-to-upload-and-show-images"></a><span data-ttu-id="413f1-450">工作 1-更新產品詳細資料頁面，即可上傳和顯示的影像</span><span class="sxs-lookup"><span data-stu-id="413f1-450">Task 1 - Updating the Product Details Page to Upload and Show Images</span></span>

<span data-ttu-id="413f1-451">在這項工作，您將更新，讓使用者可指定產品的影像 URL，並顯示在唯讀狀態檢視中的產品詳細資料頁面。</span><span class="sxs-lookup"><span data-stu-id="413f1-451">In this task, you will update the product details page to allow the user to specify an image URL for the product and display it in the read-only view.</span></span> <span data-ttu-id="413f1-452">您將以同步方式下載並建立指定的映像的本機副本。</span><span class="sxs-lookup"><span data-stu-id="413f1-452">You will create a local copy of the specified image by downloading it synchronously.</span></span> <span data-ttu-id="413f1-453">在下一個工作中，您將會更新此實作，讓它以非同步方式運作。</span><span class="sxs-lookup"><span data-stu-id="413f1-453">In the next task, you will update this implementation to make it work asynchronously.</span></span>

1. <span data-ttu-id="413f1-454">開啟**Visual Studio 2012**並載入**開始**方案位於**Source\Ex3 Async\Begin**從這個實驗室中的資料夾。</span><span class="sxs-lookup"><span data-stu-id="413f1-454">Open **Visual Studio 2012** and load the **Begin** solution located in **Source\Ex3-Async\Begin** from this lab's folder.</span></span> <span data-ttu-id="413f1-455">或者，您可以繼續工作，在您現有的方案，從先前的工作。</span><span class="sxs-lookup"><span data-stu-id="413f1-455">Alternatively, you can continue working on your existing solution from the previous exercises.</span></span>

   1. <span data-ttu-id="413f1-456">如果您開啟提供**開始**方案，您必須下載某些遺漏的 NuGet 套件才能繼續。</span><span class="sxs-lookup"><span data-stu-id="413f1-456">If you opened the provided **Begin** solution, you will need to download some missing NuGet packages before continue.</span></span> <span data-ttu-id="413f1-457">若要這樣做，請在 [方案總管] 中，按一下**WebFormsLab**專案，然後選取**管理 NuGet 封裝**。</span><span class="sxs-lookup"><span data-stu-id="413f1-457">To do this, in the Solution Explorer, click the **WebFormsLab** project and select **Manage NuGet Packages**.</span></span>
   2. <span data-ttu-id="413f1-458">在**管理 NuGet 封裝**] 對話方塊中，按一下 [**還原**才能下載遺漏的封裝。</span><span class="sxs-lookup"><span data-stu-id="413f1-458">In the **Manage NuGet Packages** dialog, click **Restore** in order to download missing packages.</span></span>
   3. <span data-ttu-id="413f1-459">最後，按一下 建置方案**建置** | **建置方案**。</span><span class="sxs-lookup"><span data-stu-id="413f1-459">Finally, build the solution by clicking **Build** | **Build Solution**.</span></span>

      > [!NOTE]
      > <span data-ttu-id="413f1-460">使用 NuGet 的優點之一是您不需要在專案中，所有的程式庫的出貨減少專案大小。</span><span class="sxs-lookup"><span data-stu-id="413f1-460">One of the advantages of using NuGet is that you don't have to ship all the libraries in your project, reducing the project size.</span></span> <span data-ttu-id="413f1-461">NuGet 的強大工具，請藉由指定封裝版本在 Packages.config 檔案中，您將會成功下載所有必要的程式庫第一次您執行專案。</span><span class="sxs-lookup"><span data-stu-id="413f1-461">With NuGet Power Tools, by specifying the package versions in the Packages.config file, you will be able to download all the required libraries the first time you run the project.</span></span> <span data-ttu-id="413f1-462">這就是為什麼您必須從這個實驗室中開啟現有的方案後執行這些步驟。</span><span class="sxs-lookup"><span data-stu-id="413f1-462">This is why you will have to run these steps after you open an existing solution from this lab.</span></span>
2. <span data-ttu-id="413f1-463">開啟**ProductDetails.aspx**頁面上的來源和加入欄位以顯示產品映像在 FormView 的 ItemTemplate 中。</span><span class="sxs-lookup"><span data-stu-id="413f1-463">Open the **ProductDetails.aspx** page source and add a field in the FormView's ItemTemplate to show the product image.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample30.aspx)]
3. <span data-ttu-id="413f1-464">加入欄位，以在 FormView 的 EditTemplate 中指定的影像 URL。</span><span class="sxs-lookup"><span data-stu-id="413f1-464">Add a field to specify the image URL in the FormView's EditTemplate.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample31.aspx)]
4. <span data-ttu-id="413f1-465">開啟**ProductDetails.aspx.cs**程式碼後置檔案，然後加入下列命名空間指示詞。</span><span class="sxs-lookup"><span data-stu-id="413f1-465">Open the **ProductDetails.aspx.cs** code-behind file and add the following namespace directives.</span></span>

    <span data-ttu-id="413f1-466">(程式碼片段- *Web Form 實驗室-Ex03-命名空間*)</span><span class="sxs-lookup"><span data-stu-id="413f1-466">(Code Snippet - *Web Forms Lab - Ex03 - Namespaces*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample32.cs)]
5. <span data-ttu-id="413f1-467">建立**UpdateProductImage**方法，可儲存在本機的遠端影像**映像**資料夾，然後更新 「 產品 」 實體與新的映像位置值。</span><span class="sxs-lookup"><span data-stu-id="413f1-467">Create an **UpdateProductImage** method to store remote images in the local **Images** folder and update the product entity with the new image location value.</span></span>

    <span data-ttu-id="413f1-468">(程式碼片段- *Web Form 實驗室-Ex03-UpdateProductImage*)</span><span class="sxs-lookup"><span data-stu-id="413f1-468">(Code Snippet - *Web Forms Lab - Ex03 - UpdateProductImage*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample33.cs)]
6. <span data-ttu-id="413f1-469">更新**UpdateProduct**方法呼叫**UpdateProductImage**方法。</span><span class="sxs-lookup"><span data-stu-id="413f1-469">Update the **UpdateProduct** method to call the **UpdateProductImage** method.</span></span>

    <span data-ttu-id="413f1-470">(程式碼片段- *Web Form 實驗室-Ex03-UpdateProductImage 呼叫*)</span><span class="sxs-lookup"><span data-stu-id="413f1-470">(Code Snippet - *Web Forms Lab - Ex03 - UpdateProductImage Call*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample34.cs)]
7. <span data-ttu-id="413f1-471">執行應用程式，然後嘗試上傳產品的影像。</span><span class="sxs-lookup"><span data-stu-id="413f1-471">Run the application and try to upload an image for a product.</span></span> <span data-ttu-id="413f1-472">例如，您可以使用下列的影像 URL，從 Office 剪輯藝術： [[http://officeimg.vo.msecnd.net/images/MB900437099.jpg](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)</span><span class="sxs-lookup"><span data-stu-id="413f1-472">For example, you can use the following image URL from Office Clip Arts: [[http://officeimg.vo.msecnd.net/images/MB900437099.jpg](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)](http://officeimg.vo.msecnd.net/images/MB900437099.jpg)</span></span>

    <span data-ttu-id="413f1-473">![設定產品的影像](whats-new-in-web-forms-in-aspnet-45/_static/image23.png "設定產品的影像")</span><span class="sxs-lookup"><span data-stu-id="413f1-473">![Setting an image for a product](whats-new-in-web-forms-in-aspnet-45/_static/image23.png "Setting an image for a product")</span></span>

    <span data-ttu-id="413f1-474">*設定產品的影像*</span><span class="sxs-lookup"><span data-stu-id="413f1-474">*Setting an image for a product*</span></span>

<a id="Task_2_-_Adding_Asynchronous_Processing_to_the_Product_Details_Page"></a>
#### <a name="task-2---adding-asynchronous-processing-to-the-product-details-page"></a><span data-ttu-id="413f1-475">工作 2-新增非同步處理對產品詳細資料頁面</span><span class="sxs-lookup"><span data-stu-id="413f1-475">Task 2 - Adding Asynchronous Processing to the Product Details Page</span></span>

<span data-ttu-id="413f1-476">在這項工作，您將更新的產品詳細資料頁面，可以讓它以非同步方式運作。</span><span class="sxs-lookup"><span data-stu-id="413f1-476">In this task, you will update the product details page to make it work asynchronously.</span></span> <span data-ttu-id="413f1-477">您會使用 ASP.NET 4.5 非同步網頁處理來強化-映像下載程序-長時間執行工作。</span><span class="sxs-lookup"><span data-stu-id="413f1-477">You will enhance a long running task - the image download process - by using ASP.NET 4.5 asynchronous page processing.</span></span>

<span data-ttu-id="413f1-478">Web 應用程式中的非同步方法可以用來最佳化 ASP.NET 執行緒集區所使用的方式。</span><span class="sxs-lookup"><span data-stu-id="413f1-478">Asynchronous methods in web applications can be used to optimize the way ASP.NET thread pools are used.</span></span> <span data-ttu-id="413f1-479">在那里的 ASP.NET 中會有限的數目的參與執行緒集區執行緒的要求，因此，在所有執行緒都都忙碌中，ASP.NET，開始拒絕新要求、 傳送應用程式錯誤訊息並將您的站台無法使用。</span><span class="sxs-lookup"><span data-stu-id="413f1-479">In ASP.NET there are a limited number of threads in the thread pool for attending requests, thus, when all the threads are busy, ASP.NET starts to reject new requests, sends application error messages and makes your site unavailable.</span></span>

<span data-ttu-id="413f1-480">這些耗時的作業，在您的網站是絕佳候選項目非同步程式設計，因為它們長時間佔用指派的執行緒。</span><span class="sxs-lookup"><span data-stu-id="413f1-480">Time-consuming operations on your web site are great candidates for asynchronous programming because they occupy the assigned thread for a long time.</span></span> <span data-ttu-id="413f1-481">這包括長時間執行的要求，具有許多不同的項目頁面，以及需要離線作業，這類查詢資料庫或存取外部 web 伺服器的頁面。</span><span class="sxs-lookup"><span data-stu-id="413f1-481">This includes long running requests, pages with lots of different elements and pages that require offline operations, such querying a database or accessing an external web server.</span></span> <span data-ttu-id="413f1-482">優點是，如果頁面正在處理時，您可以使用這些作業的非同步方法，執行緒會釋出，並傳回至執行緒集區，而且可用來處理新的頁面要求。</span><span class="sxs-lookup"><span data-stu-id="413f1-482">The advantage is that if you use asynchronous methods for these operations, while the page is processing, the thread is freed and returned to the thread pool and can be used to attend to a new page request.</span></span> <span data-ttu-id="413f1-483">這表示，網頁會開始處理執行緒集區的一個執行緒，並可能在非同步處理完成之後完成中的另一處理。</span><span class="sxs-lookup"><span data-stu-id="413f1-483">This means, the page will start processing in one thread from the thread pool and might complete processing in a different one, after the async processing completes.</span></span>

1. <span data-ttu-id="413f1-484">開啟**ProductDetails.aspx**頁面。</span><span class="sxs-lookup"><span data-stu-id="413f1-484">Open the **ProductDetails.aspx** page.</span></span> <span data-ttu-id="413f1-485">新增**非同步**屬性**頁面**項目並將它設定為**true**。</span><span class="sxs-lookup"><span data-stu-id="413f1-485">Add the **Async** attribute in the **Page** element and set it to **true**.</span></span> <span data-ttu-id="413f1-486">這個屬性會告知 ASP.NET 來實作 IHttpAsyncHandler 介面。</span><span class="sxs-lookup"><span data-stu-id="413f1-486">This attribute tells ASP.NET to implement the IHttpAsyncHandler interface.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample35.aspx)]
2. <span data-ttu-id="413f1-487">將標籤加入要顯示的頁面上執行的執行緒詳細資料頁面底部。</span><span class="sxs-lookup"><span data-stu-id="413f1-487">Add a Label at the bottom of the page to show the details of the threads running the page.</span></span>

    [!code-aspx[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample36.aspx)]
3. <span data-ttu-id="413f1-488">開啟  **ProductDetails.aspx.cs**並加入下列命名空間指示詞。</span><span class="sxs-lookup"><span data-stu-id="413f1-488">Open up **ProductDetails.aspx.cs** and add the following namespace directives.</span></span>

    <span data-ttu-id="413f1-489">(程式碼片段- *Web Form 實驗室-Ex03-命名空間 2*)</span><span class="sxs-lookup"><span data-stu-id="413f1-489">(Code Snippet - *Web Forms Lab - Ex03 - Namespaces 2*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample37.cs)]
4. <span data-ttu-id="413f1-490">修改**UpdateProductImage**下載影像的非同步工作的方法。</span><span class="sxs-lookup"><span data-stu-id="413f1-490">Modify the **UpdateProductImage** method to download the image with an asynchronous task.</span></span> <span data-ttu-id="413f1-491">您將會取代**WebClient** **DownloadFile**方法**DownloadFileTaskAsync**方法，並包含**await**關鍵字。</span><span class="sxs-lookup"><span data-stu-id="413f1-491">You will replace the **WebClient** **DownloadFile** method with the **DownloadFileTaskAsync** method and include the **await** keyword.</span></span>

    <span data-ttu-id="413f1-492">(程式碼片段- *Web Form 實驗室-Ex03-UpdateProductImage 非同步*)</span><span class="sxs-lookup"><span data-stu-id="413f1-492">(Code Snippet - *Web Forms Lab - Ex03 - UpdateProductImage Async*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample38.cs)]

    <span data-ttu-id="413f1-493">RegisterAsyncTask 註冊頁面非同步工作，以在不同的執行緒中執行。</span><span class="sxs-lookup"><span data-stu-id="413f1-493">The RegisterAsyncTask registers a new page asynchronous task to be executed in a different thread.</span></span> <span data-ttu-id="413f1-494">它會接收要執行的工作 (t) 的 lambda 運算式。</span><span class="sxs-lookup"><span data-stu-id="413f1-494">It receives a lambda expression with the Task (t) to be executed.</span></span> <span data-ttu-id="413f1-495">**Await**關鍵字**DownloadFileTaskAsync**方法轉換之後以非同步方式叫用的回呼方法的其餘部分**DownloadFileTaskAsync**方法已完成。</span><span class="sxs-lookup"><span data-stu-id="413f1-495">The **await** keyword in the **DownloadFileTaskAsync** method converts the remainder of the method into a callback that is invoked asynchronously after the **DownloadFileTaskAsync** method has completed.</span></span> <span data-ttu-id="413f1-496">ASP.NET 會繼續執行的方法，藉由自動維護所有的 HTTP 要求原始值。</span><span class="sxs-lookup"><span data-stu-id="413f1-496">ASP.NET will resume the execution of the method by automatically maintaining all the HTTP request original values.</span></span> <span data-ttu-id="413f1-497">.NET 4.5 中新的非同步程式設計模型可讓您撰寫非同步程式碼看起來很像同步程式碼，並讓編譯器處理複雜的回呼函式或接續程式碼。</span><span class="sxs-lookup"><span data-stu-id="413f1-497">The new asynchronous programming model in .NET 4.5 enables you to write asynchronous code that looks very much like synchronous code, and let the compiler handle the complications of callback functions or continuation code.</span></span>
    > [!NOTE]
    > <span data-ttu-id="413f1-498">RegisterAsyncTask 和 PageAsyncTask 已經可以使用.NET 2.0 後。</span><span class="sxs-lookup"><span data-stu-id="413f1-498">RegisterAsyncTask and PageAsyncTask were already available since .NET 2.0.</span></span> <span data-ttu-id="413f1-499">Await 關鍵字是從.NET 4.5 非同步程式設計模型中新增，而且可用新 TaskAsync 方法從.NET WebClient 物件。</span><span class="sxs-lookup"><span data-stu-id="413f1-499">The await keyword is new from the .NET 4.5 asynchronous programming model and can be used together with the new TaskAsync methods from the .NET WebClient object.</span></span>
5. <span data-ttu-id="413f1-500">加入程式碼以顯示的執行緒所在的程式碼啟動並完成執行。</span><span class="sxs-lookup"><span data-stu-id="413f1-500">Add code to display the threads on which the code started and finished executing.</span></span> <span data-ttu-id="413f1-501">若要這樣做，請更新**UpdateProductImage**方法取代下列程式碼。</span><span class="sxs-lookup"><span data-stu-id="413f1-501">To do this, update the **UpdateProductImage** method with the following code.</span></span>

    <span data-ttu-id="413f1-502">(程式碼片段- *Web Form 實驗室-Ex03-顯示執行緒*)</span><span class="sxs-lookup"><span data-stu-id="413f1-502">(Code Snippet - *Web Forms Lab - Ex03 - Show threads*)</span></span>

    [!code-csharp[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample39.cs)]
6. <span data-ttu-id="413f1-503">開啟網站的**Web.config**檔案。</span><span class="sxs-lookup"><span data-stu-id="413f1-503">Open the web site's **Web.config** file.</span></span> <span data-ttu-id="413f1-504">加入下列的 appsetting owin： 變數。</span><span class="sxs-lookup"><span data-stu-id="413f1-504">Add the following appSetting variable.</span></span>

    [!code-xml[Main](whats-new-in-web-forms-in-aspnet-45/samples/sample40.xml)]
7. <span data-ttu-id="413f1-505">按**F5**執行應用程式，並上傳映像的產品。</span><span class="sxs-lookup"><span data-stu-id="413f1-505">Press **F5** to run the application and upload an image for the product.</span></span> <span data-ttu-id="413f1-506">請注意，開始及完成的程式碼可能會不同的執行緒識別碼。</span><span class="sxs-lookup"><span data-stu-id="413f1-506">Notice the threads ID where the code started and finished may be different.</span></span> <span data-ttu-id="413f1-507">這是因為非同步工作的不同執行緒上執行 ASP.NET 執行緒集區。</span><span class="sxs-lookup"><span data-stu-id="413f1-507">This is because asynchronous tasks run on a separate thread from ASP.NET thread pool.</span></span> <span data-ttu-id="413f1-508">工作完成時，ASP.NET 會將工作放回佇列中，並指派任何可用的執行緒。</span><span class="sxs-lookup"><span data-stu-id="413f1-508">When the task completes, ASP.NET puts the task back in the queue and assigns any of the available threads.</span></span>

    <span data-ttu-id="413f1-509">![以非同步方式下載映像](whats-new-in-web-forms-in-aspnet-45/_static/image24.png "非同步下載映像")</span><span class="sxs-lookup"><span data-stu-id="413f1-509">![Downloading an image asynchronously](whats-new-in-web-forms-in-aspnet-45/_static/image24.png "Downloading an image asynchronously")</span></span>

    <span data-ttu-id="413f1-510">*以非同步方式下載映像*</span><span class="sxs-lookup"><span data-stu-id="413f1-510">*Downloading an image asynchronously*</span></span>

> [!NOTE]
> <span data-ttu-id="413f1-511">此外，您可以部署此應用程式以 Azure 下列[附錄 b： 發佈 ASP.NET MVC 4 應用程式使用 Web Deploy](#AppendixB)。</span><span class="sxs-lookup"><span data-stu-id="413f1-511">Additionally, you can deploy this application to Azure following [Appendix B: Publishing an ASP.NET MVC 4 Application using Web Deploy](#AppendixB).</span></span>


* * *

<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="413f1-512">總結</span><span class="sxs-lookup"><span data-stu-id="413f1-512">Summary</span></span>

<span data-ttu-id="413f1-513">在這個實際操作實驗室中，下列概念有已經定址，並且示範：</span><span class="sxs-lookup"><span data-stu-id="413f1-513">In this hands-on lab, the following concepts have been addressed and demonstrated:</span></span>

- <span data-ttu-id="413f1-514">使用強型別資料繫結運算式</span><span class="sxs-lookup"><span data-stu-id="413f1-514">Use strongly-typed data-binding expressions</span></span>
- <span data-ttu-id="413f1-515">Web Form 中使用新的模型繫結功能</span><span class="sxs-lookup"><span data-stu-id="413f1-515">Use new model binding features in Web Forms</span></span>
- <span data-ttu-id="413f1-516">值提供者使用的頁面資料對應到程式碼後置方法</span><span class="sxs-lookup"><span data-stu-id="413f1-516">Use value providers for mapping page data to code-behind methods</span></span>
- <span data-ttu-id="413f1-517">使用資料註解的使用者輸入驗證</span><span class="sxs-lookup"><span data-stu-id="413f1-517">Use Data Annotations for user input validation</span></span>
- <span data-ttu-id="413f1-518">採取 advange 與 jQuery unobstrusive 用戶端驗證的 Web Form 中</span><span class="sxs-lookup"><span data-stu-id="413f1-518">Take advange of unobstrusive client-side validation with jQuery in Web Forms</span></span>
- <span data-ttu-id="413f1-519">實作詳細的要求驗證</span><span class="sxs-lookup"><span data-stu-id="413f1-519">Implement granular request validation</span></span>
- <span data-ttu-id="413f1-520">實作非同步處理 Web Form 中的頁面</span><span class="sxs-lookup"><span data-stu-id="413f1-520">Implement asynchronous page processing in Web Forms</span></span>

<a id="AppendixA"></a>

<a id="Appendix_A_Installing_Visual_Studio_Express_2012_for_Web"></a>
## <a name="appendix-a-installing-visual-studio-express-2012-for-web"></a><span data-ttu-id="413f1-521">附錄 a： 安裝 Visual Studio Express 2012 for Web</span><span class="sxs-lookup"><span data-stu-id="413f1-521">Appendix A: Installing Visual Studio Express 2012 for Web</span></span>

<span data-ttu-id="413f1-522">您可以安裝**Microsoft Visual Studio Express 2012 for Web**或另一個&quot;Express&quot;版本使用**[Microsoft Web Platform Installer](https://www.microsoft.com/web/downloads/platform.aspx)**.</span><span class="sxs-lookup"><span data-stu-id="413f1-522">You can install **Microsoft Visual Studio Express 2012 for Web** or another &quot;Express&quot; version using the **[Microsoft Web Platform Installer](https://www.microsoft.com/web/downloads/platform.aspx)**.</span></span> <span data-ttu-id="413f1-523">下列指示將引導您逐步完成安裝所需*Visual studio Express 2012 for Web*使用*Microsoft Web Platform Installer*。</span><span class="sxs-lookup"><span data-stu-id="413f1-523">The following instructions guide you through the steps required to install *Visual studio Express 2012 for Web* using *Microsoft Web Platform Installer*.</span></span>

1. <span data-ttu-id="413f1-524">移至[ [ https://go.microsoft.com/?linkid=9810169 ](https://go.microsoft.com/?linkid=9810169) ](https://go.microsoft.com/?linkid=9810169)。</span><span class="sxs-lookup"><span data-stu-id="413f1-524">Go to [[https://go.microsoft.com/?linkid=9810169](https://go.microsoft.com/?linkid=9810169)](https://go.microsoft.com/?linkid=9810169).</span></span> <span data-ttu-id="413f1-525">或者，如果您已安裝 Web Platform Installer，您可以開啟它，並搜尋產品&quot; <em>Visual Studio Express 2012 for Web 與 Azure SDK</em>&quot;。</span><span class="sxs-lookup"><span data-stu-id="413f1-525">Alternatively, if you already have installed Web Platform Installer, you can open it and search for the product &quot;<em>Visual Studio Express 2012 for Web with Azure SDK</em>&quot;.</span></span>
2. <span data-ttu-id="413f1-526">按一下**立即安裝**。</span><span class="sxs-lookup"><span data-stu-id="413f1-526">Click on **Install Now**.</span></span> <span data-ttu-id="413f1-527">如果您不需要**Web Platform Installer**您會重新導向至下載並安裝第一次。</span><span class="sxs-lookup"><span data-stu-id="413f1-527">If you do not have **Web Platform Installer** you will be redirected to download and install it first.</span></span>
3. <span data-ttu-id="413f1-528">一次**Web Platform Installer**開啟時，按一下 **安裝**，啟動安裝程式。</span><span class="sxs-lookup"><span data-stu-id="413f1-528">Once **Web Platform Installer** is open, click **Install** to start the setup.</span></span>

    <span data-ttu-id="413f1-529">![安裝 Visual Studio Express](whats-new-in-web-forms-in-aspnet-45/_static/image25.png "安裝 Visual Studio Express")</span><span class="sxs-lookup"><span data-stu-id="413f1-529">![Install Visual Studio Express](whats-new-in-web-forms-in-aspnet-45/_static/image25.png "Install Visual Studio Express")</span></span>

    <span data-ttu-id="413f1-530">*安裝 Visual Studio Express*</span><span class="sxs-lookup"><span data-stu-id="413f1-530">*Install Visual Studio Express*</span></span>
4. <span data-ttu-id="413f1-531">閱讀所有產品的授權和詞彙，然後按一下**我接受**才能繼續。</span><span class="sxs-lookup"><span data-stu-id="413f1-531">Read all the products' licenses and terms and click **I Accept** to continue.</span></span>

    ![接受授權條款](whats-new-in-web-forms-in-aspnet-45/_static/image26.png)

    <span data-ttu-id="413f1-533">*接受授權條款*</span><span class="sxs-lookup"><span data-stu-id="413f1-533">*Accepting the license terms*</span></span>
5. <span data-ttu-id="413f1-534">等待直到完成下載和安裝程序。</span><span class="sxs-lookup"><span data-stu-id="413f1-534">Wait until the downloading and installation process completes.</span></span>

    ![安裝進度](whats-new-in-web-forms-in-aspnet-45/_static/image27.png)

    <span data-ttu-id="413f1-536">*安裝進度*</span><span class="sxs-lookup"><span data-stu-id="413f1-536">*Installation progress*</span></span>
6. <span data-ttu-id="413f1-537">當安裝完成時，按一下**完成**。</span><span class="sxs-lookup"><span data-stu-id="413f1-537">When the installation completes, click **Finish**.</span></span>

    ![安裝已完成](whats-new-in-web-forms-in-aspnet-45/_static/image28.png)

    <span data-ttu-id="413f1-539">*安裝已完成*</span><span class="sxs-lookup"><span data-stu-id="413f1-539">*Installation completed*</span></span>
7. <span data-ttu-id="413f1-540">按一下**結束**關閉 Web Platform Installer。</span><span class="sxs-lookup"><span data-stu-id="413f1-540">Click **Exit** to close Web Platform Installer.</span></span>
8. <span data-ttu-id="413f1-541">若要開啟 Visual Studio Express for Web，請移至**啟動**畫面上，並開始書寫&quot; **VS Express**&quot;，然後按一下  **VS Express for Web**並排顯示。</span><span class="sxs-lookup"><span data-stu-id="413f1-541">To open Visual Studio Express for Web, go to the **Start** screen and start writing &quot;**VS Express**&quot;, then click on the **VS Express for Web** tile.</span></span>

    ![VS Express for Web 方塊](whats-new-in-web-forms-in-aspnet-45/_static/image29.png)

    <span data-ttu-id="413f1-543">*VS Express for Web 方塊*</span><span class="sxs-lookup"><span data-stu-id="413f1-543">*VS Express for Web tile*</span></span>

<a id="AppendixB"></a>

<a id="Appendix_B_Publishing_an_ASPNET_MVC_4_Application_using_Web_Deploy"></a>
## <a name="appendix-b-publishing-an-aspnet-mvc-4-application-using-web-deploy"></a><span data-ttu-id="413f1-544">附錄 b： 發佈 ASP.NET MVC 4 應用程式使用 Web Deploy</span><span class="sxs-lookup"><span data-stu-id="413f1-544">Appendix B: Publishing an ASP.NET MVC 4 Application using Web Deploy</span></span>

<span data-ttu-id="413f1-545">此附錄將說明如何從 Azure 入口網站建立新的網站和發行您取得依照實驗室中，利用 Web Deploy 發行功能的 Azure 所提供的應用程式。</span><span class="sxs-lookup"><span data-stu-id="413f1-545">This appendix will show you how to create a new web site from the Azure Portal and publish the application you obtained by following the lab, taking advantage of the Web Deploy publishing feature provided by Azure.</span></span>

<a id="ApxBTask1"></a>

<a id="Task_1_-_Creating_a_New_Web_Site_from_the_Windows_Azure_Portal"></a>
#### <a name="task-1---creating-a-new-web-site-from-the-azure-portal"></a><span data-ttu-id="413f1-546">工作 1-從 Azure 入口網站建立新的網站</span><span class="sxs-lookup"><span data-stu-id="413f1-546">Task 1 - Creating a New Web Site from the Azure Portal</span></span>

1. <span data-ttu-id="413f1-547">移至[Azure 管理入口網站](https://manage.windowsazure.com/)並使用與您訂用帳戶相關聯的 Microsoft 認證登入。</span><span class="sxs-lookup"><span data-stu-id="413f1-547">Go to the [Azure Management Portal](https://manage.windowsazure.com/) and sign in using the Microsoft credentials associated with your subscription.</span></span>

    > [!NOTE]
    > <span data-ttu-id="413f1-548">使用 Azure 中，您可以免費裝載 10 個 ASP.NET 網站並再擴充隨流量成長。</span><span class="sxs-lookup"><span data-stu-id="413f1-548">With Azure you can host 10 ASP.NET Web Sites for free and then scale as your traffic grows.</span></span> <span data-ttu-id="413f1-549">您可以註冊[這裡](http://aka.ms/aspnet-hol-azure)。</span><span class="sxs-lookup"><span data-stu-id="413f1-549">You can sign up [here](http://aka.ms/aspnet-hol-azure).</span></span>

    <span data-ttu-id="413f1-550">![登入 Windows Azure 入口網站](whats-new-in-web-forms-in-aspnet-45/_static/image30.png "登入 Windows Azure 入口網站")</span><span class="sxs-lookup"><span data-stu-id="413f1-550">![Log on to Windows Azure portal](whats-new-in-web-forms-in-aspnet-45/_static/image30.png "Log on to Windows Azure portal")</span></span>

    <span data-ttu-id="413f1-551">*登入入口網站*</span><span class="sxs-lookup"><span data-stu-id="413f1-551">*Log on to the Portal*</span></span>
2. <span data-ttu-id="413f1-552">按一下**新增**命令列上。</span><span class="sxs-lookup"><span data-stu-id="413f1-552">Click **New** on the command bar.</span></span>

    <span data-ttu-id="413f1-553">![建立新的網站](whats-new-in-web-forms-in-aspnet-45/_static/image31.png "建立新的網站")</span><span class="sxs-lookup"><span data-stu-id="413f1-553">![Creating a new Web Site](whats-new-in-web-forms-in-aspnet-45/_static/image31.png "Creating a new Web Site")</span></span>

    <span data-ttu-id="413f1-554">*建立新的網站*</span><span class="sxs-lookup"><span data-stu-id="413f1-554">*Creating a new Web Site*</span></span>
3. <span data-ttu-id="413f1-555">按一下**計算** | **網站**。</span><span class="sxs-lookup"><span data-stu-id="413f1-555">Click **Compute** | **Web Site**.</span></span> <span data-ttu-id="413f1-556">然後選取**快速建立**選項。</span><span class="sxs-lookup"><span data-stu-id="413f1-556">Then select **Quick Create** option.</span></span> <span data-ttu-id="413f1-557">新的網站上提供可用的 URL，然後按一下**建立網站**。</span><span class="sxs-lookup"><span data-stu-id="413f1-557">Provide an available URL for the new web site and click **Create Web Site**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="413f1-558">Azure 是您可以控制和管理在雲端中執行 web 應用程式的主機。</span><span class="sxs-lookup"><span data-stu-id="413f1-558">Azure is the host for a web application running in the cloud that you can control and manage.</span></span> <span data-ttu-id="413f1-559">快速建立 選項可讓您部署已完成的 web 應用程式，從 azure 入口網站外部。</span><span class="sxs-lookup"><span data-stu-id="413f1-559">The Quick Create option allows you to deploy a completed web application to the Azure from outside the portal.</span></span> <span data-ttu-id="413f1-560">它不包含設定資料庫的步驟。</span><span class="sxs-lookup"><span data-stu-id="413f1-560">It does not include steps for setting up a database.</span></span>

    <span data-ttu-id="413f1-561">![建立新的網站使用 快速建立](whats-new-in-web-forms-in-aspnet-45/_static/image32.png "建立新的網站使用 快速建立")</span><span class="sxs-lookup"><span data-stu-id="413f1-561">![Creating a new Web Site using Quick Create](whats-new-in-web-forms-in-aspnet-45/_static/image32.png "Creating a new Web Site using Quick Create")</span></span>

    <span data-ttu-id="413f1-562">*建立新的網站使用 快速建立*</span><span class="sxs-lookup"><span data-stu-id="413f1-562">*Creating a new Web Site using Quick Create*</span></span>
4. <span data-ttu-id="413f1-563">等候新**網站**建立。</span><span class="sxs-lookup"><span data-stu-id="413f1-563">Wait until the new **Web Site** is created.</span></span>
5. <span data-ttu-id="413f1-564">建立網站之後按一下底下的連結**URL**資料行。</span><span class="sxs-lookup"><span data-stu-id="413f1-564">Once the Web Site is created click the link under the **URL** column.</span></span> <span data-ttu-id="413f1-565">請檢查新的網站運作。</span><span class="sxs-lookup"><span data-stu-id="413f1-565">Check that the new Web Site is working.</span></span>

    <span data-ttu-id="413f1-566">![瀏覽至新的網站](whats-new-in-web-forms-in-aspnet-45/_static/image33.png "瀏覽至新的網站")</span><span class="sxs-lookup"><span data-stu-id="413f1-566">![Browsing to the new web site](whats-new-in-web-forms-in-aspnet-45/_static/image33.png "Browsing to the new web site")</span></span>

    <span data-ttu-id="413f1-567">*瀏覽至新的網站*</span><span class="sxs-lookup"><span data-stu-id="413f1-567">*Browsing to the new web site*</span></span>

    <span data-ttu-id="413f1-568">![執行網站](whats-new-in-web-forms-in-aspnet-45/_static/image34.png "執行的網站")</span><span class="sxs-lookup"><span data-stu-id="413f1-568">![Web site running](whats-new-in-web-forms-in-aspnet-45/_static/image34.png "Web site running")</span></span>

    <span data-ttu-id="413f1-569">*執行的網站*</span><span class="sxs-lookup"><span data-stu-id="413f1-569">*Web site running*</span></span>
6. <span data-ttu-id="413f1-570">返回入口網站並按一下底下的網站名稱**名稱**資料行會顯示 [管理] 頁面。</span><span class="sxs-lookup"><span data-stu-id="413f1-570">Go back to the portal and click the name of the web site under the **Name** column to display the management pages.</span></span>

    <span data-ttu-id="413f1-571">![開啟網站管理頁面](whats-new-in-web-forms-in-aspnet-45/_static/image35.png "開啟網站管理頁面")</span><span class="sxs-lookup"><span data-stu-id="413f1-571">![Opening the web site management pages](whats-new-in-web-forms-in-aspnet-45/_static/image35.png "Opening the web site management pages")</span></span>

    <span data-ttu-id="413f1-572">*開啟網站管理頁面*</span><span class="sxs-lookup"><span data-stu-id="413f1-572">*Opening the Web Site management pages*</span></span>
7. <span data-ttu-id="413f1-573">在**儀表板**頁面的 **快速概覽**區段中，按一下**下載發行設定檔**連結。</span><span class="sxs-lookup"><span data-stu-id="413f1-573">In the **Dashboard** page, under the **quick glance** section, click the **Download publish profile** link.</span></span>

    > [!NOTE]
    > <span data-ttu-id="413f1-574">*發行設定檔*包含所有 web 應用程式發行至 Azure 的每個已啟用的發行方法所需的資訊。</span><span class="sxs-lookup"><span data-stu-id="413f1-574">The *publish profile* contains all of the information required to publish a web application to Azure for each enabled publication method.</span></span> <span data-ttu-id="413f1-575">發行設定檔包含 Url、 使用者認證和資料庫連接，並對每個端點已啟用的發行集的方法進行驗證所需的字串。</span><span class="sxs-lookup"><span data-stu-id="413f1-575">The publish profile contains the URLs, user credentials and database strings required to connect to and authenticate against each of the endpoints for which a publication method is enabled.</span></span> <span data-ttu-id="413f1-576">**Microsoft WebMatrix 2**， **Microsoft Visual Studio Express for Web**和**Microsoft Visual Studio 2012**支援讀取發行設定檔自動將這些程式的組態web 應用程式發行至 Azure。</span><span class="sxs-lookup"><span data-stu-id="413f1-576">**Microsoft WebMatrix 2**, **Microsoft Visual Studio Express for Web** and **Microsoft Visual Studio 2012** support reading publish profiles to automate configuration of these programs for publishing web applications to Azure.</span></span>

    <span data-ttu-id="413f1-577">![下載網站發行設定檔](whats-new-in-web-forms-in-aspnet-45/_static/image36.png "下載網站發行設定檔")</span><span class="sxs-lookup"><span data-stu-id="413f1-577">![Downloading the web site publish profile](whats-new-in-web-forms-in-aspnet-45/_static/image36.png "Downloading the web site publish profile")</span></span>

    <span data-ttu-id="413f1-578">*下載網站發行設定檔*</span><span class="sxs-lookup"><span data-stu-id="413f1-578">*Downloading the Web Site publish profile*</span></span>
8. <span data-ttu-id="413f1-579">下載發行設定檔至已知位置。</span><span class="sxs-lookup"><span data-stu-id="413f1-579">Download the publish profile file to a known location.</span></span> <span data-ttu-id="413f1-580">進一步在這個練習中，您會看到如何使用此檔案發行至 Azure 從 Visual Studio web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="413f1-580">Further in this exercise you will see how to use this file to publish a web application to Azure from Visual Studio.</span></span>

    <span data-ttu-id="413f1-581">![儲存發行設定檔](whats-new-in-web-forms-in-aspnet-45/_static/image37.png "儲存發行設定檔")</span><span class="sxs-lookup"><span data-stu-id="413f1-581">![Saving the publish profile file](whats-new-in-web-forms-in-aspnet-45/_static/image37.png "Saving the publish profile")</span></span>

    <span data-ttu-id="413f1-582">*儲存發行設定檔*</span><span class="sxs-lookup"><span data-stu-id="413f1-582">*Saving the publish profile file*</span></span>

<a id="ApxBTask2"></a>

<a id="Task_2_-_Configuring_the_Database_Server"></a>
#### <a name="task-2---configuring-the-database-server"></a><span data-ttu-id="413f1-583">工作 2-設定資料庫伺服器</span><span class="sxs-lookup"><span data-stu-id="413f1-583">Task 2 - Configuring the Database Server</span></span>

<span data-ttu-id="413f1-584">如果您的應用程式會使用 SQL Server 資料庫，您必須建立 SQL Database 伺服器。</span><span class="sxs-lookup"><span data-stu-id="413f1-584">If your application makes use of SQL Server databases you will need to create a SQL Database server.</span></span> <span data-ttu-id="413f1-585">如果您想要部署簡單的應用程式不會使用 SQL Server，您可能會略過這項工作。</span><span class="sxs-lookup"><span data-stu-id="413f1-585">If you want to deploy a simple application that does not use SQL Server you might skip this task.</span></span>

1. <span data-ttu-id="413f1-586">您將需要 SQL Database 伺服器來儲存應用程式資料庫。</span><span class="sxs-lookup"><span data-stu-id="413f1-586">You will need a SQL Database server for storing the application database.</span></span> <span data-ttu-id="413f1-587">您可以從您的訂用帳戶，在 Azure 管理入口網站中檢視 SQL Database 伺服器**Sql 資料庫** | **伺服器** | **伺服器的儀表板**.</span><span class="sxs-lookup"><span data-stu-id="413f1-587">You can view the SQL Database servers from your subscription in the Azure Management portal at **Sql Databases** | **Servers** | **Server's Dashboard**.</span></span> <span data-ttu-id="413f1-588">如果您沒有建立伺服器，您可以建立一個使用**新增**命令列上的按鈕。</span><span class="sxs-lookup"><span data-stu-id="413f1-588">If you do not have a server created, you can create one using the **Add** button on the command bar.</span></span> <span data-ttu-id="413f1-589">請注意**伺服器名稱和 URL、 系統管理員身分登入名稱和密碼**，因為您將在下一個工作中使用它們。</span><span class="sxs-lookup"><span data-stu-id="413f1-589">Take note of the **server name and URL, administrator login name and password**, as you will use them in the next tasks.</span></span> <span data-ttu-id="413f1-590">並未建立資料庫，因為它會在後續階段中建立。</span><span class="sxs-lookup"><span data-stu-id="413f1-590">Do not create the database yet, as it will be created in a later stage.</span></span>

    <span data-ttu-id="413f1-591">![SQL Database 伺服器儀表板](whats-new-in-web-forms-in-aspnet-45/_static/image38.png "SQL Database 伺服器儀表板")</span><span class="sxs-lookup"><span data-stu-id="413f1-591">![SQL Database Server Dashboard](whats-new-in-web-forms-in-aspnet-45/_static/image38.png "SQL Database Server Dashboard")</span></span>

    <span data-ttu-id="413f1-592">*SQL Database 伺服器儀表板*</span><span class="sxs-lookup"><span data-stu-id="413f1-592">*SQL Database Server Dashboard*</span></span>
2. <span data-ttu-id="413f1-593">下一項工作在您將測試資料庫連接，從 Visual Studio，因此您需要在伺服器的清單中包含您的本機 IP 位址**允許的 IP 位址**。</span><span class="sxs-lookup"><span data-stu-id="413f1-593">In the next task you will test the database connection from Visual Studio, for that reason you need to include your local IP address in the server's list of **Allowed IP Addresses**.</span></span> <span data-ttu-id="413f1-594">若要這樣做，請按一下**設定**，選取 [IP 位址從**目前的用戶端 IP 位址**並將它貼上**起始 IP 位址**和**結束 IP 位址**文字方塊，然後按一下![add-client-ip-address-ok-button](whats-new-in-web-forms-in-aspnet-45/_static/image39.png) ] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="413f1-594">To do that, click **Configure**, select the IP address from **Current Client IP Address** and paste it on the **Start IP Address** and **End IP Address** text boxes and click the ![add-client-ip-address-ok-button](whats-new-in-web-forms-in-aspnet-45/_static/image39.png) button.</span></span>

    ![加入用戶端 IP 位址](whats-new-in-web-forms-in-aspnet-45/_static/image40.png)

    <span data-ttu-id="413f1-596">*加入用戶端 IP 位址*</span><span class="sxs-lookup"><span data-stu-id="413f1-596">*Adding Client IP Address*</span></span>
3. <span data-ttu-id="413f1-597">一次**用戶端 IP 位址**新增至允許的 IP 位址清單中，按一下**儲存**來確認變更。</span><span class="sxs-lookup"><span data-stu-id="413f1-597">Once the **Client IP Address** is added to the allowed IP addresses list, click on **Save** to confirm the changes.</span></span>

    ![確認變更](whats-new-in-web-forms-in-aspnet-45/_static/image41.png)

    <span data-ttu-id="413f1-599">*確認變更*</span><span class="sxs-lookup"><span data-stu-id="413f1-599">*Confirm Changes*</span></span>

<a id="ApxBTask3"></a>

<a id="Task_3_-_Publishing_an_ASPNET_MVC_4_Application_using_Web_Deploy"></a>
#### <a name="task-3---publishing-an-aspnet-mvc-4-application-using-web-deploy"></a><span data-ttu-id="413f1-600">工作 3-發行 ASP.NET MVC 4 應用程式使用 Web Deploy</span><span class="sxs-lookup"><span data-stu-id="413f1-600">Task 3 - Publishing an ASP.NET MVC 4 Application using Web Deploy</span></span>

1. <span data-ttu-id="413f1-601">返回至 ASP.NET MVC 4 方案。</span><span class="sxs-lookup"><span data-stu-id="413f1-601">Go back to the ASP.NET MVC 4 solution.</span></span> <span data-ttu-id="413f1-602">在**方案總管 中**，以滑鼠右鍵按一下網站專案，然後選取**發行**。</span><span class="sxs-lookup"><span data-stu-id="413f1-602">In the **Solution Explorer**, right-click the web site project and select **Publish**.</span></span>

    <span data-ttu-id="413f1-603">![發行應用程式](whats-new-in-web-forms-in-aspnet-45/_static/image42.png "發行應用程式")</span><span class="sxs-lookup"><span data-stu-id="413f1-603">![Publishing the Application](whats-new-in-web-forms-in-aspnet-45/_static/image42.png "Publishing the Application")</span></span>

    <span data-ttu-id="413f1-604">*發行網站*</span><span class="sxs-lookup"><span data-stu-id="413f1-604">*Publishing the web site*</span></span>
2. <span data-ttu-id="413f1-605">匯入發行設定檔儲存在第一項工作中。</span><span class="sxs-lookup"><span data-stu-id="413f1-605">Import the publish profile you saved in the first task.</span></span>

    <span data-ttu-id="413f1-606">![匯入發行設定檔](whats-new-in-web-forms-in-aspnet-45/_static/image43.png "匯入發行設定檔")</span><span class="sxs-lookup"><span data-stu-id="413f1-606">![Importing the publish profile](whats-new-in-web-forms-in-aspnet-45/_static/image43.png "Importing the publish profile")</span></span>

    <span data-ttu-id="413f1-607">*匯入發行設定檔*</span><span class="sxs-lookup"><span data-stu-id="413f1-607">*Importing publish profile*</span></span>
3. <span data-ttu-id="413f1-608">按一下**驗證連線**。</span><span class="sxs-lookup"><span data-stu-id="413f1-608">Click **Validate Connection**.</span></span> <span data-ttu-id="413f1-609">驗證完成後按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="413f1-609">Once Validation is complete click **Next**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="413f1-610">一旦您看到 [驗證連線] 按鈕旁邊會出現綠色核取記號完成驗證。</span><span class="sxs-lookup"><span data-stu-id="413f1-610">Validation is complete once you see a green checkmark appear next to the Validate Connection button.</span></span>

    <span data-ttu-id="413f1-611">![驗證連線](whats-new-in-web-forms-in-aspnet-45/_static/image44.png "驗證連線")</span><span class="sxs-lookup"><span data-stu-id="413f1-611">![Validating connection](whats-new-in-web-forms-in-aspnet-45/_static/image44.png "Validating connection")</span></span>

    <span data-ttu-id="413f1-612">*驗證連接*</span><span class="sxs-lookup"><span data-stu-id="413f1-612">*Validating connection*</span></span>
4. <span data-ttu-id="413f1-613">在**設定**頁面的 **資料庫**區段中，按一下您的資料庫連接文字方塊旁邊的按鈕 (也就是**DefaultConnection**)。</span><span class="sxs-lookup"><span data-stu-id="413f1-613">In the **Settings** page, under the **Databases** section, click the button next to your database connection's textbox (i.e. **DefaultConnection**).</span></span>

    <span data-ttu-id="413f1-614">![Web 部署設定](whats-new-in-web-forms-in-aspnet-45/_static/image45.png "Web 部署設定")</span><span class="sxs-lookup"><span data-stu-id="413f1-614">![Web deploy configuration](whats-new-in-web-forms-in-aspnet-45/_static/image45.png "Web deploy configuration")</span></span>

    <span data-ttu-id="413f1-615">*Web 部署設定*</span><span class="sxs-lookup"><span data-stu-id="413f1-615">*Web deploy configuration*</span></span>
5. <span data-ttu-id="413f1-616">設定資料庫連接，如下所示：</span><span class="sxs-lookup"><span data-stu-id="413f1-616">Configure the database connection as follows:</span></span>

   - <span data-ttu-id="413f1-617">在**伺服器名稱**您 SQL Database 伺服器 URL 使用下列方法類型*tcp:* 前置詞。</span><span class="sxs-lookup"><span data-stu-id="413f1-617">In the **Server name** type your SQL Database server URL using the *tcp:* prefix.</span></span>
   - <span data-ttu-id="413f1-618">在**使用者名**輸入您的伺服器系統管理員身分登入名稱。</span><span class="sxs-lookup"><span data-stu-id="413f1-618">In **User name** type your server administrator login name.</span></span>
   - <span data-ttu-id="413f1-619">在**密碼**輸入伺服器系統管理員身分登入密碼。</span><span class="sxs-lookup"><span data-stu-id="413f1-619">In **Password** type your server administrator login password.</span></span>
   - <span data-ttu-id="413f1-620">輸入新的資料庫名稱。</span><span class="sxs-lookup"><span data-stu-id="413f1-620">Type a new database name.</span></span>

     <span data-ttu-id="413f1-621">![設定目的地連接字串](whats-new-in-web-forms-in-aspnet-45/_static/image46.png "設定目的地連接字串")</span><span class="sxs-lookup"><span data-stu-id="413f1-621">![Configuring destination connection string](whats-new-in-web-forms-in-aspnet-45/_static/image46.png "Configuring destination connection string")</span></span>

     <span data-ttu-id="413f1-622">*設定目的地連接字串*</span><span class="sxs-lookup"><span data-stu-id="413f1-622">*Configuring destination connection string*</span></span>
6. <span data-ttu-id="413f1-623">然後按一下 [確定]。 </span><span class="sxs-lookup"><span data-stu-id="413f1-623">Then click **OK**.</span></span> <span data-ttu-id="413f1-624">當系統提示您建立資料庫依序按一下**是**。</span><span class="sxs-lookup"><span data-stu-id="413f1-624">When prompted to create the database click **Yes**.</span></span>

    <span data-ttu-id="413f1-625">![建立資料庫](whats-new-in-web-forms-in-aspnet-45/_static/image47.png "建立的資料庫字串")</span><span class="sxs-lookup"><span data-stu-id="413f1-625">![Creating the database](whats-new-in-web-forms-in-aspnet-45/_static/image47.png "Creating the database string")</span></span>

    <span data-ttu-id="413f1-626">*建立資料庫*</span><span class="sxs-lookup"><span data-stu-id="413f1-626">*Creating the database*</span></span>
7. <span data-ttu-id="413f1-627">您將用來連接到 azure SQL Database 的連接字串會顯示在預設連接文字方塊中。</span><span class="sxs-lookup"><span data-stu-id="413f1-627">The connection string you will use to connect to SQL Database in Azure is shown within Default Connection textbox.</span></span> <span data-ttu-id="413f1-628">然後按 [下一步] 。</span><span class="sxs-lookup"><span data-stu-id="413f1-628">Then click **Next**.</span></span>

    <span data-ttu-id="413f1-629">![連接字串指向 SQL Database](whats-new-in-web-forms-in-aspnet-45/_static/image48.png "連接字串指向 SQL 資料庫")</span><span class="sxs-lookup"><span data-stu-id="413f1-629">![Connection string pointing to SQL Database](whats-new-in-web-forms-in-aspnet-45/_static/image48.png "Connection string pointing to SQL Database")</span></span>

    <span data-ttu-id="413f1-630">*連接字串指向 SQL 資料庫*</span><span class="sxs-lookup"><span data-stu-id="413f1-630">*Connection string pointing to SQL Database*</span></span>
8. <span data-ttu-id="413f1-631">在**預覽**頁面上，按一下**發行**。</span><span class="sxs-lookup"><span data-stu-id="413f1-631">In the **Preview** page, click **Publish**.</span></span>

    <span data-ttu-id="413f1-632">![Web 應用程式發行](whats-new-in-web-forms-in-aspnet-45/_static/image49.png "發行 web 應用程式")</span><span class="sxs-lookup"><span data-stu-id="413f1-632">![Publishing the web application](whats-new-in-web-forms-in-aspnet-45/_static/image49.png "Publishing the web application")</span></span>

    <span data-ttu-id="413f1-633">*發行 web 應用程式*</span><span class="sxs-lookup"><span data-stu-id="413f1-633">*Publishing the web application*</span></span>
9. <span data-ttu-id="413f1-634">一旦在發佈程序完成時，預設瀏覽器會開啟已發行的網站。</span><span class="sxs-lookup"><span data-stu-id="413f1-634">Once the publishing process finishes, your default browser will open the published web site.</span></span>

<a id="AppendixC"></a>

<a id="Appendix_C_Using_Code_Snippets"></a>
## <a name="appendix-c-using-code-snippets"></a><span data-ttu-id="413f1-635">附錄 c： 使用程式碼片段</span><span class="sxs-lookup"><span data-stu-id="413f1-635">Appendix C: Using Code Snippets</span></span>

<span data-ttu-id="413f1-636">程式碼片段，您會有您需要在您可以的所有程式碼。</span><span class="sxs-lookup"><span data-stu-id="413f1-636">With code snippets, you have all the code you need at your fingertips.</span></span> <span data-ttu-id="413f1-637">實驗室文件會告訴您完全時您可以使用它們，如下圖所示。</span><span class="sxs-lookup"><span data-stu-id="413f1-637">The lab document will tell you exactly when you can use them, as shown in the following figure.</span></span>

<span data-ttu-id="413f1-638">![若要將程式碼插入您的專案中使用 Visual Studio 程式碼片段](whats-new-in-web-forms-in-aspnet-45/_static/image50.png "使用 Visual Studio 程式碼片段，將程式碼插入您的專案")</span><span class="sxs-lookup"><span data-stu-id="413f1-638">![Using Visual Studio code snippets to insert code into your project](whats-new-in-web-forms-in-aspnet-45/_static/image50.png "Using Visual Studio code snippets to insert code into your project")</span></span>

<span data-ttu-id="413f1-639">*若要將程式碼插入您的專案中使用 Visual Studio 程式碼片段*</span><span class="sxs-lookup"><span data-stu-id="413f1-639">*Using Visual Studio code snippets to insert code into your project*</span></span>

<span data-ttu-id="413f1-640">***若要加入的程式碼片段，使用鍵盤 （僅限 C#)***</span><span class="sxs-lookup"><span data-stu-id="413f1-640">***To add a code snippet using the keyboard (C# only)***</span></span>

1. <span data-ttu-id="413f1-641">將游標放在您想要插入的程式碼。</span><span class="sxs-lookup"><span data-stu-id="413f1-641">Place the cursor where you would like to insert the code.</span></span>
2. <span data-ttu-id="413f1-642">開始鍵入片段名稱 （不含空格或連字號）。</span><span class="sxs-lookup"><span data-stu-id="413f1-642">Start typing the snippet name (without spaces or hyphens).</span></span>
3. <span data-ttu-id="413f1-643">觀察 IntelliSense 會顯示比對程式碼片段的名稱。</span><span class="sxs-lookup"><span data-stu-id="413f1-643">Watch as IntelliSense displays matching snippets' names.</span></span>
4. <span data-ttu-id="413f1-644">選取正確的程式碼片段 （或直到選取整個程式碼片段名稱時，保留 輸入）。</span><span class="sxs-lookup"><span data-stu-id="413f1-644">Select the correct snippet (or keep typing until the entire snippet's name is selected).</span></span>
5. <span data-ttu-id="413f1-645">按下 Tab 鍵兩次的游標位置插入程式碼片段。</span><span class="sxs-lookup"><span data-stu-id="413f1-645">Press the Tab key twice to insert the snippet at the cursor location.</span></span>

<span data-ttu-id="413f1-646">![開始輸入程式碼片段名稱](whats-new-in-web-forms-in-aspnet-45/_static/image51.png "開始鍵入片段名稱")</span><span class="sxs-lookup"><span data-stu-id="413f1-646">![Start typing the snippet name](whats-new-in-web-forms-in-aspnet-45/_static/image51.png "Start typing the snippet name")</span></span>

<span data-ttu-id="413f1-647">*開始輸入程式碼片段名稱*</span><span class="sxs-lookup"><span data-stu-id="413f1-647">*Start typing the snippet name*</span></span>

<span data-ttu-id="413f1-648">![按 Tab 鍵選取反白顯示的程式碼片段](whats-new-in-web-forms-in-aspnet-45/_static/image52.png "按 Tab 鍵以選取反白顯示的程式碼片段")</span><span class="sxs-lookup"><span data-stu-id="413f1-648">![Press Tab to select the highlighted snippet](whats-new-in-web-forms-in-aspnet-45/_static/image52.png "Press Tab to select the highlighted snippet")</span></span>

<span data-ttu-id="413f1-649">*按 Tab 鍵選取反白顯示的程式碼片段*</span><span class="sxs-lookup"><span data-stu-id="413f1-649">*Press Tab to select the highlighted snippet*</span></span>

<span data-ttu-id="413f1-650">![按 Tab 鍵一次程式碼片段就會擴展](whats-new-in-web-forms-in-aspnet-45/_static/image53.png "按 Tab 鍵一次程式碼片段就會擴展")</span><span class="sxs-lookup"><span data-stu-id="413f1-650">![Press Tab again and the snippet will expand](whats-new-in-web-forms-in-aspnet-45/_static/image53.png "Press Tab again and the snippet will expand")</span></span>

<span data-ttu-id="413f1-651">*按 Tab 鍵一次程式碼片段就會擴展*</span><span class="sxs-lookup"><span data-stu-id="413f1-651">*Press Tab again and the snippet will expand*</span></span>

<span data-ttu-id="413f1-652">***若要加入的程式碼片段，使用滑鼠 （C#、 Visual Basic 和 XML）*** 1。</span><span class="sxs-lookup"><span data-stu-id="413f1-652">***To add a code snippet using the mouse (C#, Visual Basic and XML)*** 1.</span></span> <span data-ttu-id="413f1-653">以滑鼠右鍵按一下您要插入程式碼片段。</span><span class="sxs-lookup"><span data-stu-id="413f1-653">Right-click where you want to insert the code snippet.</span></span>

1. <span data-ttu-id="413f1-654">選取**插入程式碼片段**後面**我的程式碼片段**。</span><span class="sxs-lookup"><span data-stu-id="413f1-654">Select **Insert Snippet** followed by **My Code Snippets**.</span></span>
2. <span data-ttu-id="413f1-655">透過在按一下挑選清單中，從相關的程式碼片段。</span><span class="sxs-lookup"><span data-stu-id="413f1-655">Pick the relevant snippet from the list, by clicking on it.</span></span>

<span data-ttu-id="413f1-656">![以滑鼠右鍵按一下您要插入程式碼片段，然後選取 插入程式碼片段](whats-new-in-web-forms-in-aspnet-45/_static/image54.png "以滑鼠右鍵按一下您要插入程式碼片段，然後選取 插入程式碼片段")</span><span class="sxs-lookup"><span data-stu-id="413f1-656">![Right-click where you want to insert the code snippet and select Insert Snippet](whats-new-in-web-forms-in-aspnet-45/_static/image54.png "Right-click where you want to insert the code snippet and select Insert Snippet")</span></span>

<span data-ttu-id="413f1-657">*以滑鼠右鍵按一下您要插入程式碼片段，然後選取 插入程式碼片段*</span><span class="sxs-lookup"><span data-stu-id="413f1-657">*Right-click where you want to insert the code snippet and select Insert Snippet*</span></span>

<span data-ttu-id="413f1-658">![透過在按一下挑選清單中，從相關的程式碼片段](whats-new-in-web-forms-in-aspnet-45/_static/image55.png "上它即可挑選清單中，從相關的程式碼片段")</span><span class="sxs-lookup"><span data-stu-id="413f1-658">![Pick the relevant snippet from the list, by clicking on it](whats-new-in-web-forms-in-aspnet-45/_static/image55.png "Pick the relevant snippet from the list, by clicking on it")</span></span>

<span data-ttu-id="413f1-659">*透過在按一下挑選清單中，從相關的程式碼片段*</span><span class="sxs-lookup"><span data-stu-id="413f1-659">*Pick the relevant snippet from the list, by clicking on it*</span></span>
