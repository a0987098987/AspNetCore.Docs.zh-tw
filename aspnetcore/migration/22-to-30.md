...
為 MVC 提供 UI 元件的 Razor 類別庫專案，必須在`RazorLangVersion`專案檔中設定屬性。
...
`AllowSynchronousIO`啟用或停用`HttpReqeuest.Body.Read`同步 IO api，例如、 `HttpResponse.Body.Write`和。 `Stream.Flush` 這些 Api 是執行緒耗盡的來源，導致應用程式損毀。 在3.0 中`AllowSynchronousIO` ，預設為停用。 如需詳細資訊，請參閱[Kestrel 文章中的同步 IO 一節](/aspnet/core/fundamentals/servers/kestrel?view=aspnetcore-3.0#synchronous-io)。
