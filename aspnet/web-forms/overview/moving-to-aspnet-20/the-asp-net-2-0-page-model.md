...
## <a name="the-code-behind-model-in-aspnet-1x"></a><span data-ttu-id="7bdd6-114">程式碼後置模型，在 ASP.NET 1.x</span><span class="sxs-lookup"><span data-stu-id="7bdd6-114">The Code-Behind Model in ASP.NET 1.x</span></span>
...
<span data-ttu-id="7bdd6-144">指定的非同步頁面的逾時。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-144">Specified the timeout for asynchronous pages.</span></span> <span data-ttu-id="7bdd6-145">預設值為 45 秒。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-145">The default is 45 seconds.</span></span>
...
## <a name="enableeventvalidation"></a><span data-ttu-id="7bdd6-160">EnableEventValidation</span><span class="sxs-lookup"><span data-stu-id="7bdd6-160">EnableEventValidation</span></span>
...
### <a name="masterpagefile"></a><span data-ttu-id="7bdd6-177">MasterPageFile</span><span class="sxs-lookup"><span data-stu-id="7bdd6-177">MasterPageFile</span></span>
...
## <a name="setting-public-property-values-via-the--page-directive"></a><span data-ttu-id="7bdd6-194">設定公用屬性值透過 @ Page 指示詞</span><span class="sxs-lookup"><span data-stu-id="7bdd6-194">Setting Public Property Values via the @ Page Directive</span></span>
...
<span data-ttu-id="7bdd6-223">此屬性會取得或設定布林值，指定 ASP.NET 2.0 主題套用至頁面。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-223">This property gets or sets a Boolean that specifies whether or not an ASP.NET 2.0 theme applies to the page.</span></span>
...
<span data-ttu-id="7bdd6-239">這個唯讀屬性會傳回 **，則為 true**如果頁面是跨網頁回傳的一部分。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-239">This read-only property returns **true** if the page is part of a cross-page postback.</span></span> <span data-ttu-id="7bdd6-240">稍後在本單元涵蓋跨網頁回傳。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-240">Cross-page postbacks are covered later in this module.</span></span>
...
<span data-ttu-id="7bdd6-282">這個屬性會傳回為控制項建立唯一的用戶端識別碼時，做為分隔符號的字元。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-282">This property returns the character used as a separator when creating unique client IDs for controls.</span></span> <span data-ttu-id="7bdd6-283">它適用於執行階段，而非直接在程式碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-283">It is intended for use by the runtime and not directly in code.</span></span>
...
<span data-ttu-id="7bdd6-296">頁面樣式表中的屬性套用至頁面中。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-296">Applies the properties in a pages style sheet to the page.</span></span>
...
<span data-ttu-id="7bdd6-309">傳回可用的映像等的用戶端要求的相對 URL。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-309">Returns a relative URL that can be used for client requests for images, etc.</span></span>
...
## <a name="prerendercomplete-new-in-20"></a><span data-ttu-id="7bdd6-337">PreRenderComplete （新功能 2.0)</span><span class="sxs-lookup"><span data-stu-id="7bdd6-337">PreRenderComplete (New in 2.0)</span></span>
...
<span data-ttu-id="7bdd6-398">下列影片會逐步解說中的非同步頁面要求。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-398">The video below is a walkthrough of an asynchronous page request.</span></span>
...
<span data-ttu-id="7bdd6-443">請注意用戶端上的函式的簽章會有點不同。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-443">Notice that the signature of the function on the client is a bit different.</span></span> <span data-ttu-id="7bdd6-444">用戶端函式會將 5 個字串和布林值傳遞。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-444">The client-side function passes 5 strings and a Boolean.</span></span> <span data-ttu-id="7bdd6-445">（這是在上述範例中為 null） 的其他字串包含用戶端函式會處理任何錯誤的伺服器端的回呼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-445">The additional string (which is null in the above example) contains the client-side function that will handle any errors from the server-side callback.</span></span>

## <a name="step-3--hook-the-client-side-control-event"></a><span data-ttu-id="7bdd6-446">步驟 3： 將連結的用戶端控制項事件</span><span class="sxs-lookup"><span data-stu-id="7bdd6-446">Step 3 : Hook the Client-Side Control Event</span></span>

<span data-ttu-id="7bdd6-447">請注意，上述 GetCallbackEventReference 的傳回值已指派給字串變數。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-447">Notice that the return value of GetCallbackEventReference above was assigned to a string variable.</span></span> <span data-ttu-id="7bdd6-448">該字串用來將連結控制項初始化回呼用戶端事件。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-448">That string is used to hook a client-side event for the control that initiates the callback.</span></span> <span data-ttu-id="7bdd6-449">在此範例中，回呼由下拉式清單中的頁面上，我想要將連結*OnChange*事件。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-449">In this example, the callback is initiated by a dropdown on the page, so I want to hook the *OnChange* event.</span></span>

<span data-ttu-id="7bdd6-450">若要攔截用戶端事件，只要將處理常式加入用戶端標記，如下所示：</span><span class="sxs-lookup"><span data-stu-id="7bdd6-450">To hook the client-side event, simply add a handler to the client-side markup as follows:</span></span>

[!code-csharp[Main](the-asp-net-2-0-page-model/samples/sample15.cs)]

<span data-ttu-id="7bdd6-451">請記得， *cbRef* GetCallbackEventReference 呼叫的傳回值。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-451">Recall that *cbRef* is the return value from the call to GetCallbackEventReference.</span></span> <span data-ttu-id="7bdd6-452">它包含 WebForm 呼叫\_已如上所示的 DoCallback。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-452">It contains the call to WebForm\_DoCallback that was shown above.</span></span>

## <a name="step-4--register-the-client-side-script"></a><span data-ttu-id="7bdd6-453">步驟 4： 註冊用戶端指令碼</span><span class="sxs-lookup"><span data-stu-id="7bdd6-453">Step 4 : Register the Client-Side Script</span></span>

<span data-ttu-id="7bdd6-454">回想一下，GetCallbackEventReference 呼叫指定的用戶端指令碼，呼叫**ShowCompanyName**會在伺服器端回呼成功時執行。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-454">Recall that the call to GetCallbackEventReference specified that a client-side script called **ShowCompanyName** would be executed when the server-side callback succeeds.</span></span> <span data-ttu-id="7bdd6-455">該指令碼必須加入至頁面使用 ClientScriptManager 執行個體。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-455">That script needs to be added to the page using a ClientScriptManager instance.</span></span> <span data-ttu-id="7bdd6-456">（ClientScriptManager 類別將會是這稍後在本單元中的）。因此，您執行該類似：</span><span class="sxs-lookup"><span data-stu-id="7bdd6-456">(The ClientScriptManager class will be dicussed later in this module.) You do that like so:</span></span>

[!code-javascript[Main](the-asp-net-2-0-page-model/samples/sample16.js)]

## <a name="step-5--call-the-methods-of-the-icallbackeventhandler-interface"></a><span data-ttu-id="7bdd6-457">步驟 5： 呼叫 ICallbackEventHandler 介面的方法</span><span class="sxs-lookup"><span data-stu-id="7bdd6-457">Step 5 : Call the Methods of the ICallbackEventHandler Interface</span></span>

<span data-ttu-id="7bdd6-458">ICallbackEventHandler 包含您要在您的程式碼中實作的兩個方法。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-458">The ICallbackEventHandler contains two methods that you need to implement in your code.</span></span> <span data-ttu-id="7bdd6-459">它們**RaiseCallbackEvent**並**GetCallbackEvent**。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-459">They are **RaiseCallbackEvent** and **GetCallbackEvent**.</span></span>

<span data-ttu-id="7bdd6-460">**RaiseCallbackEvent**會做為引數的字串，並且會傳回任何內容。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-460">**RaiseCallbackEvent** takes a string as an argument and returns nothing.</span></span> <span data-ttu-id="7bdd6-461">字串引數會傳遞從用戶端呼叫 WebForm\_DoCallback。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-461">The string argument is passed from the client-side call to WebForm\_DoCallback.</span></span> <span data-ttu-id="7bdd6-462">在此案例中，該值是*值*呼叫 ddlCompany 下拉式清單中的屬性。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-462">In this case, that value is the *value* attribute of the dropdown called ddlCompany.</span></span> <span data-ttu-id="7bdd6-463">您的伺服器端程式碼應該置於 RaiseCallbackEvent 方法。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-463">Your server-side code should be placed in the RaiseCallbackEvent method.</span></span> <span data-ttu-id="7bdd6-464">例如，如果您的回呼正在對外部資源的 WebRequest，該程式碼應該置於 RaiseCallbackEvent。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-464">For example, if your callback is making a WebRequest against an external resource, that code should be placed in RaiseCallbackEvent.</span></span>

<span data-ttu-id="7bdd6-465">**GetCallbackEvent**負責處理回呼的傳回至用戶端。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-465">**GetCallbackEvent** is responsible for processing the return of the callback to the client.</span></span> <span data-ttu-id="7bdd6-466">它會採用任何引數，並傳回字串。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-466">It takes no arguments and returns a string.</span></span> <span data-ttu-id="7bdd6-467">它會傳回的字串會傳遞做為引數至用戶端函式，在此情況下*ShowCompanyName*。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-467">The string that it returns will be passed as an argument to the client-side function, in this case *ShowCompanyName*.</span></span>

<span data-ttu-id="7bdd6-468">完成上述步驟後，您已準備好在 ASP.NET 2.0 中執行的指令碼回呼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-468">Once you have completed the above steps, you are ready to perform a script callback in ASP.NET 2.0.</span></span>


![](the-asp-net-2-0-page-model/_static/image4.png)


[<span data-ttu-id="7bdd6-469">開啟全螢幕影片</span><span class="sxs-lookup"><span data-stu-id="7bdd6-469">Open Full-Screen Video</span></span>](the-asp-net-2-0-page-model/_static/callback1.wmv)


<span data-ttu-id="7bdd6-470">支援在 ASP.NET 中的指令碼回呼支援讓 XMLHttp 呼叫任何瀏覽器中。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-470">Script callbacks in ASP.NET are supported in any browser that supports making XMLHttp calls.</span></span> <span data-ttu-id="7bdd6-471">其中包含所有現代瀏覽器中使用今天。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-471">That includes all of the modern browsers in use today.</span></span> <span data-ttu-id="7bdd6-472">其他 （包括即將推出 IE 7） 的現代瀏覽器使用的內建的 XMLHttp 物件時，Internet Explorer 會使用 XMLHttp ActiveX 物件。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-472">Internet Explorer uses the XMLHttp ActiveX object while other modern browsers (including the upcoming IE 7) use an intrinsic XMLHttp object.</span></span> <span data-ttu-id="7bdd6-473">若要以程式設計方式判斷是否瀏覽器支援回呼，您可以使用**Request.Browser.SupportCallback**屬性。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-473">To programmatically determine if a browser supports callbacks, you can use the **Request.Browser.SupportCallback** property.</span></span> <span data-ttu-id="7bdd6-474">這個屬性會傳回 **，則為 true**如果要求的用戶端支援指令碼回呼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-474">This property will return **true** if the requesting client supports script callbacks.</span></span>

## <a name="working-with-client-script-in-aspnet-20"></a><span data-ttu-id="7bdd6-475">使用 ASP.NET 2.0 中的用戶端指令碼</span><span class="sxs-lookup"><span data-stu-id="7bdd6-475">Working with Client Script in ASP.NET 2.0</span></span>

<span data-ttu-id="7bdd6-476">透過使用 ClientScriptManager 類別來管理 ASP.NET 2.0 中的用戶端指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-476">Client scripts in ASP.NET 2.0 are managed via the use of the ClientScriptManager class.</span></span> <span data-ttu-id="7bdd6-477">ClientScriptManager 類別會追蹤的用戶端指令碼使用的型別和名稱。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-477">The ClientScriptManager class keeps track of client scripts using a type and a name.</span></span> <span data-ttu-id="7bdd6-478">這可防止相同的指令碼要以程式設計方式插入頁面上一次以上。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-478">This prevents the same script from being programmatically inserted on a page more than once.</span></span>

> [!NOTE]
> <span data-ttu-id="7bdd6-479">指令碼已成功註冊頁面上之後，任何後續的嘗試註冊相同的指令碼只會在指令碼中未註冊第二次。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-479">After a script has been successfully registered on a page, any subsequent attempt to register the same script will simply result in the script not being registered a second time.</span></span> <span data-ttu-id="7bdd6-480">加入任何重複的指令碼，並沒有發生例外狀況。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-480">No duplicate scripts are added and no exception occurs.</span></span> <span data-ttu-id="7bdd6-481">若要避免不必要的計算，有可用來判斷指令碼是否已經註冊，讓您不會嘗試向一次以上的方法。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-481">To avoid unnecessary computation, there are methods that you can use to determine if a script is already registered so that you do not attempt to register it more than once.</span></span>


<span data-ttu-id="7bdd6-482">ClientScriptManager 方法應該是所有目前的 ASP.NET 開發人員所熟悉的：</span><span class="sxs-lookup"><span data-stu-id="7bdd6-482">The methods of the ClientScriptManager should be familiar to all current ASP.NET developers:</span></span>

## <a name="registerclientscriptblock"></a><span data-ttu-id="7bdd6-483">RegisterClientScriptBlock</span><span class="sxs-lookup"><span data-stu-id="7bdd6-483">RegisterClientScriptBlock</span></span>

<span data-ttu-id="7bdd6-484">這個方法會將指令碼，來呈現頁面的頂端。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-484">This method adds a script to the top of the rendered page.</span></span> <span data-ttu-id="7bdd6-485">這可用於新增將用戶端明確呼叫的函式。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-485">This is useful for adding functions that will be explicitly called on the client.</span></span>

<span data-ttu-id="7bdd6-486">有兩個多載的版本，這個方法。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-486">There are two overloaded versions of this method.</span></span> <span data-ttu-id="7bdd6-487">三個四個引數是在它們之間通用的。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-487">Three of four arguments are common among them.</span></span> <span data-ttu-id="7bdd6-488">包括：</span><span class="sxs-lookup"><span data-stu-id="7bdd6-488">They are:</span></span>

`type (string)`

<span data-ttu-id="7bdd6-489">***型別***引數會識別指令碼的類型。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-489">The ***type*** argument identifies a type for the script.</span></span> <span data-ttu-id="7bdd6-490">它通常是個不錯的主意，若要使用的頁面類型 （這點。GetType()) 類型。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-490">It is generally a good idea to use the page's type (this.GetType()) for the type.</span></span>

`key (string)`

<span data-ttu-id="7bdd6-491">***金鑰***引數是使用者定義索引鍵的指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-491">The ***key*** argument is a user-defined key for the script.</span></span> <span data-ttu-id="7bdd6-492">這應該是唯一的每個指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-492">This should be unique for each script.</span></span> <span data-ttu-id="7bdd6-493">如果您嘗試加入具有相同的索引鍵和型別已新增的指令碼的指令碼，它將不會加入。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-493">If you attempt to add a script with the same key and type of an already added script, it will not be added.</span></span>

`script (string)`

<span data-ttu-id="7bdd6-494">***指令碼***引數是字串，包含要新增的實際指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-494">The ***script*** argument is a string containing the actual script to add.</span></span> <span data-ttu-id="7bdd6-495">建議您在建立指令碼，然後在指派的 StringBuilder 上使用 tostring （） 方法使用 StringBuilder***指令碼***引數。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-495">It's recommended that you use a StringBuilder to create the script and then use the ToString() method on the StringBuilder to assign the ***script*** argument.</span></span>

<span data-ttu-id="7bdd6-496">如果您使用多載的 RegisterClientScriptBlock 只接受三個引數時，您必須包括指令碼項目 (&lt;指令碼&gt;並&lt;/script&gt;) 指令碼中。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-496">If you use the overloaded RegisterClientScriptBlock that only takes three arguments, you must include script elements (&lt;script&gt; and &lt;/script&gt;) in your script.</span></span>

<span data-ttu-id="7bdd6-497">您可以選擇使用 RegisterClientScriptBlock 接受第四個引數的多載。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-497">You may choose to use the overload of RegisterClientScriptBlock that takes a fourth argument.</span></span> <span data-ttu-id="7bdd6-498">第四個引數是布林值，指定 ASP.NET 應該為您新增指令碼項目。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-498">The fourth argument is a Boolean that specifies whether or not ASP.NET should add script elements for you.</span></span> <span data-ttu-id="7bdd6-499">如果這個引數是 **，則為 true**，您的指令碼不應包含的指令碼項目明確。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-499">If this argument is **true**, your script should not include the script elements explicitly.</span></span>

<span data-ttu-id="7bdd6-500">您可以使用 IsClientScriptBlockRegistered 方法來判斷是否已登錄的指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-500">Use the IsClientScriptBlockRegistered method to determine if a script has already been registered.</span></span> <span data-ttu-id="7bdd6-501">這可讓您避免嘗試重新註冊已註冊的指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-501">This allows you to avoid an attempt to re-register a script that has already been registered.</span></span>

### <a name="registerclientscriptinclude-new-in-20"></a><span data-ttu-id="7bdd6-502">RegisterClientScriptInclude （新功能 2.0)</span><span class="sxs-lookup"><span data-stu-id="7bdd6-502">RegisterClientScriptInclude (New in 2.0)</span></span>

<span data-ttu-id="7bdd6-503">RegisterClientScriptInclude 標記建立指令碼區塊連結至外部指令碼檔案。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-503">The RegisterClientScriptInclude tag creates a script block that links to an external script file.</span></span> <span data-ttu-id="7bdd6-504">它有兩個多載。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-504">It has two overloads.</span></span> <span data-ttu-id="7bdd6-505">其中一個採用索引鍵和 URL。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-505">One takes a key and a URL.</span></span> <span data-ttu-id="7bdd6-506">第二個加入的指定類型的第三個引數。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-506">The second adds a third argument specifying the type.</span></span>

<span data-ttu-id="7bdd6-507">例如，下列程式碼會產生指令碼區塊連結至 jsfunctions.js 根目錄中的應用程式的 [指令碼] 資料夾：</span><span class="sxs-lookup"><span data-stu-id="7bdd6-507">For example, the following code generates a script block that links to jsfunctions.js in the root of the scripts folder of the application:</span></span>

[!code-csharp[Main](the-asp-net-2-0-page-model/samples/sample17.cs)]

<span data-ttu-id="7bdd6-508">此程式碼會產生下列程式碼在呈現的頁面：</span><span class="sxs-lookup"><span data-stu-id="7bdd6-508">This code produces the following code in the rendered page:</span></span>

[!code-html[Main](the-asp-net-2-0-page-model/samples/sample18.html)]

> [!NOTE]
> <span data-ttu-id="7bdd6-509">在頁面底部轉譯的指令碼區塊。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-509">The script block is rendered at the bottom of the page.</span></span>


<span data-ttu-id="7bdd6-510">您可以使用 IsClientScriptIncludeRegistered 方法來判斷是否已登錄的指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-510">Use the IsClientScriptIncludeRegistered method to determine if a script has already been registered.</span></span> <span data-ttu-id="7bdd6-511">這可讓您避免嘗試重新註冊的指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-511">This allows you to avoid an attempt to re-register a script.</span></span>

## <a name="registerstartupscript"></a><span data-ttu-id="7bdd6-512">RegisterStartupScript</span><span class="sxs-lookup"><span data-stu-id="7bdd6-512">RegisterStartupScript</span></span>

<span data-ttu-id="7bdd6-513">RegisterStartupScript 方法採用相同的引數，做為 RegisterClientScriptBlock 方法。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-513">The RegisterStartupScript method takes the same arguments as the RegisterClientScriptBlock method.</span></span> <span data-ttu-id="7bdd6-514">在頁面載入之後但在這段用戶端之前，就會執行向 RegisterStartupScript 指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-514">A script registered with RegisterStartupScript executes after the page loads but before the OnLoad client-side event.</span></span> <span data-ttu-id="7bdd6-515">在 1.X 中，向 RegisterStartupScript 的指令碼已置於結尾之前&lt;/形成&gt;標記時向 RegisterClientScriptBlock 的指令碼放置在開頭之後立即&lt;表單&gt;標記。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-515">In 1.X, scripts registered with RegisterStartupScript were placed just before the closing &lt;/form&gt; tag while scripts registered with RegisterClientScriptBlock were placed immediately after the opening &lt;form&gt; tag.</span></span> <span data-ttu-id="7bdd6-516">在 ASP.NET 2.0 中，同時會放在結尾之前，立即&lt;/形成&gt;標記。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-516">In ASP.NET 2.0, both are placed immediately before the closing &lt;/form&gt; tag.</span></span>

> [!NOTE]
> <span data-ttu-id="7bdd6-517">如果您向 RegisterStartupScript 函式，該函式將不會執行，直到明確地在用戶端程式碼中呼叫。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-517">If you register a function with RegisterStartupScript, that function will not execute until you explicitly call it in client-side code.</span></span>


<span data-ttu-id="7bdd6-518">您可以使用 IsStartupScriptRegistered 方法來判斷是否已登錄的指令碼，並避免嘗試重新註冊的指令碼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-518">Use the IsStartupScriptRegistered method to determine if a script has already been registered and avoid an attempt to re-register a script.</span></span>

## <a name="other-clientscriptmanager-methods"></a><span data-ttu-id="7bdd6-519">其他 ClientScriptManager 方法</span><span class="sxs-lookup"><span data-stu-id="7bdd6-519">Other ClientScriptManager Methods</span></span>

<span data-ttu-id="7bdd6-520">以下是一些 ClientScriptManager 類別的其他有用的方法。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-520">Here are some of the other useful methods of the ClientScriptManager class.</span></span>


|  <span data-ttu-id="7bdd6-521"><strong>GetCallbackEventReference</strong></span><span class="sxs-lookup"><span data-stu-id="7bdd6-521"><strong>GetCallbackEventReference</strong></span></span>   |                                                 <span data-ttu-id="7bdd6-522">請參閱稍早在本單元中的指令碼回呼。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-522">See script callbacks earlier in this module.</span></span>                                                 |
|-----------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|
|  <span data-ttu-id="7bdd6-523"><strong>GetPostBackClientHyperlink</strong></span><span class="sxs-lookup"><span data-stu-id="7bdd6-523"><strong>GetPostBackClientHyperlink</strong></span></span>  |                <span data-ttu-id="7bdd6-524">取得 JavaScript 參考 (javascript:&lt;呼叫&gt;)，可用來從用戶端事件回傳。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-524">Gets a JavaScript reference (javascript:&lt;call&gt;) that can be used to post back from a client-side event.</span></span>                 |
|  <span data-ttu-id="7bdd6-525"><strong>GetPostBackEventReference</strong></span><span class="sxs-lookup"><span data-stu-id="7bdd6-525"><strong>GetPostBackEventReference</strong></span></span>   |                                   <span data-ttu-id="7bdd6-526">取得字串，可用來起始從用戶端 post。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-526">Gets a string that can be used to initiate a post back from the client.</span></span>                                    |
|      <span data-ttu-id="7bdd6-527"><strong>GetWebResourceUrl</strong></span><span class="sxs-lookup"><span data-stu-id="7bdd6-527"><strong>GetWebResourceUrl</strong></span></span>       | <span data-ttu-id="7bdd6-528">內嵌組件中的資源傳回的 URL。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-528">Returns a URL to a resource that is embedded in an assembly.</span></span> <span data-ttu-id="7bdd6-529">必須用於搭配<strong>RegisterClientScriptResource</strong>。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-529">Must be used in conjunction with <strong>RegisterClientScriptResource</strong>.</span></span> |
| <span data-ttu-id="7bdd6-530"><strong>RegisterClientScriptResource</strong></span><span class="sxs-lookup"><span data-stu-id="7bdd6-530"><strong>RegisterClientScriptResource</strong></span></span> |     <span data-ttu-id="7bdd6-531">向頁面註冊 Web 資源。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-531">Registers a Web resource with the page.</span></span> <span data-ttu-id="7bdd6-532">這些是內嵌在組件，而且新的 WebResource.axd 處理常式所處理的資源。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-532">These are resources embedded in an assembly and handled by the new WebResource.axd handler.</span></span>      |
|     <span data-ttu-id="7bdd6-533"><strong>RegisterHiddenField</strong></span><span class="sxs-lookup"><span data-stu-id="7bdd6-533"><strong>RegisterHiddenField</strong></span></span>      |                                                 <span data-ttu-id="7bdd6-534">向頁面註冊隱藏的表單欄位。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-534">Registers a hidden form field with the page.</span></span>                                                 |
|  <span data-ttu-id="7bdd6-535"><strong>RegisterOnSubmitStatement</strong></span><span class="sxs-lookup"><span data-stu-id="7bdd6-535"><strong>RegisterOnSubmitStatement</strong></span></span>   |                                  <span data-ttu-id="7bdd6-536">註冊用戶端程式碼的 HTML 表單提交時執行。</span><span class="sxs-lookup"><span data-stu-id="7bdd6-536">Registers client-side code that executes when the HTML form is submitted.</span></span>                                   |
