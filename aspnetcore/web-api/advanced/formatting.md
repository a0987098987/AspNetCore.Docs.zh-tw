...
* 系統會顯示要求標頭。 例如， `Accept`標頭。 前面`Accept`的程式碼會忽略標頭。
...
控制器動作可以傳回 Poco （簡單的 CLR 物件）。 當 POCO 傳回時，執行時間會自動建立`ObjectResult`包裝物件的。 用戶端會取得已格式化的序列化物件。 如果傳回的物件是`null`，則會`204 No Content`傳迴響應。

傳回物件類型：

[!code-csharp[](./formatting/sample/Controllers/AuthorsController.cs?name=snippet_alias)]

在上述程式碼中，有效作者別名的要求會傳回含有`200 OK`作者資料的回應。 對無效別名的要求會傳回`204 No Content`回應。

### <a name="the-accept-header"></a>Accept 標頭

當*negotiation*要求中出現`Accept`標頭時，就會進行內容協商。 當要求包含 accept 標頭時，ASP.NET Core：
...
如果 Accept 標頭包含`*/*`，除非在上`RespectBrowserAcceptHeader` <xref:Microsoft.AspNetCore.Mvc.MvcOptions>將設為 True，否則會忽略標頭。
...
某些功能可能不適用於以`System.Text.Json`為基礎的格式器，而且需要參考以`Newtonsoft.Json`為基礎的格式器。 如果應用程式`Newtonsoft.Json`有下列情況，請繼續使用以為基礎的格式器：
...
若要限制回應格式，請套用[`[Produces]`](xref:Microsoft.AspNetCore.Mvc.ProducesAttribute)篩選準則。 就像大部分的`[Produces]` [篩選器](xref:mvc/controllers/filters)一樣，可以在動作、控制器或全域範圍套用：
...
有些特殊案例是使用內建格式器所實作。 根據預設， `string`傳回類型的格式為*text/純文字*（如果透過`Accept`標頭要求，則為*text/html* ）。 您可以藉由移除來刪除這個<xref:Microsoft.AspNetCore.Mvc.Formatters.StringOutputFormatter>行為。 已移除方法中的`ConfigureServices`格式器。 傳回時，具有模型物件傳回類型的`204 No Content`動作會`null`返回。 您可以藉由移除來刪除這個<xref:Microsoft.AspNetCore.Mvc.Formatters.HttpNoContentOutputFormatter>行為。 下列程式碼會移除 `StringOutputFormatter` 和 `HttpNoContentOutputFormatter`。
...
若沒有`StringOutputFormatter`，內建的 JSON 格式器格式`string`會傳回類型。 如果已移除內建 JSON 格式器，而且有 XML 格式器，則 XML 格式器格式`string`會傳回類型。 否則， `string`傳回類型會`406 Not Acceptable`傳回。
