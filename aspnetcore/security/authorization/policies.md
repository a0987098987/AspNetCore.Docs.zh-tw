...
原則會套用至 Razor Pages, 方法是`[Authorize]`使用具有原則名稱的屬性。 例如：
...
`MinimumAgeHandler` 上述`services.AddSingleton<IAuthorizationHandler, MinimumAgeHandler>();`程式碼會藉由叫用將註冊為 singleton。 您可以使用任何內建[服務存留期](xref:fundamentals/dependency-injection#service-lifetimes)來註冊處理常式。
...
在某些情況下, 在程式碼中可以簡單地表達原則。 `Func<AuthorizationHandlerContext, bool>`當您`RequireAssertion`使用原則產生器來設定原則時, 可以提供。
...
您`HandleRequirementAsync`在授權處理常式中執行的方法有兩個參數`AuthorizationHandlerContext` : 和`TRequirement`您正在處理的。 MVC 或 Jabbr 這類架構可自由地將任何物件加入至`Resource` `AuthorizationHandlerContext`上的屬性, 以傳遞額外的資訊。

例如, MVC 會在`Resource`屬性中傳遞[AuthorizationFilterCoNtext](/dotnet/api/?term=AuthorizationFilterContext)的實例。 這個屬性可讓您`HttpContext`存取`RouteData`MVC 和 Razor Pages 所提供的、和所有其他專案。

`Resource`屬性的使用是架構特有的。 使用屬性中的`Resource`資訊會將您的授權原則限制為特定架構。 您應該`Resource` `is`使用關鍵字來轉換屬性, 然後確認轉換已成功, 以確保您的程式`InvalidCastException`代碼在其他架構上執行時不會損毀:
