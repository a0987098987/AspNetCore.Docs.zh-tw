...
這是通常會有不同的環境，例如開發、 測試和實際執行不同的組態設定。 下列反白顯示的程式碼會將兩個組態提供者加入至三個來源：

1. JSON 提供者，讀取*appsettings.json*
2. JSON 提供者，讀取*appsettings。\<EnvironmentName >.json*
3. 環境變數提供者
...
繫結中的自訂類別`ConfigureServices`中`Startup`類別：
