...
如果以 .NET Core 為目標，請在專案`Version`檔中移除中繼套件參考的屬性。 包含`Version`屬性會產生下列警告：
...
如果應用程式呼叫<xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*>的方式`CreateDefaultBuilder`是在`Program`類別的[CreateWebHostBuilder 方法](xref:fundamentals/host/web-host#set-up-a-host)中呼叫， `ConfigureKestrel`請呼叫來設定 Kestrel 伺服器`UseKestrel`而不是，以避免與[IIS 同進程裝載模型](xref:host-and-deploy/iis/index#in-process-hosting-model)產生衝突：
...
將 Dockerfile `FROM`中的線條*Dockerfile*變更為使用上表2.2 資料行中的新影像標記。
