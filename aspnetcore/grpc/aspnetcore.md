...
用於 gRPC 的 Kestrel 端點應使用 TLS 來保護。 在開發期間，會在 ASP.NET Core 開發憑證存在`https://localhost:5001`時，自動建立以 TLS 保護的端點。 不需要進行任何設定。 `https`前置詞會驗證 Kestrel 端點是否使用 TLS。
...
#### <a name="protocol-negotiation"></a>通訊協定協商
