...
> [!NOTE]
> `DirectoryInfo`和`FileInfo`類別位於[`System.IO`命名空間](https://msdn.microsoft.com/en-us/library/system.io.aspx)。 因此，您必須以其命名空間名稱的這些類別名稱的前面上或匯入類別檔案的命名空間 (透過`Imports System.IO`)。
