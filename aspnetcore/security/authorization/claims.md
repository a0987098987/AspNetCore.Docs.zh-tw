...
以宣告為基礎的授權檢查是宣告式的-開發人員會將它們內嵌在其程式碼中，針對控制器或控制站中的某個動作，指定目前使用者必須擁有的宣告，並選擇性地將宣告必須保留以存取要求的資源。 宣告需求是以原則為基礎，開發人員必須建立並註冊表示宣告需求的原則。

最簡單的宣告原則類型會尋找宣告是否存在，且不會檢查值。

首先，您必須建立並註冊原則。 這會在授權服務設定過程中發生，通常會在*Startup.cs*檔案中納入 `ConfigureServices()`。
...
在此情況下，`EmployeeOnly` 原則會檢查目前身分識別上是否存在 `EmployeeNumber` 宣告。

然後，您可以使用 `AuthorizeAttribute` 屬性上的 `Policy` 屬性來套用原則，以指定原則名稱。
...
如果您的控制器受到 `AuthorizeAttribute` 屬性的保護，但想要允許匿名存取特定動作，請套用 `AllowAnonymousAttribute` 屬性。
...
在上述範例中，滿足 `EmployeeOnly` 原則的任何識別都可以存取 `Payslip` 動作，因為該原則會在控制器上強制執行。 不過，為了呼叫 `UpdateSalary` 動作，身分識別必須*同時*滿足 `EmployeeOnly` 原則和 `HumanResources` 原則。
