...
> 
> <span data-ttu-id="1fc5d-131">閱讀更多有關中的 HTML 編碼[使用表單](https://go.microsoft.com/fwlink/?LinkId=202892)。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-131">You can read more about HTML encoding in [Working with Forms](https://go.microsoft.com/fwlink/?LinkId=202892).</span></span>
...
<span data-ttu-id="1fc5d-170">在瀏覽器中顯示的結果 (按一下後**送出**):</span><span class="sxs-lookup"><span data-stu-id="1fc5d-170">The result displayed in a browser (after clicking **Submit**):</span></span>
...
<span data-ttu-id="1fc5d-205">但首先，很少的技術背景。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-205">But first, a little technical background.</span></span>
...
### <a name="code-and-markup-comments"></a><span data-ttu-id="1fc5d-270">程式碼 （和標記） 的註解</span><span class="sxs-lookup"><span data-stu-id="1fc5d-270">Code (and Markup) Comments</span></span>
...
<span data-ttu-id="1fc5d-300">您宣告變數使用`var`關鍵字 （如果您不想要指定型別） 或使用類型的名稱：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-300">You declare a variable using the `var` keyword (if you don't want to specify a type) or by using the name of the type:</span></span>
...
<span data-ttu-id="1fc5d-317">下表列出一些常見的轉換和測試方法的變數。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-317">The following table lists some common conversion and test methods for variables.</span></span>
...
|--------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------|
|        <span data-ttu-id="1fc5d-335">`+` `-` `*` `/`</span><span class="sxs-lookup"><span data-stu-id="1fc5d-335">`+` `-` `*` `/`</span></span>         |                                                            <span data-ttu-id="1fc5d-336">數學運算子用在數值運算式。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-336">Math operators used in numerical expressions.</span></span>                                                             |    [!code-css[Main](introducing-razor-syntax-c/samples/sample34.css)]    |
...
- <span data-ttu-id="1fc5d-395">`myImagesFolder`: `http://myserver/myapp/images`</span><span class="sxs-lookup"><span data-stu-id="1fc5d-395">`myImagesFolder`: `http://myserver/myapp/images`</span></span>
...
<span data-ttu-id="1fc5d-410">在此範例中，如果第一個條件，在如果區塊不成立，`else if`條件會檢查。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-410">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="1fc5d-411">如果符合該條件中的陳述式`else if`區塊會執行。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-411">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="1fc5d-412">如果不符合任何一個條件中的陳述式`else`區塊會執行。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-412">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="1fc5d-413">您可以加入任意數目的 else 的 if 區塊，並關閉與`else`為封鎖&quot;一切&quot;條件。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-413">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="1fc5d-414">若要測試條件數量龐大，請使用`switch`區塊：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-414">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="1fc5d-415">要測試的值位於括號內 (在範例中，`weekday`變數)。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-415">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="1fc5d-416">每個個別的測試會使用`case`結尾是冒號 （:） 的陳述式。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-416">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="1fc5d-417">如果值`case`陳述式的測試值相符，該案例區塊中的程式碼會執行。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-417">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="1fc5d-418">關閉與每個 case 陳述式`break`陳述式。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-418">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="1fc5d-419">(如果您忘記包含在每個中斷`case`封鎖，請從下一個程式碼`case`陳述式也會執行。)A`switch`區塊通常會有`default`陳述式的最後一個案例&quot;一切&quot;其他情況下都是的則為 true 時執行的選項。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-419">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="1fc5d-420">在瀏覽器中顯示最後兩個條件式區塊的結果：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-420">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor-Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="1fc5d-422">迴圈的程式碼</span><span class="sxs-lookup"><span data-stu-id="1fc5d-422">Looping Code</span></span>

<span data-ttu-id="1fc5d-423">您通常需要重複執行相同的陳述式。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-423">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="1fc5d-424">您的迴圈。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-424">You do this by looping.</span></span> <span data-ttu-id="1fc5d-425">例如，您通常會執行相同的陳述式，每個項目集合中的資料。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-425">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="1fc5d-426">如果您知道完全多少次您想要執行迴圈，您可以使用`for`迴圈。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-426">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="1fc5d-427">這種迴圈是特別適用於計數或向下計數：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-427">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="1fc5d-428">迴圈開頭`for`每個以分號結束關鍵字，後面接著括號中，三個陳述式。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-428">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="1fc5d-429">在括弧內，第一個陳述式 (`var i=10;`) 建立計數器，並將它初始化為 10。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-429">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="1fc5d-430">您不需要名稱計數器`i`&#8212;您可以使用任何變數。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-430">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="1fc5d-431">當`for`迴圈會執行，此計數器會自動遞增。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-431">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="1fc5d-432">第二個陳述式 (`i < 21;`) 設定您想要計算的幅度的條件。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-432">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="1fc5d-433">您想在此情況下，請移至最多 20 個 （也就繼續執行時，此計數器會小於 21）。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-433">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="1fc5d-434">第三個陳述式 (`i++` ) 會使用遞增運算子，只需指定計數器應有 1 加入至它在每次執行迴圈。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-434">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="1fc5d-435">在括號內是程式碼會執行迴圈的每個反覆項目。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-435">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="1fc5d-436">標記會建立新的段落 (`<p>`元素) 在每次且會將行加入輸出中顯示的值`i`（計數器）。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-436">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="1fc5d-437">當您執行此頁面時，此範例會建立 11 行顯示輸出，以指出項目編號每一行文字。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-437">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor-Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="1fc5d-439">如果您使用集合或陣列，您通常使用`foreach`迴圈。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-439">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="1fc5d-440">集合是一組類似的物件，而`foreach`迴圈可讓您執行集合中每個項目的工作。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-440">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="1fc5d-441">這種類型的迴圈很方便的集合，因為與不同的是`for`迴圈中，您不必遞增計數器或設定的限制。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-441">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="1fc5d-442">相反地，`foreach`迴圈的程式碼會繼續執行此集合直到完成為止。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-442">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="1fc5d-443">例如，下列程式碼會傳回中的項目`Request.ServerVariables`集合，其包含您的 web 伺服器的相關資訊的物件。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-443">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="1fc5d-444">它會使用`foreac`h 迴圈以顯示每個項目的名稱來建立新`<li>`HTML 項目符號清單中的項目。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-444">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="1fc5d-445">`foreach`關鍵字後面接著括號中您宣告的變數，表示集合中的單一項目 (在範例中， `var item`)，後面接著`in`關鍵字，後面接著您想要重複使用的集合。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-445">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="1fc5d-446">本文的`foreach`迴圈中，您可以存取目前的項目，使用您稍早宣告的變數。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-446">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor-Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="1fc5d-448">若要建立更通用的迴圈，請使用`while`陳述式：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-448">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="1fc5d-449">A`while`迴圈開頭`while`關鍵字，後面接著括號，您可以指定迴圈繼續的時間長度 (這裡，只要`countNum`小於 50)，然後重複區塊。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-449">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="1fc5d-450">通常遞增迴圈 （加入） 或遞減 （被減數） 變數或物件用來計算。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-450">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="1fc5d-451">在範例中，`+=`運算子會加入 1 到`countNum`每次執行迴圈。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-451">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="1fc5d-452">(以遞減的變數向下計數在迴圈中，您會使用遞減運算子`-=`)。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-452">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="1fc5d-453">物件和集合</span><span class="sxs-lookup"><span data-stu-id="1fc5d-453">Objects and Collections</span></span>

<span data-ttu-id="1fc5d-454">幾乎所有的作業在 ASP.NET 網站是物件，包括網頁本身。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-454">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="1fc5d-455">本節討論一些重要的物件，您將使用經常在您的程式碼中。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-455">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="1fc5d-456">頁面物件</span><span class="sxs-lookup"><span data-stu-id="1fc5d-456">Page Objects</span></span>

<span data-ttu-id="1fc5d-457">在 ASP.NET 中的最基本物件是的頁面。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-457">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="1fc5d-458">您可以存取直接沒有任何合格的物件的頁面物件的屬性。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-458">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="1fc5d-459">下列程式碼取得網頁的檔案路徑，使用`Request`頁面的物件：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-459">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="1fc5d-460">要取消選取您要參考屬性和目前的頁面物件上的方法，您可以選擇性地使用關鍵字`this`來代表在程式碼中的頁面物件。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-460">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="1fc5d-461">以下是上述程式碼範例中，與`this`新增至代表的頁面：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-461">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="1fc5d-462">您可以使用屬性的`Page`物件取得許多資訊，例如：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-462">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="1fc5d-463">`Request`.</span><span class="sxs-lookup"><span data-stu-id="1fc5d-463">`Request`.</span></span> <span data-ttu-id="1fc5d-464">如您所見，這是目前的要求，包括何種瀏覽器所做要求的頁面、 使用者識別、 等等的 URL 的相關資訊的集合。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-464">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="1fc5d-465">`Response`.</span><span class="sxs-lookup"><span data-stu-id="1fc5d-465">`Response`.</span></span> <span data-ttu-id="1fc5d-466">這是會在伺服端程式碼執行完成時傳送到瀏覽器的回應 （頁面） 的相關資訊的集合。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-466">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="1fc5d-467">例如，您可以使用此屬性寫入至回應的資訊。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-467">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="1fc5d-468">（陣列和字典） 的集合物件</span><span class="sxs-lookup"><span data-stu-id="1fc5d-468">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="1fc5d-469">A*集合*是屬於相同的類型，例如集合的物件群組`Customer`資料庫中的物件。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-469">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="1fc5d-470">ASP.NET 包含許多的內建集合，例如`Request.Files`集合。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-470">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="1fc5d-471">您通常會使用集合中的資料。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-471">You'll often work with data in collections.</span></span> <span data-ttu-id="1fc5d-472">兩個常見的集合型別是*陣列*和*字典*。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-472">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="1fc5d-473">當您想要儲存的類似項目集合，但不想要建立個別的變數來保存每個項目時，陣列會是很有用：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-473">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="1fc5d-474">使用陣列時，您宣告為特定的資料類型，例如`string`， `int`，或`DateTime`。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-474">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="1fc5d-475">若要表示變數可以包含陣列，請在宣告中加入括號 (例如`string[]`或`int[]`)。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-475">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="1fc5d-476">您可以存取項目中使用它們的位置 （索引） 的陣列，或使用`foreach`陳述式。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-476">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="1fc5d-477">陣列索引以零為起始&#8212;也就是第一個項目是在位置 0，第二個項目位於位置 1，依此類推。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-477">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="1fc5d-478">您可以判斷陣列中的項目數目，藉由取得其`Length`屬性。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-478">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="1fc5d-479">若要取得特定的項目 （如果您要搜尋的陣列） 的陣列中的位置，請使用`Array.IndexOf`方法。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-479">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="1fc5d-480">您也可以執行像是反向陣列的內容 (`Array.Reverse`方法) 或排序內容 (`Array.Sort`方法)。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-480">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="1fc5d-481">在瀏覽器中顯示的字串陣列程式碼的輸出：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-481">The output of the string array code displayed in a browser:</span></span>

![Razor-Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="1fc5d-483">字典是索引鍵/值組的集合，您提供的索引鍵 （或名稱） 來設定或擷取對應的值：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-483">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="1fc5d-484">若要建立字典，請使用`new`關鍵字，指出您要建立新的字典物件。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-484">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="1fc5d-485">您可以將變數使用字典`var`關鍵字。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-485">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="1fc5d-486">指出資料類型的字典使用角括號中的項目 ( `< >` )。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-486">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="1fc5d-487">在宣告的結尾，您必須加入一組括弧，因為這是實際的方法，建立新的字典。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-487">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="1fc5d-488">若要新增至字典的項目，您可以呼叫`Add`字典變數的方法 (`myScores`在此情況下)，然後指定索引鍵和值。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-488">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="1fc5d-489">或者，您可以使用方括號，以表示索引鍵，並執行簡單指派，如下列範例所示：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-489">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="1fc5d-490">若要從字典取得值，您可以指定索引鍵括號括住：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-490">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="1fc5d-491">呼叫具有參數的方法</span><span class="sxs-lookup"><span data-stu-id="1fc5d-491">Calling Methods with Parameters</span></span>

<span data-ttu-id="1fc5d-492">當您閱讀稍早在本文中，您使用程式設計物件的方法。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-492">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="1fc5d-493">例如，`Database`物件可能會有`Database.Connect`方法。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-493">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="1fc5d-494">許多方法也有一或多個參數。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-494">Many methods also have one or more parameters.</span></span> <span data-ttu-id="1fc5d-495">A*參數*是傳遞至方法的值以啟用方法，以完成其工作。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-495">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="1fc5d-496">例如，查看的宣告`Request.MapPath`方法，這個方法會採用三個參數：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-496">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="1fc5d-497">（行被包裝成讓它更容易閱讀。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-497">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="1fc5d-498">請記住您可以將的分行符號幾乎任何地方除了內部字串括在引號。)</span><span class="sxs-lookup"><span data-stu-id="1fc5d-498">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="1fc5d-499">這個方法會傳回至指定的虛擬路徑對應的伺服器上的實體路徑。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-499">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="1fc5d-500">方法有三個參數`virtualPath`， `baseVirtualDir`，和`allowCrossAppMapping`。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-500">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="1fc5d-501">（請注意在宣告中，列出參數的資料，它們會接受的資料類型）。當您呼叫這個方法時，您必須提供所有的三個參數的值。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-501">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="1fc5d-502">Razor 語法可讓您將參數傳遞至方法的兩個選項：*位置參數*和*具名參數*。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-502">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="1fc5d-503">若要呼叫方法，使用位置參數，您傳入參數嚴格的順序，方法宣告中所指定。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-503">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="1fc5d-504">（您將通常知道此順序閱讀文件的方法）。您必須遵循的順序，並不能略過任何參數&#8212;如果有必要，您傳遞空字串 (`""`) 或`null`位置參數，您不需要的值。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-504">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="1fc5d-505">下列範例假設您有一個名為資料夾*指令碼*在您的網站。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-505">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="1fc5d-506">程式碼會呼叫`Request.MapPath`依正確順序的三個參數的方法，並傳遞值。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-506">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="1fc5d-507">然後，它會顯示產生的對應的路徑。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-507">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="1fc5d-508">當方法有許多參數時，您可以使用具名參數，將程式碼更容易閱讀。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-508">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="1fc5d-509">若要呼叫的方法使用具名的參數，您可以指定參數名稱後面接著冒號 （:），然後此值。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-509">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="1fc5d-510">具名參數的優點是，您就可以將其傳遞任何您想要的順序。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-510">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="1fc5d-511">（的缺點是在方法呼叫不是精簡）。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-511">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="1fc5d-512">下列範例會呼叫上述方法，但使用具名參數，以提供值：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-512">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="1fc5d-513">如您所見，將參數傳遞不同的順序。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-513">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="1fc5d-514">不過，如果您執行上述範例，此範例中，它們會傳回相同的值。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-514">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="1fc5d-515">處理錯誤</span><span class="sxs-lookup"><span data-stu-id="1fc5d-515">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="1fc5d-516">Try Catch 陳述式</span><span class="sxs-lookup"><span data-stu-id="1fc5d-516">Try-Catch Statements</span></span>

<span data-ttu-id="1fc5d-517">您通常會需要陳述式可能失敗的原因，您無法控制的程式碼。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-517">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="1fc5d-518">例如: </span><span class="sxs-lookup"><span data-stu-id="1fc5d-518">For example:</span></span>

- <span data-ttu-id="1fc5d-519">如果您的程式碼會嘗試建立或存取的檔案，可能會發生各種錯誤。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-519">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="1fc5d-520">您想要的檔案可能不存在，可能會遭到鎖定，程式碼可能不具有權限，等等。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-520">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="1fc5d-521">同樣地，如果您的程式碼會嘗試以更新資料庫中的記錄，可以有權限問題、 可能會卸除資料庫的連接、 要儲存的資料可能不正確，依此類推。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-521">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="1fc5d-522">在程式設計的詞彙，這些情況下會呼叫*例外狀況*。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-522">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="1fc5d-523">如果您的程式碼遇到例外狀況，則會產生 （會擲回） 的錯誤訊息，最多只能不快使用者：</span><span class="sxs-lookup"><span data-stu-id="1fc5d-523">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor-Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="1fc5d-525">在您的程式碼可能會遇到例外狀況的情況下，以及為了避免此類型的錯誤訊息，您可以使用`try/catch`陳述式。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-525">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="1fc5d-526">在`try`陳述式中，執行您正在檢查的程式碼。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-526">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="1fc5d-527">一或多個`catch`陳述式，您可以尋找特定可能發生的錯誤 （特定類型的例外狀況）。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-527">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="1fc5d-528">您可以包含最大數量`catch`陳述式，當您需要尋找您所預期的錯誤。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-528">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="1fc5d-529">我們建議您避免使用`Response.Redirect`方法中的`try/catch`陳述式，因為它會在頁面中造成例外狀況。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-529">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>


<span data-ttu-id="1fc5d-530">下列範例顯示建立第一次要求的文字檔，然後顯示按鈕，可讓使用者開啟檔案的頁面。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-530">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="1fc5d-531">此範例刻意使用不正確的檔案名稱，因此它將會導致例外狀況。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-531">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="1fc5d-532">程式碼包含`catch`陳述式中的兩個可能的例外狀況： `FileNotFoundException`，發生錯誤，檔案名稱是否與`DirectoryNotFoundException`，發生於 ASP.NET 甚至無法尋找資料夾。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-532">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="1fc5d-533">（您可以取消此範例中的陳述式才能看到一切正常運作時，它的執行方式註解）。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-533">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="1fc5d-534">如果您的程式碼未處理例外狀況，您會看到錯誤頁面類似上一個螢幕擷取畫面。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-534">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="1fc5d-535">不過，`try/catch`一節可協助防止使用者看到這些類型的錯誤。</span><span class="sxs-lookup"><span data-stu-id="1fc5d-535">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="1fc5d-536">其他資源</span><span class="sxs-lookup"><span data-stu-id="1fc5d-536">Additional Resources</span></span>

<span data-ttu-id="1fc5d-537">**使用 Visual Basic 進行程式設計**</span><span class="sxs-lookup"><span data-stu-id="1fc5d-537">**Programming with Visual Basic**</span></span>


[<span data-ttu-id="1fc5d-538">附錄： Visual Basic 語言和語法</span><span class="sxs-lookup"><span data-stu-id="1fc5d-538">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)


<span data-ttu-id="1fc5d-539">**參考文件**</span><span class="sxs-lookup"><span data-stu-id="1fc5d-539">**Reference Documentation**</span></span>


[<span data-ttu-id="1fc5d-540">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="1fc5d-540">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="1fc5d-541">C# 語言</span><span class="sxs-lookup"><span data-stu-id="1fc5d-541">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
