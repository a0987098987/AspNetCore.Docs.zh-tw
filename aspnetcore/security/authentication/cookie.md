...
在`Startup.ConfigureServices`方法中，使用<xref:Microsoft.Extensions.DependencyInjection.AuthenticationServiceCollectionExtensions.AddAuthentication*>和<xref:Microsoft.Extensions.DependencyInjection.CookieExtensions.AddCookie*>方法建立驗證中介軟體服務：
...
在`Startup.Configure`中， `UseAuthentication`呼叫`UseAuthorization`和，以`HttpContext.User`設定要求的屬性和執行授權中介軟體。 呼叫`UseAuthentication`和方法`UseAuthorization` ，再呼叫`UseEndpoints`：
...
<xref:System.Security.Claims.ClaimsIdentity>使用任何必要<xref:System.Security.Claims.Claim>的來建立，並<xref:Microsoft.AspNetCore.Authentication.AuthenticationHttpContextExtensions.SignInAsync*>呼叫以登入使用者：
...
若要在資料庫根據`LastChanged`值變更時使 cookie 失效，請使用`LastChanged`包含資料庫目前`LastChanged`值的宣告來建立 cookie：
...
在<xref:Microsoft.AspNetCore.Authentication.AuthenticationProperties.IsPersistent>中`true` <xref:Microsoft.AspNetCore.Authentication.AuthenticationProperties>，將設為：
...
在`Startup.Configure`方法中，呼叫`UseAuthentication`方法來叫用設定`HttpContext.User`屬性的驗證中介軟體。 呼叫`UseAuthentication` `UseMvcWithDefaultRoute`或之前，請`UseMvc`先呼叫方法：
