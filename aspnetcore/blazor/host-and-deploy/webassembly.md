...
* 在不支援靜態壓縮檔案內容協商（例如 GitHub 頁面）的靜態裝載解決方案上裝載時，請考慮將應用程式設定為提取和解碼 Brotli 壓縮檔案：
  ...
  * 更新應用程式以使用此解碼器。 將 `<body>` *wwwroot/index.html*中結束記號內的標記變更為下列內容：
...
因為瀏覽器會提出要求給以網際網路為基礎的主機以取得用戶端頁面，所以網頁伺服器和裝載服務必須針對實際上不在伺服器上資源，將所有要求重新撰寫至 *index.html* 頁面。 傳回*index.html*時，應用程式的 Blazor 路由器會接管並以正確的資源回應。

部署到 IIS 伺服器時，您可以使用 URL 重寫模組搭配應用程式的已發佈*web.config*檔案。 如需詳細資訊，請參閱[IIS](#iis)一節。
...
若要使用自訂*web.config*檔案，請將自訂*web.config*檔案放在專案資料夾的根目錄，併發行專案。
...
* 停用繼承的 ASP.NET Core 模組處理常式。
  ...
  Blazor將區段新增至檔案，以移除應用程式已發佈*web.config*檔案中的處理常式 `<handlers>` ：
...
除了設定[應用程式的基底路徑](xref:blazor/host-and-deploy/index#app-base-path)之外，也會執行移除處理常式或停用繼承。 在應用程式的 *index.html* 檔案，將應用程式基底路徑設為在 IIS 中設定子應用程式時使用的 IIS 別名。
...
* 將 [錯誤文件路徑]**** 設定為 `index.html`。 Razor元件和其他非檔案端點不會位於 blob 服務所儲存之靜態內容中的實體路徑。 收到路由器應處理的其中一個資源的要求時 Blazor ，由 blob 服務產生的*404-找不*到的錯誤會將要求路由傳送至**錯誤檔路徑**。 會傳回*index.html* blob，而路由器會 Blazor 載入並處理路徑。
...
下列*nginx*檔案已簡化，示範如何設定 nginx 在磁片上找不到對應的檔案時，傳送*index.html*檔案。
...
1. 建立 Apache 設定檔。 下列範例是一個簡化的設定檔案（*blazorapp.config*）：
...
若要解決 gz 和*blazor.boot.json.br*檔案的壓縮*blazor.boot.js* ，請採用下列其中一種方法：

* 移除 gz 和*blazor.boot.json.br*檔案上的壓縮*blazor.boot.js* 。 此方法會停用壓縮。
* 重新壓縮檔案*上*更新的blazor.boot.js。
