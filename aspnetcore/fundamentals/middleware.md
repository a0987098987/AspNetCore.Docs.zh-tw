...
---
# <a name="aspnet-core-middleware-fundamentals"></a>ASP.NET Core 中介軟體基本概念
...
[將 HTTP 模組移轉至中介軟體](../migration/http-modules.md)說明 ASP.NET Core 和舊版中的要求管線差異，並提供更多中介軟體範例。
...
![要求處理模式說明要求的抵達、透過三個中介軟體進行處理，以及離開應用程式的回應。 每個中介軟體會執行自己的邏輯，並於 next() 陳述式中將要求遞交給下一個中介軟體。 在第三個中介軟體處理要求後，該要求會經前兩個中介軟體交還，以在 next() 陳述式後、離開應用程式前，逐一進行額外處理，並作為給用戶端的回應。](middleware/_static/request-delegate-pipeline.png)
