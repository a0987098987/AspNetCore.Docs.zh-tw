...
* 取代的套件參考`Microsoft.AspNetCore.All`使用的套件參考`Microsoft.AspNetCore.App`。 您可能需要新增相依性，移除了`Microsoft.AspNetCore.All`。 如需詳細資訊，請參閱 <xref:fundamentals/metapackage#migrate> 與 <xref:fundamentals/metapackage-app>。 針對.NET Framework 為目標的應用程式：
  * 加入個別的套件參考，而非中繼套件參考。
  * 更新 2.1 的每個封裝參考。
...
## <a name="rules-for-projects-targeting-the-shared-runtime"></a>共用執行階段為目標的專案的規則

ASP.NET Core 包含下列的共用執行階段：
...
* 使用`Microsoft.AspNetCore.App`使用 Entity Framework Core 2.1.1 2.1.0 會導致 NuGet 還原失敗，因為套件衝突。 建議的解決方案是升級`Microsoft.AspNetCore.App`2.1.1 到。 如需詳細資訊，請參閱 < [Microsoft.AspNetCore.App 與共用相依性的套件不能參考修補程式版本](https://github.com/aspnet/Universe/issues/1180)。
...
* [SetCompatibilityVersion](xref:fundamentals/startup#setcompatibilityversion)針對`SetCompatibilityVersion(CompatibilityVersion.Version_2_1)`。
...
* 使用身分識別 UI 2.0 程式碼，而不需變更。 完全支援使用識別 UI 2.0 程式碼。 當 g endian< 變更已對產生的身分識別程式碼時，這會是不錯的方法。
...
* 刪除產生的身分識別 scaffolder `IdentityDbContext` derrived 類別*區域/身分識別/Data/* 資料夾。
* 刪除*Areas/Identity/IdentityHostingStartup.cs*
* 更新 *_LoginPartial.cshtml*檔案：
  * 移動*Pages/_LoginPartial.cshtml*到*Pages/Shared/_LoginPartial.cshtml*
...
* 移動*pages/_layout.cshtml*到*Pages/Shared/_Layout.cshtml*
