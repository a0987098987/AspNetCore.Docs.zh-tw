...
系統會提早在管線中呼叫靜態檔案中介軟體，以便其處理要求並執行最少運算，而無需一一處理剩餘的元件。 靜態檔案中介軟體**不提供**授權檢查。 其提供的所有檔案，包括在 *wwwroot* 下的檔案，皆可公開使用。 如需保護靜態檔案的方法，請參閱[靜態檔案](xref:fundamentals/static-files)。
