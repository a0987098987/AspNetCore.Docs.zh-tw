...
當您開發新的應用程式時，您的資料模型變更常見問題，以及每次將模型變更時，它會取得與資料庫同步處理。 您已設定 Entity Framework 自動卸除並重新建立每次變更資料模型資料庫。 當您新增、 移除或變更實體類別或變更您`DbContext`類別，在下次執行應用程式它自動刪除現有的資料庫，然後建立一個新符合模型，並設測試資料。

讓資料庫保持同步資料模型的這個方法適用於也直到您將部署到生產環境應用程式。 當應用程式在生產環境中執行時，通常正在儲存您想要保留，且您不想遺失的所有項目每次您進行變更，例如加入新的資料行的資料。 [Code First 移轉](https://msdn.microsoft.com/data/jj591621)功能藉由啟用程式碼第一次更新資料庫結構描述，而不是卸除並重新建立資料庫來解決這個問題。 在本教學課程中，您要部署應用程式，並準備，您將會啟用移轉。
...
2. 應用程式中而且*Web.config*檔案中，將連接字串中的資料庫名稱變更為 ContosoUniversity2。
    ...
    這項變更設定，讓第一次移轉將會建立新的資料庫專案。 這不是必要，但您稍後就會看到的原因是個不錯的主意。
...
- 使用**伺服器總管**檢查資料庫，您可以如同在第一個教學課程中，並執行應用程式確認運作一切仍然正常相同和以前一樣。
...
7. 當已驗證的連接時，旁邊顯示綠色的核取記號**驗證連線** 按鈕。 按 [ **下一步**]。
...
8. 開啟**遠端的連接字串**底下的下拉式清單**SchoolContext**並選取您所建立之資料庫的連接字串。
9. 選取**更新資料庫**。
