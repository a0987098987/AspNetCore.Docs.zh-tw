...
<span data-ttu-id="cfe9e-163">估計的時間才能完成這個實驗室： **75 分鐘**</span><span class="sxs-lookup"><span data-stu-id="cfe9e-163">Estimated time to complete this lab: **75 minutes**</span></span>
...
1. <span data-ttu-id="cfe9e-175">開啟 Visual Studio，然後開啟**GeekQuiz.sln**方案檔從**Source\Ex1 UsingEntityFrameworkMigrations\Begin**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-175">Open Visual Studio and open the **GeekQuiz.sln** solution file from **Source\Ex1-UsingEntityFrameworkMigrations\Begin**.</span></span>
...
1. <span data-ttu-id="cfe9e-221">在 **方案總管**，按兩下**TriviaQuestion.cs**檔案位於**模型**資料夾。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-221">In **Solution Explorer**, double-click the **TriviaQuestion.cs** file located inside the **Models** folder.</span></span>
...
11. <span data-ttu-id="cfe9e-246">在  **SQL Server 物件總管**，重新整理**dbo。TriviaQuestions**資料表，並檢查**提示**資料行類型是**nvarchar(150)**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-246">In **SQL Server Object Explorer**, refresh the **dbo.TriviaQuestions** table and check that the **Hint** column type is **nvarchar(150)**.</span></span>
    ...
    <span data-ttu-id="cfe9e-248">*顯示新的條件約束*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-248">*Showing the new constraint*</span></span>
...
9. <span data-ttu-id="cfe9e-302">Web 應用程式建立之後，請按一下下方的連結**URL**檢查新的 web 應用程式是否正在執行的資料行。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-302">Once the web app is created, click the link under the **URL** column to check that the new web app is running.</span></span>
...
4. <span data-ttu-id="cfe9e-316">按一下  **SQL Database**若要查看 Azure 中的 SQL 資料庫清單</span><span class="sxs-lookup"><span data-stu-id="cfe9e-316">Click **SQL Databases** to see the list of SQL databases in Azure</span></span>
    ...
    <span data-ttu-id="cfe9e-318">*SQL Database 功能表*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-318">*SQL Database menu*</span></span>
...
8. <span data-ttu-id="cfe9e-364">開啟新**Git Bash**主控台並執行下列命令。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-364">Open a new **Git Bash** console and execute the following commands.</span></span> <span data-ttu-id="cfe9e-365">更新 *[路徑-您的應用程式]* 預留位置取代為路徑**GeekQuiz**方案，位於**Source\Ex1 DeployingWebSiteToStaging\Begin**資料夾這個實驗室中。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-365">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution, located in the **Source\Ex1-DeployingWebSiteToStaging\Begin** folder of this lab.</span></span>
    ...
    <span data-ttu-id="cfe9e-367">*Git 初始化 」 和 「 第一次認可*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-367">*Git initialization and first commit*</span></span>
...
13. <span data-ttu-id="cfe9e-385">最後，按一下**瀏覽**移至 web 應用程式的命令列中。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-385">Finally, click **Browse** in the command bar to go to the web app.</span></span>
    ...
    <span data-ttu-id="cfe9e-387">*瀏覽 web 應用程式*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-387">*Browse web app*</span></span>
...
<a id="Exercise3"></a>
### <a name="exercise-3-performing-deployment-rollback-in-production"></a><span data-ttu-id="cfe9e-419">練習 3： 在生產環境中執行部署復原</span><span class="sxs-lookup"><span data-stu-id="cfe9e-419">Exercise 3: Performing Deployment Rollback in Production</span></span>

<span data-ttu-id="cfe9e-420">其中您沒有預備位置來執行熱插拔之間預備和生產環境，例如，如果您正在使用的情況下**空閒**或是**共用**模式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-420">There are scenarios where you do not have a staging slot to perform hot swap between staging and production, for example, if you are working with **Free** or **Shared** mode.</span></span> <span data-ttu-id="cfe9e-421">在這些情況下，您應該測試您的應用程式，在測試環境中 – 在本機或遠端站台中 – 之前部署至生產環境。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-421">In those scenarios, you should test your application in a testing environment –either locally or in a remote site– before deploying to production.</span></span> <span data-ttu-id="cfe9e-422">不過，就可以在測試階段偵測不到的問題可能會發生生產網站中。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-422">However, it is possible that an issue not detected during the testing phase may arise in the production site.</span></span> <span data-ttu-id="cfe9e-423">在此情況下，務必要有機制，以輕鬆地切換至先前且更穩定版本的應用程式儘速。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-423">In this case, it is important to have a mechanism to easily switch to a previous and more stable version of the application as quickly as possible.</span></span>

<span data-ttu-id="cfe9e-424">在  **Azure App Service**，從原始檔控制的連續部署讓此可能感謝**重新部署**管理入口網站中可用動作。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-424">In **Azure App Service**, continuous deployment from source control makes this possible thanks to the **redeploy** action available in the management portal.</span></span> <span data-ttu-id="cfe9e-425">Azure 會追蹤與推送至儲存機制的認可相關聯的部署，並提供選項來重新部署您的應用程式使用任何先前的部署中，在任何時間。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-425">Azure keeps track of the deployments associated with the commits pushed to the repository and provides an option to redeploy your application using any of your previous deployments, at any time.</span></span>

<span data-ttu-id="cfe9e-426">在這個練習中，您將執行中的程式碼變更**Geek 測驗**刻意插入的應用程式*bug*。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-426">In this exercise you will perform a change to the code in the **Geek Quiz** application that intentionally injects a *bug*.</span></span> <span data-ttu-id="cfe9e-427">您將部署到生產環境應用程式，以查看錯誤，並接著您會利用重新部署功能，請回到先前的狀態。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-427">You will deploy the application to production to see the error, and then you will take advantage of the redeploy feature to go back to the previous state.</span></span>

<a id="Ex3Task1"></a>
#### <a name="task-1--updating-the-geek-quiz-application"></a><span data-ttu-id="cfe9e-428">工作 1-Geek 測驗應用程式更新</span><span class="sxs-lookup"><span data-stu-id="cfe9e-428">Task 1 – Updating the Geek Quiz Application</span></span>

<span data-ttu-id="cfe9e-429">在這個工作中，重構程式碼的一小段**TriviaController**類別來擷取從資料庫擷取選取的測驗選項，為新方法的邏輯的一部分。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-429">In this task, you will refactor a small piece of code of the **TriviaController** class to extract part of the logic that retrieves the selected quiz option from the database into a new method.</span></span>

1. <span data-ttu-id="cfe9e-430">切換至 Visual Studio 執行個體**GeekQuiz**從上一個練習的解決方案。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-430">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="cfe9e-431">在 [**方案總管] 中**，開啟**TriviaController.cs**檔案**控制器**資料夾。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-431">In **Solution Explorer**, open the **TriviaController.cs** file inside the **Controllers** folder.</span></span>
3. <span data-ttu-id="cfe9e-432">找出**StoreAsync**方法，然後選取程式碼以在下圖中反白顯示。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-432">Locate the **StoreAsync** method and select the code highlighted in the following figure.</span></span>

    ![選取的程式碼](maintainable-azure-websites-managing-change-and-scale/_static/image47.png)

    <span data-ttu-id="cfe9e-434">*選取的程式碼*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-434">*Selecting the code*</span></span>
4. <span data-ttu-id="cfe9e-435">以滑鼠右鍵按一下選取的程式碼中，展開**重構**功能表，然後選取**擷取方法...**.</span><span class="sxs-lookup"><span data-stu-id="cfe9e-435">Right-click the selected code, expand the **Refactor** menu and select **Extract Method...**.</span></span>

    ![擷取程式碼做為新的方法](maintainable-azure-websites-managing-change-and-scale/_static/image48.png)

    <span data-ttu-id="cfe9e-437">*選取 擷取方法*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-437">*Selecting Extract Method*</span></span>
5. <span data-ttu-id="cfe9e-438">在 [**擷取方法**] 對話方塊中，名稱的新方法*MatchesOption*然後按一下 **[確定]**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-438">In the **Extract Method** dialog box, name the new method *MatchesOption* and click **OK**.</span></span>

    ![指定方法名稱](maintainable-azure-websites-managing-change-and-scale/_static/image49.png)

    <span data-ttu-id="cfe9e-440">*指定擷取方法的名稱*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-440">*Specifying the name for the extracted method*</span></span>
6. <span data-ttu-id="cfe9e-441">選取的程式碼接著會擷取至**MatchesOption**方法。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-441">The selected code is then extracted into the **MatchesOption** method.</span></span> <span data-ttu-id="cfe9e-442">產生的程式碼是以下列程式碼片段所示。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-442">The resulting code is shown in the following snippet.</span></span>

    [!code-csharp[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample14.cs)]
7. <span data-ttu-id="cfe9e-443">按下**CTRL + S**以儲存變更。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-443">Press **CTRL + S** to save the changes.</span></span>

<a id="Ex3Task2"></a>
#### <a name="task-2--redeploying-the-geek-quiz-application"></a><span data-ttu-id="cfe9e-444">工作 2-重新部署 Geek 測驗應用程式</span><span class="sxs-lookup"><span data-stu-id="cfe9e-444">Task 2 – Redeploying the Geek Quiz Application</span></span>

<span data-ttu-id="cfe9e-445">您現在會推送至儲存機制，將會觸發新的部署到生產環境所做先前工作中的變更。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-445">You will now push the changes you made in the previous task to the repository, which will trigger a new deployment to the production environment.</span></span> <span data-ttu-id="cfe9e-446">然後，您將在其中進行問題使用**F12 開發工具**提供 Internet Explorer 中，再回復到先前的部署從 Azure 管理入口網站。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-446">Then, you will troubleshot an issue using the **F12 development tools** provided by Internet Explorer, and then perform a rollback to the previous deployment from the Azure management portal.</span></span>

1. <span data-ttu-id="cfe9e-447">開啟新**Git Bash**主控台部署至 Azure App Service 更新的應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-447">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
2. <span data-ttu-id="cfe9e-448">執行下列命令，以將變更推送至 Azure。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-448">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="cfe9e-449">更新 *[路徑-您的應用程式]* 預留位置取代為路徑**GeekQuiz**解決方案。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-449">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="cfe9e-450">將提示您為您的部署密碼。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-450">You will be prompted for your deployment password.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample15.cmd)]

    ![將重構程式碼推送至 Azure](maintainable-azure-websites-managing-change-and-scale/_static/image50.png)

    <span data-ttu-id="cfe9e-452">*將重構程式碼推送至 Azure*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-452">*Pushing refactored code to Azure*</span></span>
3. <span data-ttu-id="cfe9e-453">開啟 Internet Explorer 並瀏覽至您的 web 應用程式 (例如`http://<your-web-site>.azurewebsites.net`)。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-453">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="cfe9e-454">使用先前建立的認證登入。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-454">Log in using the previously created credentials.</span></span>
4. <span data-ttu-id="cfe9e-455">按下**F12**若要啟動的開發工具，選取**網路**索引標籤，然後按一下**播放**開始錄製 按鈕。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-455">Press **F12** to launch the development tools, select the **Network** tab and click the **Play** button to start recording.</span></span>

    <span data-ttu-id="cfe9e-456">![啟動網路錄製](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "啟動網路記錄")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-456">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "Starting network recording")</span></span>

    <span data-ttu-id="cfe9e-457">*啟動網路記錄*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-457">*Starting network recording*</span></span>
5. <span data-ttu-id="cfe9e-458">選取任何選項的測驗。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-458">Select any option of the quiz.</span></span> <span data-ttu-id="cfe9e-459">您會看到沒有任何反應。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-459">You will see that nothing happens.</span></span>
6. <span data-ttu-id="cfe9e-460">在 [ **F12** ] 視窗中，對應至 POST HTTP 要求的項目會顯示 HTTP **500**結果。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-460">In the **F12** window, the entry corresponding to the POST HTTP request shows an HTTP **500** result.</span></span>

    ![HTTP 500 錯誤](maintainable-azure-websites-managing-change-and-scale/_static/image52.png)

    <span data-ttu-id="cfe9e-462">*HTTP 500 錯誤*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-462">*HTTP 500 error*</span></span>
7. <span data-ttu-id="cfe9e-463">選取 [**主控台**] 索引標籤。錯誤會記錄原因的詳細資料。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-463">Select the **Console** tab. An error is logged with the details of the cause.</span></span>

    ![記錄的錯誤](maintainable-azure-websites-managing-change-and-scale/_static/image53.png)

    <span data-ttu-id="cfe9e-465">*記錄的錯誤*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-465">*Logged error*</span></span>
8. <span data-ttu-id="cfe9e-466">找出錯誤的詳細資料部分。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-466">Locate the details part of the error.</span></span> <span data-ttu-id="cfe9e-467">很明顯地，此錯誤是重構前一個步驟中認可您的程式碼所造成。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-467">Clearly, this error is caused by the code refactoring you committed in the previous steps.</span></span>

    <span data-ttu-id="cfe9e-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span><span class="sxs-lookup"><span data-stu-id="cfe9e-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span></span>
9. <span data-ttu-id="cfe9e-469">請勿關閉瀏覽器。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-469">Do not close the browser.</span></span>
10. <span data-ttu-id="cfe9e-470">在新的瀏覽器執行個體中，瀏覽至[Azure 管理入口網站](https://manage.windowsazure.com)並使用您的訂用帳戶相關聯的 Microsoft 帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-470">In a new browser instance, navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
11. <span data-ttu-id="cfe9e-471">選取 **網站**按一下您在練習 2 中建立 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-471">Select **Websites** and click the web app you created in Exercise 2.</span></span>
12. <span data-ttu-id="cfe9e-472">瀏覽至**部署**頁面。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-472">Navigate to the **Deployments** page.</span></span> <span data-ttu-id="cfe9e-473">請注意，執行的所有認可會都列出在部署歷程記錄中。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-473">Notice that all the commits performed are listed in the deployment history.</span></span>

    ![現有部署的清單](maintainable-azure-websites-managing-change-and-scale/_static/image54.png)

    <span data-ttu-id="cfe9e-475">*現有部署的清單*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-475">*List of existing deployments*</span></span>
13. <span data-ttu-id="cfe9e-476">選取 上一次認可，然後按一下**重新部署**命令列上。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-476">Select the previous commit and click **Redeploy** on the command bar.</span></span>

    ![重新部署先前的認可](maintainable-azure-websites-managing-change-and-scale/_static/image55.png)

    <span data-ttu-id="cfe9e-478">*重新部署先前的認可*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-478">*Redeploying the previous commit*</span></span>
14. <span data-ttu-id="cfe9e-479">當系統提示您確認時，按一下**是**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-479">When prompted to confirm, click **Yes**.</span></span>

    ![確認重新部署](maintainable-azure-websites-managing-change-and-scale/_static/image56.png)
15. <span data-ttu-id="cfe9e-481">當部署完成後時，切換回瀏覽器執行個體，與您的 web 應用程式，然後按**CTRL + F5**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-481">When the deployment completes, switch back to the browser instance with your web app and press **CTRL + F5**.</span></span>
16. <span data-ttu-id="cfe9e-482">按一下任何一個選項。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-482">Click any of the options.</span></span> <span data-ttu-id="cfe9e-483">翻頁動畫的動畫現在需要的地方和結果 (*正確/不正確*) 將會顯示。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-483">The flip animation will now take place and the result (*correct/incorrect*) will be displayed.</span></span>
17. <span data-ttu-id="cfe9e-484">（選擇性）若要切換**Git Bash**主控台並執行下列命令來還原成先前的認可。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-484">(Optional) Switch to the **Git Bash** console and execute the following commands to revert to the previous commit.</span></span>

    > [!NOTE]
    > <span data-ttu-id="cfe9e-485">這些命令會建立新的認可，復原 Git 存放庫中的不正確的認可所做的所有變更。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-485">These commands create a new commit that undoes all changes in the Git repository that were made in the bad commit.</span></span> <span data-ttu-id="cfe9e-486">Azure 會再重新部署應用程式使用新的認可。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-486">Azure will then redeploy the application using the new commit.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample16.cmd)]

<a id="Exercise4"></a>
### <a name="exercise-4-scaling-using-azure-storage"></a><span data-ttu-id="cfe9e-487">使用 Azure 儲存體縮放練習 4:</span><span class="sxs-lookup"><span data-stu-id="cfe9e-487">Exercise 4: Scaling Using Azure Storage</span></span>

<span data-ttu-id="cfe9e-488">**Blob**是最簡單的方式來儲存大量非結構化的文字或二進位資料如視訊、 音訊和影像。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-488">**Blobs** are the simplest way to store large amounts of unstructured text or binary data such as video, audio and images.</span></span> <span data-ttu-id="cfe9e-489">移至儲存體的應用程式的靜態內容，可協助調整應用程式所提供的映像或直接到瀏覽器的文件。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-489">Moving the static content of your application to Storage, helps to scale your application by serving images or documents directly to the browser.</span></span>

<span data-ttu-id="cfe9e-490">在此練習中，您會將移至 Blob 容器的應用程式的靜態內容。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-490">In this exercise, you will move the static content of your application to a Blob container.</span></span> <span data-ttu-id="cfe9e-491">然後您將設定您的應用程式，以新增**ASP.NET URL 重寫規則**中**Web.config**重新導向至 Blob 容器的內容。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-491">Then you will configure your application to add an **ASP.NET URL rewrite rule** in the **Web.config** to redirect your content to the Blob container.</span></span>

<a id="Ex4Task1"></a>
#### <a name="task-1--creating-an-azure-storage-account"></a><span data-ttu-id="cfe9e-492">工作 1-建立 Azure 儲存體帳戶</span><span class="sxs-lookup"><span data-stu-id="cfe9e-492">Task 1 – Creating an Azure Storage Account</span></span>

<span data-ttu-id="cfe9e-493">在這項工作中，您將了解如何建立新的儲存體帳戶，使用管理入口網站。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-493">In this task you will learn how to create a new storage account using the management portal.</span></span>

1. <span data-ttu-id="cfe9e-494">瀏覽至[Azure 管理入口網站](https://manage.windowsazure.com)並使用您的訂用帳戶相關聯的 Microsoft 帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-494">Navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
2. <span data-ttu-id="cfe9e-495">選取**新 |資料服務 |儲存體 |快速建立**開始建立新的儲存體帳戶。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-495">Select **New | Data Services | Storage | Quick Create** to start creating a new storage account.</span></span> <span data-ttu-id="cfe9e-496">輸入唯一的名稱，為該帳戶，然後選取**地區**從清單中。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-496">Enter a unique name for the account and select a **Region** from the list.</span></span> <span data-ttu-id="cfe9e-497">按一下 **建立儲存體帳戶**以繼續。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-497">Click **Create Storage Account** to continue.</span></span>

    <span data-ttu-id="cfe9e-498">![建立新的儲存體帳戶](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "建立新的儲存體帳戶")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-498">![Creating a new Storage Account](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "Creating a new Storage Account")</span></span>

    <span data-ttu-id="cfe9e-499">*建立新的儲存體帳戶*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-499">*Creating a new storage account*</span></span>
3. <span data-ttu-id="cfe9e-500">在 **儲存體**區段中，等到新的儲存體帳戶的狀態會變更為*線上*才能繼續進行下一個步驟。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-500">In the **Storage** section, wait until the status of the new storage account changes to *Online* in order to continue with the following step.</span></span>

    <span data-ttu-id="cfe9e-501">![建立儲存體帳戶](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "建立儲存體帳戶")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-501">![Storage Account created](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "Storage Account created")</span></span>

    <span data-ttu-id="cfe9e-502">*建立儲存體帳戶*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-502">*Storage Account created*</span></span>
4. <span data-ttu-id="cfe9e-503">按一下儲存體帳戶名稱，然後按一下**儀表板**在頁面頂端的連結。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-503">Click on the storage account name and then click the **Dashboard** link at the top of the page.</span></span> <span data-ttu-id="cfe9e-504">**儀表板**頁面可提供您的帳戶，並且可用於您的應用程式的服務端點的狀態相關資訊。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-504">The **Dashboard** page provides you with information about the status of the account and the service endpoints that can be used within your applications.</span></span>

    <span data-ttu-id="cfe9e-505">![顯示儲存體帳戶儀表板](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "顯示儲存體帳戶儀表板")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-505">![Displaying the Storage Account Dashboard](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "Displaying the Storage Account Dashboard")</span></span>

    <span data-ttu-id="cfe9e-506">*顯示儲存體帳戶儀表板*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-506">*Displaying the Storage Account Dashboard*</span></span>
5. <span data-ttu-id="cfe9e-507">按一下 **管理存取金鑰**導覽列中的按鈕。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-507">Click the **Manage Access Keys** button in the navigation bar.</span></span>

    <span data-ttu-id="cfe9e-508">![管理存取金鑰 按鈕](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "管理存取金鑰 按鈕")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-508">![Manage Access Keys button](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "Manage Access Keys button")</span></span>

    <span data-ttu-id="cfe9e-509">*管理存取金鑰 按鈕*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-509">*Manage Access Keys button*</span></span>
6. <span data-ttu-id="cfe9e-510">在**管理存取金鑰** 對話方塊中，複製**儲存體帳戶名稱**並**主要存取金鑰**當您將需要在下列練習中。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-510">In the **Manage Access Keys** dialog box, copy the **Storage Account Name** and **Primary Access Key** as you will need them in the following exercise.</span></span> <span data-ttu-id="cfe9e-511">然後，關閉對話方塊。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-511">Then, close the dialog box.</span></span>

    <span data-ttu-id="cfe9e-512">![管理存取金鑰 對話方塊](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "管理存取金鑰 對話方塊")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-512">![Manage Access Key dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "Manage Access Key dialog box")</span></span>

    <span data-ttu-id="cfe9e-513">*管理存取金鑰 對話方塊*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-513">*Manage Access Key dialog box*</span></span>

<a id="Ex4Task2"></a>
#### <a name="task-2--uploading-an-asset-to-azure-blob-storage"></a><span data-ttu-id="cfe9e-514">工作 2-將資產上傳至 Azure Blob 儲存體</span><span class="sxs-lookup"><span data-stu-id="cfe9e-514">Task 2 – Uploading an Asset to Azure Blob Storage</span></span>

<span data-ttu-id="cfe9e-515">在這個工作中，您將使用從 Visual Studio 的 [伺服器總管] 視窗來連接到您的儲存體帳戶。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-515">In this task, you will use the Server Explorer window from Visual Studio to connect to your storage account.</span></span> <span data-ttu-id="cfe9e-516">然後，您會建立 blob 容器，並 Geek 測驗標誌檔案上傳至容器。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-516">You will then create a blob container and upload a file with the Geek Quiz logo to the container.</span></span>

1. <span data-ttu-id="cfe9e-517">切換至 Visual Studio 執行個體**GeekQuiz**從上一個練習的解決方案。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-517">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="cfe9e-518">從功能表列中，選取**檢視**，然後按一下**伺服器總管**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-518">From the menu bar, select **View** and then click **Server Explorer**.</span></span>
3. <span data-ttu-id="cfe9e-519">在 **伺服器總管**，以滑鼠右鍵按一下**Azure**節點，然後選取**連線到 Azure...**.使用您的訂用帳戶相關聯的 Microsoft 帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-519">In **Server Explorer**, right-click the **Azure** node and select **Connect to Azure...**. Sign in using the Microsoft account associated with your subscription.</span></span>

    ![連接到 Windows Azure](maintainable-azure-websites-managing-change-and-scale/_static/image62.png)

    <span data-ttu-id="cfe9e-521">*連接到 Azure*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-521">*Connect to Azure*</span></span>
4. <span data-ttu-id="cfe9e-522">依序展開**Azure**節點，以滑鼠右鍵按一下**儲存體**，然後選取**附加外部儲存體...**.</span><span class="sxs-lookup"><span data-stu-id="cfe9e-522">Expand the **Azure** node, right-click **Storage** and select **Attach External Storage...**.</span></span>
5. <span data-ttu-id="cfe9e-523">在**加入新的儲存體帳戶**對話方塊方塊中，輸入**帳戶名稱**並**帳戶金鑰**您在上一個工作並按一下 取得**確定**.</span><span class="sxs-lookup"><span data-stu-id="cfe9e-523">In the **Add New Storage Account** dialog box, enter the **Account name** and **Account key** you obtained in the previous task and click **OK**.</span></span>

    ![加入新的儲存體帳戶對話方塊](maintainable-azure-websites-managing-change-and-scale/_static/image63.png)

    <span data-ttu-id="cfe9e-525">*加入新的儲存體帳戶對話方塊*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-525">*Add New Storage Account dialog box*</span></span>
6. <span data-ttu-id="cfe9e-526">您的儲存體帳戶應該會出現下**儲存體**節點。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-526">Your storage account should appear under the **Storage** node.</span></span> <span data-ttu-id="cfe9e-527">展開您的儲存體帳戶，以滑鼠右鍵按一下**Blob** ，然後選取**建立 Blob 容器...**.</span><span class="sxs-lookup"><span data-stu-id="cfe9e-527">Expand your storage account, right-click **Blobs** and select **Create Blob Container...**.</span></span>

    <span data-ttu-id="cfe9e-528">![建立 Blob 容器](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "建立 Blob 容器")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-528">![Create Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "Create Blob Container")</span></span>

    <span data-ttu-id="cfe9e-529">*建立 Blob 容器*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-529">*Create Blob Container*</span></span>
7. <span data-ttu-id="cfe9e-530">在 **建立 Blob 容器**對話方塊方塊中，輸入 blob 容器的名稱，然後按一下**確定**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-530">In the **Create Blob Container** dialog box, enter a name for the blob container and click **OK**.</span></span>

    <span data-ttu-id="cfe9e-531">![建立 Blob 容器 對話方塊](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "建立 Blob 容器 對話方塊")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-531">![Create Blob Container dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "Create Blob Container dialog box")</span></span>

    <span data-ttu-id="cfe9e-532">*建立 Blob 容器 對話方塊*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-532">*Create Blob Container dialog box*</span></span>
8. <span data-ttu-id="cfe9e-533">應加入新的 blob 容器**Blob**節點。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-533">The new blob container should be added to the **Blobs** node.</span></span> <span data-ttu-id="cfe9e-534">變更要將容器設為公用容器中的存取權限。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-534">Change the access permissions in the container to make the container public.</span></span> <span data-ttu-id="cfe9e-535">若要這樣做，請以滑鼠右鍵按一下**映像**容器，然後選取**屬性**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-535">To do this, right-click the **images** container and select **Properties**.</span></span>

    <span data-ttu-id="cfe9e-536">![映像容器屬性](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "映像容器屬性")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-536">![images container properties](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "images container properties")</span></span>

    <span data-ttu-id="cfe9e-537">*映像容器屬性*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-537">*Images container properties*</span></span>
9. <span data-ttu-id="cfe9e-538">在 **屬性**視窗中，將**公用讀取權限**來**容器**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-538">In the **Properties** window, set the **Public Read Access** to **Container**.</span></span>

    <span data-ttu-id="cfe9e-539">![變更公用讀取權限屬性](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "變更公用讀取權限屬性")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-539">![Changing public read access property](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "Changing public read access property")</span></span>

    <span data-ttu-id="cfe9e-540">*變更公用讀取權限屬性*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-540">*Changing public read access property*</span></span>
10. <span data-ttu-id="cfe9e-541">當系統提示您是否確定要變更的公用存取屬性，請按一下**是**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-541">When prompted if you are sure you want to change the public access property, click **Yes**.</span></span>

    <span data-ttu-id="cfe9e-542">![Microsoft Visual Studio 警告](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio 警告")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-542">![Microsoft Visual Studio warning](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio warning")</span></span>

    <span data-ttu-id="cfe9e-543">*Microsoft Visual Studio 警告*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-543">*Microsoft Visual Studio warning*</span></span>
11. <span data-ttu-id="cfe9e-544">在 **伺服器總管**，以滑鼠右鍵按一下**映像**blob 容器，然後選取**檢視 Blob 容器**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-544">In **Server Explorer**, right-click in the **images** blob container and select **View Blob Container**.</span></span>

    <span data-ttu-id="cfe9e-545">![檢視 Blob 容器](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "檢視 Blob 容器")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-545">![View Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "View Blob Container")</span></span>

    <span data-ttu-id="cfe9e-546">*檢視 Blob 容器*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-546">*View Blob Container*</span></span>
12. <span data-ttu-id="cfe9e-547">映像容器應該會開啟新視窗中，應該會顯示任何項目的圖例。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-547">The images container should open in a new window and a legend with no entries should be shown.</span></span> <span data-ttu-id="cfe9e-548">按一下 **上傳**圖示，以將檔案上傳至 blob 容器。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-548">Click the **upload** icon to upload a file to the blob container.</span></span>

    <span data-ttu-id="cfe9e-549">![無項目的映像容器](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "無項目的映像容器")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-549">![Images container with no entries](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "Images container with no entries")</span></span>

    <span data-ttu-id="cfe9e-550">*無項目的映像容器*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-550">*Images container with no entries*</span></span>
13. <span data-ttu-id="cfe9e-551">在 **上傳 Blob**對話方塊方塊中，瀏覽至**資產**實驗室的資料夾。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-551">In the **Upload Blob** dialog box, navigate to the **Assets** folder of the lab.</span></span> <span data-ttu-id="cfe9e-552">選取 **標誌 big.png**檔案，然後按一下**開啟**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-552">Select the **logo-big.png** file and click **Open**.</span></span>
14. <span data-ttu-id="cfe9e-553">請等到檔案上傳。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-553">Wait until the file is uploaded.</span></span> <span data-ttu-id="cfe9e-554">上傳完成時，檔案應該列在 映像容器。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-554">When the upload completes, the file should be listed in the images container.</span></span> <span data-ttu-id="cfe9e-555">以滑鼠右鍵按一下檔案項目，然後選取**複製 URL**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-555">Right-click the file entry and select **Copy URL**.</span></span>

    <span data-ttu-id="cfe9e-556">![複製 blob URL](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "複製 blob 檔案的 URL")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-556">![Copy blob URL](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "Copy blob file URL")</span></span>

    <span data-ttu-id="cfe9e-557">*複製 blob URL*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-557">*Copy blob URL*</span></span>
15. <span data-ttu-id="cfe9e-558">開啟 Internet Explorer，並貼上 URL。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-558">Open Internet Explorer and paste the URL.</span></span> <span data-ttu-id="cfe9e-559">下列映像應該會顯示在瀏覽器中。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-559">The following image should be shown in the browser.</span></span>

    <span data-ttu-id="cfe9e-560">![從 Windows Blob 儲存體的標誌 big.png 映像](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "標誌 big.png 映像從儲存體")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-560">![logo-big.png image from Windows Blob Storage](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "logo-big.png image from storage")</span></span>

    <span data-ttu-id="cfe9e-561">*從 Azure Blob 儲存體的標誌 big.png 映像*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-561">*logo-big.png image from Azure Blob Storage*</span></span>

<a id="Ex4Task3"></a>
#### <a name="task-3--updating-the-solution-to-consume-static-content-from-azure-blob-storage"></a><span data-ttu-id="cfe9e-562">工作 3-更新方案，以使用來自 Azure Blob 儲存體的靜態內容</span><span class="sxs-lookup"><span data-stu-id="cfe9e-562">Task 3 – Updating the Solution to Consume Static Content from Azure Blob Storage</span></span>

<span data-ttu-id="cfe9e-563">在這個工作中，您會設定**GeekQuiz**解決方案，以使用映像上傳至 Azure Blob 儲存體 （而不是位於 web 應用程式中的映像） 新增中的 ASP.NET URL 重寫規則**web.config**檔案。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-563">In this task, you will configure the **GeekQuiz** solution to consume the image uploaded to Azure Blob Storage (instead of the image located in the web app) by adding an ASP.NET URL rewrite rule in the **web.config** file.</span></span>

1. <span data-ttu-id="cfe9e-564">在 Visual Studio 中開啟**Web.config**檔案內**GeekQuiz**專案，然後找出**&lt;system.webServer&gt;** 項目。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-564">In Visual Studio, open the **Web.config** file inside the **GeekQuiz** project and locate the **&lt;system.webServer&gt;** element.</span></span>
2. <span data-ttu-id="cfe9e-565">加入下列程式碼，將 URL 重寫規則，更新您的儲存體帳戶名稱的預留位置。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-565">Add the following code to add an URL rewrite rule, updating the placeholder with your storage account name.</span></span>

    <span data-ttu-id="cfe9e-566">(程式碼片段- *WebSitesInProduction-Ex4-UrlRewriteRule*)</span><span class="sxs-lookup"><span data-stu-id="cfe9e-566">(Code Snippet - *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span></span>

    [!code-xml[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample17.xml)]

    > [!NOTE]
    > <span data-ttu-id="cfe9e-567">URL 重寫為攔截傳入的 Web 要求，並將要求重新導向至不同的資源的程序。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-567">URL rewriting is the process of intercepting an incoming Web request and redirecting the request to a different resource.</span></span> <span data-ttu-id="cfe9e-568">URL 重寫規則會告訴重寫引擎要求時必須重新導向，以及其中應該將他們重新導向。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-568">The URL rewriting rules tells the rewriting engine when a request needs to be redirected, and where should they be redirected.</span></span> <span data-ttu-id="cfe9e-569">重寫規則由兩個字串所組成： 在要求 URL 中尋找的模式 （通常使用規則運算式），並找到要取代的模式，如果字串。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-569">A rewriting rule is composed of two strings: the pattern to look for in the requested URL (usually, using regular expressions), and the string to replace the pattern with, if found.</span></span> <span data-ttu-id="cfe9e-570">如需詳細資訊，請參閱 < [URL 以 ASP.NET 重寫](https://msdn.microsoft.com/library/ms972974.aspx)。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-570">For more information, see [URL Rewriting in ASP.NET](https://msdn.microsoft.com/library/ms972974.aspx).</span></span>
3. <span data-ttu-id="cfe9e-571">按下**CTRL + S**以儲存變更。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-571">Press **CTRL + S** to save the changes.</span></span>
4. <span data-ttu-id="cfe9e-572">開啟新**Git Bash**主控台部署至 Azure App Service 更新的應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-572">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
5. <span data-ttu-id="cfe9e-573">執行下列命令，以將變更推送至 Azure。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-573">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="cfe9e-574">更新 *[路徑-您的應用程式]* 預留位置取代為路徑**GeekQuiz**解決方案。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-574">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="cfe9e-575">將提示您為您的部署密碼。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-575">You will be prompted for your deployment password.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample18.cmd)]

    ![將更新部署至 Azure](maintainable-azure-websites-managing-change-and-scale/_static/image73.png)

    <span data-ttu-id="cfe9e-577">*將更新部署至 Azure*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-577">*Deploying update to Azure*</span></span>

<a id="Ex4Task4"></a>
#### <a name="task-4--verification"></a><span data-ttu-id="cfe9e-578">工作 4-驗證</span><span class="sxs-lookup"><span data-stu-id="cfe9e-578">Task 4 – Verification</span></span>

<span data-ttu-id="cfe9e-579">在這項工作會使用**Internet Explorer**瀏覽**Geek 測驗**應用程式，並檢查 URL 重寫規則的映像運作，您會被重新導向至映像裝載在**Azure Blob儲存體**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-579">In this task you will use **Internet Explorer** to browse the **Geek Quiz** application and check that the URL rewrite rule for images works and you are redirected to the image hosted on **Azure Blob Storage**.</span></span>

1. <span data-ttu-id="cfe9e-580">開啟 Internet Explorer 並瀏覽至您的 web 應用程式 (例如`http://<your-web-site>.azurewebsites.net`)。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-580">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="cfe9e-581">使用先前建立的認證登入。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-581">Log in using the previously created credentials.</span></span>

    <span data-ttu-id="cfe9e-582">![顯示玩家測驗 web 應用程式與映像](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "顯示玩家測驗 web 應用程式與映像")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-582">![Showing the Geek Quiz web app with the image](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "Showing the Geek Quiz web app with the image")</span></span>

    <span data-ttu-id="cfe9e-583">*顯示玩家測驗 web 應用程式與映像*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-583">*Showing the Geek Quiz web app with the image*</span></span>
2. <span data-ttu-id="cfe9e-584">按下**F12**若要啟動的開發工具，選取**網路**索引標籤，並開始錄製。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-584">Press **F12** to launch the development tools, select the **Network** tab and start recording.</span></span>

    <span data-ttu-id="cfe9e-585">![啟動網路錄製](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "啟動網路記錄")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-585">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "Starting network recording")</span></span>

    <span data-ttu-id="cfe9e-586">*啟動網路記錄*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-586">*Starting network recording*</span></span>
3. <span data-ttu-id="cfe9e-587">按下**CTRL + F5**重新整理網頁。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-587">Press **CTRL + F5** to refresh the web page.</span></span>
4. <span data-ttu-id="cfe9e-588">網頁完成載入之後，您應該會看到的 HTTP 要求 **/img/logo-big.png** URL 含有 HTTP **301**結果 （重新導向） 和另一個要求`http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png`URL 含有 HTTP **200**結果。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-588">Once the page has finished loading, you should see an HTTP request for the **/img/logo-big.png** URL with an HTTP **301** result (redirect) and another request for `http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png` URL with a HTTP **200** result.</span></span>

    <span data-ttu-id="cfe9e-589">![正在驗證的 URL 重新導向](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "開發人員工具中顯示重新導向")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-589">![Verifying the URL redirect](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "Showing the redirect in Dev Tools")</span></span>

    <span data-ttu-id="cfe9e-590">*正在驗證重新導向 URL*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-590">*Verifying the URL redirect*</span></span>

<a id="Exercise5"></a>
### <a name="exercise-5-using-autoscale-for-web-apps"></a><span data-ttu-id="cfe9e-591">練習 5： 使用自動調整規模的 Web 應用程式</span><span class="sxs-lookup"><span data-stu-id="cfe9e-591">Exercise 5: Using Autoscale for Web Apps</span></span>

> [!NOTE]
> <span data-ttu-id="cfe9e-592">此練習是選擇性的因為它需要支援的 Web 負載&amp;效能測試，僅適用於**Visual Studio 2013 Ultimate Edition**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-592">This exercise is optional, since it requires support for Web Load &amp; Performance Testing which is only available for **Visual Studio 2013 Ultimate Edition**.</span></span> <span data-ttu-id="cfe9e-593">如需有關 Visual Studio 2013 的特定功能的詳細資訊，請比較版本[此處](https://www.microsoft.com/visualstudio/eng/products/compare)。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-593">For more information on specific Visual Studio 2013 features, compare versions [here](https://www.microsoft.com/visualstudio/eng/products/compare).</span></span>


<span data-ttu-id="cfe9e-594">**Azure App Service Web Apps**中執行的 web 應用程式提供的自動調整功能**標準模式**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-594">**Azure App Service Web Apps** provides the Autoscale feature for web apps running in **Standard Mode**.</span></span> <span data-ttu-id="cfe9e-595">自動調整可讓 Azure 自動調整您的 web 應用程式，根據負載的執行個體計數。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-595">Autoscale lets Azure automatically scale the instance count of your web app depending on the load.</span></span> <span data-ttu-id="cfe9e-596">啟用自動調整時，Azure 會每 5 分鐘會檢查您的 web 應用程式的 CPU，並將執行個體，視需要在該時間點的。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-596">When Autoscale is enabled, Azure checks the CPU of your web app once every five minutes and adds instances as needed at that point in time.</span></span> <span data-ttu-id="cfe9e-597">如果 CPU 使用率偏低，Azure 將移除執行個體一次每隔 2 小時以確保您的 web 應用程式的效能不會降低。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-597">If the CPU usage is low, Azure will remove instances once every two hours to ensure that the performance of your web app is not degraded.</span></span>

<span data-ttu-id="cfe9e-598">在這個練習中，您會瀏覽設定所需的步驟**自動調整規模**功能**Geek 測驗**web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-598">In this exercise you will go through the steps required to configure the **Autoscale** feature for the **Geek Quiz** web app.</span></span> <span data-ttu-id="cfe9e-599">您將會執行產生足夠的 CPU 負載，應用程式觸發程序的執行個體升級到 Visual Studio 負載測試來驗證這項功能。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-599">You will verify this feature by running a Visual Studio load test to generate enough CPU load on the application to trigger an instance upgrade.</span></span>

<a id="Ex5Task1"></a>
#### <a name="task-1--configuring-autoscale-based-on-the-cpu-metric"></a><span data-ttu-id="cfe9e-600">工作 1-設定 CPU 度量為基礎的自動調整規模</span><span class="sxs-lookup"><span data-stu-id="cfe9e-600">Task 1 – Configuring Autoscale Based on the CPU Metric</span></span>

<span data-ttu-id="cfe9e-601">在這項工作中，您將使用 Azure 管理入口網站來啟用自動調整功能，以便您在練習 2 中建立 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-601">In this task you will use the Azure management portal to enable the Autoscale feature for the web app you created in Exercise 2.</span></span>

1. <span data-ttu-id="cfe9e-602">在  [Azure 管理入口網站](https://manage.windowsazure.com/)，選取**網站**按一下您在練習 2 中建立 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-602">In the [Azure management portal](https://manage.windowsazure.com/), select **Websites** and click the web app you created in Exercise 2.</span></span>
2. <span data-ttu-id="cfe9e-603">瀏覽至**擴展**頁面。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-603">Navigate to the **Scale** page.</span></span> <span data-ttu-id="cfe9e-604">底下**容量**區段中，選取**CPU** for**依度量調整規模**組態。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-604">Under the **capacity** section, select **CPU** for the **Scale by Metric** configuration.</span></span>

    > [!NOTE]
    > <span data-ttu-id="cfe9e-605">調整時的 CPU，Azure 會動態調整應用程式使用的 CPU 使用量變更時的執行個體數目。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-605">When scaling by CPU, Azure dynamically adjusts the number of instances that the app uses if the CPU usage changes.</span></span>

    <span data-ttu-id="cfe9e-606">![選取即可依 CPU 調整規模](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "選取 CPU 計量自動調整")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-606">![Selecting to scale by CPU](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "Selecting the CPU metric for auto scaling")</span></span>

    <span data-ttu-id="cfe9e-607">*選取即可依 CPU 調整規模*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-607">*Selecting to scale by CPU*</span></span>
3. <span data-ttu-id="cfe9e-608">變更**目標 CPU**組態**20**-**40**百分比。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-608">Change the **Target CPU** configuration to **20**-**40** percent.</span></span>

    > [!NOTE]
    > <span data-ttu-id="cfe9e-609">這個範圍代表您的 web 應用程式的平均 CPU 使用量。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-609">This range represents the average CPU usage for your web app.</span></span> <span data-ttu-id="cfe9e-610">Azure 將會新增或移除執行個體，將您的 web 應用程式保留在這個範圍。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-610">Azure will add or remove instances to keep your web app in this range.</span></span> <span data-ttu-id="cfe9e-611">中指定用於調整的執行個體最小和最大數目**執行個體計數**組態。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-611">The minimum and maximum number of instances used for scaling is specified in the **Instance Count** configuration.</span></span> <span data-ttu-id="cfe9e-612">Azure 永遠不會高於或超過該限制。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-612">Azure will never go above or beyond that limit.</span></span>
    >
    > <span data-ttu-id="cfe9e-613">預設值**目標 CPU**值只會修改這個實驗室的目的。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-613">The default **Target CPU** values are modified just for the purposes of this lab.</span></span> <span data-ttu-id="cfe9e-614">藉由設定 CPU 範圍值較小，您會增加以觸發自動調整的機會中, 度的負載放在應用程式時。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-614">By configuring the CPU range with small values, you are increasing the chances to trigger Autoscale when a moderate load is placed on the application.</span></span>

    <span data-ttu-id="cfe9e-615">![變更目標會介於 20 到 40%的 CPU](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "變更會介於 20 到 40%的目標 CPU")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-615">![Changing the target CPU to be between 20 and 40 percent](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "Changing the target CPU to be between 20 and 40 percent")</span></span>

    <span data-ttu-id="cfe9e-616">*變更會介於 20 到 40%的目標 CPU*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-616">*Changing the Target CPU to be between 20 and 40 percent*</span></span>
4. <span data-ttu-id="cfe9e-617">按一下 **儲存**在命令列中儲存的變更。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-617">Click **Save** in the command bar to save the changes.</span></span>

<a id="Ex5Task2"></a>
#### <a name="task-2--load-testing-with-visual-studio"></a><span data-ttu-id="cfe9e-618">工作 2-使用 Visual Studio 負載測試</span><span class="sxs-lookup"><span data-stu-id="cfe9e-618">Task 2 – Load Testing with Visual Studio</span></span>

<span data-ttu-id="cfe9e-619">既然**自動調整規模**已設定，您將建立**Web 效能和負載測試專案**在 Visual Studio 中產生某些 web 應用程式上的 CPU 負載。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-619">Now that **Autoscale** has been configured, you will create a **Web Performance and Load Test Project** in Visual Studio to generate some CPU load on your web app.</span></span>

1. <span data-ttu-id="cfe9e-620">開啟**Visual Studio Ultimate 2013** ，然後選取**檔案 |新 |專案...** 啟動新的解決方案。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-620">Open **Visual Studio Ultimate 2013** and select **File | New | Project...** to start a new solution.</span></span>

    <span data-ttu-id="cfe9e-621">![建立新的專案](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "建立新的專案")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-621">![Creating a new project](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "Creating a new project")</span></span>

    <span data-ttu-id="cfe9e-622">*建立新的專案*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-622">*Creating a new project*</span></span>
2. <span data-ttu-id="cfe9e-623">在 [**新的專案**對話方塊中，選取**Web 效能和負載測試專案**下**Visual C# |測試**] 索引標籤。請確定 **.NET Framework 4.5**是選取，將專案命名為*WebAndLoadTestProject*，選擇**位置**然後按一下**確定**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-623">In the **New Project** dialog box, select **Web Performance and Load Test Project** under the **Visual C# | Test** tab. Make sure **.NET Framework 4.5** is selected, name the project *WebAndLoadTestProject*, choose a **Location** and click **OK**.</span></span>

    <span data-ttu-id="cfe9e-624">![建立新的 Web 和負載測試專案](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "建立新的 Web 和負載測試專案")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-624">![Creating a new Web and Load Test project](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "Creating a new Web and Load Test project")</span></span>

    <span data-ttu-id="cfe9e-625">*建立新的 Web 和負載測試專案*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-625">*Creating a new Web and Load Test project*</span></span>
3. <span data-ttu-id="cfe9e-626">在  **WebTest1.webtest**上按一下滑鼠右鍵**WebTest1**節點，然後按一下**加入要求**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-626">In the **WebTest1.webtest** Right-click the **WebTest1** node and click **Add Request**.</span></span>

    <span data-ttu-id="cfe9e-627">![將要求加入至 WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "WebTest1 中加入要求")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-627">![Adding a request to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "Adding a request to WebTest1")</span></span>

    <span data-ttu-id="cfe9e-628">*將要求加入至 WebTest1*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-628">*Adding a request to WebTest1*</span></span>
4. <span data-ttu-id="cfe9e-629">在 [**屬性**] 視窗的 [新要求] 節點中，更新**Url**屬性以指向您的 web 應用程式的 URL (例如*[ http://geek-quiz.azurewebsites.net/ ](http://geek-quiz.azurewebsites.net/)*).</span><span class="sxs-lookup"><span data-stu-id="cfe9e-629">In the **Properties** window of the new request node, update the **Url** property to point to the URL of your web app (e.g. *[http://geek-quiz.azurewebsites.net/](http://geek-quiz.azurewebsites.net/)*).</span></span>

    <span data-ttu-id="cfe9e-630">![變更 Url 屬性](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "變更 Url 屬性")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-630">![Changing the Url property](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "Changing the Url property")</span></span>

    <span data-ttu-id="cfe9e-631">*變更 Url 屬性*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-631">*Changing the Url property*</span></span>
5. <span data-ttu-id="cfe9e-632">在  **WebTest1.webtest**  視窗中，以滑鼠右鍵按一下**WebTest1** ，按一下 **加入迴圈...**.</span><span class="sxs-lookup"><span data-stu-id="cfe9e-632">In the **WebTest1.webtest** window, right-click **WebTest1** and click **Add Loop...**.</span></span>

    <span data-ttu-id="cfe9e-633">![將迴圈加入至 WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "將迴圈加入至 WebTest1")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-633">![Adding a loop to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "Adding a loop to WebTest1")</span></span>

    <span data-ttu-id="cfe9e-634">*將迴圈加入至 WebTest1*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-634">*Adding a loop to WebTest1*</span></span>
6. <span data-ttu-id="cfe9e-635">在 **新增的條件式規則和項目迴圈**對話方塊中，選取**For 迴圈**規則，並修改下列屬性。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-635">In the **Add Conditional Rule and Items to Loop** dialog box, select the **For Loop** rule and modify the following properties.</span></span>

   1. <span data-ttu-id="cfe9e-636">**終止值：** 1000年</span><span class="sxs-lookup"><span data-stu-id="cfe9e-636">**Terminating value:** 1000</span></span>
   2. <span data-ttu-id="cfe9e-637">**內容參數名稱：** 迭代器</span><span class="sxs-lookup"><span data-stu-id="cfe9e-637">**Context Parameter Name:** Iterator</span></span>
   3. <span data-ttu-id="cfe9e-638">**遞增值：** 1</span><span class="sxs-lookup"><span data-stu-id="cfe9e-638">**Increment Value:** 1</span></span>

      <span data-ttu-id="cfe9e-639">![選取 For 迴圈規則並更新內容](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "選取 For 迴圈規則並更新內容")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-639">![Selecting the For Loop rule and updating the properties](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "Selecting the For Loop rule and updating the properties")</span></span>

      <span data-ttu-id="cfe9e-640">*選取 For 迴圈規則並更新內容*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-640">*Selecting the For Loop rule and updating the properties*</span></span>
7. <span data-ttu-id="cfe9e-641">底下**迴圈中的項目**區段中，選取您要在迴圈的第一個和最後一個項目先前建立的要求。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-641">Under the **Items in loop** section, select the request you created previously to be the first and last item for the loop.</span></span> <span data-ttu-id="cfe9e-642">按一下 [確定]  繼續操作。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-642">Click **OK** to continue.</span></span>

    <span data-ttu-id="cfe9e-643">![選取此迴圈的第一個和最後一個項目](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "選取迴圈的第一個和最後一個項目")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-643">![Selecting the first and last items for the loop](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "Selecting the first and last items for the loop")</span></span>

    <span data-ttu-id="cfe9e-644">*選取此迴圈的第一個和最後一個項目*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-644">*Selecting the first and last items for the loop*</span></span>
8. <span data-ttu-id="cfe9e-645">在 [**方案總管] 中**，以滑鼠右鍵按一下**WebAndLoadTestProject**專案中，展開**新增**功能表，然後選取**負載測試...**.</span><span class="sxs-lookup"><span data-stu-id="cfe9e-645">In **Solution Explorer**, right-click the **WebAndLoadTestProject** project, expand the **Add** menu and select **Load Test...**.</span></span>

    <span data-ttu-id="cfe9e-646">![WebAndLoadTestProject 專案中加入負載測試](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "WebAndLoadTestProject 專案中加入負載測試")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-646">![Adding a Load Test to the WebAndLoadTestProject project](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "Adding a Load Test to the WebAndLoadTestProject project")</span></span>

    <span data-ttu-id="cfe9e-647">*WebAndLoadTestProject 專案中加入負載測試*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-647">*Adding a Load Test to the WebAndLoadTestProject project*</span></span>
9. <span data-ttu-id="cfe9e-648">在 **新增負載測試精靈** 對話方塊中，按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-648">In the **New Load Test Wizard** dialog box, click **Next**.</span></span>

    <span data-ttu-id="cfe9e-649">![新增負載測試精靈](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "新增負載測試精靈")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-649">![New Load Test Wizard](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "New Load Test Wizard")</span></span>

    <span data-ttu-id="cfe9e-650">*新增負載測試精靈*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-650">*New Load Test Wizard*</span></span>
10. <span data-ttu-id="cfe9e-651">在 [**案例**頁面上，選取**不使用考慮時間**然後按一下**下一步]**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-651">In the **Scenario** page, select **Do not use think times** and click **Next**.</span></span>

    <span data-ttu-id="cfe9e-652">![選取不想使用考慮時間](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "選取不想使用考慮時間")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-652">![Selecting not to use think times](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "Selecting not to use think times")</span></span>

    <span data-ttu-id="cfe9e-653">*選擇不使用考慮時間*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-653">*Selecting not to use think times*</span></span>
11. <span data-ttu-id="cfe9e-654">在 **負載模式**頁面上，請確定**常數負載**選項。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-654">In the **Load Pattern** page, make sure that the **Constant Load** option is selected.</span></span> <span data-ttu-id="cfe9e-655">變更**使用者計數**設為**250**使用者，然後按一下 [**下一步]**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-655">Change the **User Count** setting to **250** users and click **Next**.</span></span>

    <span data-ttu-id="cfe9e-656">![變更使用者計數設為 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "變更的使用者計數設為 250")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-656">![Changing the user count to 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "Changing the user count to 250")</span></span>

    <span data-ttu-id="cfe9e-657">*變更為 250 的使用者計數*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-657">*Changing the user count to 250*</span></span>
12. <span data-ttu-id="cfe9e-658">在 [**測試混合模型**頁面上，選取**依據循序測試順序**然後按一下**下一步]**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-658">In the **Test Mix Model** page, select **Based on sequential test order** and click **Next**.</span></span>

    <span data-ttu-id="cfe9e-659">![選取的測試混合模型](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "選取的測試混合模型")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-659">![Selecting the test mix model](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "Selecting the test mix model")</span></span>

    <span data-ttu-id="cfe9e-660">*選取的測試混合模型*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-660">*Selecting the test mix model*</span></span>
13. <span data-ttu-id="cfe9e-661">在 **測試混合模型**頁面上，按一下**加入...** 將測試混合。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-661">In the **Test Mix Model** page, click **Add...** to add a test to the mix.</span></span>

    <span data-ttu-id="cfe9e-662">![將測試加入至測試混合](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "將測試加入至測試混合")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-662">![Adding a test to the test mix](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "Adding a test to the test mix")</span></span>

    <span data-ttu-id="cfe9e-663">*將測試加入至測試混合*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-663">*Adding a test to the test mix*</span></span>
14. <span data-ttu-id="cfe9e-664">在 [**新增測試**] 對話方塊中，按兩下**WebTest1**若要加入至測試**選取的測試**清單。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-664">In the **Add Tests** dialog box, double-click **WebTest1** to add the test to the **Selected tests** list.</span></span> <span data-ttu-id="cfe9e-665">按一下 [確定]  繼續操作。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-665">Click **OK** to continue.</span></span>

    <span data-ttu-id="cfe9e-666">![加入 WebTest1 測試](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "加入 WebTest1 測試")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-666">![Adding the WebTest1 test](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "Adding the WebTest1 test")</span></span>

    <span data-ttu-id="cfe9e-667">*加入 WebTest1 測試*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-667">*Adding the WebTest1 test*</span></span>
15. <span data-ttu-id="cfe9e-668">回到**測試混合**頁面上，按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-668">Back in the **Test Mix** page, click **Next**.</span></span>

    <span data-ttu-id="cfe9e-669">![完成測試混合 頁面](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "完成測試混合 頁面")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-669">![Completing the Test Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "Completing the Test Mix page")</span></span>

    <span data-ttu-id="cfe9e-670">*完成測試混合 頁面*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-670">*Completing the Test Mix page*</span></span>
16. <span data-ttu-id="cfe9e-671">在 [**網路混合**頁面上，按一下**下一步]**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-671">In the **Network Mix** page, click **Next**.</span></span>

    <span data-ttu-id="cfe9e-672">![按一下 [網路混合] 頁面中的下一個](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "按一下網路混合 頁面中的下一個")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-672">![Clicking next in the Network Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "Clicking next in the Network Mix page")</span></span>

    <span data-ttu-id="cfe9e-673">*網路混合 頁面中按一下 下一步*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-673">*Clicking next in the Network Mix page*</span></span>
17. <span data-ttu-id="cfe9e-674">在 [**瀏覽器混合**頁面上，選取**Internet Explorer 10.0**作為瀏覽器類型，然後按一下**下一步]**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-674">In the **Browser Mix** page, select **Internet Explorer 10.0** as the browser type and click **Next**.</span></span>

    <span data-ttu-id="cfe9e-675">![選取 瀏覽器類型](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "選取瀏覽器類型")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-675">![Selecting the browser type](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "Selecting the browser type")</span></span>

    <span data-ttu-id="cfe9e-676">*選取 瀏覽器類型*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-676">*Selecting the browser type*</span></span>
18. <span data-ttu-id="cfe9e-677">在 [**計數器集合**頁面上，按一下**下一步]**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-677">In the **Counter Sets** page, click **Next**.</span></span>

    <span data-ttu-id="cfe9e-678">![在 計數器集合 頁面中按一下 下一步](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "按一下 下一步 在 計數器集合 頁面")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-678">![Clicking Next in the Counter Sets page](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "Clicking Next in the Counter Sets page")</span></span>

    <span data-ttu-id="cfe9e-679">*在 [計數器集合] 頁面中按一下 [下一步]*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-679">*Clicking Next in the Counter Sets page*</span></span>
19. <span data-ttu-id="cfe9e-680">在 **回合設定**頁面上，將**負載測試持續期間**來**5 分鐘**，按一下 **完成**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-680">In the **Run Settings** page, set the **Load test duration** to **5 minutes** and click **Finish**.</span></span>

    <span data-ttu-id="cfe9e-681">![設定負載測試持續期間為 5 分鐘](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "設定負載測試持續期間為 5 分鐘")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-681">![Setting the load test duration to 5 minutes](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "Setting the load test duration to 5 minutes")</span></span>

    <span data-ttu-id="cfe9e-682">*設定負載測試持續期間為 5 分鐘*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-682">*Setting the load test duration to 5 minutes*</span></span>
20. <span data-ttu-id="cfe9e-683">在 [**方案總管] 中**，按兩下**Local.settings**来探索的測試設定檔案。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-683">In **Solution Explorer**, double-click the **Local.settings** file to explore the test settings.</span></span> <span data-ttu-id="cfe9e-684">根據預設，Visual Studio 會使用您的本機電腦來執行測試。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-684">By default, Visual Studio uses your local computer to run the tests.</span></span>

    > [!NOTE]
    > <span data-ttu-id="cfe9e-685">或者，您可以設定測試專案，以在雲端中使用執行負載測試**測試計劃 Azure**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-685">Alternatively, you can configure your test project to run the load tests in the cloud using **Azure Test Plans**.</span></span> <span data-ttu-id="cfe9e-686">Azure 的測試計劃提供雲端式負載測試會模擬更真實的負載的服務，避免本機環境的條件約束，例如 CPU 容量、 可用的記憶體和網路頻寬。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-686">Azure Test Plans provides a cloud-based load testing service that simulates a more realistic load, avoiding local environment constraints like CPU capacity, available memory, and network bandwidth.</span></span> <span data-ttu-id="cfe9e-687">如需有關如何使用 執行負載測試的 Azure 測試計劃的詳細資訊，請參閱[負載測試案例](/azure/devops/test/load-test/overview?view=vsts)。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-687">For more information about using Azure Test Plans to run load tests, see [Load testing scenarios](/azure/devops/test/load-test/overview?view=vsts).</span></span>

    ![測試設定](maintainable-azure-websites-managing-change-and-scale/_static/image98.png)

<a id="Ex5Task3"></a>
#### <a name="task-3--autoscale-verification"></a><span data-ttu-id="cfe9e-689">工作 3-自動調整規模的驗證</span><span class="sxs-lookup"><span data-stu-id="cfe9e-689">Task 3 – Autoscale Verification</span></span>

<span data-ttu-id="cfe9e-690">現在，您會執行您在上一個工作中建立負載測試，看您的 web 應用程式在負載下的運作方式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-690">You will now execute the load test you created in the previous task and see how your web app behaves under load.</span></span>

1. <span data-ttu-id="cfe9e-691">在 **方案總管**，按兩下**LoadTest1.loadtest**開啟負載測試。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-691">In **Solution Explorer**, double-click **LoadTest1.loadtest** to open the load test.</span></span>

    <span data-ttu-id="cfe9e-692">![開啟 LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "開啟 LoadTest1.loadtest")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-692">![Opening LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "Opening LoadTest1.loadtest")</span></span>

    <span data-ttu-id="cfe9e-693">*開啟 LoadTest1.loadtest*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-693">*Opening LoadTest1.loadtest*</span></span>
2. <span data-ttu-id="cfe9e-694">在 [ **LoadTest1.loadtest** ] 視窗中，按一下 [執行負載測試工具箱] 中的第一個按鈕。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-694">In the **LoadTest1.loadtest** window, click the first button in the toolbox to run the load test.</span></span>

    <span data-ttu-id="cfe9e-695">![執行負載測試](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "執行負載測試")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-695">![Running the load test](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "Running the load test")</span></span>

    <span data-ttu-id="cfe9e-696">*執行負載測試*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-696">*Running the load test*</span></span>
3. <span data-ttu-id="cfe9e-697">等候完成的負載測試。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-697">Wait until the load test completes.</span></span>

    > [!NOTE]
    > <span data-ttu-id="cfe9e-698">負載測試模擬多位使用者同時將要求傳送至 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-698">The load test simulates multiple users that send requests to the web app simultaneously.</span></span> <span data-ttu-id="cfe9e-699">當測試執行時，您可以監視可用的計數器，來偵測任何錯誤、 警告或其他與負載測試回合相關的資訊。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-699">When the test is running, you can monitor the available counters to detect any errors, warnings or other information related to your load test run.</span></span>

    <span data-ttu-id="cfe9e-700">![負載測試執行](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "等待，直到負載測試完成")</span><span class="sxs-lookup"><span data-stu-id="cfe9e-700">![Load test running](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "Waiting until the load test completes")</span></span>

    <span data-ttu-id="cfe9e-701">*執行負載測試*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-701">*Load test running*</span></span>
4. <span data-ttu-id="cfe9e-702">測試完成後，返回管理入口網站，並瀏覽至**擴展**web 應用程式的頁面。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-702">Once the test completes, go back to the management portal and navigate to the **Scale** page of your web app.</span></span> <span data-ttu-id="cfe9e-703">底下**容量** 區段中，您應該會看到在圖形中的新執行個體已自動部署。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-703">Under the **capacity** section, you should see in the graph that a new instance was automatically deployed.</span></span>

    ![自動部署的新執行個體](maintainable-azure-websites-managing-change-and-scale/_static/image102.png)

    <span data-ttu-id="cfe9e-705">*自動部署的新執行個體*</span><span class="sxs-lookup"><span data-stu-id="cfe9e-705">*New instance automatically deployed*</span></span>

    > [!NOTE]
    > <span data-ttu-id="cfe9e-706">可能需要幾分鐘的時間才會出現在圖形中的變更 (請按**CTRL + F5**定期重新整理頁面)。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-706">It may take several minutes for the changes to appear in the graph (press **CTRL + F5** periodically to refresh the page).</span></span> <span data-ttu-id="cfe9e-707">如果看不到任何變更，您可以嘗試下列各項：</span><span class="sxs-lookup"><span data-stu-id="cfe9e-707">If you do not see any changes, you can try the following:</span></span>
    >
    > - <span data-ttu-id="cfe9e-708">增加的負載測試持續時間 (例如，新增到**10 分鐘**)</span><span class="sxs-lookup"><span data-stu-id="cfe9e-708">Increase the duration of the load test (e.g. to **10 minutes**)</span></span>
    > - <span data-ttu-id="cfe9e-709">降低的最大和最小值**目標 CPU** web 應用程式的自動調整規模設定中的範圍</span><span class="sxs-lookup"><span data-stu-id="cfe9e-709">Reduce the maximum and minimum values of the **Target CPU** range in the Autoscale configuration of your web app</span></span>
    > - <span data-ttu-id="cfe9e-710">使用在雲端中執行負載測試**測試計劃 Azure**。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-710">Run the load test in the cloud with **Azure Test Plans**.</span></span> <span data-ttu-id="cfe9e-711">更多資訊[這裡](/azure/devops/test/load-test/index?view=vsts)</span><span class="sxs-lookup"><span data-stu-id="cfe9e-711">More information [here](/azure/devops/test/load-test/index?view=vsts)</span></span>

* * *

<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="cfe9e-712">總結</span><span class="sxs-lookup"><span data-stu-id="cfe9e-712">Summary</span></span>

<span data-ttu-id="cfe9e-713">在這個實際操作實驗室中，您已了解如何設定及部署您的生產 web 應用程式在 Azure 中的應用程式。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-713">In this hands-on lab, you learned how to set up and deploy your application to production web apps in Azure.</span></span> <span data-ttu-id="cfe9e-714">您開始透過偵測並更新您使用的資料庫**Entity Framework Code First Migrations**，然後繼續執行部署的站台使用的新版本**Git**以及執行回復您的站台的最新穩定版本。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-714">You started by detecting and updating your databases using **Entity Framework Code First Migrations**, then continued by deploying new versions of your site using **Git** and performing rollbacks to the latest stable version of your site.</span></span> <span data-ttu-id="cfe9e-715">此外，您已了解如何調整您的應用程式使用儲存體，將您的靜態內容移至 Blob 容器。</span><span class="sxs-lookup"><span data-stu-id="cfe9e-715">Additionally, you learned how to scale your app using Storage to move your static content to a Blob container.</span></span>
