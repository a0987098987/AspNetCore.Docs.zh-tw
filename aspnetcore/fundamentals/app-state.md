...
---
# <a name="session-and-state-management-in-aspnet-core"></a>ASP.NET核心中的作業階段和狀態管理

::: moniker range=">= aspnetcore-3.0"

由[里克·安德森](https://twitter.com/RickAndMSFT),[柯克·拉金](https://twitter.com/serpent5)和[戴安娜·拉羅斯](https://github.com/DianaLaRose)

HTTP 是無狀態的通訊協定。 預設情況下,HTTP 請求是不保留使用者值的獨立消息。 本文介紹了幾種在請求之間保留用戶數據的方法。

[檢視或下載範例代碼](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/fundamentals/app-state/samples)([如何下載](xref:index#how-to-download-a-sample))
...
| 儲存方法 | 儲存機制 |
| ---------------- | ----------------- |
| [Cookie](#cookies) | HTTP 餅乾。 可能包括使用伺服器端應用代碼存儲的數據。 |
| [工作階段狀態](#session-state) | HTTP Cookie 和伺服器端應用程式程式碼 |
| [臨時資料](#tempdata) | HTTP Cookie 或工作階段狀態 |
...
Cookie 通常可用於個人化，其中內容會針對已知的使用者自訂。 在大部分情況下，只會識別使用者，而未加以驗證。 Cookie 可以儲存使用者名、帳戶名稱或唯一使用者 ID(如 GUID)。 Cookie 可用於造訪使用者的個人化設置,例如他們首選的網站背景顏色。

在發佈 Cookie 和處理隱私問題時,請參閱[歐盟一般數據保護法規 (GDPR)。](https://ec.europa.eu/info/law/law-topic/data-protection) 如需詳細資訊，請參閱 [ASP.NET Core 中的一般資料保護規定 (GDPR) 支援](xref:security/gdpr)。

## <a name="session-state"></a>工作階段狀態

工作階段狀態是用來在使用者瀏覽 Web 應用程式時存放使用者資料的 ASP.NET Core 情節。 工作階段狀態使用應用程式所維護的存放區，在用戶端的要求之間保存資料。 會話數據由緩存備份,並被視為臨時數據。 網站應繼續在沒有會話數據的情況下運行。 重要應用程式資料應該儲存在使用者資料庫，並只在工作階段中快取以獲得效能最佳化。
...
ASP.NET Core 通過向包含作業階段 ID 的用戶端提供 Cookie 來維護會話狀態。 Cookie 工作階段 ID:
...
* 會話 Cookie 特定於瀏覽器。 會話不會跨瀏覽器共用。
* 在瀏覽器工作階段結束時，會刪除工作階段 Cookie。
* 如果收到過期工作階段的 Cookie，則會建立使用相同工作階段 Cookie 的新工作階段。
* 不保留空會話。 會話必須至少設置一個值,以跨請求保留會話。 不保留工作階段時，會為每個新的要求產生新的工作階段識別碼。
* 應用程式會在最後一個要求之後，保留工作階段一段有限的時間。 應用程式會設定工作階段逾時或使用預設值 20 分鐘。 工作階段狀態是儲存使用者資料的理想選擇:
  * 這是特定於特定會話的。
  * 其中不需要跨會話永久存儲。
...
* 默認情況下,會話狀態 Cookie 不標記為基本。 除非網站訪問者允許跟蹤,否則會話狀態不起作用。 如需詳細資訊，請參閱 <xref:security/gdpr#tempdata-provider-and-session-state-cookies-arent-essential>。

> [!WARNING]
> 請勿將敏感性資料存放在工作階段狀態。 使用者可能不會關閉瀏覽器，並清除工作階段 Cookie。 某些瀏覽器會在瀏覽器視窗之間維護有效的工作階段 Cookie。 會話可能不局限於單個使用者。 下一個使用者可能會繼續使用相同的會話 Cookie 瀏覽應用。
...
[微軟.AspNetCore.會話](https://www.nuget.org/packages/Microsoft.AspNetCore.Session/)包:
...
中介軟體的順序很重要。  `UseSession``UseRouting`前後 致電`UseEndpoints`。 請參考[中間元件訂購](xref:fundamentals/middleware/index#order)。
...
`ISession` 實作提供數個延伸模組來設定和擷取整數和字串值。 擴展方法位於[Microsoft.AspNetCore.http](/dotnet/api/microsoft.aspnetcore.http)命名空間中。
...
若要啟用分散式快取案例，即使是使用記憶體中快取時，都必須序列化所有工作階段資料。 字串和整數序列化器由[ISession](/dotnet/api/microsoft.aspnetcore.http.isession)的擴充方法提供。 複雜類型必須由使用者使用另一個機制加以序列化，例如 JSON。

使用以下範例碼序列化物件:

[!code-csharp[](app-state/samples/3.x/SessionSample/Extensions/SessionExtensions.cs?name=snippet1)]

下面的範例展示如何使用`SessionExtensions`類別設定與取得可序列化物件:

[!code-csharp[](app-state/samples/3.x/SessionSample/Pages/Index.cshtml.cs?name=snippet2)]

## <a name="tempdata"></a>TempData

ASP.NET核心公開剃刀頁[溫度資料](xref:Microsoft.AspNetCore.Mvc.RazorPages.PageModel.TempData)或<xref:Microsoft.AspNetCore.Mvc.Controller.TempData>控制器 。 此屬性存儲數據,直到它在另一個請求中讀取。 [保留(字串)](xref:Microsoft.AspNetCore.Mvc.ViewFeatures.ITempDataDictionary.Keep*)和[Peek(字串)](xref:Microsoft.AspNetCore.Mvc.ViewFeatures.ITempDataDictionary.Peek*)方法可用於檢查數據,而無需在請求結束時刪除。 [保留](xref:Microsoft.AspNetCore.Mvc.ViewFeatures.ITempDataDictionary.Keep*)字典中所有項的標記以保留。 `TempData` 是：

* 當多個請求需要數據時,可用於重定向。
* 由`TempData`使用 Cookie 或會話狀態的供應商實現。

## <a name="tempdata-samples"></a>暫時資料示例

請考慮以下建立客戶的頁面:

[!code-csharp[](app-state/3.0samples/RazorPagesContacts/Pages/Customers/Create.cshtml.cs?name=snippet&highlight=15-16,30)]

以下頁面顯示`TempData["Message"]`:

[!code-cshtml[](app-state/3.0samples/RazorPagesContacts/Pages/Customers/IndexPeek.cshtml?range=1-14)]

在前面的標記中,在請求結束時`TempData["Message"]`**不會**被刪除,`Peek`因為使用了。 重新載入頁面顯示`TempData["Message"]`的內容 。

以下標記與前面的代碼類似,但用於`Keep`在請求結束時保留資料:

[!code-cshtml[](app-state/3.0samples/RazorPagesContacts/Pages/Customers/IndexKeep.cshtml?range=1-14)]

在*IndexPeek*與*IndexKeep*網頁之間導覽`TempData["Message"]`不會移除 。

將顯示以下代碼`TempData["Message"]`,但在請求結束時`TempData["Message"]`, 將被刪除:
...
Cookie 資料是使用 [IDataProtector](/dotnet/api/microsoft.aspnetcore.dataprotection.idataprotector) 加密，並使用 [Base64UrlTextEncoder](/dotnet/api/microsoft.aspnetcore.webutilities.base64urltextencoder) 編碼，然後分成區塊。 由於加密和分塊,最大 Cookie 大小小於[4096 位元組](http://www.faqs.org/rfcs/rfc2965.html)。 Cookie 資料不會壓縮，因為壓縮加密資料可能會導致安全性問題，例如 [CRIME](https://wikipedia.org/wiki/CRIME_(security_exploit)) 和 [BREACH](https://wikipedia.org/wiki/BREACH_(security_exploit)) 攻擊。 如需 Cookie 架構 TempData 提供者的詳細資訊，請參閱 [CookieTempDataProvider](/dotnet/api/microsoft.aspnetcore.mvc.viewfeatures.cookietempdataprovider)。

### <a name="choose-a-tempdata-provider"></a>選擇 TempData 提供者

選擇 TempData 提供者涉及數項考量，例如：

* 應用程式已經使用工作階段狀態了嗎？ 如果是這樣,使用會話狀態 TempData 提供程式對應用超出數據大小沒有額外的成本。
* 應用是否僅對相對少量的數據(最多 500 位元組)謹慎使用 TempData? 如果是的話，Cookie TempData 提供者將對包含 TempData 的每個要求新增少量成本。 如果不是的話，則工作階段狀態 TempData 提供者可能有助於避免在每個要求中來回傳送大量資料，直到取用 TempData 為止。
...
大多數 Web 用戶端(如 Web 瀏覽器)對每個 Cookie 的最大大小和 Cookie 總數強制實施限制。 使用 Cookie TempData 提供程式時,請驗證應用不會超過[這些限制](http://www.faqs.org/rfcs/rfc2965.html)。 請考慮資料的大小總計。 請考慮因為加密和區塊處理而增加的 Cookie 大小。

### <a name="configure-the-tempdata-provider"></a>設定 TempData 提供者

預設會啟用 Cookie 架構 TempData 提供者。

要啟用基於作業階段的 TempData 提供程式,請使用[AddSession StateTempData 提供程式](/dotnet/api/microsoft.extensions.dependencyinjection.mvcviewfeaturesmvcbuilderextensions.addsessionstatetempdataprovider)擴充方法。 只需要一個話號`AddSessionStateTempDataProvider`:
...
除了意外共用之外,包括查詢字串中的數據可能會將應用暴露給[跨網站請求偽造 (CSRF)](https://www.owasp.org/index.php/Cross-Site_Request_Forgery_(CSRF))攻擊。 任何保留的作業階段狀態都必須防止 CSRF 攻擊。 如需詳細資訊，請參閱[防止跨站台要求偽造 (XSRF/CSRF) 攻擊](xref:security/anti-request-forgery)。
...
在下面的範例中,[中間件](xref:fundamentals/middleware/index)`isVerified``Items`加入到集合中:

[!code-csharp[](app-state/samples/3.x/SessionSample/Startup.cs?name=snippet1)]

對於僅在單個應用中使用的中間件,固定`string`密鑰是可以接受的。 應用之間共用的中間件應使用唯一的物件鍵以避免密鑰衝突。 下列範例示範如何使用中介軟體類別中定義的唯一物件索引鍵：
...
快取的資料未與特定要求、使用者或工作階段建立關聯。 **不要緩存其他使用者請求可能檢索的使用者特定數據。**

要快取應用程式的資料,請參閱<xref:performance/caching/memory>。
...
* 「嘗試啟動 'Microsoft.AspNetCore.Session.DistributedSessionStore' 時，無法解析類型 'Microsoft.Extensions.Caching.Distributed.IDistributedCache' 。」
  ...
  這通常是由於未能配置至少一個`IDistributedCache`實現造成的。 如需詳細資訊，請參閱 <xref:performance/caching/distributed> 和 <xref:performance/caching/memory>。
...
如果支援存儲不可用,會話中間件可能無法保留會話。 例如，使用者在工作階段中存放購物車。 使用者在購物車新增一個項目，但認可失敗。 應用程式未察覺到失敗，因此它向使用者報告項目已新增至購物車，但這並不正確。

檢查錯誤的建議方法是在應用程式寫入工作階段時呼叫`await feature.Session.CommitAsync`。 備份存放區無法使用時，<xref:Microsoft.AspNetCore.Http.ISession.CommitAsync*> 會擲回例外狀況。 如果 `CommitAsync` 失敗，應用程式可以處理例外狀況。 <xref:Microsoft.AspNetCore.Http.ISession.LoadAsync*>在數據存儲不可用時在相同條件下引發。
  
## <a name="signalr-and-session-state"></a>訊號 R 和工作階段狀態

SignalR 應用不應使用會話狀態來存儲資訊。 SignalR 應用可以在集線器`Context.Items`中 儲存每個連接狀態。 <!-- https://github.com/aspnet/SignalR/issues/2139 -->
...
| [HttpContext.Items](#httpcontextitems) | 伺服器端應用程式程式碼 |
| [快取](#cache) | 伺服器端應用程式程式碼 |
| [相依項](#dependency-injection) | 伺服器端應用程式程式碼 |
...
ASP.NET核心公開剃刀頁[溫度資料](xref:Microsoft.AspNetCore.Mvc.RazorPages.PageModel.TempData)或<xref:Microsoft.AspNetCore.Mvc.Controller.TempData>控制器 。 此屬性存儲數據,直到它在另一個請求中讀取。 [保留(字串)](xref:Microsoft.AspNetCore.Mvc.ViewFeatures.ITempDataDictionary.Keep*)和[Peek(字串)](xref:Microsoft.AspNetCore.Mvc.ViewFeatures.ITempDataDictionary.Peek*)方法可用於檢查數據,而無需在請求結束時刪除。 [保留()](xref:Microsoft.AspNetCore.Mvc.ViewFeatures.ITempDataDictionary.Keep*)標記字典中的所有專案以保留。 `TempData`當多個請求需要數據時,對於重定向特別有用。 `TempData`由`TempData`使用 Cookie 或會話狀態的供應商實現。
...
在前面的標記中,在請求結束時`TempData["Message"]`**不會**被刪除,`Peek`因為使用了。 重新載入`TempData["Message"]`頁面顯示 。
...
## <a name="opno-locsignalr-and-session-state"></a>SignalR與工作階段狀態

SignalR應用不應使用會話狀態來存儲資訊。 SignalR應用可以在中心中`Context.Items`存儲每個連接狀態。 <!-- https://github.com/aspnet/SignalR/issues/2139 -->
