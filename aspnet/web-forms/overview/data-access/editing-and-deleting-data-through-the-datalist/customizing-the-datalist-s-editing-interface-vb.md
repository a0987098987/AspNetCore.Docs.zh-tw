...
標記和 DataList s 中的 Web 控制項`EditItemTemplate`定義其編輯介面。 在所有可編輯的 DataList 範例我們 ve 檢查到目前為止，可編輯的文字方塊中的 Web 控制項編寫介面。 在 [前述教學課程](adding-validation-controls-to-the-datalist-s-editing-interface-vb.md)我們改善的編輯階段的使用者體驗將驗證控制項加入。
...
**圖 2**： 將更新、 插入和刪除索引標籤下拉式清單設定為 （無） ([按一下以檢視完整大小的影像](customizing-the-datalist-s-editing-interface-vb/_static/image6.png))
...
**圖 3**： 顯示名稱、 供應商、 分類、 已停止的狀態，以及每個產品的價格 ([按一下以檢視完整大小的影像](customizing-the-datalist-s-editing-interface-vb/_static/image9.png))
