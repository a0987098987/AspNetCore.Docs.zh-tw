...
* 針對 2.1 版， [Razor 類別庫](xref:razor-pages/ui-class)是將 Razor 檔案的建議的解決方案。 如果您的應用程式會使用內嵌的檢視，或依賴執行階段編譯 Razor 檔案，否則新增`<CopyRefAssembliesToPublishDirectory>true</CopyRefAssembliesToPublishDirectory>`至專案檔。
