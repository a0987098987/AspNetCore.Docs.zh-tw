...
* 執行 [dotnet publish](https://docs.microsoft.com/dotnet/core/tools/dotnet-publish.md) 命令：
...
以下是您先前執行之 `docker build` 命令所使用的 Dockerfile。  它會以您在本節所做的相同方式，使用 `dotnet publish` 進行建置及部署。  
...
> [透過 HTTPS 使用 Docker 開發 ASP.NET Core 應用程式](https://github.com/dotnet/dotnet-docker/blob/master/samples/aspnetapp/aspnetcore-docker-https-development.md)
