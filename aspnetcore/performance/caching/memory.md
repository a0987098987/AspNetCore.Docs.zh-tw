...
> 使用 `SetSize` 、 `Size` 或來限制快取時，請建立單一快取 `SizeLimit` 來進行快取。 如需詳細資訊和範例，請參閱[使用 SetSize、大小和 SizeLimit 來限制](#use-setsize-size-and-sizelimit-to-limit-cache-size)快取大小。
> 共用快取是由其他架構或程式庫共用。 例如，EF Core 使用共用快取，且未指定大小。 

記憶體內部快取是使用相依性[插入](xref:fundamentals/dependency-injection)從應用程式參考的*服務*。 `IMemoryCache`在此函數中要求實例：
...
`DateTime`當超時時間內有要求時，快取的值會保留在快取中。
...
* 限制快取成長。
* <xref:Microsoft.Extensions.Caching.Memory.MemoryCache.Compact*> <xref:Microsoft.Extensions.Caching.Memory.MemoryCache.Remove*> 當可用的記憶體受到限制時，呼叫或：
...
快取專案的大小可以透過 <xref:Microsoft.Extensions.Caching.Memory.MemoryCacheEntryOptions.Size> 或擴充方法來設定 <xref:Microsoft.Extensions.Caching.Memory.MemoryCacheEntryExtensions.SetSize*> ：
...
具有優先順序的固定專案永遠不會 <xref:Microsoft.Extensions.Caching.Memory.CacheItemPriority.NeverRemove> 被移除。 下列程式碼會移除快取專案並呼叫 `Compact` ：
...
下列範例示範如果相依專案過期，如何讓快取專案過期。 <xref:Microsoft.Extensions.Primitives.CancellationChangeToken>會新增至快取的專案。 `Cancel`在上呼叫時 `CancellationTokenSource` ，會收回這兩個快取專案。

[!code-csharp[](memory/3.0sample/WebCacheSample/Controllers/HomeController.cs?name=snippet_ed)]

使用 <xref:System.Threading.CancellationTokenSource> 可讓多個快取專案收回為群組。 使用 `using` 上述程式碼中的模式時，在區塊內建立的快取專案 `using` 將會繼承觸發程式和到期設定。
...
* 大部分的應用程式 `IMemoryCache` 都已啟用。 例如， `AddMvc` 在中呼叫、、 `AddControllersWithViews` `AddRazorPages` 、 `AddMvcCore().AddRazorViewEngine` 和許多其他 `Add{Service}` 方法，會 `ConfigureServices` 啟用 `IMemoryCache` 。 對於未呼叫上述其中一種方法的應用程式 `Add{Service}` ，可能需要 <xref:Microsoft.Extensions.DependencyInjection.MemoryCacheServiceCollectionExtensions.AddMemoryCache*> 在中呼叫 `ConfigureServices` 。

## <a name="background-cache-update"></a>背景快取更新

使用[背景服務](xref:fundamentals/host/hosted-services)（例如） <xref:Microsoft.Extensions.Hosting.IHostedService> 來更新快取。 背景服務可以重新計算專案，然後只有在準備就緒時，才將它們指派給快取。
...
記憶體內部快取是使用相依性插入從您的應用程式參考的一[項](../../fundamentals/dependency-injection.md)*服務*。 `AddMemoryCache`在中呼叫 `ConfigureServices` ：

[!code-csharp[](memory/sample/WebCache/Startup.cs?highlight=9)]

`IMemoryCache`在此函數中要求實例：
...
`DateTime`當超時時間內有要求時，快取的值會保留在快取中。 下圖顯示從快取中抓取的目前時間和較舊的時間：
...
具有優先順序的固定專案永遠不會 <xref:Microsoft.Extensions.Caching.Memory.CacheItemPriority.NeverRemove> 被移除。
...
使用 `CancellationTokenSource` 可讓多個快取專案收回為群組。 使用 `using` 上述程式碼中的模式時，在區塊內建立的快取專案 `using` 將會繼承觸發程式和到期設定。
