...
以背景工作角色服務範本為基礎的應用`Microsoft.NET.Sdk.Worker`程式會使用 SDK，並具有對[Microsoft Extensions. 裝載](https://www.nuget.org/packages/Microsoft.Extensions.Hosting)封裝的明確套件參考。 例如，請參閱範例應用程式的專案檔（*BackgroundTasksSample .csproj*）。
...
呼叫[ExecuteAsync （CancellationToken）](xref:Microsoft.Extensions.Hosting.BackgroundService.ExecuteAsync*)以執行背景服務。 此實作為傳回<xref:System.Threading.Tasks.Task> ，代表背景服務的整個存留期。 在[ExecuteAsync 變成非同步](https://github.com/dotnet/extensions/issues/2149)（例如呼叫`await`）之前，不會再啟動任何進一步的服務。 避免執行長時間的封鎖初始化工作`ExecuteAsync`。 [StopAsync （CancellationToken）](xref:Microsoft.Extensions.Hosting.BackgroundService.StopAsync*)中等候`ExecuteAsync`完成的主機區塊。

呼叫[IHostedService. StopAsync](xref:Microsoft.Extensions.Hosting.IHostedService.StopAsync*)時，會觸發解除標記。 當解除標記`ExecuteAsync`引發時，您的執行應該會立即完成，以便正常地關閉服務。 否則，服務強制會在關機時間關閉。 如需詳細資訊，請參閱[IHostedService 介面](#ihostedservice-interface)一節。
...
<xref:System.Threading.Timer>不會等待先前的`DoWork`執行完成，因此所顯示的方法可能不適用於每個案例。 [連鎖：遞增](xref:System.Threading.Interlocked.Increment*)是用來將執行計數器遞增為不可部分完成的作業，這可確保多個`executionCount`執行緒不會同時更新。
...
託管服務會建立範圍來解析已設定範圍的背景工作服務，以`DoWork`呼叫其方法。 `DoWork`傳回`Task`，其等候于`ExecuteAsync`：
...
* `BackgroundProcessing`方法會傳回`Task`，它會在中`ExecuteAsync`等待。
* 佇列中的背景工作會在中`BackgroundProcessing`進行清除並執行。
* 在`StopAsync`服務停止之前，會等待工作專案。

[!code-csharp[](hosted-services/samples/3.x/BackgroundTasksSample/Services/QueuedHostedService.cs?name=snippet1&highlight=28-29,33)]

每當`MonitorLoop`在輸入裝置上選取索引`w`鍵時，服務就會處理託管服務的佇列工作：

* `IBackgroundTaskQueue`會插入`MonitorLoop`服務中。
* `IBackgroundTaskQueue.QueueBackgroundWorkItem`呼叫以將工作專案排入佇列。
* 工作專案會模擬長時間執行的背景工作：
  * 會執行三個5秒的延遲`Task.Delay`（）。
  * 如果`try-catch`工作已<xref:System.OperationCanceledException>取消，則語句會補漏白。
...
<xref:System.Threading.Timer>不會等待先前的`DoWork`執行完成，因此所顯示的方法可能不適用於每個案例。
