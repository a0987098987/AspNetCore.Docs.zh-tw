...
[檢視或下載範例程式碼](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/fundamentals/url-rewriting/samples/) \(英文\) ([如何下載](xref:index#how-to-download-a-sample))
...
若要建立 URL 重寫和重新導向規則，您可以針對每個重寫規則的擴充方法建立 [RewriteOptions](xref:Microsoft.AspNetCore.Rewrite.RewriteOptions) 類別的執行個體。 依據所需的處理順序來鏈結多個規則。 系統會使用 `RewriteOptions` 將 <xref:Microsoft.AspNetCore.Builder.RewriteBuilderExtensions.UseRewriter*> 新增至要求管線，並將其傳入 URL 重寫中介軟體：
...
* 如果要求為非`www`，<xref:Microsoft.AspNetCore.Rewrite.RewriteOptionsExtensions.AddRedirectToWwwPermanent*> &ndash; 會將要求永久重新導向至 `www` 子域。 使用 [Status308PermanentRedirect](xref:Microsoft.AspNetCore.Http.StatusCodes.Status308PermanentRedirect) 狀態代碼重新導向。

* 如果傳入要求為非`www`，<xref:Microsoft.AspNetCore.Rewrite.RewriteOptionsExtensions.AddRedirectToWww*> &ndash; 會將要求重新導向至 `www` 子域。 使用 [Status307TemporaryRedirect](xref:Microsoft.AspNetCore.Http.StatusCodes.Status307TemporaryRedirect) 狀態代碼重新導向。 多載可讓您提供回應的狀態代碼。 請使用 <xref:Microsoft.AspNetCore.Http.StatusCodes> 類別的欄位來進行狀態碼指派。
...
括弧內所含的運算式部分稱為「擷取群組」。 運算式的點 (`.`) 表示「比對任何字元」。 星號 (`*`) 表示「比對前置字元零或多次」。 因此，擷取群組 `1234/5678` 會擷取 URL 的 `(.*)` 最後這兩個路徑區段。 這個單一擷取群組會擷取您在要求 URL 中的 `redirect-rule/` 之後所提供的任何值。
...
> [!NOTE]
> 您也可以透過 <xref:Microsoft.Extensions.FileProviders.IFileProvider> 取得 <xref:Microsoft.Extensions.FileProviders.PhysicalFileProvider>。 這種方法可讓重寫規則檔案的位置更有彈性。 請確認重寫規則檔案已部署到您所提供之路徑的伺服器。
>
> ```csharp
...
| 目標 | Regex 字串及<br>比對範例 | 取代字串及<br>輸出範例 |
| ---- | ------------------------------- | -------------------------------------- |
...
若要在您的專案中包含中介軟體，請在包含 [Microsoft.AspNetCore.Rewrite](xref:fundamentals/metapackage-app) 套件的專案檔中，將套件參考新增至 [Microsoft.AspNetCore.App 中繼套件](https://www.nuget.org/packages/Microsoft.AspNetCore.Rewrite)。
