...
如此即完成處理並行衝突的簡介。 其他的方式來處理各種並行案例的相關資訊，請參閱[開放式並行存取模式](https://msdn.microsoft.com/data/jj592904)和[屬性值使用](https://msdn.microsoft.com/data/jj592677)MSDN 上。 下一個教學課程示範如何實作的每個階層的資料表繼承`Instructor`和`Student`實體。
