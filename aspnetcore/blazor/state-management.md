...
1. 開啟*Pages/_Host. cshtml*檔案, 並移除對的呼叫`Html.RenderComponentAsync`。
1. 開啟檔案, 並將對的呼叫`endpoints.MapBlazorHub()`取代`endpoints.MapBlazorHub<App>("app")`為。 `Startup.cs` `App`這是根元件的類型。 `"app"`是指定根元件位置的 CSS 選取器。
