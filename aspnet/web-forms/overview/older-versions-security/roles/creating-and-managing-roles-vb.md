...
透過設定角色架構`<roleManager>`的應用程式中的項目`Web.config`檔案。 根據預設，會停用角色支援。 若要啟用它，您必須設定[ `<roleManager>` ](https://msdn.microsoft.com/library/ms164660.aspx)項目的`enabled`屬性設定為`true`就像這樣：
