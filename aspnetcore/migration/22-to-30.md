...
* @No__t_0 類別會停用具有 `[DisableCors]` 屬性的 CORS。
...
或者，所有端點都可以設定為要求授權，而不需要 `[Authorize]` 或 `RequireAuthorization` 藉由設定 `FallbackPolicy`。 @No__t_0 與 `DefaultPolicy` 不同。 @No__t_0 是由 `[Authorize]` 或 `RequireAuthorization` 觸發，而當未設定其他原則時，則會觸發 `FallbackPolicy`。 `FallbackPolicy` 一開始設定為允許未經授權的要求。
...
從 `WebHostBuilder` 到 `HostBuilder` 的最重大變更是相依性[插入（DI）](xref:fundamentals/dependency-injection)。 使用 `HostBuilder` 時，您只能將 <xref:Microsoft.Extensions.Configuration.IConfiguration> 和 <xref:Microsoft.AspNetCore.Hosting.IHostingEnvironment> 插入 `Startup` 的函式。 @No__t_0 DI 條件約束：
