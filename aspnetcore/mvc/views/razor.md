...
Razor 支援 C#，並使用`@`轉換來自 HTML，以 C# 中的符號。 Razor 評估 C# 運算式，並將它們呈現的 HTML 輸出中。 Razor 可以從 HTML 轉換，到 C# 或特定的 Razor 標記。 當`@`符號後面[Razor 保留關鍵字](#razor-reserved-keywords)會轉換成特定的 Razor 標記，否則它會轉換成一般的 C#。
