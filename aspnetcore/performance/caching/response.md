...
通用`Cache-Control`指示詞如下表所示。

| 指示詞                                                       | 動作 |
| --------------------------------------------------------------- | ------ |
| [公立](https://tools.ietf.org/html/rfc7234#section-5.2.2.5)   | 快取可能會儲存回應。 |
...
| [雜](https://tools.ietf.org/html/rfc7234#section-5.4)  | 存在於與 HTTP/1.0 快取的回溯相容`no-cache`性，以進行設定行為。 如果`Cache-Control`標頭存在，則會`Pragma`忽略標頭。 |
...
快取[控制標頭的 HTTP 1.1](https://tools.ietf.org/html/rfc7234#section-5.2)快取規格需要快取以接受用戶端`Cache-Control`所傳送的有效標頭。 用戶端可以使用`no-cache`標頭值提出要求，並強制服務器為每個要求產生新的回應。

如果您考慮`Cache-Control` HTTP 快取的目標，一律接受用戶端要求標頭是合理的。 在官方規格中，快取的目的是要降低跨用戶端、proxy 和伺服器網路上滿足要求的延遲和網路負擔。 這不一定是控制源伺服器負載的方式。

使用回應快取[中介軟體](xref:performance/caching/middleware)時，不會有開發人員控制此快取行為，因為中介軟體會遵守官方快取規格。 [中介軟體的規劃增強功能](https://github.com/dotnet/AspNetCore/issues/2612)，是在決定提供快取回應時，設定`Cache-Control`中介軟體忽略要求標頭的機會。 規劃的增強功能可讓您有機會更妥善控制伺服器負載。
...
必須啟用回應快取[中介軟體](xref:performance/caching/middleware)才能設定<xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByQueryKeys>屬性。 否則，就會擲回執行時間例外狀況。 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByQueryKeys>屬性沒有對應的 HTTP 標頭。 屬性是由回應快取中介軟體所處理的 HTTP 功能。 若要讓中介軟體提供快取的回應，查詢字串和查詢字串值必須符合先前的要求。 例如，請考慮下表所示的要求和結果順序。
...
* 如果<xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByQueryKeys>已設定，則更新回應快取 HTTP 功能。

### <a name="vary"></a>相同

只有在設定<xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByHeader>屬性時，才會寫入這個標頭。 屬性會設定為`Vary`屬性的值。 下列範例會使用<xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByHeader>屬性：
...
<xref:Microsoft.AspNetCore.Mvc.CacheProfile.NoStore>覆寫大部分的其他屬性。 當這個屬性設定為`true`時， `Cache-Control`標頭會設定為`no-store`。 如果<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>設定為`None`：

* `Cache-Control` 設定為 `no-store,no-cache`。
* `Pragma` 設定為 `no-cache`。

如果<xref:Microsoft.AspNetCore.Mvc.CacheProfile.NoStore>是`false` ， <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>而且`None`是`Cache-Control`、和`Pragma` ，則會`no-cache`設定為。

<xref:Microsoft.AspNetCore.Mvc.CacheProfile.NoStore>對於錯誤網頁， `true`通常會設定為。 範例應用程式中的 [Cache2] 頁面會產生回應標頭，以指示用戶端不要儲存回應。
...
若要啟用快<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Duration>取，必須將設為正值， <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>而且必須是`Any` （預設值）或`Client`。 架構會將`Cache-Control`標頭設定為位置值，後面接著`max-age`回應的。

<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>的選項`Any`和`Client`會分別轉譯`Cache-Control`為和`public` `private`的標頭值。 如[NoStore 和 Location. None](#nostore-and-locationnone)區段中所述，將<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>設定`None`為`Cache-Control` ，並`Pragma`將標`no-cache`頭設定為。
...
下列範例顯示來自範例應用程式的 Cache3 頁面模型，以及設定<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Duration>和保留預設<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>值所產生的標頭：
...
快取設定檔在中Razor `Startup.ConfigureServices`設定 MVC/Pages 時，可以設定為選項，而不是複製許多控制器動作屬性的回應快取設定。 在參考的快取設定檔中找到的值是用來<xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute>做為的預設值，而且會由屬性上指定的任何屬性加以覆寫。

設定快取設定檔。 下列範例顯示範例應用程式中的30秒快取設定檔`Startup.ConfigureServices`：

[!code-csharp[](response/samples/2.x/ResponseCacheSample/Startup.cs?name=snippet1)]

範例應用程式的 Cache4 頁面模型會參考`Default30`快取設定檔：

[!code-csharp[](response/samples/2.x/ResponseCacheSample/Pages/Cache4.cshtml.cs?name=snippet)]

<xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute>可套用至：

* Razor頁面處理常式（類別&ndash; ）屬性無法套用至處理常式方法。
* MVC 控制器（類別）。
* MVC 動作（方法） &ndash;方法層級屬性會覆寫在類別層級屬性中指定的設定。

由`Default30`快取設定檔套用至 Cache4 頁面回應的產生標頭：
