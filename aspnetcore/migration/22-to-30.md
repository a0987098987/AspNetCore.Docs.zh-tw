...
* 設定[目標 Framework Moniker (TFM)](/dotnet/standard/frameworks)到`netcoreapp3.0`:
...
* 更新`Startup.ConfigureServices`呼叫`AddNewtonsoftJson`。
  ...
  `AddNewtonsoftJson` 適用於新的 MVC 服務註冊方法：
...
在 3.0 中，執行階段編譯是選擇性的案例。 若要啟用執行階段編譯，請參閱 https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilation 。
