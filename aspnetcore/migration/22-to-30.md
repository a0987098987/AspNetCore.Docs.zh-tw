...
如果需要同步 IO，可以藉由在使用中的伺服器上設定 `AllowSynchronousIO` 選項（例如，在呼叫 `ConfigureKestrel`時）來啟用此功能（例如，如果使用 Kestrel）。 請注意，伺服器（Kestrel、HttpSys、TestServer 等）都有自己的 `AllowSynchronousIO` 選項，這不會影響其他伺服器。 您可以使用 `IHttpBodyControlFeature.AllowSynchronousIO` 選項，以每個要求為基礎來啟用所有伺服器的同步 IO：
...
如果應用程式使用自訂的[授權處理常式](xref:security/authorization/policies#authorization-handlers)，請注意端點路由會將不同的資源類型傳遞給處理常式，而不是 MVC。 預期授權處理常式內容資源屬於類型 <xref:Microsoft.AspNetCore.Mvc.Filters.AuthorizationFilterContext> 的處理常式（ [MVC 篩選器所提供](xref:security/authorization/policies#accessing-mvc-request-context-in-handlers)的資源類型）將需要更新，以處理 <xref:Microsoft.AspNetCore.Routing.RouteEndpoint> 類型的資源（由端點路由指定給授權處理常式的資源類型）。
...
* [某些 Cookie SameSite 預設值已變更為 [無] （aspnet/公告 #348）](https://github.com/aspnet/Announcements/issues/348)。
