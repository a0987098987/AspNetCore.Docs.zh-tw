...
[檢視或下載範例程式碼](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/host-and-deploy/health-checks/samples) \(英文\) ([如何下載](xref:index#how-to-download-a-sample))
...
ASP.NET Core 應用程式會以隱含方式參考[HealthChecks](https://www.nuget.org/packages/Microsoft.AspNetCore.Diagnostics.HealthChecks)套件。 若要使用 Entity Framework Core 執行健康情況檢查，請將套件參考新增至[HealthChecks. microsoft.entityframeworkcore](https://www.nuget.org/packages/Microsoft.Extensions.Diagnostics.HealthChecks.EntityFrameworkCore)套件。
...
在 <xref:Microsoft.Extensions.DependencyInjection.HealthCheckServiceCollectionExtensions.AddHealthChecks*> 中，使用 `Startup.ConfigureServices` 登錄健康狀態檢查服務。 藉由呼叫 `Startup.Configure`中的 `MapHealthChecks` 來建立健康狀態檢查端點。
...
下列 `ExampleHealthCheck` 類別示範健全狀況檢查的配置。 健康情況檢查邏輯會放在 `CheckHealthAsync` 方法中。 下列範例會將虛擬變數 `healthCheckResultHealthy`設定為 `true`。 如果 `healthCheckResultHealthy` 的值設定為 `false`，則會傳回[HealthCheckResult 狀況不良](xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckResult.Unhealthy*)狀態。
...
`ExampleHealthCheck` 類型會新增至 `Startup.ConfigureServices`中具有 <xref:Microsoft.Extensions.DependencyInjection.HealthChecksBuilderAddCheckExtensions.AddCheck*> 的健全狀況檢查服務：
...
呼叫 <xref:Microsoft.Extensions.DependencyInjection.HealthChecksBuilderAddCheckExtensions.AddTypeActivatedCheck*> 以將引數傳遞至健康情況檢查的執行。 在下列範例中，`TestHealthCheckWithArgs` 會接受整數和字串，以便在呼叫 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck.CheckHealthAsync*> 時使用：
...
`TestHealthCheckWithArgs` 是藉由使用傳遞至實作為的整數和字串來呼叫 `AddTypeActivatedCheck` 進行註冊：
...
在 `Startup.Configure`中，使用端點 URL 或相對路徑，在端點產生器上呼叫 `MapHealthChecks`：
...
呼叫 `RequireHost`，為健康狀態檢查端點指定一或多個允許的主機。 主機應該是 Unicode 而不是 punycode，而且可能包含埠。 如果未提供集合，則會接受任何主機。
...
呼叫 `RequireAuthorization` 以在健康情況檢查要求端點上執行授權中介軟體。 `RequireAuthorization` 多載接受一或多個授權原則。 如果未提供原則，則會使用預設的授權原則。
...
雖然從瀏覽器手動執行健康情況檢查不是常見的使用案例，但可以藉由呼叫健全狀況檢查端點上的 `RequireCors` 來啟用 CORS 中介軟體。 `RequireCors` 多載會接受 CORS 原則產生器委派（`CorsPolicyBuilder`）或原則名稱。 如果未提供原則，則會使用預設的 CORS 原則。 如需詳細資訊，請參閱 <xref:security/cors>。
...
根據預設，健康情況檢查中介軟體會執行所有已註冊的健全狀況檢查。 若要執行健康狀態檢查子集，請對 <xref:Microsoft.AspNetCore.Diagnostics.HealthChecks.HealthCheckOptions.Predicate> 選項提供傳回布林值的函式。 在下列範例中，會依函式條件陳述式中的標籤 (`Bar`) 篩選出 `bar_tag` 健康狀態檢查。只有在健康狀態檢查的 `true` 屬性符合 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckRegistration.Tags> 或 `foo_tag` 時，才傳回 `baz_tag`：
...
在 `Startup.Configure`中，`Predicate` 會篩選出 ' Bar ' 的健康情況檢查。 只有 Foo 和 Baz.png execute。：
...
在 `Startup.Configure`中，將[HealthCheckOptions. ResponseWriter](xref:Microsoft.AspNetCore.Diagnostics.HealthChecks.HealthCheckOptions.ResponseWriter)選項設定為用於寫入回應的委派：
...
範例應用程式的第一個範例示範如何使用 <xref:System.Text.Json?displayProperty=fullName>：
...
在範例應用程式中，將*CustomWriterStartup.cs*中的 `SYSTEM_TEXT_JSON`[預處理器](xref:index#preprocessor-directives-in-sample-code)指示詞批註，以啟用 `Newtonsoft.Json` 版本的 `WriteResponse`。

健康情況檢查 API 不會針對複雜的 JSON 傳回格式提供內建支援，因為此格式是您選擇的監視系統所特有。 視需要自訂上述範例中的回應。 如需有關使用 `System.Text.Json`進行 JSON 序列化的詳細資訊，請參閱[如何在 .net 中序列化和](/dotnet/standard/serialization/system-text-json-how-to)還原序列化 json。
...
在 <xref:Microsoft.Extensions.DependencyInjection.HealthCheckServiceCollectionExtensions.AddHealthChecks*> 中，使用 `Startup.ConfigureServices` 登錄健康狀態檢查服務。 範例應用程式會使用資料庫的連接字串 (`AddSqlServer`DbHealthStartup.cs *) 來呼叫*  方法：

[!code-csharp[](health-checks/samples/3.x/HealthChecksSample/DbHealthStartup.cs?name=snippet_ConfigureServices)]

健康狀態檢查端點是藉由呼叫 `Startup.Configure`中的 `MapHealthChecks` 所建立：
...
在範例應用程式中，`AppDbContext` 提供給 `AddDbContextCheck`，並在 `Startup.ConfigureServices` （*DbCoNtextHealthStartup.cs*）中註冊為服務：
...
健康狀態檢查是 <xref:Microsoft.Extensions.DependencyInjection.HealthChecksBuilderAddCheckExtensions.AddCheck*> 中使用 `Startup.ConfigureServices` 隨託管服務一起登錄。 由於託管服務必須在健康狀態檢查上設定此屬性，因此也會在服務容器 (*LivenessProbeStartup.cs*) 中登錄健康狀態檢查：

[!code-csharp[](health-checks/samples/3.x/HealthChecksSample/LivenessProbeStartup.cs?name=snippet_ConfigureServices)]

健康狀態檢查端點是藉由呼叫 `Startup.Configure`中的 `MapHealthChecks` 所建立。 在範例應用程式中，健康狀態檢查端點會建立于：

* 準備就緒檢查的 `/health/ready`。 整備度檢查使用 `ready` 標籤來篩選健康狀態檢查。
* 活動檢查的 `/health/live`。 活動檢查會藉由傳回[HealthCheckOptions](xref:Microsoft.AspNetCore.Diagnostics.HealthChecks.HealthCheckOptions.Predicate)中的 `false` 來篩選出 `StartupHostedServiceHealthCheck` （如需詳細資訊，請參閱[篩選健全狀況檢查](#filter-health-checks)）

在下列範例程式碼中：

* 準備就緒檢查會將所有已註冊的檢查與「就緒」標記搭配使用。
* `Predicate` 排除所有檢查，並傳回 200-Ok。
...
在 <xref:Microsoft.Extensions.DependencyInjection.HealthCheckServiceCollectionExtensions.AddHealthChecks*> 中，使用 `Startup.ConfigureServices` 登錄健康狀態檢查服務。 <xref:Microsoft.Extensions.DependencyInjection.HealthChecksBuilderAddCheckExtensions.AddCheck*> 會登錄為服務，而不是將健康狀態檢查傳遞至 `MemoryHealthCheck` 以啟用檢查。 所有 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck> 登錄的服務都可供健康狀態檢查服務和中介軟體使用。 建議將健康狀態檢查服務登錄為單一服務。

在範例應用程式的*CustomWriterStartup.cs*中：

[!code-csharp[](health-checks/samples/3.x/HealthChecksSample/CustomWriterStartup.cs?name=snippet_ConfigureServices&highlight=4)]

健康狀態檢查端點是藉由呼叫 `Startup.Configure`中的 `MapHealthChecks` 所建立。 當健康情況檢查執行時，會提供 `WriteResponse` 委派給 < HealthChecks > HealthCheckOptions，以輸出自訂 JSON 回應：
...
`WriteResponse` 委派會將 `CompositeHealthCheckResult` 格式化為 JSON 物件，並產生健康情況檢查回應的 JSON 輸出。 如需詳細資訊，請參閱[自訂輸出](#customize-output)一節。
...
在具有 URL 模式的 `MapHealthChecks` 上呼叫 `RequireHost`，以指定埠以限制對指定埠的健康狀態檢查要求。 這通常會用於容器環境，以公開監視服務的連接埠。
...
在範例應用程式中，`Startup.Configure` 中端點的 `RequireHost` 呼叫會從設定中指定管理埠：
...
端點會在 `Startup.Configure`的範例應用程式中建立。 在下列範例程式碼中：
...
> [!NOTE]
> 您可以在程式碼中明確設定管理埠，以避免在範例應用程式中建立*launchsettings.json。* 在建立 <xref:Microsoft.Extensions.Hosting.HostBuilder> 的*Program.cs*中，新增對 <xref:Microsoft.AspNetCore.Server.Kestrel.Core.KestrelServerOptions.ListenAnyIP*> 的呼叫，並提供應用程式的管理埠端點。 在*ManagementPortStartup.cs*的 `Configure` 中，指定具有 `RequireHost`的管理埠：
>
> *Program.cs*：
>
> ```csharp
...
1. 寫入健康狀態檢查，將 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheck> 介面當做獨立類別來實作。 此類別可能依賴[相依性插入 (DI)](xref:fundamentals/dependency-injection)、類型啟用和[具名選項](xref:fundamentals/configuration/options)來存取組態資料。
   ...
   在健康情況檢查中，`CheckHealthAsync`的邏輯：

   * 方法中會使用 `data1` 和 `data2` 來執行探查的健康情況檢查邏輯。
   * 會處理 `AccessViolationException`。

   當發生 <xref:System.AccessViolationException> 時，會傳回 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckRegistration.FailureStatus>，並提供 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckResult>，讓使用者能夠設定健康情況檢查失敗狀態。
...
* <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckPublisherOptions.Delay> &ndash; 應用程式啟動後，在執行 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheckPublisher> 實例之前所套用的初始延遲。 在啟動後就會套用延遲，但不會套用至後續的反覆項目。 預設值是五秒鐘。
...
* <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckPublisherOptions.Predicate> &ndash; 如果 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckPublisherOptions.Predicate> 是 `null` （預設值），健全狀況檢查發行者服務就會執行所有已註冊的健全狀況檢查。 若要執行一部分的健康狀態檢查，請提供可篩選該組檢查的函式。 每個期間都會評估該述詞。
* <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckPublisherOptions.Timeout> &ndash; 執行所有 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheckPublisher> 實例健全狀況檢查的超時時間。 若要在沒有逾時的情況下執行，請使用 <xref:System.Threading.Timeout.InfiniteTimeSpan>。 預設值為 30 秒。
...
* 如果健康情況檢查狀態為 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthStatus.Healthy>，則為資訊（<xref:Microsoft.Extensions.Logging.LoggerExtensions.LogInformation*>）。
* 如果狀態為 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthStatus.Degraded> 或 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthStatus.Unhealthy>，則為錯誤（<xref:Microsoft.Extensions.Logging.LoggerExtensions.LogError*>）。

[!code-csharp[](health-checks/samples/3.x/HealthChecksSample/ReadinessPublisher.cs?name=snippet_ReadinessPublisher&highlight=18-27)]

在範例應用程式的 `LivenessProbeStartup` 範例中，`StartupHostedService` 整備檢查具有 2 秒的啟動延遲，且每 30 秒會執行一次檢查。 為了啟用 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.IHealthCheckPublisher> 實作，此範例會在`ReadinessPublisher`相依性插入 (DI)[ 容器中將 ](xref:fundamentals/dependency-injection) 註冊為單一服務：
...
在下列範例中，如果收到 `api/HealthCheck` 端點的 GET 要求，`MapWhen` 會將要求管線分支以啟動健康狀態檢查中介軟體：
...
在 <xref:Microsoft.Extensions.DependencyInjection.HealthCheckServiceCollectionExtensions.AddHealthChecks*> 中，使用 `Startup.ConfigureServices` 登錄健康狀態檢查服務。 使用 `Startup.Configure`的要求處理管線中的 <xref:Microsoft.AspNetCore.Builder.HealthCheckApplicationBuilderExtensions.UseHealthChecks*>，為健康狀態檢查中介軟體新增端點。
...
`ExampleHealthCheck` 類型會使用 <xref:Microsoft.Extensions.DependencyInjection.HealthChecksBuilderAddCheckExtensions.AddCheck*>新增至 `Startup.ConfigureServices` 中的健康狀態檢查服務：
...
健全狀況檢查系統不會針對複雜的 JSON 傳回格式提供內建支援，因為此格式是您選擇的監視系統所特有。 您可以視需要自訂上述範例中的 `JObject`，以符合您的需求。
...
在 `Startup.Configure`中呼叫應用程式處理管線中的「健康情況檢查」中介軟體：
...
在範例應用程式中，`UseHealthChecks` 會在 `Startup.Configure`中新增健全狀況檢查中介軟體。
...
在 `Startup.Configure`的應用程式處理管線中呼叫健全狀況檢查中介軟體。 在範例應用程式中，健康狀態檢查端點是在 `/health/ready` (針對整備度檢查) 和 `/health/live` (針對活躍度檢查) 建立。 整備度檢查使用 `ready` 標籤來篩選健康狀態檢查。 活躍度檢查會藉由在 `StartupHostedServiceHealthCheck`HealthCheckOptions.Predicate`false` 中傳回 [ 來篩選掉 ](xref:Microsoft.AspNetCore.Diagnostics.HealthChecks.HealthCheckOptions.Predicate) (如需詳細資訊，請參閱[篩選健康狀態檢查](#filter-health-checks))：
...
如果應用程式使用超過指定的記憶體閾值（在範例應用程式中為 1 GB），`MemoryHealthCheck` 會回報狀況不良的狀態。 <xref:Microsoft.Extensions.Diagnostics.HealthChecks.HealthCheckResult> 包含應用程式的記憶體回收行程 (GC) 資訊 (*MemoryHealthCheck.cs*)：
...
在 `Startup.Configure`的應用程式處理管線中呼叫健全狀況檢查中介軟體。 當健康狀態檢查執行時，會將 `WriteResponse` 委派提供給 `ResponseWriter` 屬性以輸出自訂 JSON 回應：
...
在 <xref:Microsoft.Extensions.DependencyInjection.HealthCheckServiceCollectionExtensions.AddHealthChecks*> 中，使用 `Startup.ConfigureServices` 登錄健康狀態檢查服務。 呼叫 <xref:Microsoft.AspNetCore.Builder.HealthCheckApplicationBuilderExtensions.UseHealthChecks*> 會指定管理連接埠 (*ManagementPortStartup.cs*)：

[!code-csharp[](health-checks/samples/2.x/HealthChecksSample/ManagementPortStartup.cs?name=snippet1&highlight=17)]

> [!NOTE]
> 您可以在程式碼中明確設定 URL 和管理連接埠，避免在範例應用程式中建立 *launchSettings.json* 檔案。 在 *建立所在的*Program.cs<xref:Microsoft.AspNetCore.Hosting.WebHostBuilder> 中，新增 <xref:Microsoft.AspNetCore.Hosting.HostingAbstractionsWebHostBuilderExtensions.UseUrls*> 呼叫並提供應用程式的正常回應端點和管理連接埠端點。 在 *呼叫所在的*ManagementPortStartup.cs<xref:Microsoft.AspNetCore.Builder.HealthCheckApplicationBuilderExtensions.UseHealthChecks*> 中，明確指定管理連接埠。
>
> *Program.cs*：
>
> ```csharp
