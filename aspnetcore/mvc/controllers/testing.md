...
第一項測試會確認當`ModelState`無效，相同`ViewResult`傳回與`GET`要求。 請注意，測試不會嘗試將傳入無效的模型。 不會運作，還是因為模型繫結不執行 (雖然[整合測試](xref:mvc/controllers/testing#integration-testing)會使用練習模型繫結)。 在此情況下，不是正在測試模型繫結。 這些單元測試只有測試動作方法中的程式碼。

第二個測試會驗證當`ModelState`是有效的新`BrainstormSession`加入 （透過儲存機制），而且方法會傳回`RedirectToActionResult`與預期的屬性。 模擬不呼叫的呼叫是通常會被忽略，但呼叫`Verifiable`結尾的安裝程式呼叫可讓它在測試中驗證。 這是呼叫`mockRepo.Verify`，如果未呼叫的預期的方法，這將會失敗測試。
