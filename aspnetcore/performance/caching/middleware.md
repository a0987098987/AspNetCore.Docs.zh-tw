...
將應用程式設定為使用中介軟體搭配<xref:Microsoft.AspNetCore.Builder.ResponseCachingExtensions.UseResponseCaching*>擴充方法，這會將中介軟體新增至中`Startup.Configure`的要求處理管線：

[!code-csharp[](middleware/samples/3.x/ResponseCachingMiddleware/Startup.cs?name=snippet2&highlight=16)]

範例應用程式會新增標頭，以在後續要求中控制快取：

* 快取[控制](https://tools.ietf.org/html/rfc7234#section-5.2) &ndash;會快取最多10秒的快取回應。
* 只有在後續要求的[接受編碼](https://tools.ietf.org/html/rfc7231#section-5.3.4)標頭與原始要求相符時，才[會](https://tools.ietf.org/html/rfc7231#section-7.1.4) &ndash;設定中介軟體來提供快取的回應。
...
當不使用`[ResponseCache]`屬性時，回應快取可以與`VaryByQueryKeys`不同。 <xref:Microsoft.AspNetCore.ResponseCaching.ResponseCachingFeature>直接使用[HttpCoNtext 功能](xref:Microsoft.AspNetCore.Http.HttpContext.Features)：
...
| `Cache-Control` | 中介軟體只會考慮以`public` cache 指示詞標示的快取回應。 使用下列參數來控制快取：<ul><li>最大壽命</li><li>最大-過時&#8224;</li><li>最小-全新</li><li>must-revalidate</li><li>no-cache</li><li>否-存放區</li><li>僅限-快取</li><li>private</li><li>public</li><li>s-maxage</li><li>proxy-重新驗證&#8225;</li></ul>&#8224;如果沒有指定任何限制`max-stale`，中介軟體就不會採取任何動作。<br>&#8225;`proxy-revalidate`與具有相同的效果`must-revalidate`。<br><br>如需詳細資訊，請參閱[RFC 7231：要求](https://tools.ietf.org/html/rfc7234#section-5.2.1)快取控制指示詞。 |
| `Pragma` | 要求`Pragma: no-cache`中的標頭會產生與相同的`Cache-Control: no-cache`效果。 此標頭會由`Cache-Control`標頭中的相關指示詞覆寫（如果有的話）。 視為與 HTTP/1.0 的回溯相容性。 |
...
| `Vary` | `Vary`標頭是用來依另一個標頭來改變快取的回應。 例如，藉由包含標頭的`Vary: Accept-Encoding`編碼方式來快取回應，這會快取`Accept-Encoding: gzip`標`Accept-Encoding: text/plain`頭和個別要求的回應。 永遠不會儲存標頭值`*`為的回應。 |
| `Expires` | 除非其他`Cache-Control`標頭覆寫，否則不會儲存或抓取此標頭所視為過時的回應。 |
...
| `Age` | 會`Age`忽略原始回應中所傳送的標頭。 中介軟體會在服務快取回應時計算新的值。 |
...
* `Pragma: no-cache`如果`Cache-Control`標頭不存在，標頭不能出現，因為`Cache-Control`標頭會在`Pragma`出現時覆寫標頭。
* `Set-Cookie`標頭不得存在。
* `Vary`標頭參數必須有效，而且不等於`*`。
* `Content-Length`標頭值（如果設定）必須符合回應主體的大小。
* <xref:Microsoft.AspNetCore.Http.Features.IHttpSendFileFeature>未使用。
* 回應不得過時，如`Expires`標頭和`max-age`和`s-maxage`快取指示詞所指定。
...
* 回應必須根據[RFC 7234](https://tools.ietf.org/html/rfc7234)規格進行快取。 例如， `no-store`指示詞不能存在於要求或回應標頭欄位中。 如需詳細資訊，請參閱*第3節：在* [RFC 7234](https://tools.ietf.org/html/rfc7234)的快取中儲存回應。
