...
以 .NET Core 為目標的專案應該使用版本大於或等於 .NET Core 2.2 的[TFM](/dotnet/standard/frameworks) 。 在專案檔中，使用 `netcoreapp2.2`更新 `<TargetFramework>` 節點的內部文字：
...
若要採用[IIS 的同進程裝載模型](xref:host-and-deploy/iis/index#in-process-hosting-model)，請將具有值 `InProcess` 的 `<AspNetCoreHostingModel>` 屬性加入至專案檔中的 `<PropertyGroup>`：
...
* 在新增 ASP.NET Core 模組（`name="aspNetCore"`）的 `<handlers>` 專案中，將 `modules` 屬性值從 [`AspNetCoreModule`] 變更為 [`AspNetCoreModuleV2`]。
...
如果以 .NET Core 為目標，請在專案檔中移除中繼套件參考的 `Version` 屬性。 包含 `Version` 屬性會產生下列警告：
...
如果您的解決方案依賴[global json](/dotnet/core/tools/global-json)檔案以特定 .NET Core SDK 版本為目標，請將其 `version` 屬性更新為安裝在您電腦上的2.2 版本：
...
如果應用程式未呼叫 `CreateDefaultBuilder`，並以手動方式在 `Program` 類別中建立主機，請在呼叫 `ConfigureKestrel`**之前**呼叫 <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*>：
...
將 `Startup.ConfigureServices` 中的相容性版本更新為 `Version_2_2`：
...
在 ASP.NET Core 2.2 中，如果原則允許任何來源並允許認證，CORS 中介軟體就會以萬用字元來源（`*`）來回應。 指定萬用字元來源（`*`）時，不支援認證，瀏覽器將不允許 CORS 要求。 如需詳細資訊，包括在用戶端上修正問題的選項，請參閱[MDN web](https://developer.mozilla.org/docs/Web/HTTP/CORS/Errors/CORSNotSupportingCredentials)檔。

若要更正伺服器上的這個問題，請採取下列其中一個動作：

* 修改 CORS 原則，使其不再允許認證。 也就是，在設定原則時，請移除對 <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowCredentials*> 的呼叫。
* 如果 CORS 要求需要認證才會成功，請修改原則以指定允許的主機。 例如，使用 `builder.WithOrigins("https://api.example1.com", "https://example2.com")`，而不是使用 <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyOrigin*>。
...
變更*Dockerfile*中的 `FROM` 行，以使用上表2.2 資料行中的新影像標記。
...
如果您的應用程式在 `Startup` 類別中記錄提供者初始化、篩選和設定載入，請將該程式碼移至 `Program.Main`：
