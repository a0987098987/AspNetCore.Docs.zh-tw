...
清單控制項包含 ListBox、 DropDownList、 CheckBoxList、 RadioButtonList 和 BulletedList。 這些控制項可以繫結至資料來源的資料。 這些資料來源中的一個欄位做的顯示文字，並可以選擇性地使用第二個欄位為項目的值。 項目也可以新增以靜態方式在設計階段，您可以混合使用靜態項目和動態新增資料來源的項目。

若要資料繫結清單控制項，將資料來源控制項加入頁面。 指定資料來源控制項的 SELECT 命令，然後將清單控制項中的 [DataSourceID] 屬性為資料來源控制項的 ID。 使用**DataTextField**並**DataValueField**屬性定義的顯示文字和控制項的值。 此外，您可以使用**DataTextFormatString**屬性，即可控制顯示文字的外觀，如下所示：

| **運算式** | **描述** |
| --- | --- |
| 價格： {0:C} | 適用於數值/十進位資料。 顯示常值 」 價格: 」 後面接著貨幣格式的數字。 貨幣格式取決於在文化特性屬性中指定的文化特性設定**網頁**指示詞或 Web.config 檔案中。 |
| {0:D4} | 適用於整數資料。 不能與十進位數字。 整數會顯示在以零填補的欄位有四個字元寬。 |
| {0:N2}% | 針對數值資料。 顯示 2 位小數位數的數字精確度其後為常值"%"。 |
| {0:000.0} | 適用於數值/十進位資料。 數字會四捨五入到一個小數位數。 小於三位數的數字會填補零。 |
| {0:D} | 適用於日期/時間資料。 顯示完整日期格式 ("星期四，August 06，1996")。 日期格式取決於頁面或 Web.config 檔案的文化設定。 |
| {0:d} | 適用於日期/時間資料。 顯示簡短日期格式 ("12/31/99")。 |
| {0:yy-MM-dd} | 適用於日期/時間資料。 數值年-月-日格式 (96-08-06) 中顯示日期 |

## <a name="gridview"></a>GridView

GridView 控制項可讓表格式資料顯示和編輯使用宣告式方法，並且是 DataGrid 控制項的後續版本。 下列功能可在 GridView 控制項中。
...
- 內建的分頁功能。
- 內建的資料列選取範圍的功能。
- 以程式設計方式存取 GridView 物件模型，以動態方式設定屬性，處理事件，等等。
- 多個索引鍵的欄位。
- 超連結資料行的多個資料欄位。
- 自訂佈景主題和樣式的外觀。
...
在 GridView 控制項中的每個資料行被以 DataControlField 物件。 根據預設，AutoGenerateColumns 屬性設為 **，則為 true**，這會每個欄位 AutoGeneratedField 物件建立資料來源中。 每個欄位接著會轉譯為每個欄位會出現在資料來源的順序在 GridView 控制項中的資料行中。 您可以手動控制欄位會出現在哪個資料行**GridView**藉由設定控制**AutoGenerateColumns**屬性設**false**然後定義您自己資料行欄位集合。 不同的資料行的欄位類型決定在控制項中的資料行的行為。

下表列出可用的不同的資料行欄位型別。

| **資料行欄位類型** | **描述** |
| --- | --- |
| BoundField | 資料來源中顯示欄位的值。 這是 GridView 控制項的預設資料行類型。 |
| ButtonField | 在 GridView 控制項中顯示每個項目的命令按鈕。 這可讓您建立的自訂按鈕控制項，例如 [新增] 或 [移除] 按鈕的資料行。 |
| CheckBoxField | 在 GridView 控制項中顯示每個項目的核取方塊。 這個資料行的欄位型別通常用於顯示的布林值的欄位。 |
| CommandField | 顯示預先定義的命令按鈕，可執行選取、 編輯或刪除作業。 |
| HyperLinkField | 資料來源中的欄位的值顯示為超連結。 這個資料行的欄位型別可讓您的第二個欄位繫結至超連結的 URL。 |
| ImageField | 在 GridView 控制項中顯示每個項目的影像。 |
| TemplateField | 顯示使用者定義內容根據指定的範本在 GridView 控制項中的每個項目。 這個資料行的欄位型別可讓您建立自訂的資料行欄位。 |

若要以宣告方式定義資料行欄位集合，請先將左右**&lt;資料行&gt;** 之間的開頭和結尾標記的 GridView 控制項的標記。 接下來，列出您想要包含的開頭和結尾之間的資料行欄位**&lt;資料行&gt;** 標記。 指定的資料行加入資料行集合中列出的順序。 **資料行**集合會儲存所有資料行中控制項的欄位，並可讓您以程式設計方式管理在 GridView 控制項中的資料行欄位。

明確宣告的資料行欄位可以顯示在具有自動產生的資料行欄位的組合。 同時使用時，明確宣告的資料行欄位會呈現第一次後, 接的自動產生的資料行欄位。

## <a name="binding-to-data"></a>繫結至資料

GridView 控制項可以繫結至資料來源控制項 (例如**SqlDataSource**， **ObjectDataSource**等等)，以及實作 System.Collections.IEnumerable 任何資料來源介面 （例如 System.Data.DataView、 System.Collections.ArrayList 或 System.Collections.Hashtable）。 請使用下列方法之一的 GridView 控制項繫結至適當的資料來源類型：

- 若要繫結至資料來源控制項，設定 GridView 控制項中的 [DataSourceID] 屬性的資料來源控制項的識別碼值。 GridView 控制項自動繫結至指定的資料來源控制項，並可讓您利用資料來源的控制項的功能來執行排序、 更新、 刪除和分頁功能。 這是要繫結至資料的慣用的方法。
- 要繫結至資料來源實作 System.Collections.IEnumerable 介面，以程式設計方式將 GridView 控制項的 DataSource 屬性設資料來源，然後呼叫 DataBind 方法。 使用此方法時，在 GridView 控制項不提供內建排序、 更新、 刪除和分頁功能。 您必須自行提供這項功能。

## <a name="data-operations"></a>資料作業

GridView 控制項提供許多內建功能，可讓使用者排序、 更新、 刪除、 選取和逐頁瀏覽控制項中的項目。 GridView 控制項繫結至資料來源控制項，在 GridView 控制項就可以利用資料來源控制項的功能，並提供自動排序、 更新和刪除功能。

> [!NOTE]
> GridView 控制項可支援排序、 更新和刪除與其他類型的資料來源;不過，您必須提供這些作業的實作中的適當事件處理常式。


排序功能可以讓使用者按一下資料行的標頭排序 GridView 控制項相對於特定的資料行中的項目。 若要啟用排序，請將 AllowSorting 屬性設定為 **，則為 true**。

中的按鈕時，會啟用自動更新、 刪除和選取項目功能**ButtonField**或是**TemplateField**資料行 欄位中的，使用 編輯、 刪除 和 選取，命令名稱分別是已按下。 GridView 控制項可以自動將**CommandField**資料行欄位的編輯、 刪除或選取的按鈕，如果 [autogenerateeditbutton]、 [autogeneratedeletebutton] 或 AutoGenerateSelectButton 屬性設定為 **，則為 true**分別。

> [!NOTE]
> 將記錄插入至資料來源不直接支援 GridView 控制項。 不過，就可以使用的 GridView 控制項搭配 DetailsView 或 FormView 控制項來插入記錄。


而不是顯示資料來源中的所有記錄，在此同時，在 GridView 控制項可以自動分成記錄頁面。 若要啟用分頁，請將 AllowPaging 屬性設定為 **，則為 true**。

## <a name="customizing-the-user-interface"></a>自訂使用者介面

您可以設定不同的組件控制項的樣式屬性，來自訂 GridView 控制項的外觀。 下表列出不同的樣式屬性。

| **樣式屬性** | **描述** |
| --- | --- |
| AlternatingRowStyle | 在 GridView 控制項中替代資料列樣式設定。 當設定這個屬性時，資料列會顯示交替 RowStyle 設定並**AlternatingRowStyle**設定。 |
| EditRowStyle | 正在編輯的 GridView 控制項中的資料列樣式設定。 |
| EmptyDataRowStyle | 資料來源不包含任何記錄時，在 GridView 控制項中顯示之空白資料列樣式設定。 |
| FooterStyle | GridView 控制項的頁尾資料列樣式設定。 |
| HeaderStyle | GridView 控制項的標頭資料列樣式設定。 |
| PagerStyle | GridView 控制項的頁面巡覽區資料列樣式設定。 |
| RowStyle | 在 GridView 控制項中的資料列樣式設定。 當**AlternatingRowStyle**也會設定屬性，則資料列會顯示交替**RowStyle**設定和**AlternatingRowStyle**設定。 |
...
| RowCancelingEdit | 發生於按一下資料列的 [取消] 按鈕時，但在 GridView 控制項之前離開編輯模式。 若要停止取消的作業通常使用這個事件。 |
| RowCommand | 在 GridView 控制項中按一下按鈕時，就會發生。 若要在控制項中按一下按鈕時執行的工作通常使用這個事件。 |
| RowCreated | 在 GridView 控制項中建立新的資料列時，就會發生。 若要修改的資料列的內容，建立資料列時，通常使用這個事件。 |
| RowDataBound | 發生於資料列繫結至 GridView 控制項中的資料。 這個事件通常用於資料列繫結至資料修改的資料列的內容。 |
| RowDeleted | 發生於按一下資料列的 刪除 按鈕，但之後 GridView 控制項從 資料來源中刪除記錄。 此事件通常用來檢查刪除作業的結果。 |
| RowDeleting | 發生於按一下資料列的 [刪除] 按鈕，但之前 GridView 控制項，請刪除資料來源的記錄。 此事件通常用來取消刪除作業。 |
| RowEditing | 發生於按一下資料列的 [編輯] 按鈕時，但之前 GridView 控制項進入編輯模式。 此事件通常用來取消編輯作業。 |
| RowUpdated | 發生於按一下資料列的 [更新] 按鈕，但之後 GridView 控制項更新資料列。 此事件通常會用來檢查更新作業的結果中。 |
| RowUpdating | 發生於按一下資料列的 [更新] 按鈕，但之前 GridView 控制項更新資料列。 此事件通常用來取消更新作業。 |
| SelectedIndexChanged | 發生於按一下資料列的 [選取] 按鈕時，但之後 GridView 控制項處理選取作業。 這個事件通常用於執行工作，在控制項中選取一個資料列之後。 |
| SelectedIndexChanging | 發生於按一下資料列的 [選取] 按鈕時，但之前 GridView 控制項處理選取作業。 若要取消了選取作業通常使用這個事件。 |
| 排序 | 發生於按一下排序資料行的超連結時，但之後 GridView 控制項處理排序作業。 這個事件通常用於執行工作，在使用者按一下超連結以排序資料行之後。 |
| 排序 | 發生於按一下排序資料行的超連結時，但之前 GridView 控制項處理排序作業。 通常在取消排序作業，或執行自訂排序常式，會使用這個事件。 |

## <a name="formview"></a>FormView

FormView 控制項來顯示資料來源的單一記錄。 就像是在 DetailsView 控制項，只不過它會顯示使用者定義的範本，而不是資料列欄位。 建立您自己的範本，讓您更大的彈性來控制資料的顯示方式。 FormView 控制項支援下列功能：
...
- 若要繫結至資料來源控制項，設定 FormView 控制項的 [DataSourceID] 屬性的資料來源控制項的識別碼值。 FormView 控制項自動繫結至指定的資料來源控制項，並可以利用資料來源控制項的功能，執行插入、 更新、 刪除和分頁功能。 這是要繫結至資料的慣用的方法。
...
| HyperLinkField | 資料來源中的欄位的值顯示為超連結。 這個資料列的欄位型別可讓您的第二個欄位繫結至超連結的 URL。 |
| ImageField | 在 DetailsView 控制項中顯示影像。 |
| TemplateField | 顯示使用者定義內容根據指定的範本在 DetailsView 控制項中的資料列。 這個資料列的欄位型別可讓您建立自訂的資料列欄位。 |
...
- 若要繫結至資料來源控制項，設定 DetailsView 控制項中的 [DataSourceID] 屬性的資料來源控制項的識別碼值。 在 DetailsView 控制項自動繫結至指定的資料來源控制項。 這是要繫結至資料的慣用的方法。
