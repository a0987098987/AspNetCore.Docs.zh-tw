...
預設會停用 HTTP/2。 如需設定的詳細資訊，請參閱 [Kestrel 選項](#kestrel-options)和[端點設定](#endpoint-configuration)一節。
...
基於安全性考量，必須有反向 Proxy 才能進行中公眾面向邊緣伺服器部署 (公開到來自網際網路的流量)。 Kestrel 的 1.x 版沒有針對攻擊的完整防禦補充，例如適當的逾時、大小上限，以及並行連線限制。
