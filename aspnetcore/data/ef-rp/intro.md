...
---
# <a name="getting-started-with-razor-pages-and-entity-framework-core-using-visual-studio-1-of-8"></a>使用 Visual Studio 開始使用 Razor 頁面與 Entity Framework Core (1/8)
...
* 為 **Students**、**Courses**、**Instructors**、**Departments** 新增功能表輸入，並刪除 **Contact** 功能表輸入。
...
`ID` 屬性會成為資料庫 (DB) 資料表中的主索引鍵資料行，並對應至這個類別。 EF Core 預設會解譯名為 `ID` 或 `classnameID` 作為主索引鍵的屬性。
...
`Grade` 屬性為 `enum`。 問號之後的 `Grade` 類型宣告表示 `Grade` 屬性可為 Null。 為 Null 的等級不同於零等級：Null 表示等級未知或尚未指派。
...
上述的連接字串使用 `ConnectRetryCount=0` 以防止 [SQLClient](https://docs.microsoft.com/dotnet/framework/data/adonet/ef/sqlclient-for-the-entity-framework) 停滯。
...
建置專案。 建置時會產生類似如下錯誤：
 ...
 全域變更 `_context.Student` 至 `_context.Students` (亦即，在 `Student` 新增一個 "s")。 找到並更新 7 個發生次數。 我們希望下一個版本能修正[這個 Bug](https://github.com/aspnet/Scaffolding/issues/633)。
...
* `DbSet` 屬性的名稱會用作資料表的名稱。 對於未由 `DbSet` 屬性引用的實體，實體類別名稱會用作資料表名稱。
...
* `async` 關鍵字會指示編譯器：
  ...
  * 為方法主體的組件產生回呼。
  * 自動建立傳回的 [Task](https://docs.microsoft.com/dotnet/api/system.threading.tasks.task?view=netframework-4.7) 物件。 如需詳細資訊，請參閱 [Task 傳回型別](https://docs.microsoft.com/dotnet/csharp/programming-guide/concepts/async/async-return-types#BKMK_TaskReturnType)。

* 隱含傳回型別 `Task` 代表進行中的工作。

* `await` 關鍵字會導致編譯器將方法分成兩個組件。 第一個組件會和以非同步方式啟動的作業一起結束。 第二個組件會在作業完成時，放入回呼方法。
