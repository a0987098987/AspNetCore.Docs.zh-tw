...
在專案檔中，將每`Microsoft.AspNetCore.*`個封裝參考`Version`的屬性更新為3.1.0 或更新版本。 例如：
...
HTTP `SameSite` cookie 的屬性（attribute）會在 ASP.NET Core 3.0 和3.1 之間變更。 如需採取動作，請參閱下列資源：
...
ASP.NET Core 3.1 引進`Component`標記協助程式。 標記協助程式可以取代`RenderComponentAsync<TComponent>` Blazor專案中的 HTML Helper 方法。 例如：
...
如需詳細資訊，請參閱將[元件整合至Razor頁面和 MVC 應用程式](/aspnet/core/blazor/integrate-components?view=aspnetcore-3.1)。
