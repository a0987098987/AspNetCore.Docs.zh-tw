...
Web API 有一或多個衍生自 <xref:Microsoft.AspNetCore.Mvc.ControllerBase> 的控制器類別。 例如，Web API 專案範本會建立值控制器：
...
<xref:Microsoft.AspNetCore.Mvc> 命名空間提供的屬性，可用來設定 Web API 控制器和動作方法的行為。 下列範例會使用屬性來指定接受的 HTTP 方法和傳回的狀態碼：
...
|[[HttpGet]](<xref:Microsoft.AspNetCore.Mvc.HttpGetAttribute>)  |識別支援 HTTP GET 方法的動作。|
...
[[ApiController]](xref:Microsoft.AspNetCore.Mvc.ApiControllerAttribute) 屬性可以套用至控制器類別，以啟用 API 特定行為：
...
### <a name="apicontroller-on-specific-controllers"></a>特定控制站上的 ApiController
...
### <a name="apicontroller-on-multiple-controllers"></a>多個控制站上的 ApiController

在多個控制站上使用同一屬性的方法之一，就是建立以 `[ApiController]` 屬性標註的自訂基底控制器類別。 以下範例顯示自訂的基底類別和從它衍生的控制站：

[!code-csharp[](index/samples/2.x/Controllers/MyControllerBase.cs?name=snippet_MyControllerBase)]

[!code-csharp[](index/samples/2.x/Controllers/PetsController.cs?name=snippet_Inherit)]

### <a name="apicontroller-on-an-assembly"></a>組件上的 ApiController

如果[相容性版本](<xref:mvc/compatibility-version>)設定為 2.2 或更新版本，`[ApiController]` 屬性就可以套用至組件。 以這種方式標註會將 Web API 行為套用至組件中的所有控制器。 沒有任何方法可以退出個別控制器。 將 `Startup` 組件層級屬性套用至類別，如下列範例所示：
...
使用 2.2 或更新版本的相容性版本，HTTP 400 回應的預設回應類型為 <xref:Microsoft.AspNetCore.Mvc.ValidationProblemDetails>。 `ValidationProblemDetails` 類型符合 [RFC 7807 規格](https://tools.ietf.org/html/rfc7807)。

若要變更 <xref:Microsoft.AspNetCore.Mvc.SerializableError> 的預設回應，請在 `Startup.ConfigureServices` 中將 `SuppressUseValidationProblemDetailsForInvalidModelStateResponses` 屬性設定為 `true`，如下列範例所示：

[!code-csharp[](index/samples/2.x/Startup.cs?name=snippet_ConfigureApiBehaviorOptions&highlight=3,9)]

### <a name="customize-badrequest-response"></a>自訂 BadRequest 回應

若要自訂驗證錯誤所產生的回應，請使用 <xref:Microsoft.AspNetCore.Mvc.ApiBehaviorOptions.InvalidModelStateResponseFactory>。 在 `services.AddMvc().SetCompatibilityVersion` 後面，新增下列醒目提示程式碼：

[!code-csharp[](index/samples/2.x/Startup.cs?name=snippet_ConfigureBadRequestResponse&highlight=3-20)]

### <a name="disable-automatic-400"></a>停用自動 400

若要停用自動 400 行為，請將 <xref:Microsoft.AspNetCore.Mvc.ApiBehaviorOptions.SuppressModelStateInvalidFilter> 屬性設定為 `true`。 在 `services.AddMvc().SetCompatibilityVersion` 後面的 `Startup.ConfigureServices` 中，新增下列醒目提示程式碼：
...
若要停用繫結來源推斷，請將 <xref:Microsoft.AspNetCore.Mvc.ApiBehaviorOptions.SuppressInferBindingSourcesForParameters> 設定為 `true`。 在 `services.AddMvc().SetCompatibilityVersion` 後，於 `Startup.ConfigureServices` 中新增下列程式碼：

[!code-csharp[](index/samples/2.x/Startup.cs?name=snippet_ConfigureApiBehaviorOptions&highlight=3,6)]

## <a name="multipartform-data-request-inference"></a>多部分/表單資料要求推斷

當以 [[FromForm]](xref:Microsoft.AspNetCore.Mvc.FromFormAttribute) 屬性標註動作參數，而 `[ApiController]` 屬性套用推斷規則時：會推斷 `multipart/form-data` 要求內容類型。

若要停用預設行為，請在 `Startup.ConfigureServices` 中將 <xref:Microsoft.AspNetCore.Mvc.ApiBehaviorOptions.SuppressConsumesConstraintForFormFileParameters> 設定為 `true`，如下列範例所示：
...
### <a name="customize-problemdetails-response"></a>自訂 ProblemDetails 回應
...
當 `ProblemDetails` 屬性設定為 `SuppressMapClientErrors` 時，會停用自動建立 `true`。 將下列程式碼加入 `Startup.ConfigureServices`：
