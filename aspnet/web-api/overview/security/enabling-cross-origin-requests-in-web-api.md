...
稍早所述方式預檢要求可能包括存取控制-頭 access-control-request-headers 標頭中，列出應用程式所設定的 HTTP 標頭 (所謂的 「 撰寫要求標頭 」)。 *標頭*參數**[EnableCors]**屬性會指定允許哪些作者要求標頭。 若要允許任何標頭，設定*標頭*至 「\*"。 白名單特定的標頭，設定*標頭*允許的標頭以逗號分隔清單：
