...
在 ASP.NET Core 2.2 和更早版本<xref:System.Collections.Generic.IEnumerable%601>中，從動作傳回會導致序列化程式進行同步集合反復專案。 結果是封鎖呼叫，而且執行緒集區的可能會耗盡。 為了說明，請想像，Entity Framework （EF） Core 是用於 Web API 的資料存取需求。 在序列化期間，會同步列舉下列動作的傳回型別：
...
上述`IAsyncEnumerable<Product>`動作的對應項是：
...
當<xref:Microsoft.AspNetCore.Mvc.IActionResult>動作中可能有多個`ActionResult`傳回類型時，傳回類型是適當的。 `ActionResult` 類型代表各種 HTTP 狀態碼。 衍生自`ActionResult`的任何非抽象類別都符合資格，做為有效的傳回型別。 此分類中的一些常見傳回類型<xref:Microsoft.AspNetCore.Mvc.BadRequestResult>為（400） <xref:Microsoft.AspNetCore.Mvc.NotFoundResult> 、（404）和<xref:Microsoft.AspNetCore.Mvc.OkObjectResult> （200）。 或者，您可以使用類別<xref:Microsoft.AspNetCore.Mvc.ControllerBase>中的便利方法，從`ActionResult`動作傳回類型。 例如， `return BadRequest();`是的簡短形式`return new BadRequestResult();`。
...
* 當所代表的產品`id`不存在於基礎資料存放區時，會傳回404狀態碼。 便利<xref:Microsoft.AspNetCore.Mvc.ControllerBase.NotFound*>的方法會被叫用為`return new NotFoundResult();`的速記。
...
* 當建立產品時， <xref:Microsoft.AspNetCore.Mvc.ControllerBase.CreatedAtAction*>便利性方法會產生201狀態碼。 呼叫`CreatedAtAction`的替代方法是`return new CreatedAtActionResult(nameof(GetById), "Products", new { id = product.Id }, product);`。 在此程式碼路徑中`Product` ，會在回應主體中提供物件。 提供`Location`包含新建立之產品 URL 的回應標頭。
...
* 可以[`[ProducesResponseType]`](xref:Microsoft.AspNetCore.Mvc.ProducesResponseTypeAttribute)排除屬性`Type`的屬性。 例如，`[ProducesResponseType(200, Type = typeof(Product))]` 簡化為 `[ProducesResponseType(200)]`。 該動作的預期傳回型別會改為從 `ActionResult<T>` 中的 `T` 推斷。
...
* 在下列情況中，<xref:Microsoft.AspNetCore.Mvc.ControllerBase.BadRequest*>ASP.NET Core 執行時間會傳回400狀態碼（）：
  * 已[`[ApiController]`](xref:Microsoft.AspNetCore.Mvc.ApiControllerAttribute)套用屬性，而且模型驗證失敗。
  * 產品描述包含 "XYZ Widget"。
* 當建立產品時， <xref:Microsoft.AspNetCore.Mvc.ControllerBase.CreatedAtAction*>方法會產生201狀態碼。 在此程式碼路徑中`Product` ，會在回應主體中提供物件。 提供`Location`包含新建立之產品 URL 的回應標頭。
