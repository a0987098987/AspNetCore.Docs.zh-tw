...
> 
> <span data-ttu-id="47baa-131">您可以深入了解中的 HTML 編碼[使用表單](https://go.microsoft.com/fwlink/?LinkId=202892)。</span><span class="sxs-lookup"><span data-stu-id="47baa-131">You can read more about HTML encoding in [Working with Forms](https://go.microsoft.com/fwlink/?LinkId=202892).</span></span>
...
<span data-ttu-id="47baa-170">在瀏覽器中顯示結果 (按一下後**送出**):</span><span class="sxs-lookup"><span data-stu-id="47baa-170">The result displayed in a browser (after clicking **Submit**):</span></span>
...
<span data-ttu-id="47baa-205">但首先，小小的技術背景。</span><span class="sxs-lookup"><span data-stu-id="47baa-205">But first, a little technical background.</span></span>
...
<span data-ttu-id="47baa-300">您宣告變數使用`var`關鍵字 （如果您不想要指定型別） 或使用型別名稱：</span><span class="sxs-lookup"><span data-stu-id="47baa-300">You declare a variable using the `var` keyword (if you don't want to specify a type) or by using the name of the type:</span></span>
...
<span data-ttu-id="47baa-317">下表列出一些常見的轉換和測試方法的變數。</span><span class="sxs-lookup"><span data-stu-id="47baa-317">The following table lists some common conversion and test methods for variables.</span></span>
...
* * *
<span data-ttu-id="47baa-334">::: 資料列:::::: 資料行::: `ToString()` ::: 資料行後端:::::: 資料行::: 將其他任何資料類型轉換為字串。</span><span class="sxs-lookup"><span data-stu-id="47baa-334">:::row::: :::column::: `ToString()` :::column-end::: :::column::: Converts any other data type to a string.</span></span>
...
* * *
<span data-ttu-id="47baa-353">::: 資料列:::::: 資料行::: `!=` ::: 資料行後端:::::: 資料行::: 不等比較。</span><span class="sxs-lookup"><span data-stu-id="47baa-353">:::row::: :::column::: `!=` :::column-end::: :::column::: Inequality.</span></span> <span data-ttu-id="47baa-354">傳回`true`值是否不相等。</span><span class="sxs-lookup"><span data-stu-id="47baa-354">Returns `true` if the values are not equal.</span></span>
...
* * *
<span data-ttu-id="47baa-375">::: 資料列:::::: 資料行::: `[]` ::: 資料行後端:::::: 資料行::: 方括號。</span><span class="sxs-lookup"><span data-stu-id="47baa-375">:::row::: :::column::: `[]` :::column-end::: :::column::: Brackets.</span></span> <span data-ttu-id="47baa-376">用來存取陣列或集合中的值。</span><span class="sxs-lookup"><span data-stu-id="47baa-376">Used for accessing values in arrays or collections.</span></span>
...
<span data-ttu-id="47baa-411">`Server.MapPath`方法會將轉換的虛擬路徑 (例如 */default.cshtml*) 成絕對實體路徑 (例如*C:\WebSites\MyWebSiteFolder\default.cshtml*)。</span><span class="sxs-lookup"><span data-stu-id="47baa-411">The `Server.MapPath` method converts a virtual path (like */default.cshtml*) to an absolute physical path (like *C:\WebSites\MyWebSiteFolder\default.cshtml*).</span></span> <span data-ttu-id="47baa-412">每當您需要完整的實體路徑使用這個方法。</span><span class="sxs-lookup"><span data-stu-id="47baa-412">You use this method any time you need a complete physical path.</span></span> <span data-ttu-id="47baa-413">當您讀取或寫入的文字檔或 web 伺服器上的映像檔，就會是一個典型的例子。</span><span class="sxs-lookup"><span data-stu-id="47baa-413">A typical example is when you're reading or writing a text file or image file on the web server.</span></span>

<span data-ttu-id="47baa-414">您通常不知道您的網站裝載站台伺服器上的絕對實體路徑，因此這個方法可以將路徑轉換您知道 — 虛擬路徑，來為您在伺服器上對應的路徑。</span><span class="sxs-lookup"><span data-stu-id="47baa-414">You typically don't know the absolute physical path of your site on a hosting site's server, so this method can convert the path you do know — the virtual path — to the corresponding path on the server for you.</span></span> <span data-ttu-id="47baa-415">您將虛擬路徑傳遞至檔案或資料夾的方法，並傳回的實體路徑：</span><span class="sxs-lookup"><span data-stu-id="47baa-415">You pass the virtual path to a file or folder to the method, and it returns the physical path:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample46.cshtml)]

### <a name="referencing-the-virtual-root-the--operator-and-href-method"></a><span data-ttu-id="47baa-416">參考的虛擬根目錄： ~ 運算子和 Href 方法</span><span class="sxs-lookup"><span data-stu-id="47baa-416">Referencing the virtual root: the ~ operator and Href method</span></span>

<span data-ttu-id="47baa-417">在  *.cshtml*或 *.vbhtml*檔案中，您可以參考虛擬根目錄的路徑使用`~`運算子。</span><span class="sxs-lookup"><span data-stu-id="47baa-417">In a *.cshtml* or *.vbhtml* file, you can reference the virtual root path using the `~` operator.</span></span> <span data-ttu-id="47baa-418">這是非常好用，因為您可以移動的頁面，在網站中，而且它們包含其他頁面的任何連結不會中斷。</span><span class="sxs-lookup"><span data-stu-id="47baa-418">This is very handy because you can move pages around in a site, and any links they contain to other pages won't be broken.</span></span> <span data-ttu-id="47baa-419">如果您曾經將您的網站移至不同的位置還有好用。</span><span class="sxs-lookup"><span data-stu-id="47baa-419">It's also handy in case you ever move your website to a different location.</span></span> <span data-ttu-id="47baa-420">以下是一些範例：</span><span class="sxs-lookup"><span data-stu-id="47baa-420">Here are some examples:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample47.cshtml)]

<span data-ttu-id="47baa-421">如果網站是`http://myserver/myapp`，以下是 ASP.NET 如何處理此頁面會執行的這些路徑：</span><span class="sxs-lookup"><span data-stu-id="47baa-421">If the website is `http://myserver/myapp`, here's how ASP.NET will treat these paths when the page runs:</span></span>

- <span data-ttu-id="47baa-422">`myImagesFolder`: `http://myserver/myapp/images`</span><span class="sxs-lookup"><span data-stu-id="47baa-422">`myImagesFolder`: `http://myserver/myapp/images`</span></span>
- <span data-ttu-id="47baa-423">`myStyleSheet` : `http://myserver/myapp/styles/Stylesheet.css`</span><span class="sxs-lookup"><span data-stu-id="47baa-423">`myStyleSheet` : `http://myserver/myapp/styles/Stylesheet.css`</span></span>

<span data-ttu-id="47baa-424">（您實際上不會看到這些路徑做為變數的值，但，是它們的是，ASP.NET 會將路徑）。</span><span class="sxs-lookup"><span data-stu-id="47baa-424">(You won't actually see these paths as the values of the variable, but ASP.NET will treat the paths as if that's what they were.)</span></span>

<span data-ttu-id="47baa-425">您可以使用`~`運算子伺服端程式碼 （如上所述） 並在標記中，像這樣：</span><span class="sxs-lookup"><span data-stu-id="47baa-425">You can use the `~` operator both in server code (as above) and in markup, like this:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample48.html)]

<span data-ttu-id="47baa-426">在標記中，您可以使用`~`運算子來建立資源，例如映像檔案、 其他網頁和 CSS 檔案的路徑。</span><span class="sxs-lookup"><span data-stu-id="47baa-426">In markup, you use the `~` operator to create paths to resources like image files, other web pages, and CSS files.</span></span> <span data-ttu-id="47baa-427">ASP.NET 頁面執行時，尋找整個頁面 （程式碼和標記），並解決所有`~`參考適當的路徑。</span><span class="sxs-lookup"><span data-stu-id="47baa-427">When the page runs, ASP.NET looks through the page (both code and markup) and resolves all the `~` references to the appropriate path.</span></span>

## <a name="conditional-logic-and-loops"></a><span data-ttu-id="47baa-428">條件式邏輯和迴圈</span><span class="sxs-lookup"><span data-stu-id="47baa-428">Conditional Logic and Loops</span></span>

<span data-ttu-id="47baa-429">ASP.NET server 程式碼可讓您執行以條件為基礎的工作，並撰寫重複陳述式的程式碼次數 （也就是程式碼會執行迴圈）。</span><span class="sxs-lookup"><span data-stu-id="47baa-429">ASP.NET server code lets you perform tasks based on conditions and write code that repeats statements a specific number of times (that is, code that runs a loop).</span></span>

### <a name="testing-conditions"></a><span data-ttu-id="47baa-430">測試條件</span><span class="sxs-lookup"><span data-stu-id="47baa-430">Testing Conditions</span></span>

<span data-ttu-id="47baa-431">若要測試您所使用的簡單條件`if`陳述式，此傳回 true 或 false 取決於您所指定的測試：</span><span class="sxs-lookup"><span data-stu-id="47baa-431">To test a simple condition you use the `if` statement, which returns true or false based on a test you specify:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample49.cshtml)]

<span data-ttu-id="47baa-432">`if`關鍵字開始區塊。</span><span class="sxs-lookup"><span data-stu-id="47baa-432">The `if` keyword starts a block.</span></span> <span data-ttu-id="47baa-433">實際的測試 （條件） 是括號括住，並傳回 true 或 false。</span><span class="sxs-lookup"><span data-stu-id="47baa-433">The actual test (condition) is in parentheses and returns true or false.</span></span> <span data-ttu-id="47baa-434">執行測試結果為 true 的陳述式會括在大括號。</span><span class="sxs-lookup"><span data-stu-id="47baa-434">The statements that run if the test is true are enclosed in braces.</span></span> <span data-ttu-id="47baa-435">`if`陳述式可包含`else`，指定當條件為 false 時要執行的陳述式區塊：</span><span class="sxs-lookup"><span data-stu-id="47baa-435">An `if` statement can include an `else` block that specifies statements to run if the condition is false:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample50.cshtml)]

<span data-ttu-id="47baa-436">您可以加入多個條件使用`else if`區塊：</span><span class="sxs-lookup"><span data-stu-id="47baa-436">You can add multiple conditions using an `else if` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample51.cshtml)]

<span data-ttu-id="47baa-437">在此範例中，如果第一個條件，在 if 區塊不是 true，`else if`在檢查條件。</span><span class="sxs-lookup"><span data-stu-id="47baa-437">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="47baa-438">如果符合該條件中的陳述式`else if`區塊會執行。</span><span class="sxs-lookup"><span data-stu-id="47baa-438">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="47baa-439">如果條件符合中的陳述式`else`區塊會執行。</span><span class="sxs-lookup"><span data-stu-id="47baa-439">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="47baa-440">您可以新增任意數目的 else 的 if 區塊中使用，並關閉與`else`封鎖&quot;一切&quot;條件。</span><span class="sxs-lookup"><span data-stu-id="47baa-440">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="47baa-441">若要測試大量的條件，請使用`switch`區塊：</span><span class="sxs-lookup"><span data-stu-id="47baa-441">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="47baa-442">要測試的值位於括號內 (在範例中，`weekday`變數)。</span><span class="sxs-lookup"><span data-stu-id="47baa-442">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="47baa-443">每個個別的測試會使用`case`陳述式的結尾是冒號 （:）。</span><span class="sxs-lookup"><span data-stu-id="47baa-443">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="47baa-444">如果值`case`陳述式符合測試值，該案例區塊中的程式碼會執行。</span><span class="sxs-lookup"><span data-stu-id="47baa-444">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="47baa-445">關閉與每個 case 陳述式`break`陳述式。</span><span class="sxs-lookup"><span data-stu-id="47baa-445">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="47baa-446">(如果您忘記在每個包含中斷`case`封鎖，請從下一個程式碼`case`陳述式也會執行。)A`switch`區塊通常會有`default`陳述式的最後一個案例作為&quot;一切&quot;執行如果沒有任何其他情況下，則為 true 的選項。</span><span class="sxs-lookup"><span data-stu-id="47baa-446">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="47baa-447">在瀏覽器中顯示的最後兩個條件式區塊的結果：</span><span class="sxs-lookup"><span data-stu-id="47baa-447">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor-Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="47baa-449">迴圈的程式碼</span><span class="sxs-lookup"><span data-stu-id="47baa-449">Looping Code</span></span>

<span data-ttu-id="47baa-450">您通常需要重複執行相同的陳述式。</span><span class="sxs-lookup"><span data-stu-id="47baa-450">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="47baa-451">您可以進行迴圈。</span><span class="sxs-lookup"><span data-stu-id="47baa-451">You do this by looping.</span></span> <span data-ttu-id="47baa-452">例如，您經常執行相同的陳述式，每個項目集合中的資料。</span><span class="sxs-lookup"><span data-stu-id="47baa-452">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="47baa-453">如果您知道確實多少次您想要執行迴圈，您可以使用`for`迴圈。</span><span class="sxs-lookup"><span data-stu-id="47baa-453">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="47baa-454">這種迴圈是特別適用於計算或向下計數：</span><span class="sxs-lookup"><span data-stu-id="47baa-454">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="47baa-455">迴圈開頭`for`每個以分號結束關鍵字，後面接著括號中的三個陳述式。</span><span class="sxs-lookup"><span data-stu-id="47baa-455">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="47baa-456">在括弧內，第一個陳述式 (`var i=10;`) 建立計數器，並且將它初始化為 10。</span><span class="sxs-lookup"><span data-stu-id="47baa-456">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="47baa-457">您不需要名稱計數器`i`&#8212;您可以使用任何變數。</span><span class="sxs-lookup"><span data-stu-id="47baa-457">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="47baa-458">當`for`迴圈執行時，會自動遞增的計數器。</span><span class="sxs-lookup"><span data-stu-id="47baa-458">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="47baa-459">第二個陳述式 (`i < 21;`) 設定條件，以您想要計算的幅度。</span><span class="sxs-lookup"><span data-stu-id="47baa-459">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="47baa-460">您想在此情況下，請移至最多 20 個 （也就繼續執行時的計數器是小於 21）。</span><span class="sxs-lookup"><span data-stu-id="47baa-460">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="47baa-461">第三個陳述式 (`i++` ) 會使用遞增運算子，只需指定 計數器應有 1 加入至它在每次執行迴圈。</span><span class="sxs-lookup"><span data-stu-id="47baa-461">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="47baa-462">括號內是迴圈的每個反覆項目，就會執行的程式碼。</span><span class="sxs-lookup"><span data-stu-id="47baa-462">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="47baa-463">標記會建立新的段落 (`<p>`項目) 每次，且輸出中顯示的值中加入一行`i`（計數器）。</span><span class="sxs-lookup"><span data-stu-id="47baa-463">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="47baa-464">當您執行此頁面時，此範例會建立 11 行顯示輸出，表示項目數目每一行中的文字。</span><span class="sxs-lookup"><span data-stu-id="47baa-464">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor-Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="47baa-466">如果您正在使用集合或陣列，您經常使用`foreach`迴圈。</span><span class="sxs-lookup"><span data-stu-id="47baa-466">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="47baa-467">集合是一組類似的物件，而`foreach`迴圈可讓您執行的工作集合中的每個項目上。</span><span class="sxs-lookup"><span data-stu-id="47baa-467">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="47baa-468">這種類型的迴圈是方便的集合，因為不像`for`迴圈中，您不需要遞增計數器，或設定限制。</span><span class="sxs-lookup"><span data-stu-id="47baa-468">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="47baa-469">相反地，`foreach`迴圈的程式碼會繼續透過集合直到完成為止。</span><span class="sxs-lookup"><span data-stu-id="47baa-469">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="47baa-470">例如，下列程式碼會傳回中的項目`Request.ServerVariables`集合，也就是物件，包含 web 伺服器的相關資訊。</span><span class="sxs-lookup"><span data-stu-id="47baa-470">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="47baa-471">它會使用`foreac`h 的迴圈，以顯示每個項目的名稱來建立新`<li>`HTML 項目符號清單中的項目。</span><span class="sxs-lookup"><span data-stu-id="47baa-471">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="47baa-472">`foreach`關鍵字後面接著括號您用來宣告變數，表示集合中的單一項目 (在範例中， `var item`)，後面接著`in`關鍵字，後面接著您要循環的集合。</span><span class="sxs-lookup"><span data-stu-id="47baa-472">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="47baa-473">本文的`foreach`迴圈中，您可以存取目前的項目使用您稍早宣告的變數。</span><span class="sxs-lookup"><span data-stu-id="47baa-473">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor-Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="47baa-475">若要建立更通用的迴圈，請使用`while`陳述式：</span><span class="sxs-lookup"><span data-stu-id="47baa-475">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="47baa-476">A`while`迴圈開頭`while`關鍵字，後面接著括號，您可以指定迴圈繼續的時間長度 (這裡，只要`countNum`小於 50)，然後重複的區塊。</span><span class="sxs-lookup"><span data-stu-id="47baa-476">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="47baa-477">迴圈通常遞增 （加入） 或遞減 （減去） 的變數或用於計算的物件。</span><span class="sxs-lookup"><span data-stu-id="47baa-477">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="47baa-478">在範例中，`+=`運算子會加入至 1`countNum`每次執行迴圈。</span><span class="sxs-lookup"><span data-stu-id="47baa-478">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="47baa-479">(以遞減的向下計數的迴圈中的變數，您會使用遞減運算子`-=`)。</span><span class="sxs-lookup"><span data-stu-id="47baa-479">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="47baa-480">物件和集合</span><span class="sxs-lookup"><span data-stu-id="47baa-480">Objects and Collections</span></span>

<span data-ttu-id="47baa-481">幾乎在 ASP.NET 網站中所有項目是物件，包括網頁本身。</span><span class="sxs-lookup"><span data-stu-id="47baa-481">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="47baa-482">本章節將討論一些重要的物件，您將使用經常在您的程式碼。</span><span class="sxs-lookup"><span data-stu-id="47baa-482">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="47baa-483">頁面物件</span><span class="sxs-lookup"><span data-stu-id="47baa-483">Page Objects</span></span>

<span data-ttu-id="47baa-484">在 ASP.NET 中最基本的物件是頁面。</span><span class="sxs-lookup"><span data-stu-id="47baa-484">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="47baa-485">您可以存取任何合格的物件不直接的 page 物件的屬性。</span><span class="sxs-lookup"><span data-stu-id="47baa-485">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="47baa-486">下列程式碼會取得頁面的檔案路徑，使用`Request`頁面物件：</span><span class="sxs-lookup"><span data-stu-id="47baa-486">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="47baa-487">要取消選取您要參考的屬性和目前的頁面物件上的方法，您可以選擇性地使用關鍵字`this`來代表您的程式碼中的頁面物件。</span><span class="sxs-lookup"><span data-stu-id="47baa-487">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="47baa-488">以下是上述的程式碼範例中，使用`this`新增至代表的頁面：</span><span class="sxs-lookup"><span data-stu-id="47baa-488">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="47baa-489">您可以使用屬性`Page`物件，以取得大量資訊，例如：</span><span class="sxs-lookup"><span data-stu-id="47baa-489">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="47baa-490">`Request`.</span><span class="sxs-lookup"><span data-stu-id="47baa-490">`Request`.</span></span> <span data-ttu-id="47baa-491">如您所見，這是目前的要求，包括進行要求的頁面、 使用者身分識別等 URL 的瀏覽器的何種類型的相關資訊的集合。</span><span class="sxs-lookup"><span data-stu-id="47baa-491">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="47baa-492">`Response`.</span><span class="sxs-lookup"><span data-stu-id="47baa-492">`Response`.</span></span> <span data-ttu-id="47baa-493">這是會在伺服器程式碼執行完成時傳送到瀏覽器的回應 （頁面） 的相關資訊的集合。</span><span class="sxs-lookup"><span data-stu-id="47baa-493">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="47baa-494">例如，您可以使用此屬性寫入至回應的資訊。</span><span class="sxs-lookup"><span data-stu-id="47baa-494">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="47baa-495">（陣列和字典） 的集合物件</span><span class="sxs-lookup"><span data-stu-id="47baa-495">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="47baa-496">A*集合*是一組相同的類型，例如的集合物件`Customer`資料庫的物件。</span><span class="sxs-lookup"><span data-stu-id="47baa-496">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="47baa-497">ASP.NET 包含許多的內建集合，例如`Request.Files`集合。</span><span class="sxs-lookup"><span data-stu-id="47baa-497">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="47baa-498">您通常會使用在集合中的資料。</span><span class="sxs-lookup"><span data-stu-id="47baa-498">You'll often work with data in collections.</span></span> <span data-ttu-id="47baa-499">兩個常見的集合類型*陣列*並*字典*。</span><span class="sxs-lookup"><span data-stu-id="47baa-499">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="47baa-500">當您想要儲存一組類似的項目，但不想要建立個別的變數來保存每個項目時，陣列是很有用：</span><span class="sxs-lookup"><span data-stu-id="47baa-500">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="47baa-501">使用陣列時，您宣告特定的資料類型，例如`string`， `int`，或`DateTime`。</span><span class="sxs-lookup"><span data-stu-id="47baa-501">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="47baa-502">若要指出變數可以包含陣列，您將加入宣告的方括號 (例如`string[]`或`int[]`)。</span><span class="sxs-lookup"><span data-stu-id="47baa-502">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="47baa-503">您可以存取項目中使用它們的位置 （索引） 的陣列，或使用`foreach`陳述式。</span><span class="sxs-lookup"><span data-stu-id="47baa-503">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="47baa-504">陣列索引以零為起始&#8212;也就是第一個項目是在位置 0，第二個項目位於位置 1，依此類推。</span><span class="sxs-lookup"><span data-stu-id="47baa-504">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="47baa-505">您可以判斷陣列中的項目數，藉由取得其`Length`屬性。</span><span class="sxs-lookup"><span data-stu-id="47baa-505">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="47baa-506">若要取得特定項目的陣列中 （如果您要搜尋的陣列） 的位置，請使用`Array.IndexOf`方法。</span><span class="sxs-lookup"><span data-stu-id="47baa-506">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="47baa-507">您也可以執行像是反向陣列的內容 (`Array.Reverse`方法) 或排序內容 (`Array.Sort`方法)。</span><span class="sxs-lookup"><span data-stu-id="47baa-507">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="47baa-508">在瀏覽器中顯示的字串陣列程式碼的輸出：</span><span class="sxs-lookup"><span data-stu-id="47baa-508">The output of the string array code displayed in a browser:</span></span>

![Razor-Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="47baa-510">字典是索引鍵/值組的集合，您可在此提供金鑰 （或名稱），以設定或擷取對應的值：</span><span class="sxs-lookup"><span data-stu-id="47baa-510">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="47baa-511">若要建立字典，您使用`new`關鍵字指出您要建立新的字典物件。</span><span class="sxs-lookup"><span data-stu-id="47baa-511">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="47baa-512">您可以將字典指派給變數使用`var`關鍵字。</span><span class="sxs-lookup"><span data-stu-id="47baa-512">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="47baa-513">指出資料類型的字典使用角括弧中的項目 ( `< >` )。</span><span class="sxs-lookup"><span data-stu-id="47baa-513">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="47baa-514">宣告的結尾，在中，您必須新增一組括號，因為這是實際的方法，建立新的字典。</span><span class="sxs-lookup"><span data-stu-id="47baa-514">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="47baa-515">若要新增至字典的項目，您可以呼叫`Add`字典變數的方法 (`myScores`在此情況下)，然後指定 索引鍵和值。</span><span class="sxs-lookup"><span data-stu-id="47baa-515">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="47baa-516">或者，您可以使用方括號表示索引鍵，並執行簡單的指派，如下列範例所示：</span><span class="sxs-lookup"><span data-stu-id="47baa-516">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="47baa-517">若要從字典取得值，指定金鑰括號括住：</span><span class="sxs-lookup"><span data-stu-id="47baa-517">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="47baa-518">呼叫具有參數的方法</span><span class="sxs-lookup"><span data-stu-id="47baa-518">Calling Methods with Parameters</span></span>

<span data-ttu-id="47baa-519">如同您稍早在本文中，您使用程式設計物件的方法。</span><span class="sxs-lookup"><span data-stu-id="47baa-519">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="47baa-520">例如，`Database`物件可能會有`Database.Connect`方法。</span><span class="sxs-lookup"><span data-stu-id="47baa-520">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="47baa-521">許多的方法也會有一或多個參數。</span><span class="sxs-lookup"><span data-stu-id="47baa-521">Many methods also have one or more parameters.</span></span> <span data-ttu-id="47baa-522">A*參數*是一個值傳遞至方法，若要啟用以完成其工作的方法。</span><span class="sxs-lookup"><span data-stu-id="47baa-522">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="47baa-523">例如，看看的宣告`Request.MapPath`方法，後者會採用三個參數：</span><span class="sxs-lookup"><span data-stu-id="47baa-523">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="47baa-524">（行已換行，讓它更容易閱讀。</span><span class="sxs-lookup"><span data-stu-id="47baa-524">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="47baa-525">請記住，您可以將幾乎任何放置除了內部字串換行符號包含在引號中。)</span><span class="sxs-lookup"><span data-stu-id="47baa-525">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="47baa-526">這個方法會傳回對應的伺服器上的實體路徑，以指定的虛擬路徑。</span><span class="sxs-lookup"><span data-stu-id="47baa-526">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="47baa-527">方法的三個參數都`virtualPath`， `baseVirtualDir`，和`allowCrossAppMapping`。</span><span class="sxs-lookup"><span data-stu-id="47baa-527">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="47baa-528">（請注意，在宣告中，列出參數的資料類型，它們會接受的資料）。當您呼叫這個方法時，您必須提供所有的三個參數的值。</span><span class="sxs-lookup"><span data-stu-id="47baa-528">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="47baa-529">Razor 語法可讓您將參數傳遞至方法的兩個選項：*位置參數*並*具名參數*。</span><span class="sxs-lookup"><span data-stu-id="47baa-529">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="47baa-530">若要呼叫使用位置參數的方法，您會以嚴格的順序，指定在方法宣告中傳遞參數。</span><span class="sxs-lookup"><span data-stu-id="47baa-530">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="47baa-531">（您將通常知道此順序，請閱讀文件的方法）。您必須遵循的順序，以及您不能略過的任何參數&#8212;如果有必要，您傳遞空字串 (`""`) 或`null`您不需要的值為位置參數。</span><span class="sxs-lookup"><span data-stu-id="47baa-531">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="47baa-532">下列範例假設您有一個名為的資料夾*指令碼*在您的網站。</span><span class="sxs-lookup"><span data-stu-id="47baa-532">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="47baa-533">程式碼會呼叫`Request.MapPath`方法，並傳遞正確的順序的三個參數值。</span><span class="sxs-lookup"><span data-stu-id="47baa-533">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="47baa-534">然後，它會顯示產生的對應的路徑。</span><span class="sxs-lookup"><span data-stu-id="47baa-534">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="47baa-535">當方法有許多參數時，您可以使用具名參數，讓您的程式碼更容易閱讀。</span><span class="sxs-lookup"><span data-stu-id="47baa-535">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="47baa-536">若要呼叫方法，使用具名的參數，您可以指定參數名稱後面接著冒號 （:），然後按一下 值。</span><span class="sxs-lookup"><span data-stu-id="47baa-536">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="47baa-537">具名參數的優點是，您就可以將其傳遞任何您想要的順序。</span><span class="sxs-lookup"><span data-stu-id="47baa-537">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="47baa-538">（一項缺點是，在方法呼叫並不會精簡）。</span><span class="sxs-lookup"><span data-stu-id="47baa-538">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="47baa-539">下列範例會呼叫與上述相同的方法，但使用具名參數，以提供值：</span><span class="sxs-lookup"><span data-stu-id="47baa-539">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="47baa-540">如您所見，參數會傳遞不同的順序。</span><span class="sxs-lookup"><span data-stu-id="47baa-540">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="47baa-541">不過，如果您執行先前的範例和此範例中，它們會傳回相同的值。</span><span class="sxs-lookup"><span data-stu-id="47baa-541">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="47baa-542">處理錯誤</span><span class="sxs-lookup"><span data-stu-id="47baa-542">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="47baa-543">Try Catch 陳述式</span><span class="sxs-lookup"><span data-stu-id="47baa-543">Try-Catch Statements</span></span>

<span data-ttu-id="47baa-544">陳述式通常必須在程式碼中可能會失敗的原因，您無法控制。</span><span class="sxs-lookup"><span data-stu-id="47baa-544">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="47baa-545">例如: </span><span class="sxs-lookup"><span data-stu-id="47baa-545">For example:</span></span>

- <span data-ttu-id="47baa-546">如果您的程式碼會嘗試建立或存取的檔案，可能會發生各種錯誤。</span><span class="sxs-lookup"><span data-stu-id="47baa-546">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="47baa-547">您想要的檔案可能不存在，它可能會遭到鎖定，程式碼可能不具有權限，等等。</span><span class="sxs-lookup"><span data-stu-id="47baa-547">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="47baa-548">同樣地，如果您的程式碼會嘗試更新資料庫中的記錄，可以有權限問題，可能會卸除資料庫的連接，要儲存的資料可能無效，依此類推。</span><span class="sxs-lookup"><span data-stu-id="47baa-548">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="47baa-549">在程式設計的詞彙中，這些情況下則稱為*例外狀況*。</span><span class="sxs-lookup"><span data-stu-id="47baa-549">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="47baa-550">如果您的程式碼發生例外狀況，則會產生 （擲回） 的錯誤訊息，最惱人的使用者：</span><span class="sxs-lookup"><span data-stu-id="47baa-550">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor-Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="47baa-552">在您的程式碼可能會遇到例外狀況的情況下，並以避免此類型的錯誤訊息，您可以使用`try/catch`陳述式。</span><span class="sxs-lookup"><span data-stu-id="47baa-552">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="47baa-553">在 `try`陳述式中，執行您的程式碼。</span><span class="sxs-lookup"><span data-stu-id="47baa-553">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="47baa-554">在一或多個`catch`陳述式，您可以尋找特定可能發生的錯誤 （特定類型的例外狀況）。</span><span class="sxs-lookup"><span data-stu-id="47baa-554">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="47baa-555">您可以包含更多`catch`陳述式，為您要尋找您預期的錯誤。</span><span class="sxs-lookup"><span data-stu-id="47baa-555">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="47baa-556">我們建議您避免使用`Response.Redirect`方法中的`try/catch`陳述式，因為它會在網頁中造成例外狀況。</span><span class="sxs-lookup"><span data-stu-id="47baa-556">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>


<span data-ttu-id="47baa-557">下列範例顯示建立第一個要求上的文字檔案，然後顯示按鈕，讓使用者可以開啟檔案的頁面。</span><span class="sxs-lookup"><span data-stu-id="47baa-557">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="47baa-558">此範例刻意使用不正確的檔案名稱，如此會導致例外狀況。</span><span class="sxs-lookup"><span data-stu-id="47baa-558">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="47baa-559">程式碼包含`catch`陳述式中的兩個可能的例外狀況： `FileNotFoundException`，就會出現錯誤，檔案名稱是否與`DirectoryNotFoundException`，發生於 ASP.NET 即使找不到資料夾。</span><span class="sxs-lookup"><span data-stu-id="47baa-559">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="47baa-560">（您可以取消此範例中的陳述式以查看一切正常運作時，它的執行方式註解）。</span><span class="sxs-lookup"><span data-stu-id="47baa-560">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="47baa-561">如果您的程式碼未處理例外狀況，您會看到先前的螢幕擷取畫面類似的錯誤頁面。</span><span class="sxs-lookup"><span data-stu-id="47baa-561">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="47baa-562">不過，`try/catch`一節可協助避免使用者看到這些錯誤類型。</span><span class="sxs-lookup"><span data-stu-id="47baa-562">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="47baa-563">其他資源</span><span class="sxs-lookup"><span data-stu-id="47baa-563">Additional Resources</span></span>

<span data-ttu-id="47baa-564">**使用 Visual Basic 進行程式設計**</span><span class="sxs-lookup"><span data-stu-id="47baa-564">**Programming with Visual Basic**</span></span>


[<span data-ttu-id="47baa-565">附錄： Visual Basic 語言和語法</span><span class="sxs-lookup"><span data-stu-id="47baa-565">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)


<span data-ttu-id="47baa-566">**參考文件**</span><span class="sxs-lookup"><span data-stu-id="47baa-566">**Reference Documentation**</span></span>


[<span data-ttu-id="47baa-567">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="47baa-567">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="47baa-568">C# 語言</span><span class="sxs-lookup"><span data-stu-id="47baa-568">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
