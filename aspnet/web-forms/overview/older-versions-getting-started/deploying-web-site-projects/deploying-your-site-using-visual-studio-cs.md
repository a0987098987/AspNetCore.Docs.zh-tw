...
若要部署新的檔案選取它們，然後按一下 將它們傳送到遠端站台的箭號圖示。 傳輸完成後`Policy.aspx`和`Policy.aspx.cs`檔案放在 Unchanged 狀態與本機和遠端網站。
...
複製網站工具也會指出，是否自上次複製作業後刪除檔案的位置。 刪除`Privacy.aspx`從本機專案，然後重新整理複製網站工具。 `Privacy.aspx`和`Privacy.aspx.cs`檔案保留列在左窗格中，但已刪除狀態，指出他們已在上次複製作業之後已經移除。
