...
一旦部署了應用程式，而且有需要保存的資料後，在需要變更結構描述時就無法卸除資料庫。 Entity Framework [Code First 移轉](http://docs.efproject.net/en/latest/platforms/aspnetcore/new-db.html)可讓您更新結構描述並移轉資料庫，而不會遺失資料。 移轉是使用 SQL Server 時的常用功能，但 SQLlite 不支援許多移轉結構描述作業，因此只能執行非常簡單的移轉。 如需詳細資訊，請參閱 [SQLite 限制](https://docs.microsoft.com/ef/core/providers/sqlite/limitations)。
...
在我們更新資料庫以包含新欄位之前，應用程式無法運作。 如果立即執行它，則會收到下列 `SqliteException`：
