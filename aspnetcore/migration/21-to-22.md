...
.NET Framework 為目標的 ASP.NET Core 應用程式不支援同處理序主控模型。
...
如果應用程式呼叫<xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*>中`CreateDefaultBuilder`方法`Program`類別中，呼叫`ConfigureKestrel`若要設定 Kestrel 伺服器，而不是`UseKestrel`以免 je v konfliktu [IIS 同處理序主控模型](xref:fundamentals/servers/index#in-process-hosting-model):
...
如果應用程式並不會呼叫`CreateDefaultBuilder`，呼叫<xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*>**之前**呼叫`ConfigureKestrel`:
