...
* 會使用 Web API 者即為用戶端 (行動裝置應用程式、瀏覽器等)。 本教學課程不會建立用戶端。 將使用 [Postman](https://www.getpostman.com/) \(英文\) 或 [curl](https://curl.haxx.se/docs/manpage.html) \(英文\) 做為測試應用程式的用戶端。

* 「模型」是代表應用程式中資料的物件。 在此情況下，唯一的模型是待辦事項。 模型以 C# 類別表示，也稱為簡單的 C# 物件 (**P**lain **O**ld **C**LR **O**bject, POCO)。
