...
GRPC 用戶端類型會使用相依性插入（DI）註冊為暫時性。 現在可以在 DI 所建立的類型中直接插入和取用用戶端。 ASP.NET Core MVC 控制器、SignalR hub 和 gRPC services 是可自動插入 gRPC 用戶端的地方：
...
在 gRPC 服務中，由處理`EnableCallContextPropagation()`站所建立的 gRPC 用戶端可以設定為，以自動將期限和取消權杖傳播至子呼叫。 擴充方法可在[Grpc. AspNetCore. ClientFactory](https://www.nuget.org/packages/Grpc.AspNetCore.Server.ClientFactory) NuGet 套件中取得。 `EnableCallContextPropagation()`
