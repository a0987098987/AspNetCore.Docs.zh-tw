...
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Get*><xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.GetAsync*> &ndash;會接受字串索引鍵，並在快取中找到`byte[]`快取的專案做為陣列。
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Set*>， <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.SetAsync*> &ndash;會使用字串索引鍵`byte[]` ，將專案（陣列）新增至快取。
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Refresh*><xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.RefreshAsync*>會根據索引鍵重新整理快取中的專案，重設其滑動到期時間（如果有的話&ndash; ）。
...
在中`Startup.ConfigureServices`註冊的<xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache>執行。 本主題所描述的架構提供的實作為包括：
...
執行`sql-cache create`命令，在 SQL Server 中建立資料表。 提供 SQL Server 實例`Data Source`（）、資料庫（`Initial Catalog`）、架構（例如`dbo`）和資料表名稱（例如`TestCache`）：
...
> [!NOTE]
> 應用程式應該使用的實例來<xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache>操作快取值，而<xref:Microsoft.Extensions.Caching.SqlServer.SqlServerCache>不是。

範例應用程式<xref:Microsoft.Extensions.Caching.SqlServer.SqlServerCache>會在的非開發環境中`Startup.ConfigureServices`執行：
...
應用程式會在的非開發環境<xref:Microsoft.Extensions.Caching.StackExchangeRedis.RedisCache>中，<xref:Microsoft.Extensions.DependencyInjection.StackExchangeRedisCacheServiceCollectionExtensions.AddStackExchangeRedisCache*>使用實例（）來設定快取`Startup.ConfigureServices`執行：
...
範例應用程式<xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache>會將插入`IndexModel` ，以供 [索引] 頁面使用。
...
若要使用 NCache 分散式快取，請參考[AspNetCore 應用程式中繼套件](xref:fundamentals/metapackage-app)，並將套件參考新增至[NCache.](https://www.nuget.org/packages/NCache.Microsoft.Extensions.Caching.OpenSource)套件。 NCache 套件不包含在`Microsoft.AspNetCore.App`封裝中，因此您必須在專案檔中分別參考 NCache 套件。
