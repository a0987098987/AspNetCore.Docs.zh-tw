...
* 不需要字串緩衝區，因為`PipeReader`會處理沒有再被使用的位元組。
* 編碼的字串會直接新增至所傳回字串的清單。
* 系統不會配置資源給字串建立作業，除了記憶體所使用的字串 (`ToArray()` 呼叫除外) 。

## <a name="adapters"></a>配接器

 `Body`和`BodyReader/BodyWriter`的屬性都適用于`HttpRequest`和。 `HttpResponse`。當您將`Body`設定為不同的資料流時，調整器會自動將每個類型調整成同一種資料流。如果您將`HttpRequest.Body`設定為新的資料流，`HttpRequest.BodyReader`會自動設為新的`PipeReader`(`HttpRequest.Body`的包裝)。

## <a name="startasync"></a>StartAsync

`HttpResponse.StartAsync`用來表示標頭是不可修改的，且執行 `OnStarting` 回呼。 使用 Kestrel 做為伺服器時，在`StartAsync` `PipeReader`使用之前呼叫會`GetMemory`保證所傳回的記憶體屬於 Kestrel 的內部<xref:System.IO.Pipelines.Pipe> ，而不是外部緩衝區。
