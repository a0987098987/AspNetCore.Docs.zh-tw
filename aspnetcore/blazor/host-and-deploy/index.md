...
若要為 Blazor 應用程式的基底路徑提供 `https://www.contoso.com/CoolApp/` 設定， `<base>` 標記的 `href` 屬性會設為*Pages/_Host. cshtml*檔案（ Blazor 伺服器）或*wwwroot/index.html*檔案（WebAssembly）中的相對根路徑 Blazor ：
...
若要設定應用程式的基底路徑，請在 `<base>` `<head>` *Pages/_Host. cshtml*檔案（ Blazor 伺服器）或*wwwroot/index.html*檔案（WebAssembly）的標記元素內更新標記 Blazor 。 將 `href` 屬性值設定為 `/{RELATIVE URL PATH}/` （需要尾端斜線），其中 `{RELATIVE URL PATH}` 是應用程式的完整相對 URL 路徑。
