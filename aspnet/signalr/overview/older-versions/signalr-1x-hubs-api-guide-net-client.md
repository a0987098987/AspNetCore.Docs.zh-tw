...
Proxy 物件是安全執行緒。 事實上，如果您呼叫`HubConnection.CreateHubProxy`多次使用相同`hubName`，您得到相同的快取`IHubProxy`物件。
...
如需詳細資訊，請參閱 <<c0> [ 了解和處理連線存留期事件 SignalR](../guide-to-the-api/handling-connection-lifetime-events.md)。
