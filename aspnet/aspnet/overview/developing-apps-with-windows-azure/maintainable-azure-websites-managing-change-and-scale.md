...
<span data-ttu-id="f3ad9-165">完成本實驗室估計時間： **75 分鐘**</span><span class="sxs-lookup"><span data-stu-id="f3ad9-165">Estimated time to complete this lab: **75 minutes**</span></span>
...
1. <span data-ttu-id="f3ad9-177">開啟 Visual Studio，並開啟**GeekQuiz.sln**方案檔從**Source\Ex1 UsingEntityFrameworkMigrations\Begin**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-177">Open Visual Studio and open the **GeekQuiz.sln** solution file from **Source\Ex1-UsingEntityFrameworkMigrations\Begin**.</span></span>
...
9. <span data-ttu-id="f3ad9-206">在**SQL Server 物件總管**視窗中，連接到 LocalDB 執行個體，以滑鼠右鍵按一下**SQL Server**節點，然後選取**加入 SQL Server...**選項。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-206">In the **SQL Server Object Explorer** window, connect to your LocalDB instance by right-clicking the **SQL Server** node and selecting **Add SQL Server...** option.</span></span>
    ...
    <span data-ttu-id="f3ad9-208">*將 SQL Server 執行個體加入至 SQL Server 物件總管*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-208">*Adding a SQL Server instance to SQL Server Object Explorer*</span></span>
...
2. <span data-ttu-id="f3ad9-269">按一下**新增**在頁面底部命令列中。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-269">Click **New** in the command bar at the bottom of the page.</span></span>
    ...
    <span data-ttu-id="f3ad9-271">*建立新的 web 應用程式*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-271">*Creating a new web app*</span></span>
...
<a id="Exercise3"></a>
### <a name="exercise-3-performing-deployment-rollback-in-production"></a><span data-ttu-id="f3ad9-421">練習 3： 在生產環境中執行部署復原</span><span class="sxs-lookup"><span data-stu-id="f3ad9-421">Exercise 3: Performing Deployment Rollback in Production</span></span>

<span data-ttu-id="f3ad9-422">其中您沒有預備位置來執行熱交換之間預備和生產環境，例如，如果您正在使用的情況下**免費**或**共用**模式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-422">There are scenarios where you do not have a staging slot to perform hot swap between staging and production, for example, if you are working with **Free** or **Shared** mode.</span></span> <span data-ttu-id="f3ad9-423">在這些情況下，您應該測試應用程式在測試環境中 （本機或遠端網站中） 部署到生產環境之前。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-423">In those scenarios, you should test your application in a testing environment –either locally or in a remote site– before deploying to production.</span></span> <span data-ttu-id="f3ad9-424">不過，很可能在測試階段期間未偵測到的問題可能會發生在生產網站。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-424">However, it is possible that an issue not detected during the testing phase may arise in the production site.</span></span> <span data-ttu-id="f3ad9-425">在此情況下，它是一定要有一個機制，輕鬆地儘快切換至應用程式的上一個且更穩定版本。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-425">In this case, it is important to have a mechanism to easily switch to a previous and more stable version of the application as quickly as possible.</span></span>

<span data-ttu-id="f3ad9-426">在**Azure App Service**，從原始檔控制的連續部署可讓此可能有**重新部署**管理入口網站中可用的動作。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-426">In **Azure App Service**, continuous deployment from source control makes this possible thanks to the **redeploy** action available in the management portal.</span></span> <span data-ttu-id="f3ad9-427">Azure 會追蹤在認可推送到儲存機制相關聯的部署，並提供選項，以重新部署應用程式使用任何先前的部署中，在任何時間。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-427">Azure keeps track of the deployments associated with the commits pushed to the repository and provides an option to redeploy your application using any of your previous deployments, at any time.</span></span>

<span data-ttu-id="f3ad9-428">在這個練習中，您將執行中的程式碼變更**玩家測驗**刻意插入的應用程式*bug*。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-428">In this exercise you will perform a change to the code in the **Geek Quiz** application that intentionally injects a *bug*.</span></span> <span data-ttu-id="f3ad9-429">您將部署到生產環境以查看錯誤，應用程式，然後您會利用重新部署功能，請回到先前的狀態。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-429">You will deploy the application to production to see the error, and then you will take advantage of the redeploy feature to go back to the previous state.</span></span>

<a id="Ex3Task1"></a>
#### <a name="task-1--updating-the-geek-quiz-application"></a><span data-ttu-id="f3ad9-430">工作 1-更新玩家受測應用程式</span><span class="sxs-lookup"><span data-stu-id="f3ad9-430">Task 1 – Updating the Geek Quiz Application</span></span>

<span data-ttu-id="f3ad9-431">在這項工作，您將會重構一小段程式碼**TriviaController**類別來擷取從資料庫擷取選取的測驗選項，為新方法的邏輯的一部分。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-431">In this task, you will refactor a small piece of code of the **TriviaController** class to extract part of the logic that retrieves the selected quiz option from the database into a new method.</span></span>

1. <span data-ttu-id="f3ad9-432">切換至 Visual Studio 執行個體與**GeekQuiz**在上一個作業中的方案。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-432">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="f3ad9-433">在**方案總管 中**，開啟**TriviaController.cs**檔案內**控制器**資料夾。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-433">In **Solution Explorer**, open the **TriviaController.cs** file inside the **Controllers** folder.</span></span>
3. <span data-ttu-id="f3ad9-434">找出**StoreAsync**方法，然後選取程式碼以在下圖中反白顯示。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-434">Locate the **StoreAsync** method and select the code highlighted in the following figure.</span></span>

    ![選取程式碼](maintainable-azure-websites-managing-change-and-scale/_static/image47.png)

    <span data-ttu-id="f3ad9-436">*選取程式碼*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-436">*Selecting the code*</span></span>
4. <span data-ttu-id="f3ad9-437">以滑鼠右鍵按一下選取的程式碼中，展開**重構**功能表，然後選取**擷取方法...**.</span><span class="sxs-lookup"><span data-stu-id="f3ad9-437">Right-click the selected code, expand the **Refactor** menu and select **Extract Method...**.</span></span>

    ![擷取程式碼做為新方法](maintainable-azure-websites-managing-change-and-scale/_static/image48.png)

    <span data-ttu-id="f3ad9-439">*選取擷取方法*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-439">*Selecting Extract Method*</span></span>
5. <span data-ttu-id="f3ad9-440">在**擷取方法**對話方塊中，新的方法名稱*MatchesOption*按一下**確定**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-440">In the **Extract Method** dialog box, name the new method *MatchesOption* and click **OK**.</span></span>

    ![指定方法名稱](maintainable-azure-websites-managing-change-and-scale/_static/image49.png)

    <span data-ttu-id="f3ad9-442">*指定擷取方法的名稱*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-442">*Specifying the name for the extracted method*</span></span>
6. <span data-ttu-id="f3ad9-443">選取的程式碼接著會擷取到**MatchesOption**方法。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-443">The selected code is then extracted into the **MatchesOption** method.</span></span> <span data-ttu-id="f3ad9-444">產生的程式碼是以下列程式碼片段所示。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-444">The resulting code is shown in the following snippet.</span></span>

    [!code-csharp[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample14.cs)]
7. <span data-ttu-id="f3ad9-445">按**CTRL + S**儲存的變更。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-445">Press **CTRL + S** to save the changes.</span></span>

<a id="Ex3Task2"></a>
#### <a name="task-2--redeploying-the-geek-quiz-application"></a><span data-ttu-id="f3ad9-446">工作 2 – 重新部署玩家受測應用程式</span><span class="sxs-lookup"><span data-stu-id="f3ad9-446">Task 2 – Redeploying the Geek Quiz Application</span></span>

<span data-ttu-id="f3ad9-447">您現在會將推送您對先前工作中的儲存機制將會觸發新的部署到生產環境的變更。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-447">You will now push the changes you made in the previous task to the repository, which will trigger a new deployment to the production environment.</span></span> <span data-ttu-id="f3ad9-448">然後，您將使用問題使用**F12 開發工具**由 Internet Explorer 中，提供，然後執行回復至先前的部署從 Azure 管理入口網站。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-448">Then, you will troubleshot an issue using the **F12 development tools** provided by Internet Explorer, and then perform a rollback to the previous deployment from the Azure management portal.</span></span>

1. <span data-ttu-id="f3ad9-449">開啟新**Git Bash**主控台部署至 Azure App Service 更新的應用程式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-449">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
2. <span data-ttu-id="f3ad9-450">執行下列命令以將變更推送至 Azure。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-450">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="f3ad9-451">更新*[您的應用程式的路徑]*預留位置路徑**GeekQuiz**方案。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-451">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="f3ad9-452">系統會提示您做為部署密碼。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-452">You will be prompted for your deployment password.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample15.cmd)]

    ![將重構的程式碼推送至 Azure](maintainable-azure-websites-managing-change-and-scale/_static/image50.png)

    <span data-ttu-id="f3ad9-454">*將重構的程式碼推送至 Azure*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-454">*Pushing refactored code to Azure*</span></span>
3. <span data-ttu-id="f3ad9-455">開啟 Internet Explorer 並瀏覽至您的 web 應用程式 (例如`http://<your-web-site>.azurewebsites.net`)。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-455">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="f3ad9-456">使用先前建立的認證登入。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-456">Log in using the previously created credentials.</span></span>
4. <span data-ttu-id="f3ad9-457">按**F12**若要啟動開發工具，請選取**網路**索引標籤上，按一下 [**播放**開始錄製] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-457">Press **F12** to launch the development tools, select the **Network** tab and click the **Play** button to start recording.</span></span>

    <span data-ttu-id="f3ad9-458">![啟動網路錄製](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "啟動網路錄製")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-458">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "Starting network recording")</span></span>

    <span data-ttu-id="f3ad9-459">*啟動網路錄製*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-459">*Starting network recording*</span></span>
5. <span data-ttu-id="f3ad9-460">選取測驗的任何選項。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-460">Select any option of the quiz.</span></span> <span data-ttu-id="f3ad9-461">您會看到沒有任何反應。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-461">You will see that nothing happens.</span></span>
6. <span data-ttu-id="f3ad9-462">在**F12**視窗中，對應至 POST HTTP 要求的項目會顯示 HTTP **500**結果。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-462">In the **F12** window, the entry corresponding to the POST HTTP request shows an HTTP **500** result.</span></span>

    ![HTTP 500 錯誤](maintainable-azure-websites-managing-change-and-scale/_static/image52.png)

    <span data-ttu-id="f3ad9-464">*HTTP 500 錯誤*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-464">*HTTP 500 error*</span></span>
7. <span data-ttu-id="f3ad9-465">選取**主控台** 索引標籤。將錯誤記錄的原因詳細資料。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-465">Select the **Console** tab. An error is logged with the details of the cause.</span></span>

    ![記錄的錯誤](maintainable-azure-websites-managing-change-and-scale/_static/image53.png)

    <span data-ttu-id="f3ad9-467">*記錄的錯誤*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-467">*Logged error*</span></span>
8. <span data-ttu-id="f3ad9-468">找出錯誤的詳細資料部分。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-468">Locate the details part of the error.</span></span> <span data-ttu-id="f3ad9-469">很明顯地，這個錯誤是重構前一個步驟中認可您的程式碼所造成。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-469">Clearly, this error is caused by the code refactoring you committed in the previous steps.</span></span>

    <span data-ttu-id="f3ad9-470">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span><span class="sxs-lookup"><span data-stu-id="f3ad9-470">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span></span>
9. <span data-ttu-id="f3ad9-471">請勿關閉瀏覽器。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-471">Do not close the browser.</span></span>
10. <span data-ttu-id="f3ad9-472">在新的瀏覽器執行個體中，瀏覽至[Azure 管理入口網站](https://manage.windowsazure.com)並使用與您訂用帳戶相關聯的 Microsoft 帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-472">In a new browser instance, navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
11. <span data-ttu-id="f3ad9-473">選取**網站**按一下您在練習 2 中建立 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-473">Select **Websites** and click the web app you created in Exercise 2.</span></span>
12. <span data-ttu-id="f3ad9-474">瀏覽至**部署**頁面。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-474">Navigate to the **Deployments** page.</span></span> <span data-ttu-id="f3ad9-475">請注意，執行的所有認可會列在 部署歷程記錄。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-475">Notice that all the commits performed are listed in the deployment history.</span></span>

    ![現有的部署清單](maintainable-azure-websites-managing-change-and-scale/_static/image54.png)

    <span data-ttu-id="f3ad9-477">*現有的部署清單*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-477">*List of existing deployments*</span></span>
13. <span data-ttu-id="f3ad9-478">選取 上一次認可，然後按一下**重新部署**命令列上。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-478">Select the previous commit and click **Redeploy** on the command bar.</span></span>

    ![上一次認可重新部署](maintainable-azure-websites-managing-change-and-scale/_static/image55.png)

    <span data-ttu-id="f3ad9-480">*上一次認可重新部署*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-480">*Redeploying the previous commit*</span></span>
14. <span data-ttu-id="f3ad9-481">當提示確認，請按一下 **是**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-481">When prompted to confirm, click **Yes**.</span></span>

    ![確認重新部署](maintainable-azure-websites-managing-change-and-scale/_static/image56.png)
15. <span data-ttu-id="f3ad9-483">完成部署之後，切換回瀏覽器執行個體與您的 web 應用程式按**CTRL + F5**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-483">When the deployment completes, switch back to the browser instance with your web app and press **CTRL + F5**.</span></span>
16. <span data-ttu-id="f3ad9-484">按一下任何一個選項。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-484">Click any of the options.</span></span> <span data-ttu-id="f3ad9-485">位置和結果，將立即帶翻轉動畫 (*正確/不正確*) 將會顯示。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-485">The flip animation will now take place and the result (*correct/incorrect*) will be displayed.</span></span>
17. <span data-ttu-id="f3ad9-486">（選擇性）切換至**Git Bash**主控台，然後執行下列命令，還原成先前的認可。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-486">(Optional) Switch to the **Git Bash** console and execute the following commands to revert to the previous commit.</span></span>

    > [!NOTE]
    > <span data-ttu-id="f3ad9-487">這些命令會建立新的認可，復原將 Git 儲存機制中不正確的認可中所做的所有變更。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-487">These commands create a new commit that undoes all changes in the Git repository that were made in the bad commit.</span></span> <span data-ttu-id="f3ad9-488">Azure 會再重新部署應用程式使用新的認可。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-488">Azure will then redeploy the application using the new commit.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample16.cmd)]

<a id="Exercise4"></a>
### <a name="exercise-4-scaling-using-azure-storage"></a><span data-ttu-id="f3ad9-489">練習 4： 可讓您調整使用 Azure 儲存體</span><span class="sxs-lookup"><span data-stu-id="f3ad9-489">Exercise 4: Scaling Using Azure Storage</span></span>

<span data-ttu-id="f3ad9-490">**Blob**是最簡單的方式來儲存大量非結構化的文字或二進位資料，例如視訊、 音訊和影像。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-490">**Blobs** are the simplest way to store large amounts of unstructured text or binary data such as video, audio and images.</span></span> <span data-ttu-id="f3ad9-491">移至儲存體的應用程式的靜態內容，有助於藉由提供影像或文件，以瀏覽器直接調整您的應用程式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-491">Moving the static content of your application to Storage, helps to scale your application by serving images or documents directly to the browser.</span></span>

<span data-ttu-id="f3ad9-492">在此練習中，您會將移到 Blob 容器的應用程式的靜態內容。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-492">In this exercise, you will move the static content of your application to a Blob container.</span></span> <span data-ttu-id="f3ad9-493">然後您將設定您的應用程式加入**ASP.NET URL 重寫規則**中**Web.config**重新導向至 Blob 容器的內容。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-493">Then you will configure your application to add an **ASP.NET URL rewrite rule** in the **Web.config** to redirect your content to the Blob container.</span></span>

<a id="Ex4Task1"></a>
#### <a name="task-1--creating-an-azure-storage-account"></a><span data-ttu-id="f3ad9-494">工作 1 – 建立 Azure 儲存體帳戶</span><span class="sxs-lookup"><span data-stu-id="f3ad9-494">Task 1 – Creating an Azure Storage Account</span></span>

<span data-ttu-id="f3ad9-495">在這項工作中，您將學習如何建立新的儲存體帳戶，使用管理入口網站。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-495">In this task you will learn how to create a new storage account using the management portal.</span></span>

1. <span data-ttu-id="f3ad9-496">瀏覽至[Azure 管理入口網站](https://manage.windowsazure.com)並使用與您訂用帳戶相關聯的 Microsoft 帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-496">Navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
2. <span data-ttu-id="f3ad9-497">選取**新 |資料服務 |儲存體 |快速建立**開始建立新的儲存體帳戶。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-497">Select **New | Data Services | Storage | Quick Create** to start creating a new storage account.</span></span> <span data-ttu-id="f3ad9-498">輸入唯一的名稱做為帳戶選取**區域**從清單中。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-498">Enter a unique name for the account and select a **Region** from the list.</span></span> <span data-ttu-id="f3ad9-499">按一下**建立儲存體帳戶**才能繼續。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-499">Click **Create Storage Account** to continue.</span></span>

    <span data-ttu-id="f3ad9-500">![建立新的儲存體帳戶](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "建立新的儲存體帳戶")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-500">![Creating a new Storage Account](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "Creating a new Storage Account")</span></span>

    <span data-ttu-id="f3ad9-501">*建立新的儲存體帳戶*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-501">*Creating a new storage account*</span></span>
3. <span data-ttu-id="f3ad9-502">在**儲存體**區段中，等到新的儲存體帳戶的狀態變更為*線上*才能繼續進行下一個步驟。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-502">In the **Storage** section, wait until the status of the new storage account changes to *Online* in order to continue with the following step.</span></span>

    <span data-ttu-id="f3ad9-503">![建立儲存體帳戶](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "建立儲存體帳戶")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-503">![Storage Account created](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "Storage Account created")</span></span>

    <span data-ttu-id="f3ad9-504">*建立儲存體帳戶*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-504">*Storage Account created*</span></span>
4. <span data-ttu-id="f3ad9-505">按一下儲存體帳戶名稱，然後按一下**儀表板**在頁面頂端的連結。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-505">Click on the storage account name and then click the **Dashboard** link at the top of the page.</span></span> <span data-ttu-id="f3ad9-506">**儀表板**頁面為您提供的帳戶，可用於您的應用程式的服務端點的狀態資訊。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-506">The **Dashboard** page provides you with information about the status of the account and the service endpoints that can be used within your applications.</span></span>

    <span data-ttu-id="f3ad9-507">![顯示儲存體帳戶儀表板](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "顯示儲存體帳戶儀表板")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-507">![Displaying the Storage Account Dashboard](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "Displaying the Storage Account Dashboard")</span></span>

    <span data-ttu-id="f3ad9-508">*顯示儲存體帳戶儀表板*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-508">*Displaying the Storage Account Dashboard*</span></span>
5. <span data-ttu-id="f3ad9-509">按一下**管理存取金鑰**導覽列中的按鈕。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-509">Click the **Manage Access Keys** button in the navigation bar.</span></span>

    <span data-ttu-id="f3ad9-510">![管理存取金鑰 按鈕](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "管理存取金鑰 按鈕")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-510">![Manage Access Keys button](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "Manage Access Keys button")</span></span>

    <span data-ttu-id="f3ad9-511">*管理存取金鑰 按鈕*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-511">*Manage Access Keys button*</span></span>
6. <span data-ttu-id="f3ad9-512">在**管理存取金鑰**對話方塊中，複製**儲存體帳戶名稱**和**主要存取金鑰**當您將需要在下列練習中。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-512">In the **Manage Access Keys** dialog box, copy the **Storage Account Name** and **Primary Access Key** as you will need them in the following exercise.</span></span> <span data-ttu-id="f3ad9-513">然後，關閉對話方塊。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-513">Then, close the dialog box.</span></span>

    <span data-ttu-id="f3ad9-514">![管理存取金鑰對話方塊](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "管理存取金鑰 對話方塊")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-514">![Manage Access Key dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "Manage Access Key dialog box")</span></span>

    <span data-ttu-id="f3ad9-515">*管理存取金鑰對話方塊*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-515">*Manage Access Key dialog box*</span></span>

<a id="Ex4Task2"></a>
#### <a name="task-2--uploading-an-asset-to-azure-blob-storage"></a><span data-ttu-id="f3ad9-516">工作 2-將資產上傳至 Azure Blob 儲存體</span><span class="sxs-lookup"><span data-stu-id="f3ad9-516">Task 2 – Uploading an Asset to Azure Blob Storage</span></span>

<span data-ttu-id="f3ad9-517">在這個工作中，您將使用從 Visual Studio 的 [伺服器總管] 視窗，來連接至儲存體帳戶。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-517">In this task, you will use the Server Explorer window from Visual Studio to connect to your storage account.</span></span> <span data-ttu-id="f3ad9-518">接著建立 blob 容器，並有一些測驗標誌檔案上傳至容器。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-518">You will then create a blob container and upload a file with the Geek Quiz logo to the container.</span></span>

1. <span data-ttu-id="f3ad9-519">切換至 Visual Studio 執行個體與**GeekQuiz**在上一個作業中的方案。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-519">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="f3ad9-520">從功能表列中，選取**檢視**，然後按一下 **伺服器總管**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-520">From the menu bar, select **View** and then click **Server Explorer**.</span></span>
3. <span data-ttu-id="f3ad9-521">在**伺服器總管**，以滑鼠右鍵按一下**Azure**節點，然後選取**連接至 Azure...**.使用與您訂用帳戶相關聯的 Microsoft 帳戶登入。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-521">In **Server Explorer**, right-click the **Azure** node and select **Connect to Azure...**. Sign in using the Microsoft account associated with your subscription.</span></span>

    ![連接到 Windows Azure](maintainable-azure-websites-managing-change-and-scale/_static/image62.png)

    <span data-ttu-id="f3ad9-523">*連接到 Azure*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-523">*Connect to Azure*</span></span>
4. <span data-ttu-id="f3ad9-524">展開**Azure**  節點，以滑鼠右鍵按一下**儲存體**選取**附加外部儲存體...**.</span><span class="sxs-lookup"><span data-stu-id="f3ad9-524">Expand the **Azure** node, right-click **Storage** and select **Attach External Storage...**.</span></span>
5. <span data-ttu-id="f3ad9-525">在**加入新的儲存體帳戶**對話方塊方塊中，輸入**帳戶名稱**和**帳戶金鑰**您在上一項工作，然後按一下取得**確定**.</span><span class="sxs-lookup"><span data-stu-id="f3ad9-525">In the **Add New Storage Account** dialog box, enter the **Account name** and **Account key** you obtained in the previous task and click **OK**.</span></span>

    ![加入新的儲存體帳戶對話方塊](maintainable-azure-websites-managing-change-and-scale/_static/image63.png)

    <span data-ttu-id="f3ad9-527">*加入新的儲存體帳戶對話方塊*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-527">*Add New Storage Account dialog box*</span></span>
6. <span data-ttu-id="f3ad9-528">儲存體帳戶應該會出現在**儲存體**節點。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-528">Your storage account should appear under the **Storage** node.</span></span> <span data-ttu-id="f3ad9-529">展開您的儲存體帳戶，以滑鼠右鍵按一下**Blob**選取**建立 Blob 容器...**.</span><span class="sxs-lookup"><span data-stu-id="f3ad9-529">Expand your storage account, right-click **Blobs** and select **Create Blob Container...**.</span></span>

    <span data-ttu-id="f3ad9-530">![建立 Blob 容器](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "建立 Blob 容器")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-530">![Create Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "Create Blob Container")</span></span>

    <span data-ttu-id="f3ad9-531">*建立 Blob 容器*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-531">*Create Blob Container*</span></span>
7. <span data-ttu-id="f3ad9-532">在**建立 Blob 容器**對話方塊方塊中，輸入 blob 容器的名稱，然後按一下**確定**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-532">In the **Create Blob Container** dialog box, enter a name for the blob container and click **OK**.</span></span>

    <span data-ttu-id="f3ad9-533">![建立 Blob 容器 對話方塊](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "建立 Blob 容器 對話方塊")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-533">![Create Blob Container dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "Create Blob Container dialog box")</span></span>

    <span data-ttu-id="f3ad9-534">*建立 Blob 容器 對話方塊*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-534">*Create Blob Container dialog box*</span></span>
8. <span data-ttu-id="f3ad9-535">應該加入新的 blob 容器**Blob**節點。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-535">The new blob container should be added to the **Blobs** node.</span></span> <span data-ttu-id="f3ad9-536">變更要將容器設為公用容器中的存取權限。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-536">Change the access permissions in the container to make the container public.</span></span> <span data-ttu-id="f3ad9-537">若要這樣做，請以滑鼠右鍵按一下**映像**容器，然後選取**屬性**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-537">To do this, right-click the **images** container and select **Properties**.</span></span>

    <span data-ttu-id="f3ad9-538">![映像容器屬性](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "映像容器屬性")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-538">![images container properties](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "images container properties")</span></span>

    <span data-ttu-id="f3ad9-539">*映像容器屬性*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-539">*Images container properties*</span></span>
9. <span data-ttu-id="f3ad9-540">在**屬性**視窗中，將**公用讀取權限**至**容器**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-540">In the **Properties** window, set the **Public Read Access** to **Container**.</span></span>

    <span data-ttu-id="f3ad9-541">![變更公用讀取權限屬性](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "變更公用讀取權限屬性")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-541">![Changing public read access property](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "Changing public read access property")</span></span>

    <span data-ttu-id="f3ad9-542">*變更公用讀取權限屬性*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-542">*Changing public read access property*</span></span>
10. <span data-ttu-id="f3ad9-543">當系統提示您是否確定要變更的公用存取屬性，請按一下**是**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-543">When prompted if you are sure you want to change the public access property, click **Yes**.</span></span>

    <span data-ttu-id="f3ad9-544">![Microsoft Visual Studio 警告](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio 警告")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-544">![Microsoft Visual Studio warning](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio warning")</span></span>

    <span data-ttu-id="f3ad9-545">*Microsoft Visual Studio 警告*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-545">*Microsoft Visual Studio warning*</span></span>
11. <span data-ttu-id="f3ad9-546">在**伺服器總管**，以滑鼠右鍵按一下**映像**blob 容器，然後選取**檢視 Blob 容器**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-546">In **Server Explorer**, right-click in the **images** blob container and select **View Blob Container**.</span></span>

    <span data-ttu-id="f3ad9-547">![檢視 Blob 容器](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "檢視 Blob 容器")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-547">![View Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "View Blob Container")</span></span>

    <span data-ttu-id="f3ad9-548">*檢視 Blob 容器*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-548">*View Blob Container*</span></span>
12. <span data-ttu-id="f3ad9-549">容器映像應該會開啟新視窗中，應該會顯示任何項目的圖例。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-549">The images container should open in a new window and a legend with no entries should be shown.</span></span> <span data-ttu-id="f3ad9-550">按一下**上傳**檔案上傳至 blob 容器的圖示。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-550">Click the **upload** icon to upload a file to the blob container.</span></span>

    <span data-ttu-id="f3ad9-551">![無項目的影像容器](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "無項目的影像容器")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-551">![Images container with no entries](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "Images container with no entries")</span></span>

    <span data-ttu-id="f3ad9-552">*無項目的影像容器*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-552">*Images container with no entries*</span></span>
13. <span data-ttu-id="f3ad9-553">在**上傳 Blob**對話方塊方塊中，瀏覽至**資產**實驗室的資料夾。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-553">In the **Upload Blob** dialog box, navigate to the **Assets** folder of the lab.</span></span> <span data-ttu-id="f3ad9-554">選取**標誌 big.png**檔案，然後按一下 **開啟**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-554">Select the **logo-big.png** file and click **Open**.</span></span>
14. <span data-ttu-id="f3ad9-555">請等到檔案上傳。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-555">Wait until the file is uploaded.</span></span> <span data-ttu-id="f3ad9-556">完成上傳的檔案應該列映像容器中。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-556">When the upload completes, the file should be listed in the images container.</span></span> <span data-ttu-id="f3ad9-557">檔案項目上按一下滑鼠右鍵，然後選取**複製 URL**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-557">Right-click the file entry and select **Copy URL**.</span></span>

    <span data-ttu-id="f3ad9-558">![複製 blob URL](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "複製 blob 檔案的 URL")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-558">![Copy blob URL](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "Copy blob file URL")</span></span>

    <span data-ttu-id="f3ad9-559">*複製 blob URL*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-559">*Copy blob URL*</span></span>
15. <span data-ttu-id="f3ad9-560">開啟 Internet Explorer 並貼上 URL。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-560">Open Internet Explorer and paste the URL.</span></span> <span data-ttu-id="f3ad9-561">下列映像應該會顯示在瀏覽器。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-561">The following image should be shown in the browser.</span></span>

    <span data-ttu-id="f3ad9-562">![從 Windows Blob 儲存體的標誌 big.png 映像](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "標誌 big.png 映像，從儲存體")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-562">![logo-big.png image from Windows Blob Storage](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "logo-big.png image from storage")</span></span>

    <span data-ttu-id="f3ad9-563">*從 Azure Blob 儲存體的標誌 big.png 映像*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-563">*logo-big.png image from Azure Blob Storage*</span></span>

<a id="Ex4Task3"></a>
#### <a name="task-3--updating-the-solution-to-consume-static-content-from-azure-blob-storage"></a><span data-ttu-id="f3ad9-564">工作 3-更新方案來使用 Azure Blob 儲存體的靜態內容</span><span class="sxs-lookup"><span data-stu-id="f3ad9-564">Task 3 – Updating the Solution to Consume Static Content from Azure Blob Storage</span></span>

<span data-ttu-id="f3ad9-565">在這個工作中，您將設定**GeekQuiz**解決方案，以使用映像上傳至 Azure Blob 儲存體 （而不是位於 web 應用程式中的影像） 加入 ASP.NET URL 重寫規則，在**web.config**檔案。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-565">In this task, you will configure the **GeekQuiz** solution to consume the image uploaded to Azure Blob Storage (instead of the image located in the web app) by adding an ASP.NET URL rewrite rule in the **web.config** file.</span></span>

1. <span data-ttu-id="f3ad9-566">在 Visual Studio 中開啟**Web.config**檔案內**GeekQuiz**專案，並找出 **&lt;system.webServer&gt;** 項目。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-566">In Visual Studio, open the **Web.config** file inside the **GeekQuiz** project and locate the **&lt;system.webServer&gt;** element.</span></span>
2. <span data-ttu-id="f3ad9-567">加入下列程式碼，將 URL 重寫規則，更新您的儲存體帳戶名稱的預留位置。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-567">Add the following code to add an URL rewrite rule, updating the placeholder with your storage account name.</span></span>

    <span data-ttu-id="f3ad9-568">(程式碼片段- *WebSitesInProduction-Ex4-UrlRewriteRule*)</span><span class="sxs-lookup"><span data-stu-id="f3ad9-568">(Code Snippet - *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span></span>

    [!code-xml[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample17.xml)]

    > [!NOTE]
    > <span data-ttu-id="f3ad9-569">URL 重寫為攔截傳入的 Web 要求，並將要求重新導向至不同的資源的程序。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-569">URL rewriting is the process of intercepting an incoming Web request and redirecting the request to a different resource.</span></span> <span data-ttu-id="f3ad9-570">重寫規則的 URL 會告訴重寫引擎要求時必須重新導向，而且其中應該他們重新導向。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-570">The URL rewriting rules tells the rewriting engine when a request needs to be redirected, and where should they be redirected.</span></span> <span data-ttu-id="f3ad9-571">重寫規則由兩個字串所組成： 尋找所要求 URL 中的模式 （通常會使用規則運算式），並找到要取代的模式，如果字串。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-571">A rewriting rule is composed of two strings: the pattern to look for in the requested URL (usually, using regular expressions), and the string to replace the pattern with, if found.</span></span> <span data-ttu-id="f3ad9-572">如需詳細資訊，請參閱[ASP.NET 中的 URL 重寫](https://msdn.microsoft.com/en-us/library/ms972974.aspx)。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-572">For more information, see [URL Rewriting in ASP.NET](https://msdn.microsoft.com/en-us/library/ms972974.aspx).</span></span>
3. <span data-ttu-id="f3ad9-573">按**CTRL + S**儲存的變更。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-573">Press **CTRL + S** to save the changes.</span></span>
4. <span data-ttu-id="f3ad9-574">開啟新**Git Bash**主控台部署至 Azure App Service 更新的應用程式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-574">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
5. <span data-ttu-id="f3ad9-575">執行下列命令以將變更推送至 Azure。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-575">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="f3ad9-576">更新*[您的應用程式的路徑]*預留位置路徑**GeekQuiz**方案。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-576">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="f3ad9-577">系統會提示您做為部署密碼。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-577">You will be prompted for your deployment password.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample18.cmd)]

    ![將更新部署至 Azure](maintainable-azure-websites-managing-change-and-scale/_static/image73.png)

    <span data-ttu-id="f3ad9-579">*將更新部署至 Azure*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-579">*Deploying update to Azure*</span></span>

<a id="Ex4Task4"></a>
#### <a name="task-4--verification"></a><span data-ttu-id="f3ad9-580">工作 4-驗證</span><span class="sxs-lookup"><span data-stu-id="f3ad9-580">Task 4 – Verification</span></span>

<span data-ttu-id="f3ad9-581">在這項工作會使用**Internet Explorer**瀏覽**玩家測驗**應用程式，並檢查 URL 重寫規則的映像運作，而且您會被重新導向至裝載於映像**Azure Blob儲存體**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-581">In this task you will use **Internet Explorer** to browse the **Geek Quiz** application and check that the URL rewrite rule for images works and you are redirected to the image hosted on **Azure Blob Storage**.</span></span>

1. <span data-ttu-id="f3ad9-582">開啟 Internet Explorer 並瀏覽至您的 web 應用程式 (例如`http://<your-web-site>.azurewebsites.net`)。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-582">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="f3ad9-583">使用先前建立的認證登入。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-583">Log in using the previously created credentials.</span></span>

    <span data-ttu-id="f3ad9-584">![顯示玩家測驗 web 應用程式與映像](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "顯示玩家測驗 web 應用程式與映像")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-584">![Showing the Geek Quiz web app with the image](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "Showing the Geek Quiz web app with the image")</span></span>

    <span data-ttu-id="f3ad9-585">*顯示玩家測驗 web 應用程式與映像*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-585">*Showing the Geek Quiz web app with the image*</span></span>
2. <span data-ttu-id="f3ad9-586">按**F12**若要啟動開發工具，請選取**網路**索引標籤上，並開始錄製。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-586">Press **F12** to launch the development tools, select the **Network** tab and start recording.</span></span>

    <span data-ttu-id="f3ad9-587">![啟動網路錄製](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "啟動網路錄製")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-587">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "Starting network recording")</span></span>

    <span data-ttu-id="f3ad9-588">*啟動網路錄製*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-588">*Starting network recording*</span></span>
3. <span data-ttu-id="f3ad9-589">按**CTRL + F5**重新整理網頁。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-589">Press **CTRL + F5** to refresh the web page.</span></span>
4. <span data-ttu-id="f3ad9-590">在網頁完成載入時，您應該會看到的 HTTP 要求**/img/logo-big.png**含有 HTTP URL **301**結果 （重新導向） 和另一個要求`http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png`URL 含有 HTTP **200**結果。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-590">Once the page has finished loading, you should see an HTTP request for the **/img/logo-big.png** URL with an HTTP **301** result (redirect) and another request for `http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png` URL with a HTTP **200** result.</span></span>

    <span data-ttu-id="f3ad9-591">![正在驗證重新導向 URL](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "開發人員工具中顯示重新導向")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-591">![Verifying the URL redirect](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "Showing the redirect in Dev Tools")</span></span>

    <span data-ttu-id="f3ad9-592">*正在驗證重新導向 URL*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-592">*Verifying the URL redirect*</span></span>

<a id="Exercise5"></a>
### <a name="exercise-5-using-autoscale-for-web-apps"></a><span data-ttu-id="f3ad9-593">練習 5： 使用自動調整規模的 Web 應用程式</span><span class="sxs-lookup"><span data-stu-id="f3ad9-593">Exercise 5: Using Autoscale for Web Apps</span></span>

> [!NOTE]
> <span data-ttu-id="f3ad9-594">這個練習中是選擇性的因為它要求支援 Web 負載&amp;效能測試時才可供**Visual Studio 2013 Ultimate Edition**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-594">This exercise is optional, since it requires support for Web Load &amp; Performance Testing which is only available for **Visual Studio 2013 Ultimate Edition**.</span></span> <span data-ttu-id="f3ad9-595">如需特定的 Visual Studio 2013 功能的詳細資訊，請比較版本[這裡](https://www.microsoft.com/visualstudio/eng/products/compare)。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-595">For more information on specific Visual Studio 2013 features, compare versions [here](https://www.microsoft.com/visualstudio/eng/products/compare).</span></span>


<span data-ttu-id="f3ad9-596">**Azure App Service Web 應用程式**中執行的 web 應用程式提供的自動調整功能**標準模式**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-596">**Azure App Service Web Apps** provides the Autoscale feature for web apps running in **Standard Mode**.</span></span> <span data-ttu-id="f3ad9-597">自動調整規模可讓 Azure 自動調整您的 web 應用程式，根據負載的執行個體計數。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-597">Autoscale lets Azure automatically scale the instance count of your web app depending on the load.</span></span> <span data-ttu-id="f3ad9-598">啟用自動調整時，Azure 會檢查您的 web 應用程式的 CPU 一次每隔五分鐘，並將執行個體，視需要在該點的時間。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-598">When Autoscale is enabled, Azure checks the CPU of your web app once every five minutes and adds instances as needed at that point in time.</span></span> <span data-ttu-id="f3ad9-599">如果 CPU 使用量過低時，Azure 將會移除執行個體一次每隔兩小時以確保您的 web 應用程式的效能不會效能降低。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-599">If the CPU usage is low, Azure will remove instances once every two hours to ensure that the performance of your web app is not degraded.</span></span>

<span data-ttu-id="f3ad9-600">在此練習將逐步執行設定所需的步驟**自動調整規模**之功能**玩家測驗**web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-600">In this exercise you will go through the steps required to configure the **Autoscale** feature for the **Geek Quiz** web app.</span></span> <span data-ttu-id="f3ad9-601">您將執行 Visual Studio 負載測試，以便產生足夠的 CPU 負載，以觸發執行個體升級應用程式上，以驗證這項功能。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-601">You will verify this feature by running a Visual Studio load test to generate enough CPU load on the application to trigger an instance upgrade.</span></span>

<a id="Ex5Task1"></a>
#### <a name="task-1--configuring-autoscale-based-on-the-cpu-metric"></a><span data-ttu-id="f3ad9-602">工作 1-根據 CPU 度量的自動調整設定</span><span class="sxs-lookup"><span data-stu-id="f3ad9-602">Task 1 – Configuring Autoscale Based on the CPU Metric</span></span>

<span data-ttu-id="f3ad9-603">在這項工作中，您將使用 Azure 管理入口網站來啟用自動調整功能，您在練習 2 中建立 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-603">In this task you will use the Azure management portal to enable the Autoscale feature for the web app you created in Exercise 2.</span></span>

1. <span data-ttu-id="f3ad9-604">在[Azure 管理入口網站](https://manage.windowsazure.com/)，選取**網站**按一下您在練習 2 中建立 web 應用程式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-604">In the [Azure management portal](https://manage.windowsazure.com/), select **Websites** and click the web app you created in Exercise 2.</span></span>
2. <span data-ttu-id="f3ad9-605">瀏覽至**標尺**頁面。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-605">Navigate to the **Scale** page.</span></span> <span data-ttu-id="f3ad9-606">在下**容量**區段中，選取**CPU**如**依度量調整**組態。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-606">Under the **capacity** section, select **CPU** for the **Scale by Metric** configuration.</span></span>

    > [!NOTE]
    > <span data-ttu-id="f3ad9-607">依 CPU 調整，當 Azure 動態調整應用程式中使用的 CPU 使用量變更時的執行個體數目。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-607">When scaling by CPU, Azure dynamically adjusts the number of instances that the app uses if the CPU usage changes.</span></span>

    <span data-ttu-id="f3ad9-608">![選取依 CPU 調整即可](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "選取自動調整大小的 CPU 計量")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-608">![Selecting to scale by CPU](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "Selecting the CPU metric for auto scaling")</span></span>

    <span data-ttu-id="f3ad9-609">*選取依 CPU 調整即可*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-609">*Selecting to scale by CPU*</span></span>
3. <span data-ttu-id="f3ad9-610">變更**目標 CPU**設定**20**-**40**百分比。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-610">Change the **Target CPU** configuration to **20**-**40** percent.</span></span>

    > [!NOTE]
    > <span data-ttu-id="f3ad9-611">這個範圍代表您 web 應用程式的平均 CPU 使用量。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-611">This range represents the average CPU usage for your web app.</span></span> <span data-ttu-id="f3ad9-612">Azure 會加入或移除執行個體，將您的 web 應用程式保留在這個範圍。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-612">Azure will add or remove instances to keep your web app in this range.</span></span> <span data-ttu-id="f3ad9-613">用於調整的執行個體最小和最大數目指定在**執行個體計數**組態。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-613">The minimum and maximum number of instances used for scaling is specified in the **Instance Count** configuration.</span></span> <span data-ttu-id="f3ad9-614">Azure 絕不會高於或超過該限制。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-614">Azure will never go above or beyond that limit.</span></span>
    > 
    > <span data-ttu-id="f3ad9-615">預設值**目標 CPU**只針對這個實驗室的目的修改值。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-615">The default **Target CPU** values are modified just for the purposes of this lab.</span></span> <span data-ttu-id="f3ad9-616">藉由設定 CPU 範圍值較小，您要在一般負載放在應用程式時增加到觸發程序自動調整的機會。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-616">By configuring the CPU range with small values, you are increasing the chances to trigger Autoscale when a moderate load is placed on the application.</span></span>

    <span data-ttu-id="f3ad9-617">![變更目標必須介於 20 到 40%的 CPU](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "變更必須介於 20 到 40%的目標 CPU")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-617">![Changing the target CPU to be between 20 and 40 percent](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "Changing the target CPU to be between 20 and 40 percent")</span></span>

    <span data-ttu-id="f3ad9-618">*變更必須介於 20 到 40%的目標 CPU*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-618">*Changing the Target CPU to be between 20 and 40 percent*</span></span>
4. <span data-ttu-id="f3ad9-619">按一下**儲存**在命令列中儲存的變更。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-619">Click **Save** in the command bar to save the changes.</span></span>

<a id="Ex5Task2"></a>
#### <a name="task-2--load-testing-with-visual-studio"></a><span data-ttu-id="f3ad9-620">工作 2： 使用 Visual Studio 測試的負載</span><span class="sxs-lookup"><span data-stu-id="f3ad9-620">Task 2 – Load Testing with Visual Studio</span></span>

<span data-ttu-id="f3ad9-621">既然**自動調整規模**已經過設定，您將建立**Web 效能和負載測試專案**在 Visual Studio 中產生某些 web 應用程式上的 CPU 負載。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-621">Now that **Autoscale** has been configured, you will create a **Web Performance and Load Test Project** in Visual Studio to generate some CPU load on your web app.</span></span>

1. <span data-ttu-id="f3ad9-622">開啟**Visual Studio Ultimate 2013**選取**檔案 |新 |專案...**啟動新的解決方案。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-622">Open **Visual Studio Ultimate 2013** and select **File | New | Project...** to start a new solution.</span></span>

    <span data-ttu-id="f3ad9-623">![建立新的專案](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "建立新的專案")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-623">![Creating a new project](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "Creating a new project")</span></span>

    <span data-ttu-id="f3ad9-624">*建立新的專案*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-624">*Creating a new project*</span></span>
2. <span data-ttu-id="f3ad9-625">在**新專案**對話方塊中，選取**Web 效能和負載測試專案**下**Visual C# |測試** 索引標籤。請確定**.NET Framework 4.5**是選取，將專案命名為*WebAndLoadTestProject*，選擇**位置**按一下**確定**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-625">In the **New Project** dialog box, select **Web Performance and Load Test Project** under the **Visual C# | Test** tab. Make sure **.NET Framework 4.5** is selected, name the project *WebAndLoadTestProject*, choose a **Location** and click **OK**.</span></span>

    <span data-ttu-id="f3ad9-626">![建立新的 Web 和負載測試專案](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "建立新的 Web 和負載測試專案")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-626">![Creating a new Web and Load Test project](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "Creating a new Web and Load Test project")</span></span>

    <span data-ttu-id="f3ad9-627">*建立新的 Web 和負載測試專案*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-627">*Creating a new Web and Load Test project*</span></span>
3. <span data-ttu-id="f3ad9-628">在**WebTest1.webtest**以滑鼠右鍵按一下**WebTest1**節點，然後按一下**加入要求**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-628">In the **WebTest1.webtest** Right-click the **WebTest1** node and click **Add Request**.</span></span>

    <span data-ttu-id="f3ad9-629">![將要求加入至 WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "將要求加入至 WebTest1")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-629">![Adding a request to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "Adding a request to WebTest1")</span></span>

    <span data-ttu-id="f3ad9-630">*將要求加入至 WebTest1*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-630">*Adding a request to WebTest1*</span></span>
4. <span data-ttu-id="f3ad9-631">在**屬性**視窗的 [新要求] 節點中，更新**Url**屬性以指向您的 web 應用程式的 URL (例如 *[http://geek-quiz.azurewebsites.net/](http://geek-quiz.azurewebsites.net/)*).</span><span class="sxs-lookup"><span data-stu-id="f3ad9-631">In the **Properties** window of the new request node, update the **Url** property to point to the URL of your web app (e.g. *[http://geek-quiz.azurewebsites.net/](http://geek-quiz.azurewebsites.net/)*).</span></span>

    <span data-ttu-id="f3ad9-632">![變更 Url 屬性](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "變更 Url 屬性")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-632">![Changing the Url property](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "Changing the Url property")</span></span>

    <span data-ttu-id="f3ad9-633">*變更 Url 屬性*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-633">*Changing the Url property*</span></span>
5. <span data-ttu-id="f3ad9-634">在**WebTest1.webtest**視窗中，以滑鼠右鍵按一下**WebTest1**按一下**新增迴圈...**.</span><span class="sxs-lookup"><span data-stu-id="f3ad9-634">In the **WebTest1.webtest** window, right-click **WebTest1** and click **Add Loop...**.</span></span>

    <span data-ttu-id="f3ad9-635">![將迴圈加入至 WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "將迴圈加入至 WebTest1")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-635">![Adding a loop to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "Adding a loop to WebTest1")</span></span>

    <span data-ttu-id="f3ad9-636">*將迴圈加入至 WebTest1*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-636">*Adding a loop to WebTest1*</span></span>
6. <span data-ttu-id="f3ad9-637">在**加入條件式規則和項目迴圈**對話方塊中，選取**For 迴圈 」**規則，並修改下列屬性。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-637">In the **Add Conditional Rule and Items to Loop** dialog box, select the **For Loop** rule and modify the following properties.</span></span>

    1. <span data-ttu-id="f3ad9-638">**結束值：** 1000年</span><span class="sxs-lookup"><span data-stu-id="f3ad9-638">**Terminating value:** 1000</span></span>
    2. <span data-ttu-id="f3ad9-639">**內容參數名稱：**迭代器</span><span class="sxs-lookup"><span data-stu-id="f3ad9-639">**Context Parameter Name:** Iterator</span></span>
    3. <span data-ttu-id="f3ad9-640">**遞增值：** 1</span><span class="sxs-lookup"><span data-stu-id="f3ad9-640">**Increment Value:** 1</span></span>

    <span data-ttu-id="f3ad9-641">![選取 For 迴圈 」 規則，並更新屬性](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "選取 For 迴圈 」 規則，並更新屬性")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-641">![Selecting the For Loop rule and updating the properties](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "Selecting the For Loop rule and updating the properties")</span></span>

    <span data-ttu-id="f3ad9-642">*選取 For 迴圈 」 規則，並更新屬性*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-642">*Selecting the For Loop rule and updating the properties*</span></span>
7. <span data-ttu-id="f3ad9-643">在下**迴圈中的項目**區段中，選取您要在迴圈的第一個和最後一個項目先前建立的要求。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-643">Under the **Items in loop** section, select the request you created previously to be the first and last item for the loop.</span></span> <span data-ttu-id="f3ad9-644">按一下 [確定]  繼續操作。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-644">Click **OK** to continue.</span></span>

    <span data-ttu-id="f3ad9-645">![選取此迴圈的第一個和最後一個項目](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "選取迴圈的第一個和最後一個項目")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-645">![Selecting the first and last items for the loop](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "Selecting the first and last items for the loop")</span></span>

    <span data-ttu-id="f3ad9-646">*選取此迴圈的第一個和最後一個項目*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-646">*Selecting the first and last items for the loop*</span></span>
8. <span data-ttu-id="f3ad9-647">在**方案總管] 中**，以滑鼠右鍵按一下**WebAndLoadTestProject**專案中，展開 [**新增**功能表，然後選取**負載測試...**.</span><span class="sxs-lookup"><span data-stu-id="f3ad9-647">In **Solution Explorer**, right-click the **WebAndLoadTestProject** project, expand the **Add** menu and select **Load Test...**.</span></span>

    <span data-ttu-id="f3ad9-648">![將負載測試加入至 WebAndLoadTestProject 專案](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "WebAndLoadTestProject 專案中加入負載測試")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-648">![Adding a Load Test to the WebAndLoadTestProject project](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "Adding a Load Test to the WebAndLoadTestProject project")</span></span>

    <span data-ttu-id="f3ad9-649">*WebAndLoadTestProject 專案中加入負載測試*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-649">*Adding a Load Test to the WebAndLoadTestProject project*</span></span>
9. <span data-ttu-id="f3ad9-650">在**新增負載測試精靈**對話方塊中，按一下 **下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-650">In the **New Load Test Wizard** dialog box, click **Next**.</span></span>

    <span data-ttu-id="f3ad9-651">![新增負載測試精靈](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "新增負載測試精靈")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-651">![New Load Test Wizard](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "New Load Test Wizard")</span></span>

    <span data-ttu-id="f3ad9-652">*新增負載測試精靈*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-652">*New Load Test Wizard*</span></span>
10. <span data-ttu-id="f3ad9-653">在**案例**頁面上，選取**不使用考慮時間**按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-653">In the **Scenario** page, select **Do not use think times** and click **Next**.</span></span>

    <span data-ttu-id="f3ad9-654">![選取不想使用考慮時間](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "選取不想使用考慮時間")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-654">![Selecting not to use think times](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "Selecting not to use think times")</span></span>

    <span data-ttu-id="f3ad9-655">*選擇不使用考慮時間*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-655">*Selecting not to use think times*</span></span>
11. <span data-ttu-id="f3ad9-656">在**負載模式**頁面上，請確定**常數負載**選取選項。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-656">In the **Load Pattern** page, make sure that the **Constant Load** option is selected.</span></span> <span data-ttu-id="f3ad9-657">變更**使用者計數**設**250**使用者，然後按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-657">Change the **User Count** setting to **250** users and click **Next**.</span></span>

    <span data-ttu-id="f3ad9-658">![使用者計數變更到 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "變更到 250 使用者計數")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-658">![Changing the user count to 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "Changing the user count to 250")</span></span>

    <span data-ttu-id="f3ad9-659">*變更到 250 使用者計數*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-659">*Changing the user count to 250*</span></span>
12. <span data-ttu-id="f3ad9-660">在**測試混合模型**頁面上，選取**依據循序測試順序**按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-660">In the **Test Mix Model** page, select **Based on sequential test order** and click **Next**.</span></span>

    <span data-ttu-id="f3ad9-661">![選取的測試混合模型](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "選取的測試混合模型")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-661">![Selecting the test mix model](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "Selecting the test mix model")</span></span>

    <span data-ttu-id="f3ad9-662">*選取的測試混合模型*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-662">*Selecting the test mix model*</span></span>
13. <span data-ttu-id="f3ad9-663">在**測試混合模型**頁面上，按一下**新增...**若要將測試加入至混合。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-663">In the **Test Mix Model** page, click **Add...** to add a test to the mix.</span></span>

    <span data-ttu-id="f3ad9-664">![將測試加入至測試混合](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "將測試加入至測試混合")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-664">![Adding a test to the test mix](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "Adding a test to the test mix")</span></span>

    <span data-ttu-id="f3ad9-665">*將測試加入至測試混合*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-665">*Adding a test to the test mix*</span></span>
14. <span data-ttu-id="f3ad9-666">在**加入測試**對話方塊中，按兩下**WebTest1**若要加入至測試**選取的測試**清單。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-666">In the **Add Tests** dialog box, double-click **WebTest1** to add the test to the **Selected tests** list.</span></span> <span data-ttu-id="f3ad9-667">按一下 [確定]  繼續操作。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-667">Click **OK** to continue.</span></span>

    <span data-ttu-id="f3ad9-668">![新增 WebTest1 測試](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "新增 WebTest1 測試")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-668">![Adding the WebTest1 test](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "Adding the WebTest1 test")</span></span>

    <span data-ttu-id="f3ad9-669">*新增 WebTest1 測試*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-669">*Adding the WebTest1 test*</span></span>
15. <span data-ttu-id="f3ad9-670">回到**測試混合**頁面上，按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-670">Back in the **Test Mix** page, click **Next**.</span></span>

    <span data-ttu-id="f3ad9-671">![正在完成測試混合頁面](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "完成測試混合 頁面")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-671">![Completing the Test Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "Completing the Test Mix page")</span></span>

    <span data-ttu-id="f3ad9-672">*完成測試混合 頁面*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-672">*Completing the Test Mix page*</span></span>
16. <span data-ttu-id="f3ad9-673">在**網路混合**頁面上，按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-673">In the **Network Mix** page, click **Next**.</span></span>

    <span data-ttu-id="f3ad9-674">![按一下 [網路混合] 頁面中的下一個](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "按一下網路混合 頁面中的下一個")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-674">![Clicking next in the Network Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "Clicking next in the Network Mix page")</span></span>

    <span data-ttu-id="f3ad9-675">*網路混合頁面中按一下 [下一步]*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-675">*Clicking next in the Network Mix page*</span></span>
17. <span data-ttu-id="f3ad9-676">在**瀏覽器混合**頁面上，選取**Internet Explorer 10.0**作為瀏覽器類型按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-676">In the **Browser Mix** page, select **Internet Explorer 10.0** as the browser type and click **Next**.</span></span>

    <span data-ttu-id="f3ad9-677">![選取瀏覽器類型](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "選取瀏覽器類型")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-677">![Selecting the browser type](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "Selecting the browser type")</span></span>

    <span data-ttu-id="f3ad9-678">*選取瀏覽器類型*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-678">*Selecting the browser type*</span></span>
18. <span data-ttu-id="f3ad9-679">在**計數器集合**頁面上，按一下**下一步**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-679">In the **Counter Sets** page, click **Next**.</span></span>

    <span data-ttu-id="f3ad9-680">![計數器集合] 頁面中按一下 [下一步](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "按一下中的下一個計數器集合 頁面")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-680">![Clicking Next in the Counter Sets page](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "Clicking Next in the Counter Sets page")</span></span>

    <span data-ttu-id="f3ad9-681">*計數器集合 頁面中按一下 下一步*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-681">*Clicking Next in the Counter Sets page*</span></span>
19. <span data-ttu-id="f3ad9-682">在**回合設定**頁面上，設定**負載測試持續期間**至**5 分鐘**按一下**完成**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-682">In the **Run Settings** page, set the **Load test duration** to **5 minutes** and click **Finish**.</span></span>

    <span data-ttu-id="f3ad9-683">![設定負載測試持續期間為 5 分鐘](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "設定負載測試持續期間為 5 分鐘")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-683">![Setting the load test duration to 5 minutes](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "Setting the load test duration to 5 minutes")</span></span>

    <span data-ttu-id="f3ad9-684">*設定負載測試持續期間為 5 分鐘*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-684">*Setting the load test duration to 5 minutes*</span></span>
20. <span data-ttu-id="f3ad9-685">在**方案總管 中**，連按兩下**Local.settings**瀏覽測試設定檔案。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-685">In **Solution Explorer**, double-click the **Local.settings** file to explore the test settings.</span></span> <span data-ttu-id="f3ad9-686">根據預設，Visual Studio 會使用本機電腦執行的測試。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-686">By default, Visual Studio uses your local computer to run the tests.</span></span>

    > [!NOTE]
    > <span data-ttu-id="f3ad9-687">或者，您可以設定您的測試專案來執行負載測試在雲端中使用**Visual Studio Online (VSO)**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-687">Alternatively, you can configure your test project to run the load tests in the cloud using **Visual Studio Online (VSO)**.</span></span> <span data-ttu-id="f3ad9-688">VSO 提供雲端式負載測試會模擬更真實的負載的服務，避免本機環境條件約束，例如 CPU 容量總計、 可用記憶體和網路頻寬。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-688">VSO provides a cloud-based load testing service that simulates a more realistic load, avoiding local environment constraints like CPU capacity, available memory and network bandwidth.</span></span> <span data-ttu-id="f3ad9-689">如需使用 VSO 執行負載測試的詳細資訊，請參閱[本文](https://www.visualstudio.com/get-started/load-test-your-app-vs)。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-689">For more information about using VSO to run load tests, see [this article](https://www.visualstudio.com/get-started/load-test-your-app-vs).</span></span>

    ![測試設定](maintainable-azure-websites-managing-change-and-scale/_static/image98.png)

<a id="Ex5Task3"></a>
#### <a name="task-3--autoscale-verification"></a><span data-ttu-id="f3ad9-691">工作 3-自動調整規模驗證</span><span class="sxs-lookup"><span data-stu-id="f3ad9-691">Task 3 – Autoscale Verification</span></span>

<span data-ttu-id="f3ad9-692">您現在將會執行您在前一項工作中建立負載測試，並查看您的 web 應用程式在負載下的運作方式。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-692">You will now execute the load test you created in the previous task and see how your web app behaves under load.</span></span>

1. <span data-ttu-id="f3ad9-693">在**方案總管 中**，連按兩下**LoadTest1.loadtest**開啟負載測試。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-693">In **Solution Explorer**, double-click **LoadTest1.loadtest** to open the load test.</span></span>

    <span data-ttu-id="f3ad9-694">![開啟 LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "開啟 LoadTest1.loadtest")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-694">![Opening LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "Opening LoadTest1.loadtest")</span></span>

    <span data-ttu-id="f3ad9-695">*開啟 LoadTest1.loadtest*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-695">*Opening LoadTest1.loadtest*</span></span>
2. <span data-ttu-id="f3ad9-696">在**LoadTest1.loadtest**視窗中，按一下要執行負載測試工具箱 中的第一個按鈕。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-696">In the **LoadTest1.loadtest** window, click the first button in the toolbox to run the load test.</span></span>

    <span data-ttu-id="f3ad9-697">![執行負載測試](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "執行負載測試")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-697">![Running the load test](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "Running the load test")</span></span>

    <span data-ttu-id="f3ad9-698">*執行負載測試*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-698">*Running the load test*</span></span>
3. <span data-ttu-id="f3ad9-699">等待直到完成負載測試。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-699">Wait until the load test completes.</span></span>

    > [!NOTE]
    > <span data-ttu-id="f3ad9-700">負載測試會模擬多個同時將要求傳送至 web 應用程式的使用者。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-700">The load test simulates multiple users that send requests to the web app simultaneously.</span></span> <span data-ttu-id="f3ad9-701">當執行測試時，您可以監視可用的計數器，來偵測所有錯誤、 警告或其他與您的負載測試回合相關的資訊。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-701">When the test is running, you can monitor the available counters to detect any errors, warnings or other information related to your load test run.</span></span>

    <span data-ttu-id="f3ad9-702">![負載測試執行](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "等待，直到負載測試完成")</span><span class="sxs-lookup"><span data-stu-id="f3ad9-702">![Load test running](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "Waiting until the load test completes")</span></span>

    <span data-ttu-id="f3ad9-703">*執行負載測試*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-703">*Load test running*</span></span>
4. <span data-ttu-id="f3ad9-704">測試完成之後，請返回管理入口網站並瀏覽至**標尺**web 應用程式頁面。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-704">Once the test completes, go back to the management portal and navigate to the **Scale** page of your web app.</span></span> <span data-ttu-id="f3ad9-705">在下**容量** 區段中，您應該會看到圖表中的新執行個體自動部署。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-705">Under the **capacity** section, you should see in the graph that a new instance was automatically deployed.</span></span>

    ![自動部署的新執行個體](maintainable-azure-websites-managing-change-and-scale/_static/image102.png)

    <span data-ttu-id="f3ad9-707">*自動部署的新執行個體*</span><span class="sxs-lookup"><span data-stu-id="f3ad9-707">*New instance automatically deployed*</span></span>

    > [!NOTE]
    > <span data-ttu-id="f3ad9-708">可能需要幾分鐘的時間才會出現在圖表中的變更 (請按**CTRL + F5**定期重新整理頁面)。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-708">It may take several minutes for the changes to appear in the graph (press **CTRL + F5** periodically to refresh the page).</span></span> <span data-ttu-id="f3ad9-709">如果看不到任何變更，您可以嘗試下列各項：</span><span class="sxs-lookup"><span data-stu-id="f3ad9-709">If you do not see any changes, you can try the following:</span></span>
    > 
    > - <span data-ttu-id="f3ad9-710">增加負載測試的持續時間 (例如以**10 分鐘**)</span><span class="sxs-lookup"><span data-stu-id="f3ad9-710">Increase the duration of the load test (e.g. to **10 minutes**)</span></span>
    > - <span data-ttu-id="f3ad9-711">減少的最大和最小值**目標 CPU**您 web 應用程式的自動調整規模設定中的範圍</span><span class="sxs-lookup"><span data-stu-id="f3ad9-711">Reduce the maximum and minimum values of the **Target CPU** range in the Autoscale configuration of your web app</span></span>
    > - <span data-ttu-id="f3ad9-712">透過在雲端執行負載測試**Visual Studio Online**。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-712">Run the load test in the cloud with **Visual Studio Online**.</span></span> <span data-ttu-id="f3ad9-713">更多資訊[這裡](https://www.visualstudio.com/en-us/get-started/load-test-your-app-vs.aspx)</span><span class="sxs-lookup"><span data-stu-id="f3ad9-713">More information [here](https://www.visualstudio.com/en-us/get-started/load-test-your-app-vs.aspx)</span></span>

* * *

<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="f3ad9-714">總結</span><span class="sxs-lookup"><span data-stu-id="f3ad9-714">Summary</span></span>

<span data-ttu-id="f3ad9-715">在這個實際操作實驗室中，您學會如何設定和應用程式部署至生產環境 web 應用程式在 Azure 中。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-715">In this hands-on lab, you learned how to set up and deploy your application to production web apps in Azure.</span></span> <span data-ttu-id="f3ad9-716">首先會偵測並使用您資料庫更新**Entity Framework Code First 移轉**，然後繼續執行部署新版本的站台使用**Git**以及執行回復您的站台的最新穩定版本。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-716">You started by detecting and updating your databases using **Entity Framework Code First Migrations**, then continued by deploying new versions of your site using **Git** and performing rollbacks to the latest stable version of your site.</span></span> <span data-ttu-id="f3ad9-717">此外，您會學到如何調整您的應用程式使用儲存體將靜態內容移至 Blob 容器。</span><span class="sxs-lookup"><span data-stu-id="f3ad9-717">Additionally, you learned how to scale your app using Storage to move your static content to a Blob container.</span></span>
