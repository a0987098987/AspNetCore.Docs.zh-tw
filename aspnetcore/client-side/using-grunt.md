...
這個範例會使用空的 ASP.NET Core 專案做為起點，顯示如何從可用的用戶端建置程序自動化。

完成的範例會清除目標部署目錄、 結合 JavaScript 檔案、 檢查程式碼品質、 總是 JavaScript 檔案內容和部署到您的 web 應用程式的根目錄。 我們將使用下列套件：

* **grunt**: grunt 所完成的工作執行器套件。
...
* **監看 contrib grunt 式**： 監看檔案 」 活動的外掛程式。
