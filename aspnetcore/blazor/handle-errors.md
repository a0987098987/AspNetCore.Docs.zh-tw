...
Blazor 伺服器端是可設定狀態的架構。 當使用者與應用程式互動時, 它們會維持與伺服器的連線, 稱為「*線路*」。 線路包含作用中的元件實例, 以及其他許多狀態層面, 例如:
...
Blazor 會將大部分未處理的例外狀況視為其發生所在之線路的嚴重問題。 如果線路因未處理的例外狀況而終止, 則使用者只需重載頁面來建立新的線路, 即可繼續與應用程式互動。 在終止的電路以外的線路 (也就是其他使用者或其他瀏覽器索引標籤的線路) 不受影響。 此案例類似于將損毀&mdash;的應用程式損毀的桌面應用程式, 必須重新開機, 但不會影響其他應用程式。
...
* 會叫用元件的函式。
* 系統會叫用透過指示詞或[@inject](xref:blazor/dependency-injection#request-a-service-in-a-component) [[插入]](xref:blazor/dependency-injection#request-a-service-in-a-component)屬性提供給元件之函式的任何非 singleton DI 服務的函式。 
...
* `ProductRepository.GetProductByIdAsync`方法中擲回的例外狀況是`try-catch`由語句處理。
* `catch`執行區塊時:
...
`.razor`元件檔案中的宣告式標記會編譯成名C# `BuildRenderTree`為的方法。 當元件轉譯時, `BuildRenderTree`會執行並建立資料結構, 以描述所轉譯元件的元素、文字和子元件。

轉譯邏輯可能會擲回例外狀況。 當`@someObject.PropertyName`評估但`@someObject`為`null`時, 就會發生此案例的範例。 轉譯邏輯所擲回的未處理例外狀況對線路而言是嚴重的。
...
例如, 元件可能會從 UI 移除, 因為使用者已流覽至另一個頁面。 當執行的元件<xref:System.IDisposable?displayProperty=fullName>從 UI 中移除時, 架構會呼叫元件的<xref:System.IDisposable.Dispose*>方法。 
...
如需有關元件處置的詳細資訊<xref:blazor/components#component-disposal-with-idisposable>, 請參閱。
...
* 如果對的呼叫`InvokeAsync<T>`同步失敗, 就會發生 .net 例外狀況。 例如, 對`InvokeAsync<T>`的呼叫失敗, 因為無法序列化提供的引數。 開發人員程式碼必須攔截例外狀況。 如果事件處理常式或元件生命週期方法中的應用程式程式碼不會處理例外狀況, 則產生的例外狀況對線路而言是嚴重的。
...
* 根據預設, 的呼叫`InvokeAsync<T>`必須在特定期間內完成, 否則呼叫會超時。預設的超時時間為一分鐘。 Timeout 會保護程式碼不會遺失網路連線, 或永遠不會傳回完成訊息的 JavaScript 程式碼。 如果呼叫超時, 則產生`Task`的會失敗<xref:System.OperationCanceledException>並出現。 使用記錄來設陷並處理例外狀況。
...
您可以使用`Html.RenderComponentAsync`來資源清單 Blazor 元件, 使其呈現的 HTML 標籤會當做使用者初始 HTTP 要求的一部分傳回。 其運作方式如下:

* 建立新的電路, 其中包含屬於相同頁面的所有資源清單元件。
* 產生初始 HTML。
* 將線路`disconnected`視為, 直到使用者的瀏覽器建立 SignalR 連線回到同一部伺服器, 以恢復線路上的互動性。

如果任何元件在進行預入期間擲回未處理的例外狀況 (例如, 在生命週期方法或轉譯邏輯期間):

* 這是電路的嚴重例外狀況。
* 從`Html.RenderComponentAsync`呼叫中擲回呼叫堆疊的例外狀況。 因此, 除非開發人員程式碼明確攔截到例外狀況, 否則整個 HTTP 要求都會失敗。

在一般情況下, 如果無法轉譯, 則繼續建立並轉譯元件並沒有意義, 因為無法轉譯運作中的元件。

若要容忍在自動處理期間可能發生的錯誤, 錯誤處理邏輯必須放在可能會擲回例外狀況的元件內部。 使用[try-catch](/dotnet/csharp/language-reference/keywords/try-catch)語句搭配錯誤處理和記錄。 不要將呼叫`RenderComponentAsync`包裝`try-catch`在語句中, 而是將錯誤處理邏輯放`RenderComponentAsync`在所呈現的元件中。
