...
如需詳細資訊，請參閱[AddDefaultIdentity source](https://github.com/aspnet/AspNetCore/blob/release/2.2/src/Identity/UI/src/IdentityServiceCollectionUIExtensions.cs#L47-L63) 。
...
服務會在中`ConfigureServices`加入。 典型模式是呼叫所有 `Add{Service}` 方法，然後呼叫 `services.Configure{Service}` 方法。
...
::: moniker range="<= aspnetcore-2.0"
   ...
   藉由`UseAuthentication` `Configure`在方法中呼叫來啟用應用程式的身分識別。 `UseAuthentication`將驗證[中介軟體](xref:fundamentals/middleware/index)新增至要求管線。
...
::: moniker range="= aspnetcore-1.1"
   ...
   藉由`UseIdentity` `Configure`在方法中呼叫來啟用應用程式的身分識別。 `UseIdentity`將以 cookie 為基礎的驗證[中介軟體](xref:fundamentals/middleware/index)新增至要求管線。
...
::: moniker range=">= aspnetcore-2.1"
   ...
   當使用者按一下 [**註冊**] 連結時， `RegisterModel.OnPostAsync`就會叫用動作。 使用者是由`_userManager`物件上的[CreateAsync](/dotnet/api/microsoft.aspnetcore.identity.usermanager-1.createasync#Microsoft_AspNetCore_Identity_UserManager_1_CreateAsync__0_System_String_)所建立。 `_userManager`由相依性插入所提供）：
...
::: moniker range="= aspnetcore-2.0"
   ...
   當使用者按一下 [**註冊**] 連結時， `Register`就會在上`AccountController`叫用動作。 動作會藉由`_userManager`在物件上`CreateAsync`呼叫（由相依性插入`AccountController`提供給）來建立使用者： `Register`

   [!code-csharp[](identity/sample/src/ASPNET-IdentityDemo/Controllers/AccountController.cs?name=snippet_register&highlight=11)]

::: moniker-end

   如果成功建立使用者，則會呼叫來`_signInManager.SignInAsync`登入使用者。
...
提交登入頁面上的表單時， `OnPostAsync`會呼叫動作。 `PasswordSignInAsync`會在`_signInManager`物件上呼叫（由相依性插入所提供）。

   [!code-csharp[](identity/sample/WebApp1/Areas/Identity/Pages/Account/Login.cshtml.cs?name=snippet&highlight=10-11)]

   基類`Controller` 會`User`公開可從控制器方法存取的屬性。 例如，您可以列舉`User.Claims`並做出授權決策。 如需詳細資訊，請參閱 <xref:security/authorization/introduction>。

::: moniker-end

::: moniker range="= aspnetcore-2.0"

當使用者選取 [**登入**] 連結時，或在存取需要驗證的網頁時，就會顯示登入表單。 當使用者在登入頁面上提交表單時， `AccountController` `Login`會呼叫動作。

動作會呼叫`PasswordSignInAsync` `_signInManager`物件上的（ `AccountController`由相依性插入所提供）。 `Login`

[!code-csharp[](identity/sample/src/ASPNET-IdentityDemo/Controllers/AccountController.cs?name=snippet_login&highlight=13-14)]

基底（`Controller`或`PageModel`）類別會公開`User`屬性。 例如， `User.Claims`可以列舉以進行授權決策。
...
[**登出**] 連結`LogoutModel.OnPost`會叫用動作。 
...
身分識別的主要套件是[AspNetCore。](https://www.nuget.org/packages/Microsoft.AspNetCore.Identity/) 此套件包含 ASP.NET Core 身分識別的核心介面集，並且包含`Microsoft.AspNetCore.Identity.EntityFrameworkCore`在中。
