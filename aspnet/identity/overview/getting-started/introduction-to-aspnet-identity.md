...
- 提供者系統可讓您變更 支援的資料存放區，但系統針對假設適用於關聯式資料庫設計。 您可以撰寫的提供者，將成員資格資訊儲存在非關聯式儲存體機制，例如 Azure 儲存體資料表，但是，您必須解決關聯式的設計，撰寫太多程式碼和許多`System.NotImplementedException`不的方法的例外狀況適用於 NoSQL 資料庫。
...
前提是使用者會輸入登入的使用者名稱和已註冊您自己的應用程式中的密碼不再有效。 Web 已經成為其他社交。 使用者透過社交管道，例如 Facebook、 Twitter 和其他社交網站的即時與彼此互動。 開發人員想要讓使用者能夠使用其社交身分識別登入，以便它們可以在其於網站上有豐富的體驗。 現代的成員資格系統必須啟用重新導向以登入來驗證提供者，例如 Facebook、 Twitter 和其他項目。
...
- **NuGet 套件**
    ...
    - ASP.NET 身分識別，則會重新分配以 NuGet 套件安裝隨附於 Visual Studio 2017 的 ASP.NET MVC、 Web Form 和 Web API 範本中。 您可以從 NuGet 資源庫下載這個 NuGet 封裝。
...
ASP.NET 身分識別可在 Visual Studio 2017 專案範本的 ASP.NET MVC、 Web Form、 Web API 和 SPA 之間。 在本逐步解說中，我們將說明的專案範本如何使用 ASP.NET 身分識別來將功能新增至註冊、 登入和登出使用者。
...
2. 建立的專案會包含下列三個封裝，ASP.NET 身分識別。
   ...
   此套件包含用來在 ASP.NET 應用程式中插入 OWIN 驗證中使用 ASP.NET Identity 的功能。 您將登功能新增至您的應用程式和呼叫 OWIN 的 Cookie 驗證中介軟體以產生 cookie 時，會使用這項目。
3. 建立使用者。  
   啟動應用程式，然後按一下**註冊**連結，以建立使用者。 下圖顯示 [註冊] 頁面所收集的使用者名稱和密碼。  
  
    ![](introduction-to-aspnet-identity/_static/image2.png)  
  
   當使用者選取**註冊** 按鈕，`Register`帳戶控制器的動作會建立使用者，方法是呼叫 ASP.NET 身分識別 API，以反白顯示如下：
...
下圖顯示 ASP.NET 身分識別系統的元件 (上，選取[這](introduction-to-aspnet-identity/_static/image3.png)或將它放大圖表上)。 以綠色的封裝是由 ASP.NET 身分識別系統所組成。 所有其他套件是使用 ASP.NET 身分識別系統在 ASP.NET 應用程式時所需的相依性。
