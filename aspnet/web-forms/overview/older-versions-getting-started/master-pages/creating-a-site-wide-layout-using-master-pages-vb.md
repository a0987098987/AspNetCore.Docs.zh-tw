...
宣告式標記中的第一行是[`@Master`指示詞](https://msdn.microsoft.com/en-us/library/ms228176.aspx)。 `@Master`指示詞是類似於[`@Page`指示詞](https://msdn.microsoft.com/en-us/library/ydy4x04a.aspx)出現在 ASP.NET 網頁中。 它會定義的伺服器端語言 (VB) 和資訊的位置和主版頁面的程式碼後置類別的繼承。
